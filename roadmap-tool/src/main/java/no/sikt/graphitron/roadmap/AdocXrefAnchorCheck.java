...
/**
 * Scans the staged AsciiDoc site for cross-file {@code xref:<file>.adoc#anchor} references
 * naming an anchor the target page does not publish.
 *
 * <p>Such a reference renders as a working link that goes nowhere useful: the reader lands at
 * the top of the right page instead of the section. Asciidoctor does not and cannot report it,
 * because the target is a separate document that it never resolves, so the mistake ships
 * silently and reproduces. The same-file forms are out of scope; Asciidoctor already reports
 * those, at INFO.
 *
 ...
 * still could not resolve a target that has no authored {@code .adoc} at all. The cost is that
 * every path in a finding is build output; {@link #authoredSource} pays it back.
 *
 * <p><b>An unresolvable target is reported, never failed.</b> A reference whose target
 * {@code .adoc} is absent from staging is a wrong <em>path</em>, a different failure with a
 * different fix: it 404s on the first click, so it is self-reporting in a way a wrong anchor
 * never is. Counting them keeps under-coverage visible without making every quoted example in
 * roadmap prose a build break.
 */
final class AdocXrefAnchorCheck {
    ...
    /** A cross-file anchored reference, as collected from one staged page. */
     ...
     * </ul>
     *
     * <p>Returns 0 when every cross-file anchored reference resolves, and 64 when invoked with
     * no root or a non-directory root. A dangling anchor throws {@link BuildFailure} rather
     * than returning non-zero, for the reason {@link AdocMarkdownTableCheck#run} spells out:
     * {@link Main} turns a non-zero return into {@code System.exit}, and this check runs in the
     * Maven JVM via {@code exec:java}, where that would kill Maven before it prints
     * {@code BUILD FAILURE}.
     */
    static int run(List<String> args) throws IOException {
        if (args.size() != 1) {
            System.err.println("usage: check-adoc-xrefs <staging-dir>");
            return 64;
        }
        ...
        List<Reference> unresolvable = new ArrayList<>();
        for (Reference r : references) {
            ...
            if (!published.contains(r.anchor())) {
                findings.add(new Finding(r, target, published));
            }
        }

        if (!unresolvable.isEmpty()) {
            // Printed, not failed: under-coverage stays visible without a wrong path becoming a
            // build break. A non-zero count is a finding to look at, not noise to tune out.
            System.out.println("check-adoc-xrefs: " + unresolvable.size()
                + " anchored reference(s) could not be resolved to a staged .adoc and were not checked.");
            for (Reference r : unresolvable) {
        ...
        if (findings.isEmpty()) {
            System.out.println("check-adoc-xrefs: " + references.size()
                + " cross-file anchored reference(s), all resolving to an explicit anchor.");
            return 0;
        }

        System.err.println("check-adoc-xrefs: found " + findings.size()
        ...
        throw new BuildFailure("cross-file xrefs naming anchors the target pages do not publish");
    }

    /** Every cross-file anchored reference in the staged tree, in walk order. */
    ...
    /**
     * The cross-file anchored references in one page's {@code adoc} source.
     *
        ...
        for (int n = 0; n < lines.length; n++) {
            ...
            while (m.find()) {
                ...
                references.add(new Reference(
                    file, n + 1, target.substring(0, hash), target.substring(hash + 1)));
            }
        }
        return references;
    }
