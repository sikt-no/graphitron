...
This will be available after the first run of Graphitron codegen. A [nodeIdStrategy](#global-node-identification)
...
By default, Graphitron assumes that every data fetcher and the associated queries should be generated automatically.
To override this assumption, use the **[service](#services)** and **[notGenerated](#notgenerated-directive)** directives.

#### Fields
Each field will have a corresponding output in queries. Assume we have this type in the schema:
...
When using this field in a query, Graphitron will generate the following jOOQ-row:
```java
DSL.row(
        A.a // "A" corresponds to the jOOQ table column "a" is fetched from.
).mapping(Functions.nullOnAllNull(A::new)) // "A" is the DTO-type that will be used for the GraphQL response.
```

#### Arguments
Arguments are used as conditions for queries. Assume we have a query with an argument in the schema:
...
When using this argument in a query, Graphitron will generate the following jOOQ-condition:
```java
.where(A.b.eq(b))  // "A" corresponds to the jOOQ table column "b" is contained within. The second "b" is the value of the argument in the schema.
```
...
This feature is incomplete and currently has low utility.
...
Set this on any query or mutation field that should result in a new data fetcher in order to
disable Graphitron's generation for it.
Since graphql still requires the data fetchers, they will have to be implemented and [wired manually](https://javadoc.io/static/com.graphql-java/graphql-java/10.0/graphql/schema/idl/RuntimeWiring.html).
...
type AType {
  otherType: OtherType @splitQuery @notGenerated # Require new data fetcher for this field, but do not generate it automatically.
}
...
<!-- Litt tidlig å nevne subqueries nå -->
...
Also note that setting both the _table_ and _key_ parameters in the same _reference element_ is redundant, since either is enough to make the join.
...
for queries, however we need to set it on input types so that Graphitron will know which records should be mutated.
...
type EditResponse {
    id: ID! # Note, delete mutations need to work with types that have 'id' fields.
}
...
Note that mutations need either the **mutationType** or the **service** directive set, but not both, in order to be generated as mutations.
Mutations that should not be generated should set the **notGenerated** directive.
...
In this example, to enable Graphitron to resolve `Customer.address`, ensure that the `CustomerRecord` returned from your service has the `CUSTOMER_ID` field (the primary key of the `CUSTOMER` table) set.
If the key field(s) are missing, the split query resolver will not have enough information to fetch the related `Address`.
...
There are some rules that services must follow. Services that are not on the top level must return a map.
...
In other words, top level services should return a list.
If the service is not at the top level it should return a map with the ID as key and the value should be set to a list of records.
...
automatically become a lookup. Only arguments for a top-level field can be keys, or input types referenced from one.
