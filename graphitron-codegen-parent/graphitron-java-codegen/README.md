...
- [Directives](#directives)
  - [Common directives](#common-directives)
    - [Split database queries with @splitQuery](#split-database-queries-with-splitquery)
      - [Joining on self referencing FKs requires @splitQuery](#joining-on-self-referencing-fks-requires-splitquery)
  ...
  - [Custom logic with @service](#custom-logic-with-service)
    ...
    - [Resolving @table fields after services](#resolving-table-fields-after-services)
...
- [Global node identification](#global-node-identification)
  - [Mark a type as globally identifiable with @node](#mark-a-type-as-globally-identifiable-with-node)
  - [Mark an ID as globally unique with @nodeId](#mark-an-id-as-globally-unique-with-nodeid)
...
* `validateAffectedRows` - Checks the affected row count that the batched write of an update or upsert mutation reports, and fails the mutation if a statement in it affected no rows. Enabled by default. A write that reaches no rows is valid SQL that the database does not report as an error, so without this check the mutation returns a payload built by a separate read of the untouched row and looks successful. Deletes are not checked, because their payload reports back the ids that are no longer present, so a row that could not be deleted comes back as null rather than as a false success. Inserts are not checked either, because a write that inserts nothing raises an error of its own.
...
Note that for `@table` fields on `@record` types reachable from `@service`, `@splitQuery` is applied automatically. See [Resolving @table fields after services](#resolving-table-fields-after-services).

##### Joining on self referencing FKs requires @splitQuery
...
**Restrictions:**
- The containing type must have access to a table, either by specifying `@table` or inheriting it from previous types.
- The target type is forbidden from having a `@table` directive set.
...
#### Resolving @table fields after services
...
The Java record is then expected to have `customer` of type `CustomerRecord`, and its primary key fields to be given the correct value(s) in the service.

> **Note:** `@splitQuery` can still be used explicitly and remains required for `@table` fields on plain (non-`@record`) types.

#### Auto-fetching @table types from database
...
> **Note:** If the return type does _not_ have `@table`, the service result is mapped directly to the GraphQL response type using the standard [response mapping](#response-mapping) rules.
...
### Mark a type as globally identifiable with @node
...
- `typeName` — The name of the node type the ID refers to.
  ...
  - The provided type must have the [**@node**](#mark-a-type-as-globally-identifiable-with-node) directive.
