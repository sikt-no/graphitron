...
public class SQLCondition extends CodeReferenceWrapper {
    ...
    public CodeBlock formatToString(List<CodeBlock> methodInputs) {
        ...
        return CodeBlock
                ...
                .add(contextFields.isEmpty() ? "" : ", ")  // Next: Make these be sent to the queries as well.
                .add(StringUtils.repeat("$L", ", ", contextFields.size()), contextFields.keySet().stream().map(it -> "_" + it).toList().toArray())
                .add(")")
                .build();
    }
