...
public class OperationMethodGenerator extends DataFetcherMethodGenerator {
    ...
    private CodeBlock validateInputs(InputParser parser) {
        ...
        for (InputField inputRecord : parser.getJOOQRecords().values()) {
            ...
            var itemVarName = isListed ? asIterable(inputVarName) : inputVarName;
            ...
            for (InputField recordField : type.getFields()) {
                if (processedSchema.isNodeIdField(recordField)) {
                    ...
                    var unpackedVarName = "_unpacked_" + recordField.getName();
                    ...
                    for (int j = i + 1; j < mappings.size(); j++) {
                        ...
                        code.beginControlFlow("if (!$T.equals($L, $L))",
                                ...
                                .addStatement("throw new $T($S)",
                                        ...
                                        "Conflicting values for column " + columnName + " between fields " +
                                                mapping1.field.getName() + " and " + mapping2.field.getName())
                                .endControlFlow();
                    }
                }
            }
