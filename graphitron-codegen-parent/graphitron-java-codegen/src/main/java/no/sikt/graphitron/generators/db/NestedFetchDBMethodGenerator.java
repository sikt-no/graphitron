...
public abstract class NestedFetchDBMethodGenerator extends FetchDBMethodGenerator {
    ...
    String generateHelperMethodName(ObjectField target) {
        ...
        var callingMethodName = asQueryMethodName(target instanceof VirtualSourceField ? target.getContainerTypeName() : target.getName(), localObject.getName());
        var returnTypeName = processedSchema.getRecordType(target).getName();
        return generateNestedMethodName(callingMethodName, returnTypeName);
    }
