...
public class DeleteQueryTest extends MutationQueryTest {
        ...
        assertGeneratedContentContains("nonNullableListedInput",
                ...
                .where(in.size() > 0 ?
                            DSL.row(CUSTOMER.CUSTOMER_ID).in(
                                IntStream.range(0, in.size()).mapToObj(internal_it_ ->
                                    DSL.row(DSL.val(in.get(internal_it_).getId()))
                                ).toList()
                            ) : DSL.falseCondition()
                        )
                """
    ...
    void inputListWithNodeId() {
        assertGeneratedContentContains("inputListWithNodeId",
                "DSL.row(nodeIdStrategy.hasId(\"CustomerNode\", in.get(internal_it_).getId(), CUSTOMER"
        );
    }
    ...
    void wrappedOutput() {
        assertGeneratedContentContains("wrappedOutput",
                "deleteFrom(CUSTOMER)",
                ".where(nodeIdStrategy.hasId(\"CustomerNode\", in.getId(), CUSTOMER."
        );
    }
    ...
    void wrappedScalar() {
        assertGeneratedContentContains("wrappedScalar",
                "deleteFrom(CUSTOMER)",
                ".where(nodeIdStrategy.hasId(\"CustomerNode\", in.getId(), CUSTOMER.fields"
        );
    }
    ...
    void wrappedScalarListed() {
        assertGeneratedContentContains("wrappedScalarListed",
                "deleteFrom(CUSTOMER)",
                ".where(in.size() > 0",
                "hasId(\"CustomerNode\", in.get(internal_it_).getId(), CUSTOMER"
        );
    }
}
