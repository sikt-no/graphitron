...
public class ResolverTest extends GeneratorTest {
    ...
    void deleteListed() {
        assertGeneratedContentContains(
                "deleteListed",
                "in.stream().map(internal_it_ -> internal_it_.getId()).filter(internal_it_ -> !_result.contains(internal_it_)).collect(Collectors.toList())"
        );
    }
    ...
    void deleteWrappedListed() {
        assertGeneratedContentContains(
                "deleteWrappedListed",
                "new CustomerOutput(in.stream().map(internal_it_ -> internal_it_.getId()).filter(internal_it_ -> !_result.getId().contains(internal_it_)).collect(Collectors.toList()))"
        );
    }
}
