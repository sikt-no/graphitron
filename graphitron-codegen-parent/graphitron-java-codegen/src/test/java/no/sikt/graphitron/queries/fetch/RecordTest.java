...
public class RecordTest extends GeneratorTest {
    ...
    @DisplayName("Listed input jOOQ record with an additional reference")
    void listedInputJOOQRecordWithReference() {
        assertGeneratedContentContains(
                "listedInputJOOQRecordWithReference",
                "customerForQuery(DSLContext ctx, List<CustomerRecord> inRecordList,",
                "DSL.row(customer_2952383337_address_left.DISTRICT).in(" // Code will not compile because the CustomerRecord does not have district.
        );
    }
