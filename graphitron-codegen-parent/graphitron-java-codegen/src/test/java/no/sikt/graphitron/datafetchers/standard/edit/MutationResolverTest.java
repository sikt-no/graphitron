...
public class MutationResolverTest extends GeneratorTest {
    ...
    void wrappedOutput() {
        assertGeneratedContentContains("wrappedOutput",
                "loadWrapped(" +
                        "(ctx, selectionSet) -> MutationDBQueries.mutationForMutation(ctx, in, selectionSet)," +
                        "(_result) -> new Wrapper(_result));"
                );
    }
    ...
    void validation() {
        ...
        assertGeneratedContentContains("default",
                ...
                "inRecord = transform.customerInputTableToJOOQRecord(in, \"in\", \"in\");",
                "transform.validate(); return new DataFetcherHelper(env).load(",
                "mutationForMutation(ctx, in,"
        );
