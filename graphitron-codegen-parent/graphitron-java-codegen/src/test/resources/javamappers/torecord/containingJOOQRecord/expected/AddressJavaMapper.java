    ...
    public static List<MapperNestedJavaRecord> toJavaRecord(List<Address> _mi_address,
        ...
        if (_mi_address != null) {
            for (int _niit_address = 0; _niit_address < _mi_address.size(); _niit_address++) {
                ...
                if (_mi_customer != null && _iv_args.hasField("customer")) {
                    _mo_mapperNestedJavaRecord.setCustomer(_iv_transform.customerInputTableToJOOQRecord(_mi_customer, _iv_args.child("customer"), _iv_path + "[" + _niit_address + "]/customer"));
                }
