...
public class QueryGeneratedDataFetcher {
    ...
    public static DataFetcher<CompletableFuture<Node>> node(NodeIdStrategy _iv_nodeIdStrategy) {
        return _iv_env -> {
            ...
            var _iv_typeId = _iv_nodeIdStrategy.getTypeId(_mi_id);
            if (_iv_typeId == null) {
            ...
            switch (_iv_typeId) {
                case "Customer": return _iv_fetcher.loadInterface(_iv_loaderName, _mi_id, (_iv_ctx, _iv_ids, _iv_selectionSet) -> CustomerDBQueries.customerForNode(_iv_ctx, _iv_nodeIdStrategy, _iv_ids, _iv_selectionSet));
