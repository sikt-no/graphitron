...
- `UpdateRowsWalker.classifyInto` and `DeleteRowsWalker.classifyInto`. These are *parallel* methods, not one shared helper: the DELETE one's javadoc calls itself "the DELETE analogue of `UpdateRowsWalker.classifyInto`". So two gates; collapsing the duplication is not this item's job, per the gate-shape rule below.
...
- "lifts to local FK columns (no join)" (`ColumnBackedArg`'s contrast clause), and the same claim restated in `FieldBuilder.classifyArgument`'s `@reference` comment ("Unlike the @nodeId FK-target arm above (which lifts to local FK columns and emits no join)").
