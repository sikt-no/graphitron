...
The factory returns `DomainReturnType`, not `Claim`: the `ReturnTypeRef` arms it is asked about include ones with no backing class to name, and `NoClaim` is the honest answer there rather than a caller-side null check that reintroduces the placeholder by another route. `fromBinaryName` is `private` on `RowsMethodShape` today; widen it to package-private (`DomainReturnType` is in the same package) rather than adding a fourth spelling of the `$`-splitting.

### Producer answers

* `ChildField.ServiceRecordField.domainReturnType()` answers the per-key element, the same `V` the emitted `DataLoader<K, V>` is typed with, through the factory over `returnType()`. `Map` never reaches `env.getSource()`; `V` does. `FieldBuilder` mints this leaf from two classify arms (`ServiceDirectiveResolver.Resolved.Result` and `Resolved.Scalar`), so the fork is spelled out per arm the same way the record-read leaf's is:
  * `ResultReturnType` with non-null `fqClassName`: the factory's backing-class claim, which is `RowsMethodShape.strictPerKeyType`'s grounded backing class for this arm.
  * `ResultReturnType` with null `fqClassName`: `NoClaim`, for the same reason as the record-read leaf's null fork below, and this population matters more than a completeness note. It is grouped (the SDL return is an Object type), it is live (`ServiceRecordField.elementType()`'s "other null-perKey returns (a backing-less `ResultReturnType`)" branch is exactly it), and `strictPerKeyType` answers `null` there. An implementer who fills that null by falling back to reflected `peelToClassName` re-mints the placeholder this item retires, on a population the check actually compares.
  * `ScalarReturnType`: whatever `strictPerKeyType` grounds (the Java class for the five built-in scalars), else `NoClaim` for an enum or custom scalar. Never grouped, so this choice is visible only in the rejection rendering. It still matters: a scalar-returning batch leaf answers `Plain(java.util.Map)` today, which is the same false statement as the object case, sitting somewhere nobody currently looks.
...
The list-shaped `JavaAccessor` component named above is the second population in that gap, on the same terms: the claim is the SDL type's bound class, the wrapper erasure means nothing checks the accessor's element class, and a wrong element class surfaces at run time. Both populations share the shape of the trade: the unverified fact is a single reader's, not a disagreement between two producers, so rejecting it was never something the check was measuring. Unverifiable is not disagreeing, and the placeholder's accidental rejection of either was not an invariant.
...
`MutationServiceRecordField`'s javadoc paragraph claiming non-carrier shapes "either agree or are filtered by upstream classifier rejections" is replaced by the `fqClassName` fork it now implements, with a `{@link}` to the query twin so the reference gate pins the symmetry the two leaves are supposed to hold.
