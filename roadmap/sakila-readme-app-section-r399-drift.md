...
- Same drift class, surfaced by the Spec-time survey: a doubled `graphitron-rewrite/graphitron-rewrite/` path segment in absolute GitHub URLs at `docs/quick-start.adoc` lines 21, 64, and 66 (four URLs) and `docs/manual/tutorial/index.adoc` line 7. (The same string in `roadmap/changelog.md` is a historical path inside a history entry, not a link; leave it.)
...
- **Ride-along typo fix**: the doubled `graphitron-rewrite/graphitron-rewrite/` path segment in the absolute GitHub URLs at `docs/quick-start.adoc` lines 21/64/66 and `docs/manual/tutorial/index.adoc` line 7 (invisible to any local guard; grep the doubled segment rather than trusting the line numbers, and skip the historical-path hit in `roadmap/changelog.md`).
