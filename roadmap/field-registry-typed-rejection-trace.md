...
Under the claim relations the trace this item types is replaced: `FieldRegistry.classify`'s same-coordinate conflict handling is the key constraint in embryonic form (R589's words), and claim plus violation rows carry what the trace carries. Fork (b), threading `TableRef` into `classifyInput`, is a dead end; the item collapses to the record widening or retires with the registry.
