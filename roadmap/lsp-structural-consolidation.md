...
- Bundle `catalog` / `sourceIndex` / `snapshot` / `validationReport` into one
  immutable `BuildOutput` record behind a single `volatile` reference, so `setBuildOutput` is an atomic
  swap and readers never see a torn set. The per-field `volatile`s do not make a multi-field read
  atomic: a reader interleaving with `setBuildOutput`'s separate writes can still pair a new snapshot
  with an old catalog. The single-reference swap is the fix, and the bundle must include `sourceIndex`,
  not just the original triple. **Restated since this item opened:** the second-reader half of the
  argument was `graphitron-mcp`, which no longer reads a `Workspace` at all: it answers from the
  session's store handle and reader, so the field it read alongside the snapshot is gone with the
  projection behind it. What survives is the LSP's own multi-field reads, which is the case the swap
  has to be argued on now.
...
- **`intellij-lsp-plugin`**: a downstream surface of the same server; it benefits from the cleaner
  seams but is not blocked on this.
- **`graphitron-mcp`**: no longer an intersection at all. It once read the same live `Workspace` the
  LSP reads, which made Slice 5's torn-read fix a two-consumer concern and put `LspVocabulary` on the
  path of Slice 6's optional split. It now declares no edge to `graphitron-lsp` in any scope and
  answers every tool from the session's store handle and reader, so both slices are free of it: the
  two surfaces are still "one model, two views", but what they share is the store the dev session
  captures into rather than a Java vocabulary over it.
