...
depends-on: [remove-convention-scalar-resolution]
created: 2026-07-10
...
**Depends on R464.** The only fallout from the extended-scalars bump in the dry run was the convention-table drift-guard test (`ScalarTypeResolverTest.conventionTable_coversEveryExtendedScalarsField`) tripping on six new constants (`YearMonth`, `Year`, `AccurateDuration`, `NominalDuration`, `SecondsSinceEpoch`, `HexColorCode`), each demanding a map-or-exclude decision. R464 deletes that convention layer and its drift test outright. Sequencing R464 first means this upgrade is a pure two-line version bump with nothing to curate; doing them in the other order would waste effort triaging six scalars that are about to be removed from the resolution path entirely. If R464 is ever abandoned, this item reabsorbs the scalar-triage work as its own scope.
