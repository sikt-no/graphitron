...
Bout 5 (census rows 191-241, 51 workable files, verified, reactor green with both guards):

190. `graphitron/.../compile/AbiSignature.java`: 46 -> 36. Refactor-candidate promise and transient spec-clause tags cut; invariants kept pinned to `{@link RecompileSet}`.
191. `graphitron/.../MultiTableChildReferencePathRejectionPipelineTest.java`: 46 -> 45. Stale "deferred capability" rules corrected to the live `@referenceFor` steer (verified against the FieldBuilder messages).
192. `graphitron/.../methodgraph/ReentryCommandClosureTest.java`: 46 -> 46. Slice numbering removed in place; oracle pins kept.
193. `graphitron/.../walker/ErrorChannelWalker.java`: 45 -> 23. Spec-signature substrate section and stages list cut; dedup contract pinned to `{@link MappingsConstantNameDedup}`.
194. `graphitron/.../RejectionKind.java`: 44 -> 43. Stale kebab-prefix format claim repointed to `#messageLabel`/`#displayName`.
195. `graphitron/.../generators/util/SelectionOccurrencesClassGenerator.java`: 44 -> 29. Per-method breakdown deduplicated onto the emitted javadoc.
196. `graphitron/.../TypedAccessorSchemaQualifiedIdentityPipelineTest.java`: 44 -> 30. Before/after compare narration restated present-tense.
197. `graphitron/.../IdReferenceShimClassificationTest.java`: 44 -> 43. Stale `ColumnField` names corrected to the live `ColumnBackedField`.
198. `graphitron-lsp/.../parsing/BundledLibraryLookup.java`: 44 -> 36. Platform table reduced to `{@link #resolvePlatform}`.
199. `graphitron/.../NodeIndex.java`: 43 -> 29. Build-mechanics recap cut; purity invariant kept pinned.
200. `graphitron-sakila-service/.../NestedFilmReviewPayloadHolder.java`: 43 -> 40. Pass-relative narration restated present-tense; compile-gate pin kept.
201. `graphitron-sakila-example/.../MultiTablePolymorphicJoinedCorrelationExecutionTest.java`: 43 -> 43. Slice labels renamed to their content.
202. `graphitron/.../TableNameComparisonCaseGuardTest.java`: 42 -> 32. Bug provenance restated; predicate pinned to `TableRef#sameTable`.
203. `graphitron/.../SourceShapeProjectionTest.java`: 42 -> 32. Code-block restatement reduced to the method link.
204. `graphitron/.../model/ParticipantCorrelation.java`: 40 -> 35. Rename narration and slice vocabulary cut; constructor restatements deleted.
205. `graphitron/.../VariantCoverageTest.java`: 40 -> 36. Milestone narration and plan-file citation cut.
206. `graphitron/.../QualifiedTerminalReferenceColumnPipelineTest.java`: 40 -> 35. Ambiguity-fix narration restated present-tense.
207. `graphitron/.../RetiredDimensionTypesAreGoneTest.java`: 42 -> 24. Pivot narration, spec quotes, and per-method walkthrough cut.
208. `graphitron/.../classifieddsl/DimensionTuple.java`: 40 -> 36.
209. `graphitron/.../generators/util/SelectMethodBody.java`: 36 -> 32. Dead federation-plan slug citation cut; ON-vs-USING contract pinned to a named execution test.
210. `graphitron/.../model/ResultKeyAliasedField.java`: 38 -> 38. Defect-history phrasing restated present-tense in place.
211. `graphitron/.../generators/schema/ConstraintViolationsClassGenerator.java`: 38 -> 17. Generated-body restatement cut; dangling spec-file pointer deleted; verify stage corrected a misattributed check to `{@link HandlerAccessorCheck}` (the one wrongful rewrite of the bout).
212. `graphitron-maven-plugin/.../DevDatabaseBinding.java`: 38 -> 38. No trimmable claim (env-override list and fail-closed contract verified live against `DevMojo`).
213. `graphitron/.../generators/ArgumentValueSource.java`: 37 -> 34. Status-quo narration cut; silent-failure rationale pinned via `{@link}` upgrades.
214. `graphitron/.../SynthesizeFkJoinReorderedKeysTest.java`: 37 -> 27. Regression-history paragraph cut; ordering invariant pinned to the jOOQ key accessors.
215. `graphitron/.../model/FkTargetConditionFilter.java`: 36 -> 36. Anecdote cut; composite-key claim upgraded to `{@link JoinPathEmitter#emitCorrelationWhere}`.
216. `graphitron/.../generators/schema/InputTypeGenerator.java`: 36 -> 32.
217. `graphitron/.../test/compile/EmittedCodeHarness.java`: 36 -> 26. Compiler-mechanics paragraph cut; seam rationale kept pinned.
218. `graphitron/.../classifieddsl/ClassifiedDsl.java`: 36 -> 32. Directive-signature restatement of `PRELUDE` cut.
219. `graphitron-jakarta-rest/.../AbstractGraphitronApplication.java`: 36 -> 34.
220. `graphitron/.../model/AccessorRef.java`: 35 -> 28. Duplicate cardinality prose cut; cardinality ownership pinned to `{@link KeyLift.Accessor#arity()}`.
221. `graphitron/.../generators/schema/GraphitronDevExecutorGeneratorTest.java`: 35 -> 34. Verify stage restored the distinct-types precondition on the compile-gate coverage claim (the one wrongful deletion of the bout).
222. `graphitron-sakila-service/.../AddressRecordService.java`: 35 -> 22. Phase-label provenance restated present-tense, pinned to the driving execution tests.
223. `graphitron/.../model/WhereFilter.java`: 34 -> 28. Paragraph citing four retired builder intermediates deleted (grep-verified gone); stale two-vs-three implementations count fixed.
224. `graphitron/.../RoadmapReferenceGuardTest.java`: 34 -> 31. Duplicated inline comment reduced to a pointer; otherwise no trimmable claim.
225. `graphitron-lsp/.../definition/DeclarationDefinitions.java`: 34 -> 31.
226. `graphitron-lsp/.../state/FileSnapshot.java`: 34 -> 31.
227. `graphitron/.../generators/util/PolymorphicSelectionSetClassGenerator.java`: 33 -> 26. Consumer coupling pinned to `{@link MultiTablePolymorphicEmitter}`.
228. `graphitron/.../model/TargetShape.java`: 32 -> 29. Slice promises restated as the present carry-no-payload fact; dead `mapping()` reference deleted.
229. `graphitron/.../generators/ChannelCatchArmEmitter.java`: 32 -> 11. Slice/commit narration and per-arm emitted-code bullets cut; arm contracts deduplicated onto `{@link ErrorChannel}` docs.
230. `graphitron-sakila-example/.../AddressOccupantsListBatchingTest.java`: 32 -> 19. N+1-history narration cut; exact-count rationale kept.
231. `graphitron-lsp/.../SdlActionDriftTest.java`: 31 -> 21. Hierarchy-collapse narration cut; guard contract pinned to `{@link SdlAction#targets()}`.
232. `graphitron/.../model/RoutineChain.java`: 30 -> 20. Extraction narration and invariant bullets restating throw messages cut; consumer coupling pinned to `{@link RoutineCallEmitter}`.
233. `graphitron/.../model/ConditionFilter.java`: 30 -> 20. Delegation restatements and extensibility promise cut.
234. `graphitron/.../generators/util/OneOfDirectiveSdlGenerator.java`: 30 -> 27. Future-proofing comment restated as the idempotence-guard contract; caller map upgraded to `{@link}`s.
235. `graphitron/.../schema/DirectiveSupportTypes.java`: 30 -> 30. Membership enumeration deduplicated onto the constant; pins kept.
236. `graphitron/.../model/PivotSpec.java`: 29 -> 28.
237. `graphitron/.../model/SourceKeyTest.java`: 29 -> 20. Slice-history dispositions cut; arm-to-wrap tables reduced to the `{@link KeyLift#wrap()}` derivation.
238. `graphitron/.../generators/schema/GraphitronClientExceptionClassGenerator.java`: 27 -> 18. Future-subtype promise deleted (the generated class is final); surfacing contract pinned to `{@link ErrorRouterClassGenerator}`.
239. `graphitron/.../generators/schema/GraphQLValueEmitter.java`: 27 -> 17. Supported-shapes list restating the instanceof chain cut.
240. `graphitron/.../NestedInputFieldEffectiveNonNullPipelineTest.java`: 26 -> 18. Bug narration restated present-tense; execution coverage pinned to a named live test.

Claims routed out of the sweep (follow-ons to file at Done): `GraphitronType.ResultType#fqClassName` null caveat vs non-null construction paths; `GraphitronType` InputType garbled "(or `@table`)" parenthetical; `TypeFetcherGenerator` L2085 comment contradicting `DSL.noCondition()` emission; `TypeFetcherGenerator` stale "Mapped is not produced yet" claims (x2) whose adjacent throw-message string literals repeat the stale claim; `RecordBindingResolver#fromAnyProducer` is dead API (zero callers). From bout 2: `JooqRecordInstantiationEmitter#openDescent` javadoc's graphql-java nested present-null coercion claim (load-bearing external-library behavior with no named execution test to pin to); `ScalarTypeResolver#resolveFromDirectiveValue` javadoc's rationale citing a per-arm LSP `ClassNotFound` fix-it that does not exist in `graphitron-lsp` main sources. From bout 3: `JoinPathEmitter#emitCorrelationWhere` javadoc's claim that the empty-slot fallback's emitted `DSL.noCondition()` stub is runtime-throwing (behavioral claim about generated output with no named pin); `MultiTablePolymorphicEmitter` (two comments) and `RowsMethodCall` still cite the removed `buildSplitQueryDataFetcher`/`buildRecordBasedDataFetcher` methods (the former survived the bout-1 pass; the latter is below the census cutoff). Also for the generated-output item: "slice" mentions inside `ConnectionRuntimeClassGenerator` `.addJavadoc` string literals render into generated output. From bout 4: `BuildContext`'s two `§`-numbered channel-rule comments (rule-family names pinned to fixtures, but the numbered rule spec they cite resolves to no live artifact); `Source.OnlyChild`'s row-correctness contract (load-bearing, explicitly marked as machine-unenforced; needs an enforcer or a pin). String-literal findings for the generated-output item (untouchable by this sweep's comment-only constraint): emitted javadoc in `ErrorRouterClassGenerator`, `FieldBuilder`, and `WithErrorChannel` cites the dead `error-handling-parity.md` spec file and carries future-work phrasing; `GraphitronConnectionInstrumentationGenerator`'s emitted incremental-delivery rejection message says "it is a named follow-on"; `BuildContext` emits "file a roadmap item if this shape needs admission" / "a future Backlog item may admit multi-data carriers" diagnostics. Test-source note: `ClassifiedDslTest`'s `OPERATION_KNOWN_GAPS` map values cite roadmap ids in string literals (out of the guard's string-scan scope by design). From bout 5 (no comment-region claims were routed), string-literal findings for the generated-output item: `ChannelCatchArmEmitter`'s invariant-throw message narrates its own deletion plan ("until it is deleted in slice-1 commit 4"); `GraphitronClientExceptionClassGenerator`'s emitted javadoc references a future query `@error` handler; `ConstraintViolationsClassGenerator`'s emitted javadoc literals restate its class doc at length. Further test-source notes (guard-exempt by design): assertion/display literals citing roadmap ids in `TableNameComparisonCaseGuardTest`, `SourceShapeProjectionTest` (`NOT_CORPUS_COVERED`), `VariantCoverageTest` (`NO_CASE_REQUIRED`), `RetiredDimensionTypesAreGoneTest`, `AddressOccupantsListBatchingTest`, and `IdReferenceShimClassificationTest`; `TypedAccessorSchemaQualifiedIdentityPipelineTest`'s fixture producer method names embed an item id in code identifiers.

Follow-ons for every routed claim are filed: comment-drift corrections in `roadmap/generator-comment-drift-corrections.md`, unpinnable-contract enforcement in `roadmap/unpinned-comment-contract-enforcement.md`, and the emitted-text findings appended to `roadmap/generated-output-hygiene-sweep.md`. Acceptance census re-run (context only, no numeric gate): before the sweep, 58917 comment lines across the in-scope trees (24.9% of 236187 source lines) with 241 files over the worklist cutoff; after bout 5, 54838 comment lines (23.6% of 232087) with 161 files over the cutoff.
