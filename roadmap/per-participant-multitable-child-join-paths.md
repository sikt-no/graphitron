...
**Shipped.** The classifier lowers a multi-hop route to `JoinedCorrelation` (`FieldBuilder.classifyParticipantRoute`); the emitter declares one aliased local per intermediate hop table (`declareBranchAliases`), bridges them back toward the parent with `JoinPathEmitter.emitBackwardBridging` (`branchBridgingJoins`), and value-binds hop-0 against `parentRecord` (single) or `parentInput` (batched). All three cardinality forms share the bridging emit. Execution fixture: `Film.membersViaJunction` / `firstMemberViaJunction` / `membersViaJunctionConnection` reach the `actor`-backed participant through `film -> film_actor -> actor`, with an auto-discovered `inventory` sibling keeping the field multi-table (`MultiTablePolymorphicJoinedCorrelationExecutionTest`).
...
**Shipped.** A route carrying any `On.Predicate` hop lowers to `JoinedCorrelation`; when hop-0 is the predicate the emitter joins the aliased parent table (`branchParentAlias` / `declareBranchAliases`) on the two-arg condition (`branchBridgingJoins`, via `emitTwoArgMethodCall`) and pins the parent alias to the parent's bound key (`parentKeyBoundWhere` for the single form, `parentInputKeyPredicate` for the batched forms). Intermediate `filter:` hops (`{key:, condition:}`) are ANDed onto the branch WHERE (`hopFilterTerms`); a filter on hop-0 has no parent alias to bind an FK route against and is out of the emittable set. Execution fixture: `Film.castByCondition` / `firstByCondition` correlate the `actor` participant with `filmActorsViaCondition` (an `EXISTS` over `film_actor`), auto-discovered `inventory` sibling (`MultiTablePolymorphicJoinedCorrelationExecutionTest`). Every shape `@referenceFor` can state is now emittable.
