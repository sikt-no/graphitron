...
The change is a fold, and it needs no new capture. The view joins `jvm_method` per descriptor, so an overload set is already several rows, and the outer `UNION` already collapses declarations that arrive at the same table, which is why an agreeing set needs nothing: it resolves to one route today and keeps doing so. Only the disagreeing set moves.

- **Arrival disagreement suppresses the route.** Where a `(graph_name, class_name, method)` pair has arrival rows naming more than one distinct `sql_table`, the pair routes nothing, matching `resolveConditionJoinTarget`'s rejection on a bound slot whose `TableRef`s disagree. The guard sits on the existing `arrival` CTE, whose key is already the pair; nothing else in the view moves.
- **A sixth verdict, and the vocabulary stays closed and total.** `intent_condition_method_route_defect` gains `TARGET_DISAGREEMENT_ACROSS_OVERLOADS`, ordered after `WILDCARD_TARGET_PARAMETER` and before the `TARGET_NOT_A_TABLE_CLASS` fall-through, so a set whose slots mix a wildcard with concrete tables keeps reading as the wildcard case on both sides (the resolver's own reading, per the admission deliverable). The relation's totality claim survives by construction: a pair the route view now suppresses lands in the defect population the same query already drives.
- **The stale rationale goes with it.** The route view's no-return-type-guard sentence and its `method` column comment ("two overloads of it are two routes here") both assert the pre-admission rule and are rewritten in the same commit, as is the `WILDCARD_TARGET_PARAMETER` line in the defect view's verdict comment where it explains itself in terms of what the resolver refuses. The DDL is the model's source of truth and the schema reference renders from these comments, so a comment asserting a resolver behaviour that no longer holds is a defect in the model, not documentation drift.

What stays out is the rest of the admission rule, and the reason is capture rather than preference. `jvm_method` carries `return_type`, `declared_return_type` and `returns_condition`, but no static flag and no declared-exception list, so the binding-shape agreement the admission deliverable demands is not expressible over the census as it stands. That leaves a set disagreeing on shape rather than on arrival still routing in the store while the build rejects it with `AmbiguousMethod`. It is a real gap and it is pre-existing: the store is already more permissive than the generator there, since today *any* second declaration is an `AmbiguousMethod` and the store routes a pair whose other declaration simply contributes no arrival row. Admission narrows that gap rather than widening it, which is why closing it belongs to whoever next widens the census, not here (see out of scope).
...
- **Census-side route agreement**: model tier, against a seeded store, which is where what a view returns given rows is pinned. `ConditionMethodRouteTest` gains the disagreeing set (two declarations of one name arriving at different tables) asserting no route, beside its existing agreeing-set case; `ConditionMethodRouteDefectTest` gains the `TARGET_DISAGREEMENT_ACROSS_OVERLOADS` case and re-asserts the pairing case that makes the vocabulary total, so a suppressed route is named exactly once. The mixed wildcard-and-concrete set belongs here too, pinning that both sides still read it as `WILDCARD_TARGET_PARAMETER`, because that is the case where the two rules could most easily be spelled differently in the two modules.
...
- Mirroring the *binding-shape* half of admission over the census: `jvm_method` carries no static flag and no declared-exception list, so the shape agreement the admission deliverable demands is not expressible over the store as captured, and a set disagreeing on shape rather than on arrival keeps routing there while the build rejects it. Pre-existing and narrowed rather than widened by admission (see the census deliverable), so it belongs to whoever next widens the census, with the shape rule as the statement to capture against.
...
- The census-side route agrees with the resolver on every set the store can judge: an agreeing set routes as it does today, a set disagreeing on arrival routes nothing and is named once by `TARGET_DISAGREEMENT_ACROSS_OVERLOADS`, a mixed wildcard-and-concrete set reads as the wildcard case on both sides, and the defect vocabulary stays closed and total over the unrouted population. The route and defect views carry no sentence asserting the pre-admission rule.
