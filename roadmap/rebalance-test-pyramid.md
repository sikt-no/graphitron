...
**Correction found in implementation.** The premise holds for `destFile` and `dataFile` but is false for `outputDirectory`: the report mojo's `plugin.xml` binds `${jacoco.destFile}` and `${jacoco.dataFile}` but gives `outputDirectory` no property expression at all, so `-Djacoco.outputDirectory` is silently ignored and the tier report overwrites the combined one at `target/site/jacoco`. Repaired in the spirit of this decision rather than its letter: the root pom declares a `jacoco.outputDirectory` property defaulting to the mojo's own default (`${project.reporting.outputDirectory}/jacoco`) and the `pluginManagement` entry routes the parameter through it, which makes the recipe's `-Djacoco.outputDirectory` work as written because user properties override project properties. Verified by producing `jacoco`, `jacoco-unit` and `jacoco-pipeline` side by side.
