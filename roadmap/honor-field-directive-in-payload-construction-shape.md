...
2. **All-fields-ctor arm.** Ctor selection by parameter arity is name-independent and unchanged. The directive is load-bearing only at the errors-slot location:
...
1. **Gathering stays builder-internal.** In `resolveErrorChannel` (`FieldBuilder.java:2977-2980`), replace the `sdlFieldNames` name list with a per-field carrier, a FieldBuilder-internal `record PayloadSdlField(String sdlFieldName, String javaBaseName)` where `javaBaseName` is the house idiom `argString(f, DIR_FIELD, ARG_NAME).orElse(f.getName())`. `resolvePayloadConstructionShape`'s signature changes from `List<String>` to `List<PayloadSdlField>` (`null` still admitted for the skip-setter-predicate case). This is deliberately *not* the R202 carry-on-the-model pattern: the sole consumer is this one classify-time site, and what lands on the model is the resolved artifact (the setter `Method` on `SetterBinding`, the int on `ErrorsSlot.CtorParameterIndex`), never the carrier itself. Architect-reviewed: promoting it to a model type would leak gathering scaffolding into the model.
...
- **Pipeline** (`ErrorChannelClassificationTest`):
  ...
  4. Rejects: unresolvable directive value on the ctor arm; directive on the errors field of a name-less POJO; blank `@field(name: "")`.
