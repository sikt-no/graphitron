...
`reset` was the only caller of `declarePartitionSelectivity`, a test-side restatement of the sweep
`GraphitronModelStore` runs at creation. Once `reset` restores the partition column itself, nothing
calls the restatement, so it is removed, and the private `resetEveryColumn` it sat beside folds into
its one remaining caller, `resetIncludingTheDeclaration`. No behaviour changes: `reset`'s javadoc now
names the creation-time declaration over `GraphPartition.keyedBaseTables` as the end state it matches.
