...
- **Call sites.** The ~25 `Rejection.deferred(...)` sites drop the slug argument; each summary must stand alone as a fact once the suffix is gone (most already say "... is not yet supported"). Two non-factory constructors also pass a slug and must be swept: `TypeFetcherGenerator.deferredFor(...)` (`:332`) and its `STUBBED_VARIANTS` `CompositeColumnReferenceField` entry (slug `nodeidreferencefield-join-projection-form`), which construct `Rejection.Deferred` directly. Remove the `FieldBuilder` DML-pair slug-threading (`PairVerdict.Deferred(planSlug)`, the `deferredSlug` fold). A few of these summaries also name a roadmap slug or `R<n>` ID *in the prose itself*, so dropping the argument alone leaves the transient reference in the rendered message: `MutationInputResolver` (`:369`) reads "... are designed under R145 (mutation-cardinality-safety-upsert)" and passes the same slug as the arg. At every deferred site the item edits, rewrite the summary to state the fact with no slug and no `R<n>` (for `:369`, something like "UPSERT is not supported under the cardinality-safety regime and is not yet designed"). This in-message purge is what the item title's "message literals" clause names, and it is what makes the `no roadmap path` line under Done true at these sites.
...
The guard matches the `roadmap/` substring, which pins the composed-path shape but cannot catch a bare slug or `R<n>` token dropped into a summary (a hyphenated slug is not distinguishable from ordinary prose). Those in-prose references are removed by the call-site edits above and caught in review, not by the guard. A cheap, tractable addition the implementer may fold in: assert that no `Deferred.message()` render contains a `\bR\d+\b` token; that arm is the one this item hardens to a pure function of `summary`, so the assertion is satisfiable once `:369` is cleaned and catches an in-prose `R<n>` regression there.

## Done

- `planSlug` removed from the model and every call site; `message()` renders `summary` only.
- The deferred summaries that named a slug or `R<n>` in the prose itself (e.g. `MutationInputResolver:369`) are rewritten to carry neither, so the rendered message holds no roadmap path, slug, or `R<n>`.
...
- In-prose roadmap references (`R<n>` IDs and slugs) in *non-deferred* message literals: `Rejection.structural(...)` prose (including the UPSERT-deferral twins at `FieldBuilder:4701` and `MutationField:609`, which restate the same fact `:369` carries), walker and `IllegalStateException` throw messages, and `@Disabled` test annotations. This is a broad, heterogeneous habitat spanning contributor-facing throws and test scaffolding, not the author-facing deferred-rejection channel this item governs; it is better swept uniformly in one literal-purge sibling than piecemeal here. Consequence accepted for now: the UPSERT deferral renders with the slug on the structural paths (`:4701`, `:609`) and without it on the deferred path (`:369`) until that sibling lands.
