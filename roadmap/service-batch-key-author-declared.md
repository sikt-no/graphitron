...
* **Pipeline**, the `Sources`-less rejection: one fixture per parent kind (table and record), each asserting an `UnclassifiedField` naming the coordinate and the missing `Sources` parameter. The record-parent one is also the pin for the root-branch flip's residue, since it is the coordinate that would otherwise have lost its validation regime. A Connection-returning record-parent child `@service` needs its own fixture on the other side of the flip, because the root arm is what rejects it today and `validateChildServiceReturnType` does not; assert the verdict the child path now gives it rather than assuming the root message survives.
