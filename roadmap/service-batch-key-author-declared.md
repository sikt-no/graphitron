...
The direction is right; these are child coordinates and were only getting root treatment by the collapse this section removes. The hazard is that the replacement is not equivalent on the `Sources`-less case: `validateChildServiceReturnType` returns immediately when the method declares no `Sourced` parameter, so a record-parent child `@service` without `Sources` would go from strict root return-type comparison to no return-type validation at all, and a Connection-returning one from rejected to unchecked on that axis. The classify-time rejection settled below closes this: the coordinate that would have lost its validation regime stops being constructible, so there is no unvalidated residue for the flip to strand. The pins are in the Tests section, in both directions.
...
* The new component inherits `sourceKey`'s nullability: all four classify sites thread `sourced == null ? null : buildServiceSourceKey(sourced)`, so a `@service` field whose method declares no `Sources` parameter carries a null key and would carry a null key source, leaving `sourceShape()` nothing to derive from. **An invariant throw is the wrong instrument here, and the reason is a seat mismatch.** `sourceShape()` is not a generation-only surface: `GraphitronSchemaValidator.validateListRequiresOrdering` reaches it through `OutputField.requiresReFetch()` on every `SqlGeneratingField`, and `ServiceTableField` is one (`TableTargetField extends SqlGeneratingField`), so the cross-cutting check that runs *after* the leaf switch would hit the throw on the very schema whose only defect is the missing `Sources` parameter, converting the author-facing rejection this item adds into a generator crash. `CatalogBuilder`, `ProjectionCommands`, `DeliveryFactRelation` and `OperationMemberRelation` read it outside validation too. `ServiceRecordField`'s own `sourceKey` javadoc states the real guarantee and it is narrower than the one an invariant throw needs: "generation runs only post-validation, so it never observes null", which says generation, not validation.
  ...
  **Settled: the `Sources`-less child `@service` is rejected at classify time, on both parent kinds.** The leaf then never exists with a null key, both components are non-null by construction, and the derivation is total with no null branch to place a throw in. See "The `Sources`-less child is a classify-time rejection" below for why this is the tree's own answer rather than a choice this item invents.

## The `Sources`-less child is a classify-time rejection

A child `@service` whose method declares no `Sources` parameter has no batch key, and the tree has already decided that this is an error rather than a shape: `validateServiceTableField` rejects it by name on the table-bound leaf ("requires a Sources parameter for DataLoader batching"). What is missing is the mirror on `ServiceRecordField` and the seat.

The mirror gap is not a deduction from the code, it is recorded in the code. `LauncherCommands.serviceRecordRow` opens with a production guard whose message says the coordinate "has no Sources parameter, so no DataLoader key exists; the validator accepts this shape today (a recorded mirror gap; the table-bound sibling is rejected) and no emission exists for it", and the surrounding javadoc says outright that the guard exists to keep the gap loud "until a validator rejection lands". This item is where it lands. `ProjectionCommands`' `ServiceRecordField` arm reads the same fact more tolerantly (a null `sourceKey` projects no columns), but tolerating it in the projection walk does not make it emittable; the launcher still throws.

**Why classify time and not the validator the guard asks for.** A validator rejection alone does not close the hole this item opens, because the leaf is already constructed by then and `sourceShape()` runs during validation (the seat argument above). Rejecting at classify time makes the null key unconstructible, which is what the derivation needs. The validator checks stay anyway, as the mirror rather than the primary, which is the house pattern under "Validator mirrors classifier invariants" (cited at `FieldBuilder`'s polymorphic child-`@service` arm, at `GraphitronSchemaValidator`, and in `ServiceRecordCompositeCarrierValidationTest`'s javadoc). So `validateServiceTableField` keeps its check and `validateServiceRecordField` gains the one it never had; both become classifier-guaranteed rather than load-bearing.

The seat is the four classify sites where `sourced == null ? null : buildServiceSourceKey(sourced)` is threaded today (one in `classifyChildFieldOnResultType`, three in `classifyChildFieldOnTableType`). It is deliberately *not* a `ParentKeyResolution` arm: whether the method declares `Sources` is a post-reflection fact about the signature, while `ParentKeyResolution` answers a pre-reflection question about the coordinate. Folding them would put the coordinate answer back downstream of the reflection loop, which is the shape the coordinate section rejects.

**Scope of the non-null pin.** It belongs on `ChildField.ServiceTableField` and `ChildField.ServiceRecordField` only, not on `BatchKeyField`. The root leaves are not in the interface at all (`QueryField.QueryServiceRecordField` and its mutation twin implement `ServiceField`, carry no `sourceKey` component, and must not have `Sources` by `validateRootInvariants`), and `BatchedPivotField` carries a legitimately null key that `ProjectionCommands` reads. A `requireNonNull` on each of the two components replaces the `(sourceKey == null) == (keySource == null)` biconditional the earlier draft proposed.

Two live null branches retire with the shape, and they are the check that the rejection actually landed: `LauncherCommands.serviceRecordRow`'s first production guard becomes unreachable by construction, and `ProjectionCommands`' `ServiceRecordField` arm collapses to the plain `sr.sourceKey().columns()` read. The second guard in `serviceRecordRow` (the backing-less result return) is a different recorded gap and stays.

What the author sees does not change in severity, only in which phase names it: both classify rejections and validation errors fail the build. On a table parent the message moves from the validator to the classifier and the validator's copy stops being the one that fires.

## Implementation

* `model/ServiceKeySource.java`, `model/ParentKeyResolution.java` (new). The two sealed facts above.
* **Landed early, by the precedence item.** `ServiceCatalog` is split into `decodeServiceMethod` / `reduceClaims` / `bindServiceMethod`, `ServiceDirectiveResolver.ParentContext` is the coordinate input (`Root` / `TableParent` / `RecordParent`), `ServiceCatalog.PkLessParent` is deleted, and the `resolve` javadoc is rewritten. What is left for this item: compute `ParentKeyResolution` in the resolver's classify phase from `ParentContext` plus the decoded candidate's element class, and feed `Available.source().keyOwner().primaryKeyColumns()` to `bindServiceMethod`'s `parentPkColumns` parameter in place of the coordinate's own key columns (and retire that parameter name with the semantic change, per this item's retired-vocabulary list). Place `ParentKeyResolution` beside `ParentContext` on the resolver rather than under `model/` (its `Rejected(Rejection)` arm is the tell that it is resolver-internal gathering scaffolding; only `ServiceKeySource` reaches a leaf component). The four validations that change hands are enumerated in the coordinate section above; they are now a one-arm change on `ServiceDirectiveResolver.regimeOf`'s `RecordParent` case, mapping it from `STRICT_ROOT` to `BATCHED_CHILD`, and the `Sources`-less residue still has to be answered before that arm flips.
* `ServiceCatalog`: a `Set<XRecord>` / `List<XRecord>` element whose class does not resolve through `resolveTableByRecordClass` keeps its existing diagnostic; `dtoSourcesRejectionReason`'s trailer, which currently points at `@sourceRow` as the analogous solution for record-backed parents, is repointed at the typed-record route this item creates. Note that the DTO-sources diagnostic is gated on `bindServiceMethod`'s `parentPkColumns` being non-empty today, so it does not fire on a record-backed parent at all; re-expressing that gate against `ParentKeyResolution` widens its reach to the coordinates this item admits, which is the intended reading and should be stated in the replacement rather than inherited by accident.
...
* `ServiceDirectiveResolver`'s `RecordParent` classify arm: replace the hoisted deferral with the `ParentKeyResolution` computation. `FieldBuilder.classifyChildFieldOnResultType`'s `DIR_SERVICE` branch: the `Result` and `Scalar` arms are invariant throws today (the resolver rejects the pairing before they can be reached) and become real `ServiceRecordField` projections, the way the `TableBound` arm already builds `ServiceTableField`. The `Polymorphic` arm's deferral also lives in the resolver now and stays there (doubly out of scope, unchanged).
* `ServiceDirectiveResolver.validateTableRecordSourceParentTable`: already gated on the `TableParent` classify arm by the precedence item, so this bullet is pre-done. On the record-parent path the element type is the resolver's input and the check is vacuous there, so nothing further is owed.
* The `Sources`-less classify-time rejection at the four `buildServiceSourceKey` sites, per the section above. `LauncherCommands.serviceRecordRow`'s no-Sources production guard and `ProjectionCommands`' null-`sourceKey` branch on the `ServiceRecordField` arm retire with it; `serviceRecordRow`'s second guard (backing-less result return) is a separate recorded gap and stays.
* `ChildField.ServiceTableField` / `ChildField.ServiceRecordField`: add one `ServiceKeySource` component, non-null, alongside a now-non-null `sourceKey` (the pin is on these two leaves, not on `BatchKeyField`). No `KeyLift`, and no component named `sourceShape`: `ChildField.sourceShape()`'s default projection already answers `SourceShape.Table` for both leaves, and a same-named record component would silently override it and leave those arms dead. Update the two arms to derive from the new component, per the source-shape section above.
...
* **Pipeline** (`ServiceCoordinatePrecedenceTest`, the precedence item's class): the three `ParentKeyResolution` arms, including that a `Root` resolution still reaches the batch-at-root diagnostic and a `Rejected` resolution surfaces its own rejection rather than the argument-name mismatch. Not `ServiceCatalogTest`: after the phase split the catalog decides no coordinate question, so these pins are pipeline-shaped. The `Rejected` case is the pin that this item did not re-mask what the precedence item unmasks.
* **Pipeline**, the `Sources`-less rejection: one fixture per parent kind (table and record), each asserting an `UnclassifiedField` naming the coordinate and the missing `Sources` parameter. The record-parent one is also the pin for the root-branch flip's residue, since it is the coordinate that would otherwise have lost its validation regime. A Connection-returning record-parent child `@service` needs its own fixture on the other side of the flip, because the `STRICT_ROOT` regime is what rejects it today and `validateChildServiceReturnType` does not; assert the verdict the child path now gives it rather than assuming the root message survives. Note that the precedence item could not build that fixture: a Connection-shaped type returned from a record-backed parent's field classifies both as a `ConnectionType` and as a record carrier, so the type itself rejects before the field reaches the resolver. Budget finding a schema shape that gets past that, or record why the arm stays unpinned. The strict-return-comparison half of the same flip *is* pinned, by `ServiceCoordinatePrecedenceTest.recordParent_noBatchParameter_keepsTheStrictReturnTypeComparison`, which is the "before" side this item flips.
* **Unit**: direct-construction sites for the two service leaves (`TypeFetcherGeneratorTest` builds both) have to supply a non-null `SourceKey` and `ServiceKeySource` once the compact constructors pin them, and one case should assert the pin fires rather than only satisfying it.
...
* `ServiceCatalog.PkLessParent` (already retired by the precedence item)
...
* "the validator accepts this shape today (a recorded mirror gap; the table-bound sibling is rejected)" and "until a validator rejection lands" (`LauncherCommands.serviceRecordRow`'s no-Sources guard and its javadoc, both retired by the classify-time rejection)
* "A null sourceKey is a no-Sources service method: plain per-parent delegation, nothing to project" (`ProjectionCommands`' `ServiceRecordField` arm)
