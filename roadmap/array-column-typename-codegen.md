...
Landed the boundary type-lift as specified: `ColumnRef` and `JooqCatalog.ColumnEntry` gained a `TypeName columnType` component, decoded once via `TypeName.get(col.getType())` at the three jOOQ reflection sites (`allColumnsOf`, `findColumn`, `allColumnRefs`), threaded through every `new ColumnRef(...)` / `new ColumnEntry(...)` construction site, and the 31 `ClassName.bestGuess(<ref>.columnClass())` codegen sites now read `<ref>.columnType()`. The raw `columnClass` string is untouched, so the binary-form consumers (`EnumMappingResolver`, `SourceRowDirectiveResolver`, `GraphitronSchemaValidator`) keep reading it. The `SourceKey:207` `(TypeName)` cast is dropped. Line numbers had drifted from the Spec (e.g. `ChildField` `1072`->`1104`, `TypeFetcherGenerator` `1735`/`2825`->`1716`/`2806`); the grep counts (31 sites, 20 construction sites) held.

Two decisions beyond the letter of the Spec:

* **`DomainReturnType.Plain(ClassName)` widened to `Plain(TypeName)`.** The `ChildField` sites the Spec lists among the 31 feed `new DomainReturnType.Plain(...)`, whose component was `ClassName`; converting them to `columnType()` (a `TypeName`) forced the widening. The Spec's "Return-type widening" enumeration omitted `Plain`. Safe for the R204/R279 multi-producer domain-type-agreement check: javapoet `TypeName.equals`/`hashCode` are defined on `toString()`, so a scalar `ClassName` keeps FQN-string equality and arrays get array-string equality; `toString()` changed from `javaClass.canonicalName()` (a `ClassName`-only method) to `javaClass` (`TypeName.toString()`).
* **A 3-arg auxiliary constructor on `ColumnRef` / `ColumnEntry`** derives `columnType` from the source-form `columnClass`, to avoid churning ~161 hand-built test fixtures. This is not the rejected production string-parse helper: all production/boundary construction uses the 4-arg form with the real `TypeName.get(Class)`; the 3-arg form is test-only and returns a `null` `columnType` for the synthetic placeholder `columnClass` values some fixtures pass (empty string, `related_1`, `film_pkey`), which are never emitted and whose type is never read. Array columns never reach it. Two enforcers close the drift a convention-only "test-only" label would leave (per a principles self-check): both auxiliary constructors route through one shared decode `ColumnRef.bestGuessScalarTypeOrNull` (so they cannot diverge on placeholder tolerance), and `ColumnTypeConstructorArityGuardTest` scans the rewrite main-source tree and fails the build if any production `new ColumnRef(` / `new ColumnEntry(` omits the decoded `columnType` (i.e. regresses to the string-decoding form).

Validation rejection: both key arms landed. `@node` array-key columns are rejected in `GraphitronSchemaValidator.validateNodeType`; DataLoader / `@splitQuery` array batch-key columns are rejected in `validateField` over the `BatchKeyField.sourceKey().columns()` set. Detection is `columnType instanceof ArrayTypeName` in both; the message names the type/field, the offending column, and the reference-identity reason. No classifier plumbing was needed (the keys are already resolved on `NodeType.nodeKeyColumns()` / `SourceKey.columns()`), so the crash fix and rejection landed together as one item.

Tier realisation vs. the Tests plan:

* **Boundary decode (unit)** — `ArrayColumnTypeDecodeTest`: `columnType` is a `ClassName` for scalars (`id`, `label`) and an `ArrayTypeName` of the right element for `flags boolean[]` / `tags text[]`, with `toString()` `java.lang.Boolean[]` (never the raw descriptor); `columnClass` stays the binary descriptor for arrays.
* **Regression pin (pipeline)** — `ArrayColumnCodegenPipelineTest`: drives `TypeFetcherGenerator.generate` (the generator that reaches `buildKeyExtraction`, not `TypeClassGenerator`) over an `array_holder` parent with a `TableRecord`-wrap `@service` child; asserts no crash. Verified it reproduces `IllegalArgumentException: couldn't make a guess for [Ljava.lang.Boolean;` when the crash site is reverted.
* **Rejection (pipeline)** — `ArrayKeyColumnRejectionValidationTest`: the `@node` array-key arm plus a scalar-key control. The DataLoader batch-key arm shares the same `ArrayTypeName` detection but is unconstructable from a real schema (PostgreSQL disallows an array column as an FK target, so no DataLoader keys on one), so there is no fixture for it.
* **Structural guard (unit)** — `ColumnTypeConstructorArityGuardTest`: enforces that production construction of `ColumnRef` / `ColumnEntry` passes the decoded `columnType`, so the test-only string-decoding auxiliary constructor cannot silently reappear in production and reintroduce the crash.
* **Compilation / execution tiers** — not added via `graphitron-sakila-example`: the reachable crash is the internal full-row reconstruction, and array columns are not surfaced as GraphQL fields in that schema, so there is no low-risk way to route an array column into the example's compiled output. The emitted-form validity the compile tier would guard against (a raw descriptor instead of `Boolean[]`) is pinned directly by the boundary-decode `toString()` assertion.

Fixture: new `public.array_holder` table in `graphitron-sakila-db/src/main/resources/init.sql` (`id serial` PK, `flags boolean[]`, `tags text[]`, `label varchar`); jOOQ fixture schema version bumped `2.7`->`2.8`. New `TestFilmService.getArrayHolderRankByRecord` service stub keyed on `Set<ArrayHolderRecord>`. Full reactor build (`mvn install -Plocal-db -P!docs`) green across all modules.
