...
* **A 3-arg auxiliary constructor on `ColumnRef` / `ColumnEntry`** derives `columnType` from the source-form `columnClass` via a guarded `bestGuess`-or-null, to avoid churning ~161 hand-built test fixtures. This is not the rejected production string-parse helper: all production/boundary construction uses the 4-arg form with the real `TypeName.get(Class)`; the 3-arg form is test-only and returns a `null` `columnType` for the synthetic placeholder `columnClass` values some fixtures pass (empty string, `related_1`, `film_pkey`), which are never emitted and whose type is never read. Array columns never reach it.
