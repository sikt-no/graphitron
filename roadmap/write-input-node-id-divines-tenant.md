...
Shipped at `4bb378e`, with the round 2 and round 3 reworks following it. Neither rework changes the design. Round 2 adds evidence and updates the record, and makes the one generated-code fix the evidence turned up (the mixed-tenant refusal reaches the client, see Tests). Round 3 pins the reach arm added in delivery and corrects its stated reason (see Scope added in delivery). Where the delivery departs from the plan below, the paragraph says so in place.
...
**Scope added in delivery: the write target counts as reached.** `reachedTables` gains an arm for `MutationField.DmlTableField` that adds `f.write().table()` to the coordinate's reach. This is a separate bug fix riding along with the item, not something the goal depends on. `armOf` returns `ArgumentBound` as soon as a slot divines, before it reads the reach, so a write that binds its tenant routes with or without the arm. What the arm closes is the unbound case. A DML write whose return is an encoded id (`: ID`, `: [ID!]!`) has no `Record` return target, so before this arm such a mutation reached no table as far as the fold could tell. One that wrote a tenant-scoped table and bound no tenant classified `Untenanted` and ran on the default source: a silent cross-tenant write. It rides this item because the item's fixtures are the first id-returning tenant-scoped writes, which is where the hole showed. The arm also lets the SET-partition decline fire on an UPDATE that returns an id, since declines are only raised where the statement reaches a tenant-scoped table. `idReturningWriteToATenantScopedTableWithNoBindingRejects` pins it, and fails with the arm removed. It changes behaviour for consumers: a tenant-scoped DML write that returns an id and names no tenant built before and now rejects with `NoTenantBinding`. The manual already states the rule this enforces ("A tenant-scoped field that nothing routes is a build error, never a silent read of the wrong database"), so the manual is not changed. Before this arm the sentence was false for id-returning writes. A consumer who hits the new rejection is looking at a write that was going to the wrong database.
...
- *The reach arm added in delivery has its own case.* `idReturningWriteToATenantScopedTableWithNoBindingRejects` is an `inventory` DELETE returning `ID` whose input names no tenant; it rejects with `NoTenantBinding`, and classifies `Untenanted` with the `DmlTableField` arm removed. Added in the round 3 rework; see Scope added in delivery above.
