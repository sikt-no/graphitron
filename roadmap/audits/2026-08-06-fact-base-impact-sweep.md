...
| R569 mcp-aggregated-diagnostics | Consumer | Forward note only: pivot dimensions become violation-relation columns; the schema channels' aggregation becomes GROUP BY post-migration. (Superseded 2026-08-07 by user decision: R569 is now the store's first reader; its aggregate is a jOOQ query over a `diagnostic` union view whose arms are bridge relations, later replaced by R589's violation relation and R603 `pipeline-output-facts-family`'s compile home. The Java grouping engine is never built; the item's first-reader section carries the design.) |
