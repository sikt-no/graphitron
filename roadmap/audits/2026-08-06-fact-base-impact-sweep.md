...
| R569 mcp-aggregated-diagnostics | Consumer | Forward note only: pivot dimensions become violation-relation columns; the schema channels' aggregation becomes GROUP BY post-migration. (Applied 2026-08-06, with one correction: the compile channel is javac output over generated Java, produced after capture, so it reaches the store only via the pipeline-output family filed the same day as R603 `pipeline-output-facts-family`; until then the three-channel union seam and the wire contract outlive the migration and only the grouping engine retires.) |
