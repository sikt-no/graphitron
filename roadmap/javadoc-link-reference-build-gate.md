...
- **Positive evidence it actually ran, per in-scope module.** A javadoc gate has the same vacuous-pass failure mode `RoadmapReferenceGuardTest`'s `MIN_SCANNED_FILES` floor guards against: a module that silently skips the goal (an inherited `maven.javadoc.skip`, an empty `sourcepath`, a misbound phase) reports zero warnings and greens having checked nothing. The gate must carry evidence each in-scope module ran the check, not merely "no errors reported". Coverage is a subset of `RoadmapReferenceGuardTest.IN_SCOPE_MODULES`: `sourcepath=src/main/java` correctly excludes generated sources under `target/`, so the gate audits hand-authored links only. Two modules are structurally exempt and opt out with `maven.javadoc.skip`: `graphitron-sakila-db` (jOOQ-generated only, no hand-authored main sources) and `graphitron-sakila-example` (its one hand-authored source imports generated types not on the `src/main/java` sourcepath, so javadoc cannot resolve them; pulling generated sources in would break the hand-authored-only scope). `JavadocReferenceGateTest` pins both the covered set and the exemptions.
