...
**Reconcile with R580 before either lands.** R580 is at `Ready`, bounced from In Review over a docs finding, so its user-manual drafts are undelivered and still describe a warning: its "Draft: the warning text" section reads "Graphitron is using the node ID. Add `@nodeId` to confirm, or `@field(name: "id")` to expose the raw column instead", and its `node.adoc` subsection documents the shape as a warning an author may ignore. Those become error text. R580 also recorded that `docs/manual/reference/diagnostics-glossary.adoc` is error-only by construction and that the warning therefore had no glossary-shaped home; as an error it does, which turns R580's noted gap into a glossary entry this item should add. The two items must not ship disagreeing on whether this breaks the build.
