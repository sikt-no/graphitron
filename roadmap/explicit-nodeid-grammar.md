...
**A node-id reading that collides with a real column of the same name resolves to the node, emits `LintRule.NODE_ID_SHADOWS_COLUMN`, and is disambiguated by the author writing `@field(name:)` for the column or `@nodeId` for the node.** This holds identically for output fields, input fields and arguments. The rule is deliberately uniform: shadowing is one question, and an author who learns the answer at one coordinate must not guess wrong at the next.

R580 already shipped exactly this at the output coordinate, including the wire-format consequence: a `Node.id` field over a table with a same-named column flipped from the raw column to the encoded global id, warned, with both silencers named in the message. Generalising it is what makes rules 1, 2 and 6 one grammar rather than three coordinate-specific dialects.

The measured exposure is small but real, and the one case that matters is worth naming so nobody rediscovers it as a bug. In the fixture catalog, `baz`'s single column is literally `ID` **and** is also its node key, so a directive-less `id` on a baz-backed input resolves to the raw column today (`BuildContext.java:2669`, `Direct` extraction) and after this item decodes instead. That is a wire-format change for that coordinate: the client sends the encoded global id where it used to send the raw value. It is warned rather than silent, and `@field(name: "id")` restores the old behaviour verbatim, which is the same trade R580 accepted at output. Elsewhere the exposure is nil: `film_actor` has `actor_id`, `film_id`, `last_update` and `bar` has `ID_1`, `ID_2`, `NAME`, so neither sakila's motivating field nor rule 6's own proof case has a column to shadow.

An earlier draft of this item avoided the `baz` flip by resolving rule 6 *after* column resolution, on the argument that the Relay contract attaches to the field on the type rather than to a same-named argument. That is true as far as it goes, but it bought one coordinate's stability at the price of two different shadowing answers in one grammar, which is the worse trade. Recorded here so the placement is not re-litigated on the same reasoning.
...
* The directive-less implicit scalar-`ID` argument arm (`FieldBuilder.java:1580-1609`), **replaced rather than merely deleted**: rule 6's arm takes its place, keyed on the argument name matching the return `NodeType`'s `Node.id` field and resolving the decode through `NodeIndex.forName`. Shape it as the mirror of the `Node.id` output half at `FieldBuilder.java:7270`: it sits **ahead** of the argument's column resolution at `FieldBuilder.java:1664` (`argString(arg, DIR_FIELD, ARG_NAME).orElse(name)`) and wins against a same-named column, carrying the `LintRule.NODE_ID_SHADOWS_COLUMN` finding with both silencers named, per "One shadowing rule". The arm it replaces sits on the same side of that lookup, so the placement is not itself a change; what narrows is the predicate, from "any `ID` argument whose table carries metadata" to "an `ID` argument named for the target's `Node.id`". There is no existing notion of an argument or input field mapping to a *field* of the target type, so that name-match predicate is new code; today's binding is name-to-column only.
...
The shadowing rule needs a case per coordinate, and `baz` is the fixture for the input and argument ones: a node-backed table whose single column is literally `ID` and is also its node key. Pin, at both an input field and an argument, that a directive-less `id` there resolves to the **node** and emits `LintRule.NODE_ID_SHADOWS_COLUMN`, that `@field(name: "id")` restores the raw column, and that `@nodeId` silences the warning while keeping the node. The output equivalent shipped with R580; asserting all three coordinates together is what pins the uniformity, and a test that covers only one of them will not catch a later coordinate-specific divergence.
...
One further change is warned rather than silent and so is not in this set, but shipping it needs the same consumer question asked: a coordinate whose name *does* match `Node.id` over a table carrying a column of that name flips from raw column to encoded global id, per "One shadowing rule". `baz` is the in-tree instance. It emits `NODE_ID_SHADOWS_COLUMN` and `@field(name:)` reverts it verbatim, which is the trade R580 already accepted at output, but it is a wire-format change at a consumer even so.
