...
- **Phase 1: carved out to R581 and shipped there.** The type-bearing callers (`BuildContext.buildInputNodeIdReference`, `NodeIdLeafResolver.resolve`) now route through `BuildContext.resolveDecodeHelperForType`, and `resolveTargetKeys` reads the `NodeIndex` by-name entry ahead of the table's metadata. `resolveDecodeHelperForTable` survives in the synthesis shims and in the bare-`@nodeId` argument path in `FieldBuilder.classifyArgument`, which holds no type name to key on; folding that arm onto its enclosing type belongs with phase 2's grammar change rather than with a bug fix.
- **Phase 2 (breaking SDL change, shares R27's consumer-migration gate):** enforce rules 2-3 as build errors, let rule 4's plain-column mapping replace the shims' synthesized node semantics, and delete the table-first helper together with the shims. R27 (`retire-synthesis-shims`) remains the deletion vehicle for the shims themselves; this item widens the migration slightly beyond R27's recipe in that bare `@nodeId` on inputs/arguments needs `typeName:` added (the sakila INSERT-input fixture writes the bare form today), on top of the directive-less cases R27 already covers. R34 (`nodeid-migration-quickfix`, pivoted 2026-07-14) ships LSP quick fixes that automate the consumer-side migration from the shims' own derived facts. The theoretical transition hazard (a legacy column-named ID field like `customerId: ID` silently flipping from shim-synthesized node decode to raw column mapping) is moot in practice: per confirmation on 2026-07-13, no actual consumer relies on the shim behavior. That also means phase 2 is softer-gated than R27's written trigger suggests; the shims can likely retire without waiting out the sis migration window.
