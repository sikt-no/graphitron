...
6. **An argument naming the return type's `Node.id` is that nodeId, implicitly.** This follows from rule 1 rather than diluting it. Where a field returns a type classified as a `NodeType`, that type has exactly one field satisfying the `Node` interface, so an argument carrying that field's name is naming it, and "which node" is answered by the return type declared on the same field in SDL. Numbered last because it was coined at the pass-3 gate; logically it sits next to rule 1. Four boundaries, each load-bearing:
   ...
   * **`ID`-typed only.** An `id: Int!` argument is not a node id and must not decode. Rules 3 and 4 make this implicit, but at a coordinate this subtle it is worth an explicit guard rather than an inference.
   ...
   * **It resolves *after* column resolution, not before**, which is the one place rule 6 deliberately departs from rule 1's shape. Rule 1 sits ahead of the column lookup because the Relay contract makes `Node.id` mean the global id even against a shadowing column. That contract attaches to the *field on the type*: `FilmActor.id` **is** the interface's id by declaration, whereas `filmActorByNodeId(id:)` is an argument that happens to share the name and carries no such contract. So a real column has the better claim and wins.

   That placement is what keeps rule 6 from opening a can of worms, and it costs nothing measured against the tree. `film_actor` has columns `actor_id`, `film_id`, `last_update` and no `id`, so sakila's motivating field still misses and still decodes. `bar` has `ID_1`, `ID_2`, `NAME` and no `id`, so `LookupKeyCase.SCALAR_NODEID_LOOKUP_COMPOSITE_PK` stays green. But `baz`'s single column is literally `ID` **and** is its node key, so a directive-less `id` on a baz-backed input resolves to the raw column today (`BuildContext.java:2669`, `Direct` extraction) and would have started decoding under a rule-1-shaped placement: a silent, wire-breaking flip, and an implicit reading *added* by an item whose whole purpose is removing them. After column resolution, that coordinate keeps today's behaviour and the explicit `@nodeId(typeName: "Baz")` spelling still decodes, because the directive branch runs earlier.

   Two guards fall out of the placement rather than needing implementation. **`@field(name:)` defeats rule 6 automatically**, since it pins a column that then resolves first; no `!hasFieldDirective` mirror of `FieldBuilder.java:7273` is needed. And **no `NODE_ID_SHADOWS_COLUMN` finding belongs at these coordinates**, because nothing shadows: rule 1 warns precisely because the node wins against a column, and here it does not. An earlier draft of this item proposed that symmetry; it was wrong.
   ...
   One residue stays open rather than being closed by the rule, and should be named rather than papered over: an argument genuinely meaning something else but named `id`, on a field whose node-backed target has no `id` column (`filmsByStore(id: ID): [Film!]!` if `film` were node-backed), decodes as the return type's node. That is a pre-existing sharp edge, since today's arm decodes it too on looser grounds, and rule 6 narrows the aperture rather than widening it. `@field(name:)` or a better argument name is the author's remedy.
...
* The directive-less implicit scalar-`ID` argument arm (`FieldBuilder.java:1580-1609`), **replaced rather than merely deleted**: rule 6's arm takes its place, keyed on the argument name matching the return `NodeType`'s `Node.id` field and resolving the decode through `NodeIndex.forName`. Shape it as the mirror of the *non-`id`* output half rather than the `Node.id` half: it sits **after** the argument's column resolution at `FieldBuilder.java:1664` (`argString(arg, DIR_FIELD, ARG_NAME).orElse(name)`) and fires on a column miss, per rule 6's placement clause. Note the arm it replaces sits on the other side of that lookup, so this is a narrowing, and the narrowing is the silent change recorded under Consumer migration. There is no existing notion of an argument or input field mapping to a *field* of the target type, so the name-match predicate is new code; today's binding is name-to-column only. Do not attach a `LintRule.NODE_ID_SHADOWS_COLUMN` finding here: nothing shadows once the column wins.
...
Rule 6's placement needs a case of its own, and `baz` is the fixture for it: a node-backed table whose single column is literally `ID` and is also its node key. Pin that a directive-less `id` there resolves to the **raw column**, not a decode, and that the explicit `@nodeId(typeName: "Baz")` spelling on the same shape still decodes. That pair is what stops a later reader from "restoring symmetry" with rule 1 and silently changing the wire format.
...
The second is an `ID` **argument or input field** whose target is node-backed and whose name matches a real column. Today's arm decodes it, consulting only `catalog.nodeIdMetadata` and the `ID` type and never the name, and running ahead of the column lookup; under rule 6 the column resolves first and wins. The narrowing is the point, but it is silent, so it belongs in the same re-confirmation as the first. Two converses are *not* flips and are worth stating so the exposure is not over-read: a coordinate that matches `Node.id` on a target with no such column keeps decoding, which is why sakila needs no migration; and a coordinate that matches a real column keeps mapping it, which is why `baz`-shaped inputs are untouched. If re-confirmation turns up any consumer at all, stage *both* arms behind a WARN for one release before they change semantics, and leave rules 1-3 as errors immediately since they cannot flip anything silently.
...
**Rule 6 placed after column resolution, 2026-08-06.** The author flagged that a name-matching rule
layered over a binding mechanism that is itself name-to-column risks opening a can of worms, and it
did. Measured against the fixture catalog: `baz`'s single column is literally `ID` and is also its
node key, so a directive-less `id` on a baz-backed input maps the raw column today and would have
begun decoding under the rule-1-shaped placement rule 6 was first drafted with. That is a silent
wire-format change, and an implicit reading *added* by an item that exists to remove them. Resolving
after the column lookup keeps it, and costs nothing: `film_actor` and `bar` both lack an `id`
column, so the motivating sakila field and rule 6's own proof case are unaffected. It also makes the
`@field(name:)` defeat automatic and retires the `NODE_ID_SHADOWS_COLUMN` symmetry an earlier draft
proposed, since nothing shadows once the column wins. The departure from rule 1's placement is
principled rather than expedient: the Relay contract attaches to the field on the type, not to an
argument or input field that happens to share its name.
