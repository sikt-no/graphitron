...
R92 (`catalog-check-constraint-validation`) routes DB CHECK-derived constraints through `GeneratedConstraintMapping.toMapping(...)` inside the default `getValidator` seam; a consumer-supplied custom factory must also apply `GeneratedConstraintMapping`, or it silently drops every CHECK-derived constraint. This item's design should state how the custom factory picks those constraints up.
