...
Today's editor instructions in `docs/manual/how-to/dev-loop.adoc` ("point your LSP client at `localhost:8487` (TCP)") work for Neovim / Helix / VS Code; the "IntelliJ users" subsection there now documents an explicit LSP4IJ + `nc 127.0.0.1 8487` stdio-bridge recipe, which is a hand-rolled version of exactly the bridge this item packages. Internal users have flagged this as a blocker — IntelliJ is the dominant editor in the consuming teams, and the current path is far enough off the IntelliJ-native well-trodden track that nobody completes it. A `java -jar bridge.jar --port 8487` command (the alternative considered earlier) does not close the gap either: the user has to know where the bridge jar lives, paste an absolute path into IDE settings, and update it on every plugin upgrade.

## Background

The existing transport-and-process shape is documented in `docs/architecture/how-to/dev-loop-internals.adoc`. Five cooperating components live in one JVM: LSP server (TCP), MCP server (loopback `8488`, `DevMojo.DEFAULT_MCP_PORT`), schema watcher, classpath watcher, generator dispatch. State is in-memory only; the warm-JVM model is the reason `DevServer.serve()` constructs a *fresh* `GraphitronLanguageServer` per connection backed by a *shared* `Workspace`, so editor reattach drops the per-connection state without touching the parsed-buffer + catalog substrate. R99 (multi-module classpath-scan gap) shipped (Done) since this was drafted; it did not overlap this item's surface. R212 only adds an editor-side surface, not a generator-side one.
...
Two new modules at the repo root (the reactor root since R182 unnested the rewrite):
...
- **`graphitron-lsp-bridge`** (Maven, child of `graphitron-rewrite-parent`). Single class `no.sikt.graphitron.lsp.bridge.StdioBridge` with a `main(String[] args)` that:
  ...
  `<release>17</release>` — the bridge is the only generator-tree code that runs **outside** the consumer's `mvn` JVM (IntelliJ spawns it as a subprocess), so it has to be runnable on the JBR shipped inside IntelliJ. JBR 17 was the floor since 2022.3; JBR 21 lands across the 2024.x line, but pinning to 17 covers every supported IntelliJ version the plugin's `since-build=241` allows. This is **not** the "generated output targets Java 17" rule (which is a different principle entirely, about the SDK floor of code we emit for consumers' jOOQ-bound runtime); it's a JBR-runtime constraint specific to the bridge. Everything else in the reactor continues to compile at `<release>25</release>` per the parent pom. Zero compile dependencies; JDK 17's `InputStream.transferTo` is sufficient. Produces a shaded executable jar via `maven-shade-plugin`'s `Main-Class` manifest entry, so `java -jar` is enough; no classpath argument required. Sits inside the main reactor so a normal `mvn install -Plocal-db` builds it.

- **`graphitron-intellij-plugin`** (Gradle, *not* in the Maven reactor — sits **outside** the rewrite-build-independence invariant by design). The IntelliJ Platform Plugin SDK is Gradle-only in practice; mixing it into the Maven reactor via `gradle-maven-plugin` invites a class of build failures the project has not signed up for. The principle at stake is `development-principles.adoc`'s self-contained-reactor constraint: `mvn install` from the repo root must produce the publishable rewrite artifact set from a self-contained reactor (the `verify-standalone-build.sh` script that used to pin this was retired with R182 once the legacy reactor disappeared; the invariant survives as prose in the principles doc). The plugin is *not* in that set — it's a downstream editor surface, not a generator artifact a consumer's pom would depend on — and the precedent is `graphitron-tree-sitter-natives`, which sits inside the repo but is deliberately omitted from the parent pom's `<modules>` list for the same release-cadence-independence reason. The Gradle module reads the bridge jar from a fixed relative path (`../graphitron-lsp-bridge/target/graphitron-lsp-bridge-<version>-shaded.jar`) and copies it into the plugin's `src/main/resources/bridge.jar` at package time. `mvn install -Plocal-db` produces the bridge; `./gradlew :graphitron-intellij-plugin:buildPlugin` (run separately, JDK 25, same JDK floor the parent pom's enforcer pins) produces the plugin zip. CI gates: the existing `rewrite-build.yml` workflow keeps owning the Maven side; a new `intellij-plugin-build.yml` workflow invokes `./gradlew :graphitron-intellij-plugin:buildPlugin` on PR and trunk pushes that touch `graphitron-intellij-plugin/**` or `graphitron-lsp-bridge/**`, uploading the plugin zip as a workflow artifact. There is no standalone-build script left to teach (retired with R182); if a standalone-verification check ever returns, it should ignore the Gradle module's outputs by path. Documented as a two-step build in `docs/manual/how-to/dev-loop.adoc` and in a new release how-to modeled on `docs/architecture/how-to/release-natives.adoc`.
...
Per `workflow.adoc`'s "Plans with a user-visible surface … include a draft of the user docs as the first client of the design," here is the prose that goes into `docs/manual/how-to/dev-loop.adoc`, replacing the existing "IntelliJ users" netcat-bridge recipe, plus a sketch of the new `intellij-plugin-release.adoc`. Until this reads simply on its own, the design is wrong.

### Drafted prose for `docs/manual/how-to/dev-loop.adoc` (replacing the "IntelliJ users" subsection)
...
> ```
>
> Open any `.graphqls` file in the project. The plugin auto-attaches to the dev mojo on `localhost:8487` and surfaces diagnostics, hover, completion, go-to-definition, quick fixes, and inlay hints. The dev mojo is the source of truth for the loop's lifecycle — start it before opening files, stop it with Ctrl+C, restart it when you change `pom.xml`. The plugin attaches and reattaches automatically.
>
...
1. *What this document is.* Release plumbing for `graphitron-intellij-plugin`, parallel to `docs/architecture/how-to/release-natives.adoc`. Not user-facing — release engineers only.
2. *Build the plugin locally.* `mvn install -Plocal-db` from the repo root (produces bridge jar), then `./gradlew :graphitron-intellij-plugin:buildPlugin` (produces the plugin zip under `graphitron-intellij-plugin/build/distributions/`).
...
- `graphitron-maven-plugin/src/main/java/no/sikt/graphitron/rewrite/maven/DevMojo.java` — `DevMojo.DEFAULT_PORT` / `LOOPBACK_HOST`, the constants the plugin's settings default mirrors. `graphitron.dev.port` is the canonical override property.
- `graphitron-maven-plugin/src/main/java/no/sikt/graphitron/rewrite/maven/dev/DevServer.java` — the per-connection `DevServer.serve()` method the bridge effectively drives. It constructs a fresh `GraphitronLanguageServer` per connection backed by the constructor-supplied shared `Workspace` (see the constructor doc comment), so multi-attach (one IntelliJ + one Neovim against the same dev mojo) works out of the box.
- `graphitron-lsp/src/main/java/no/sikt/graphitron/lsp/server/Launcher.java` — the *non*-candidate stdio entry point. Cold LSP, no shared workspace, no watchers; the bridge must hit `DevServer`'s socket, not this main.
- `docs/manual/how-to/dev-loop.adoc` § "IntelliJ users" — the existing editor-side documentation surface (currently the manual LSP4IJ + `nc` bridge recipe); R212 replaces it with the plugin install path. The internals live in `docs/architecture/how-to/dev-loop-internals.adoc`.
- `graphitron-tree-sitter-natives/pom.xml` — the precedent for a standalone module published with its own release cadence. R212's `graphitron-lsp-bridge` stays inside the reactor (no release-cadence pressure) but follows the same "small standalone purpose-built module" shape.
...
- R99 (`lsp-submodule-sibling-classpath`), Done — the sibling-module classpath scan shipped; the dev goal now folds siblings' `target/classes` into the scan (see "Multi-module projects" in `dev-loop-internals.adoc`). It surfaced under the same multi-module shapes the IntelliJ plugin will mostly see in practice, but was unrelated to R212's transport work.
