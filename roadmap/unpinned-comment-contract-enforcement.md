...
Rework (one site) resolved: the review found the sweep's grep had covered six module roots but not the full `graphitron*` glob, leaving `FilmLookupNotFoundException` (`graphitron-sakila-service`) with two dead `§4` cites. Both repointed to `FieldBuilder.checkDeclaredCheckedExceptions` in prose (a `{@link}` cannot resolve from that module), and the acceptance grep re-run across all of `graphitron*/src/main/java` now shows only the whitelisted external-spec and R496/R521-owned sites. Everything else on the item passed the first review round: all five claims resolved as specified, the four spec-named test pins present and green, the claim-3 producer audit independently confirmed, and the claim-1 truth-probe correction verified.
