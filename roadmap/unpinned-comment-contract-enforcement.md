...
**Rework, one site.** The acceptance grep is not clean: `FilmLookupNotFoundException` (in `graphitron-sakila-service/src/main/java`) still carries two dead `§4` cites in its class javadoc, "which §4 would require to be covered by a matching `@error` handler" and "orthogonal to the §4 declared-exception check". Both are the dead error-channel-spec `§4` declared-checked-exception family, in a main-source comment region, inside the acceptance criterion's own `graphitron*/src/main/java` glob, and on none of the whitelists (not JLS, not LSP protocol, not R496-owned, not R521-emitted-text). The sweep evidently scoped its grep to `graphitron/src/main/java` and missed the fixture-service module, even though the same commit range edited that module's `CustomerRecordService` javadoc. Fix: repoint both to the live enforcer the sweep already used at `FieldBuilder`, `checkDeclaredCheckedExceptions`, naming it in prose the way `MethodRef`'s javadoc does (a `{@link}` cannot resolve from that module). Then re-run the acceptance grep across all of `graphitron*/src/main/java`, not one module. Everything else on the item passed review: all five claims resolved as specified, the four spec-named test pins present and green, the claim-3 producer audit independently confirmed (both `On.ColumnPairs` construction sites guarantee non-empty slots, and the name-matched path's author-error rejections are preserved, so the deleted generation-time throws were genuinely redundant), the claim-1 truth-probe correction is a real improvement over the spec's own hypothesis, and `mvn install -Plocal-db` is green.
