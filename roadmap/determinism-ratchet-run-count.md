...
* `docs/architecture/explanation/fact-model.adoc` never learns the registry. The page already
  ...
  the item was right to say it needed no new sanction, but the doctrine sentence at "materialization
  is sanctioned above where a view cannot serve" still states the narrow version this item widened,
...
* `mvn install -Plocal-db`: BUILD SUCCESS, 14/14 modules.
* `MaterializeRegistryGateTest`'s four cases are non-vacuous and check the structural half properly,
  including the no-ordering-needed claim by transitive closure over the engine's own view bodies.
* Registration is invisible to consumers as claimed: the only store-open outside `FactCapture` is
  `DevMojo`, which refreshes, and no capture path writes the base relations these two views read
  without going through `FactCapture.capture`.
* Every deliverable in the list is marked with what became of it, and the two declined ones carry
  their re-measurements. The run count staying at four is argued and correct.
* The class improvement reproduces in order of magnitude on this machine
  (`GeneratorDeterminismTest` 26.83s for three cases in a parallel reactor run, against 229.0s on
...
build on a `depends-on` slug whose file is gone. Whoever approves the next pass clears that entry in
the approval commit.

## What was implemented

**This is the reviewing reader's shortcut.** The body below was written as a plan
