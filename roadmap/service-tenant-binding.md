...
- **`NodeIdDecodePolymorphicRecord` leaf** (a `@nodeId(typeName:)` naming a multitable interface or a union, at a bean member or a top-level slot): declines, with its own text. The same wire id decodes to whichever `PolymorphicCandidate` its type id names, each with its own key columns, and a candidate may carry the tenant at a different position or not at all, so there is no single decode to route the call on. This is the service-side twin of `PruneOnMismatch`, which `accessOf` already declines for the same reason, and the decline goes through `SlotCollector.decline` like that one. It is not "mint nothing": a polymorphic id beside a divining sibling would then go unchecked, and a service handed a record from another tenant would act on it in the wrong database. Declining stops the field divining, so its subtree gets no tenant. Under the existing rule in `armOf` the decline's text surfaces at the root only when the root's own reach is tenant-scoped (a table-returning service root, a polymorphic service root over tenant-scoped participants). Under a wrapper root the tenant-scoped children reject as they do today. Nothing that runs today starts failing.
- **Every other leaf** (`Direct`, `EnumValueOf`, `JooqConvert`) mints nothing. These leaves carry no column, so there is nothing to test against the tenant column.
...
- `MultiTablePolymorphicEmitter.buildServiceMainFetcher` and `buildServiceTableInterfaceFetcher` (the polymorphic service roots) move the same way, including their stamped returns. Both carry only `fieldName` today, and `handDownOnly` takes the `OutputField`, so their callers thread the classified field through as `buildServiceFetcherCommon`'s do.
...
A method call whose parameters receive a connection-bound value, a `DSLContext` or the `$session` handle, runs on whichever connection it is handed, so which connection serves it is semantics, not plumbing. The existing `bindsSessionHandle` rung encodes half of that question. It is widened to `bindsConnection(OutputField)`, which reads the same two carriers `bindsSessionHandle` reads today:

- **A `MethodBackedField`:** its method needs a `DSLContext`, or it has a `ParamSource.SessionHandle` parameter. "Needs a `DSLContext`" is `MethodRef.CallShape.needsDsl()` where the method is a `MethodRef.Service`. That is the tree's one home for the static-vs-instance fork, and it covers both a `ParamSource.DslContext` method parameter (`CallShape.Static.needsDslLocal`) and a holder constructor taking the `DSLContext` (`CallShape.InstanceWithDslHolder.ctorParams`). A child service whose method is non-static reaches its connection the second way, `new FooService(dsl).bar(...)`, so a check on the method's own parameters alone would leave it `Untenanted` on the default source. For every other `MethodRef` (no call shape: `StaticOnly`, `ConditionFilter`), the check is a `ParamSource.DslContext` parameter.
- **A `ServiceField`:** its carrier has a `MappingEntry.FromDsl` or `MappingEntry.FromSessionHandle` entry, across `ServiceMethodCall.Instance.ctorArgs()` and `methodArgs()` as `bindsSessionHandle` already folds them. The instance-holder case is covered here because the holder's `DSLContext` rides in `ctorArgs` as `FromDsl`.

The rung keeps its place ahead of `!anyTenant -> Untenanted`: under a tenant context such a call classifies `Inherited` and routes through the handed-down tenant. One predicate rather than a second rung, so the two cannot drift. A call binding neither never touches a connection, so `Untenanted` stays right for it.
...
**The services that start divining do change behaviour, deliberately.** A wrapper-returning service that takes a tenant-scoped record id runs today, `Untenanted`, on the default source. Once this lands it runs on the tenant that id names. That is the rule above applied, and it is the fix the Goal describes. But a consumer whose service takes a tenant's record id and touches only global tables through its `DSLContext` will see that service move to the tenant database, where those tables are not. The changelog entry for this item says so. The remedy is on the consumer's side: a service that does not work on the tenant's data should not take the tenant's record.

## Implementation

- `TenantBindingIndex.Fold`: the `ServiceCall` arm in `directBinding` and its `ValueShape` walk, including the polymorphic-record decline; `bindsSessionHandle` widened to `bindsConnection`, reading `CallShape.needsDsl()`.
...
- `graphitron-sakila-service`: a `FilmRatingService` (name illustrative) with a bean method taking a `FilmRecord` member, a method taking `List<FilmActorRecord>` whose input carries the composite `FilmActor` id (the tenant at key position 1), and two child services binding a `DSLContext`: a static method taking it as a parameter, and a non-static method on a class whose constructor takes it. Each returns what `current_database()` reports on the connection it was handed, so a test can see which database served it.
- `graphitron-sakila-example`'s `multitenant.graphqls`: the fields below, one per shape, in the file's "one field per arm" style.

## Tests

- **Classification** (`TenantBindingClassificationTest`): a wrapper-returning service root with a `@nodeId` record member classifies `ArgumentBound` with a `DecodedKeySlot` at the tenant's key position, and the wrapper's tenant-scoped child classifies `Inherited` with no rejection. The same for a jOOQ-record parameter binding the tenant column through `@field` (a `Raw` slot), and for a table-returning service root. An input with two tenant-bearing `@nodeId` fields of different node types mints two slots, and two fields both named `id` at different paths mint two slots rather than one. A service root with only a `Direct` `ID` stays `Untenanted` and its child still rejects, pinning the gap R978 owns. A `@nodeId` naming a multitable interface or union declines: at a table-returning service root it rejects with the decline's own text rather than the generic "no argument maps to tenant column" message, and beside a divining sibling it stops the field divining, so the wrapper's tenant-scoped child rejects. A connection-binding child service under a bound parent classifies `Inherited`, in both shapes: a static method taking a `DSLContext` parameter, and an instance-holder service whose constructor takes it. One binding no connection stays `Untenanted`.
...
- **Route a polymorphic `@nodeId` per candidate.** When every candidate's key embeds the tenant column, the build could prove the slot names a tenant, much as node dispatch proves it for `NodeIdBound`. The runtime would then peek the id's type id and read that candidate's key position. It was not chosen for the reason the first alternative above was not: it splices the per-row node-dispatch family into `ArgumentBound`'s fixed-decode shape. No sis service needs it. If one does, it is its own item.
...
1. **The widened inherit rung misses the instance-holder child service (question 2).** The spec defines `bindsConnection` for a `MethodBackedField` as "has a `ParamSource.DslContext` or `ParamSource.SessionHandle` parameter", which reads `method().params()` only. A child service whose method is non-static gets `MethodRef.CallShape.InstanceWithDslHolder` from `ServiceCatalog` (line 508). Its `DSLContext` comes in through the holder constructor's `ctorParams`, not through the method's own parameters. So `new FooService(dsl).bar(...)`, record-returning and reaching no table under a tenant-bound parent, would still classify `Untenanted` and run on `dslDefault`. That is the silent wrong-database routing this item exists to close. The root branch does not have this problem, because it already folds `ServiceMethodCall.Instance.ctorArgs()` in. The tree has one home for this fork, `MethodRef.CallShape.needsDsl()`, whose javadoc says every emit site reads it "rather than re-stating the disjunction". To satisfy the finding, define the `MethodBackedField` half through `needsDsl()` (plus the `SessionHandle` parameter check), or say why the holder arm is excluded. Then add a holder-shaped child service to the classification and execution tests beside the parameter-shaped one.
   ...
   *Author, round 1:* addressed in Design § Inherit. `bindsConnection` reads `MethodRef.CallShape.needsDsl()` for a `MethodRef.Service`, so the holder constructor counts. It checks a `ParamSource.DslContext` parameter for the method refs that have no call shape. Implementation adds an instance-holder child service to `graphitron-sakila-service`, and Tests pins it at the classification and execution tiers beside the static one.
...
2. **The leaf enumeration omits `NodeIdDecodePolymorphicRecord`, which reaches this walk (question 2).** The `ValueShape` walk lists the leaves that mint slots and says "every other leaf (`Direct`, `EnumValueOf`, `JooqConvert`) mints nothing" because "a `Direct` leaf has no column". But `ServiceMethodCallWalker.isLeaf` and `fieldBindingShape` carry `CallSiteExtraction.NodeIdDecodePolymorphicRecord` as a `Scalar` leaf. `ServiceCatalog.nodeIdSlotExtraction` also mints it for a top-level `@nodeId` slot that names a multitable interface or union. So a polymorphic record decode can reach the service arm today. It does have columns, one key tuple per `PolymorphicCandidate`, so the stated reason does not cover it. It is also the service-side analogue of `PruneOnMismatch`, which `accessOf` explicitly declines ("no single decode to route the statement on"). The implementer's switch has to pick an answer: mint nothing (the leaf joins the accepted gap), decline with its own text as `PruneOnMismatch` does, or route per candidate. The spec should make that choice, and a classification test should pin it.
   ...
   *Author, round 1:* addressed in Design § Divine. The leaf declines through `SlotCollector.decline`, as `PruneOnMismatch` does, and the text says why: minting nothing would leave the id unchecked beside a divining sibling. Per-candidate routing is recorded under Other solutions, with the reason it was not chosen. Tests pins the decline at a table-returning root and beside a divining sibling.
...
- "Nothing that runs today starts failing" is scoped to the undecoded-id gap, and it holds there. The newly divining services are a deliberate change of behaviour, though. A wrapper-returning service that takes a tenant-scoped record id and today runs, `Untenanted`, on the default source (for example one that writes only global tables) will route to the tenant database once this lands. That follows from the "route whenever a tenant is known" rule, and I agree with the rule. The Goal or the gap section could say so, so that a consumer reading the changelog is not surprised.
  ...
  *Author, round 1:* added a paragraph under Design § What an empty reach means, stating the behaviour change and that the changelog entry names it.
