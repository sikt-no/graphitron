...
| **Lift** | — | `SplitTableField`, `SplitLookupTableField`, `RecordTableField`, `RecordLookupTableField`, `RecordTableMethodField` | `InterfaceField`, `UnionField`, `TableInterfaceField` (child) | — | `ServiceTableField`, `ServiceRecordField` (child) | — |
...
| **CarrierFollowUp** | — | `SingleRecordTableField` | — | — | — | — |
...
sealed interface Field permits QueryField, MutationField, ChildField {
    ...
    DataFetcher fetcher();        // sealed: Root | Lift(SourceKey, LoaderRegistration) | Trivial | CarrierFollowUp(...)
...
- `BatchKeyField` → `fetcher() instanceof DataFetcher.Lift(var sk, var lr)`
...
- **`CarrierFollowUp` as a fourth DataFetcher kind, or fold into Root?** `SingleRecordTableField` runs a SELECT against the response table with PK passed in from the upstream carrier — neither DataLoader-batched nor request-entry. Treating it as a fourth kind keeps the runtime axis clean; folding it into `Root(parentPkSource: CarrierPk)` keeps the kind set to three at the cost of a non-uniform Root arm.
...
- **R161** (`unify-record-dml-on-carrier-walk`) is in flight and lands the carrier-walk unification. R164's `CarrierFollowUp` (or Root-with-CarrierPk) resolution depends on R161 having settled. Hard dependency.
