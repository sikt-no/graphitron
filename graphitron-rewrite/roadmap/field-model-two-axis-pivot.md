...
- R188 (`simplify-update-mutations-drop-value`) lands ahead of R164 and commits to forward-compatibility with the `QueryBuilder.Update.filter` shape this item introduces. R188 materialises the row-identity outcome as a sealed `RowIdentity` sub-taxonomy on the UPDATE field record, whose permits are shaped to match `Filter.KeyLookup | Predicate | BulkKeyLookup` one-to-one: PK-covered → `KeyLookup`, override-claimed by `@condition` → `Predicate`, broadcast via `multiRow:` → `BulkKeyLookup`. R188's layered `WhereFilters` (input-type / mutation-field / arg / input-field, with compact-constructor invariants from `mutation-condition.where-filters-well-formed`) survives as the construction-side input to `RowIdentity.Predicate`; the layering is the load-bearing rejection surface and is not collapsed away.
- R164's lift on R188's contributions is then mechanical, not structural: (i) rename `RowIdentity` to `Filter` and hoist it from the UPDATE field record to `QueryBuilder.Update.filter`; (ii) route the existing `WhereFilters` through `Filter.Predicate` as the construction-side input rather than re-deriving the layering at emit time; (iii) preserve the four-placement `@condition` composition into `Filter.Predicate` so no resolution layer is dropped. R164's plan body must explicitly commit not to retire `WhereFilters` and not to short-circuit the four-placement composition; both are R188's load-bearing model surface that R164 inherits intact.
