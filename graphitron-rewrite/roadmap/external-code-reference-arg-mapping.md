...
**Status mechanics.** When R53 reaches `Ready`, R41's file moves to
`Discarded` (or equivalent terminal state per
`graphitron-rewrite/docs/workflow.adoc`) with a one-line front-matter pointer
`superseded-by: external-code-reference-arg-mapping`. The R41 file deletes
when R53 reaches `Done`, following the R20 → R50 tombstone pattern. R53
inherits no Spec → Ready review credit from R41; the Spec → Ready transition
for R53 is its own review.
...
10. **Tombstone R41.** When R53 reaches `Ready`, R41's front-matter gains
    `superseded-by: external-code-reference-arg-mapping` and its status moves
    to `Discarded` (or equivalent terminal state per
    `graphitron-rewrite/docs/workflow.adoc`); the file deletes when R53 reaches
    `Done`, following the R20 → R50 tombstone pattern. The `@field` directive
    docstring update lands as part of step 1 above (no separate revert step
    needed because the per-arg-override line never reaches main).
