...
- **B4c-1, per-parent inline child connection** — shipped 2026-05-02. `ChildField.InterfaceField` / `ChildField.UnionField` + `@asConnection`. Same per-branch parent-FK WHERE shape as B3 combined with B4a/B4b's connection scaffolding; each parent invocation runs its own polymorphic UNION ALL with `pagesTable`, `.orderBy/.seek/.limit`, and a per-parent `totalCount`. Per-parent inline (one query per parent invocation); the windowed CTE batched form is the B4c-2 follow-up.
- **B4c-2, DataLoader-batched windowed CTE** — pending. Move `ChildField.InterfaceField` / `ChildField.UnionField` invocation from per-parent (`env.getSource()`) to `List<parentKey>` + DataLoader so one query covers all parents in a request. Stage-1 wraps the per-branch UNION ALL in `WITH ranked AS (...)` with `ROW_NUMBER() OVER (PARTITION BY parent_idx ORDER BY __sort__, __typename)` and the outer SELECT filters `WHERE __rn__ <= page.limit()`; per-parent scatter into `List<ConnectionResult>` matches `SplitRowsMethodEmitter`'s connection method shape. Eliminates the B4c-1 N+1 risk.
