...
- **2026-05-02** — B4c-2 follow-ups (round 2) shipped. RowN widening for composite-PK parents on child interface/union `@asConnection` fields. `MultiTablePolymorphicEmitter` adds private `rowClass(arity)` / `recordClass(arity)` helpers and threads parent PK arity through `buildBatchedConnectionFetcher` (DataLoader key element widens from `Row1<PkClass>` to `RowN<T1...Tn>`; parent PK extraction emits one typed local per column then `DSL.row(fk0, fk1, ..., fkN-1)`) and `buildBatchedConnectionRowsMethod` (`parentInput VALUES` widens from `Row2<Integer, ParentPkClass>` to `Row<N+1>` and the alias widens from `("parentInput", "idx", pk_sql)` to include all parent PK SQL names). `batchedBranchJoinPredicate` now emits a per-PK-slot AND-chain `<participant>.<fk_i>.eq(parentInput.field("<parent_pk_i_sqlname>", T_i.class))` across composite FKs (position-aligned with parent PK columns per the `JoinStep.FkJoin.sourceColumns()/targetColumns()` contract). Validator's `validateChildConnectionParentPk` now only rejects (a) empty parent PK with a clean AUTHOR_ERROR (`requires the parent type ... to have a primary key`) and (b) parent PK arity > 21 (since `parentInput` widens to `Row<N+1>` capped at jOOQ's typed `Row22`). Codegen-time tripwires in `buildBatchedConnectionFetcher` cover the same cases as defensive backstops. Tests: 3 new unit tests in `TypeFetcherGeneratorTest` covering composite-PK parent — `_widensKeyToRowN` (`DataLoader<Row2<Integer, Integer>, ConnectionResult>` plus two typed parent PK extractions plus `DSL.row(fk0, fk1)`), `_widensParentInputToRowN` (`Row3<Integer, Integer, Integer>[]` parentRows plus `DSL.row(DSL.inline(i), k.field1(), k.field2())` plus alias `("parentInput", "idx", "org_id", "project_id")`), and `_widensJoinPredicateToAndChain` (per-branch JOIN ON is the AND-chain across both PK slots). Two validator tests updated: `rejects_connection_onCompositePkParent` removed (composite parent PK is now supported); replaced with `wellFormed_connection_onCompositePkParent_noErrors` and `rejects_connection_onPkLessParent` (parallel union variant in `UnionFieldValidationTest`). The synthetic `Project` parent fixture (composite PK `(org_id, project_id)`) sits in the unit test only — sakila's composite-PK tables (film_actor, film_category) are pure junctions with no children FK-referencing them, so an execution-tier fixture for composite-PK parents would require a non-sakila DB schema and is left as a follow-up.
