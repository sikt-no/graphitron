...
- **B2b, multi-table fixture and execution tests** — pending. Adds `interface Searchable` (Film/Actor), `Query.search`, the parallel `union Document = Film | Actor`, plus the composite-PK third participant for the jsonbArray sort exercise. Drives `GraphQLQueryTest` cases (`search_returnsAllParticipantTypes`, `search_orderedByPkAcrossParticipants`, `search_emptyResult_noStage2Roundtrips`, `search_compositePkParticipant_jsonbArraySorts`).
