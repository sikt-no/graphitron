...
- **B4b, totalCount via UNION-ALL count subquery** — pending. Emit `dsl.selectCount().from(<UNION ALL as table>).fetchOne(0, Integer.class)` from a polymorphic-aware totalCount resolver path; today's `(null, null)` carrier in `ConnectionResult` short-circuits to null.
- **B4c, child-case connection pagination** — pending. `ChildField.InterfaceField` / `ChildField.UnionField` + `@asConnection`. Per-parent partitioning via `ROW_NUMBER() OVER (PARTITION BY <parent FK> ORDER BY __sort__, __typename)` so one query covers all parents in a batch; mirrors `SplitRowsMethodEmitter`'s windowed CTE shape applied across the polymorphic UNION ALL.
