...
- [x] `ErrorFieldCase.PATH_AND_MESSAGE_CLASSIFY_AS_PROPERTY_FIELDS` (L6031) -> `ChildField.PropertyField` | `path`/`message` fields on an `@error` parent | migrated: `error-field` corpus example (MyError.path/message), corpus-only on the inline/Field coordinate; sole constant, so the whole `ErrorFieldCase` enum + `errorFieldClassification` test retired with it
...
- [x] `TypeClassificationCase.ROOT_TYPE` (L5419) -> `GraphitronType.RootType` | the `Query` / `Mutation` root type | migrated: `catalog` corpus example's Query asserted via `@classifiedType(as: RootType)`, corpus-only
- [x] `ErrorTypeCase.ADMIT_EXTRA_FIELD` (L5904) -> `GraphitronType.ErrorType` | `@error` type carrying a field beyond `path`/`message` | migrated: `error-type` corpus example (ExtraFieldError with a `severity` field), corpus-only
- [x] `ErrorTypeCase.ERROR_PLUS_RECORD_IGNORES_RECORD` (L5979) -> `GraphitronType.ErrorType` | `@error` co-located with an (ignored) `@record` | migrated: `error-type` corpus example (RecordIgnoredError, `@error` + `@record`), corpus-only
- [x] `NestingTypeCase.DOES_NOT_OVERRIDE_DIRECTIVE_CLASSIFICATION` (L9586) -> `GraphitronType.TableType` | a `@table` type classifies as `TableType`, not `NestingType`. *Nuance: carries an `isNotInstanceOf(NestingType)` negative beyond the bare verdict; confirm the corpus's positive `@classifiedType(as: TableType)` coverage suffices before retiring, or keep the negative as a dedicated robustness case.* | retired: a type's classification is a single exclusive verdict, so the `catalog` example's positive `@classifiedType(as: TableType)` on `@table` Film subsumes the negative; NestingType itself stays covered by `NESTED_UNDER_TABLE_PARENT`
