...
* `graphitron-rewrite/graphitron-lsp/src/main/java/no/sikt/graphitron/lsp/hover/Hovers.java`: grow `compute`'s two overloads with an `LspSchemaSnapshot` parameter after `CompletionData`. Add a directive-name-hover branch *before* the coordinate-lookup path: if the cursor sits on `directive.nameNode()`, resolve through `DirectiveResolution.resolve` and produce a hover from the bundled directive's description or the user shape's description. Add a `User`-arm fallback in `docstringHover` that reads `InputValueShape.description()` from the snapshot when the coordinate's bundled description is empty.
...
R139's prep-note about the cross-module audit gap: phase 2 adds two more `@DependsOnClassifierCheck` markers (`Hovers.compute` and `ArgNameCompletions.generate`) on the LSP side, plus arguably one on `Diagnostics`' new arg-validation helpers. The audit's scan still doesn't see them, so they remain find-usages-only. Phase 2 *does not* widen the audit scope: the right fix is a test-tier or aggregator-module change with its own design call (sketched in R139's "Future evolution"), not a phase 2 payload. Phase 2 ships the markers in their documentation-only form and leaves the audit-widening decision to a separate item.
