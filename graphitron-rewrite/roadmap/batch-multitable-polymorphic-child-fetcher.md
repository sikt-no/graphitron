...
R102 lands the model unification and the list-arm DataLoader fix together: lift `BatchKey.RecordParentBatchKey` plus `GraphitronType.ResultType` onto the two field records, route both arms through `buildRecordParentKeyExtraction`, drop the `parentTable` parameter from `MultiTablePolymorphicEmitter` entirely. The unification is the longevity move; the SQL-count win on the list arm is one of its consequences. R102 does *not* deliver the `@record`-parent classifier arm (the deferred rejection at `FieldBuilder.java:2703` stays); R102's slot type accepts any `RecordParentBatchKey` permit so [R105](record-parent-multitable-polymorphic-classifier-arm.md) can wire that arm in on top without re-touching the records or the emitter. Today only `RowKeyed` is reachable from classification; that stays true through R102.
...
`BatchKey.RecordParentBatchKey` is documented at `BatchKey.java:25-29` as exactly "the parameter type for `buildRecordParentKeyExtraction`": the axis encodes "parent object handed up via `env.getSource()`," which is what the multi-table polymorphic emitter consumes. The other axis sub-seal `ParentKeyed` (six permits, source-side declared via `@source`) is not used here; `RowKeyed` is the only permit on both. If a future feature lands a multi-table polymorphic child whose parent-side keys come from a developer-facing source declaration, splitting the slot per-axis at that point is the same kind of sealed-switch sweep R74 / R71 handled and the type system carries the discrimination without rework.
...
- `BatchKey.RecordParentBatchKey parentKey` — the parent-object key-extraction strategy. The slot type accepts all four permits (`RowKeyed`, `LifterRowKeyed`, `AccessorKeyedSingle`, `AccessorKeyedMany`); through R102, only `RowKeyed` is constructed by classification (table-backed parents). The other three permits are wired in by [R105](record-parent-multitable-polymorphic-classifier-arm.md) without re-touching this slot or the emitter.
- `GraphitronType.ResultType parentResultType` — the parent's object shape, threaded through to `buildRecordParentKeyExtraction` so `env.getSource()` is cast and read against the right Java type. Through R102, only the table-backed parent's resolved `ResultType` (`JooqTableRecordType` and the synthetic-table case) reaches the field; the `JavaRecordType` / `PojoResultType` arms are reachable once R105 lands.

`FieldBuilder.classifyObjectReturnChildField` (the constructors at `FieldBuilder.java:520-522` and `:531-533`) is the only construction site of either record under R102. It supplies `parentKey = new BatchKey.RowKeyed(parentTable.primaryKeyColumns())` (same shape as `FieldBuilder.deriveSplitQueryBatchKey` at `:2854-2858`) and `parentResultType` from the enclosing parent's classified `ResultType`. The deferred rejection at `FieldBuilder.java:2703` for polymorphic returns on `@record` parents stays in place; R105 owns its replacement. R102 contains no work on `classifyChildFieldOnResultType`.
...
- **Per-typename dispatcher** (new, in `MultiTablePolymorphicEmitter`). The stage-2 dispatch over `participantJoinPaths` (typename → record-class scatter) is identity-shaped between the **list arm** (`buildMainFetcher` at `:284-291`) and the **root-connection arm** (`buildRootConnectionFetcher` at `:439-445`): same `byType.get(typeName)` → `perTypename<Field>(byType.get(...), env, dsl, result)` call. These two get a single shared helper. The **batched-connection-rows** dispatch (`buildBatchedConnectionRowsMethod` at `:793-…`) emits `ConnectionResult` per parent, builds a `__rn__` ranked-CTE, and threads per-parent `__idx__.eq(i)` carrier conditions through stage 2; that arm shares the parent-input VALUES emitter and the key-extraction delegation site with the others, but its dispatch is structurally different and stays inline.
...
- **Unifies a long-standing model gap.** The multi-table polymorphic emitter is the one batched-fetcher path that bypasses `GeneratorUtils.buildRecordParentKeyExtraction` and casts to `org.jooq.Record` inline. R102 closes this on the emitter side; the slot type accepts the full four-permit `RecordParentBatchKey` sub-seal even though only `RowKeyed` is reachable through classification today.
- **Direct dependency of** [R105 / `record-parent-multitable-polymorphic-classifier-arm.md`](record-parent-multitable-polymorphic-classifier-arm.md): R105 replaces the deferred rejection at `FieldBuilder.java:2703` and wires `LifterRowKeyed` / `AccessorKeyedSingle` / `AccessorKeyedMany` into the multi-table polymorphic classifier. R102 ships the structural prerequisites; R105 lights up the `@record`-parent capability on top.
