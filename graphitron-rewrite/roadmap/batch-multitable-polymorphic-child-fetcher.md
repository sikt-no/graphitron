...
The fanout symptom is one face of a deeper model gap: `MultiTablePolymorphicEmitter` is the one batched-fetcher path in the codebase that does *not* delegate parent-object key extraction to `GeneratorUtils.buildRecordParentKeyExtraction`, the canonical sealed-switch helper that handles all four parent-object shapes (`JooqTableRecordType`, `JooqRecordType`, `JavaRecordType`, `PojoResultType`) and all four `BatchKey.RecordParentBatchKey` permits. Both multi-table polymorphic arms cast `env.getSource()` to `org.jooq.Record` inline and read PK columns straight off the parent's `TableRef`, baking in a jOOQ-Record-only assumption that does not hold on `@record`-backed parents. R102 lands the list-arm DataLoader fix and the model unification together: lift `BatchKey.RecordParentBatchKey` plus `GraphitronType.ResultType` onto the field records, route both arms through `buildRecordParentKeyExtraction`, and drop the `parentTable` parameter from the multi-table emitter entirely. The longevity move is the unification; the SQL-count win is one of its consequences.

## Decision: route key extraction through `buildRecordParentKeyExtraction` with a `BatchKey.RecordParentBatchKey` slot

The "shape the emitter needs" is not the parent's `TableRef`. Inside a `DataFetcher` callback the emitter has access to `env.getSource()` — an object, not a table — and the key tuple has to be extracted *from that object*. `MultiTablePolymorphicEmitter` today bakes in the assumption that the object is a jOOQ `Record`: both the connection-arm `(($T) env.getSource()).get($T.$L.$L)` (`MultiTablePolymorphicEmitter.java:735-736`) and the list-arm `Record parentRecord = (Record) env.getSource()` (`:240`) cast unconditionally. On a `@record` parent (POJO or Java record source) this either throws at runtime or is silently rejected upstream — the multi-table polymorphic emitter is the one batched-fetcher path that does not consume the existing object-shape abstraction.

That abstraction is `GeneratorUtils.buildRecordParentKeyExtraction(BatchKey.RecordParentBatchKey, GraphitronType.ResultType, ...)` (`GeneratorUtils.java:186-196`), the canonical key-extraction switch driven by the single-table batched fetcher. Its inner `buildFkRowKey` (`:198-226`) already handles all four parent-object shapes (`JooqTableRecordType` / `JooqRecordType` / `JavaRecordType` / `PojoResultType`) and the four-permit `BatchKey.RecordParentBatchKey` sub-seal (`RowKeyed`, `LifterRowKeyed`, `AccessorKeyedSingle`, `AccessorKeyedMany`) covers every key-extraction strategy graphitron emits. The principled landing for R102 is to route both multi-table polymorphic arms through this helper.
...
- For `@record`-backed parents, classification consults the existing `@record`-parent batch-key derivation path (the code that already produces `LifterRowKeyed` / `AccessorKeyedSingle` / `AccessorKeyedMany` for single-table batched fetchers), and `parentResultType` is `JavaRecordType` or `PojoResultType` from the `@record` directive's resolved class. Multi-table polymorphic children on `@record` parents fall out as a side benefit of the unification rather than as a new feature; today's silent jOOQ-only behaviour becomes either supported or explicitly rejected at validate time, not at runtime.
...
For `@record`-backed parents, validation extends the same rule to whichever derivation arm is selected: `LifterRowKeyed` requires the lifter's column tuple to be non-empty (already implied by the lifter contract); `AccessorKeyedSingle` / `AccessorKeyedMany` require the element table's PK to be non-empty (already required by the typed-accessor contract). All three are guaranteed by the `BatchKey` canonical-constructor sweep, so the validator's only remaining job on the `@record` parent arms is to reject the case where no derivation arm matches at all — which the existing `@record`-parent classification already covers.
...
- `emitMethods` and `emitConnectionMethods` stay as distinct entry points. New private static helpers (`emitParentInputValues`, `emitPerTypenameDispatch` or equivalent names) host the moves shared between list and connection arms.
