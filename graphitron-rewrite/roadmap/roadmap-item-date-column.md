...
- **No backfill, no migration commit.** The renderer tolerates missing values (empty cell). New items stamp on create; existing items stamp the first time anyone runs `status` on them. Data fills in naturally; the column is non-uniform during the transition window, which is itself a useful signal ("these haven't moved since the feature shipped"). The validator stays permissive on the new keys — present-and-malformed fails, absent passes. If full uniformity is ever wanted, a one-shot `git log -1 --date=short --format=%ad -- <file>` walk is a manual one-liner; it does not need to be a tool subcommand.
...
- `README.md` intro paragraph in `Main.java:873` (rendered into `graphitron-rewrite/roadmap/README.md`): document `created:` / `last-updated:` alongside the existing front-matter dimensions, and note that absent dates on pre-R143 items fill in naturally on first transition.

### Phase 5: tests

- `MainTest` (or sibling unit test class): cover `create` stamping both dates today; cover `status` subcommand stamping `last-updated:` today, leaving an existing `created:` untouched, and back-filling `created:` only when absent; cover rejection of invalid transitions.
...
- Forcing uniformity on pre-R143 items via a backfill subcommand. The data fills in naturally on first transition; the column-non-uniformity during the transition window is itself a useful "this hasn't moved" signal.
