...
* **R266 (DELETE walker carrier, Done, `57cb7b0` + `f1ee7a6`)**: **landed in-tree; this spec is rebased onto it.** R266 moved DELETE off `MutationInputResolver` onto the sibling `DeleteRowsWalker` and retired `@value`, so the DELETE half of this design now mirrors the UPDATE half exactly (step 2 mirrors step 1; the two binding-path steps the earlier draft carried for DELETE PK-coverage dropped, see the note after step 4). The flatten-and-rewrap mechanism is carrier-agnostic, so the *design* has no hard dependency on R266 (it would work on either substrate); but because R266 reshaped the DELETE gate and landed first, implementing R186's pre-R266 DELETE path would have written code R266 has already torn out, and both touch the same `MutationInputResolver` lines. The `depends-on: [deleterows-walker-carrier]` edge has been dropped now that R266 is Done; it was recorded for sequencing, not because the carrier shape constrains the design.
