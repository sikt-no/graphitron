...
7. **User documentation** (first-client check):
   ...
   * No changes needed to `directives/table.adoc` (the nested grouping is *not* `@table`-backed) or to the diagnostics glossary unless step 1's `@value`-on-NestingField rejection ships before R266 retires `@value`, in which case the glossary gains one entry.
...
* **Pipeline tier (`GraphitronSchemaBuilderTest`)** — primary classification coverage:
  ...
  * `DML_NESTING_VALUE_ON_NESTING_REJECTED`: `@value` on a `NestingField` carrier on the `MutationInputResolver` payload-returning-UPDATE path trips the per-field rejection (drop on R266 landing; not applicable to the direct-UPDATE path, where R246 already makes `@value` inert).
