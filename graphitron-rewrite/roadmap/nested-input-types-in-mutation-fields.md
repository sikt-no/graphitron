...
7. **Load-bearing classifier checks** (`MutationInputResolver.java` `@LoadBearing` annotations):
   ...
   * **New audit key** `mutation-input.nesting-leaf-carries-path-extraction`: every leaf flattened out of a `NestingField` carries a `CallSiteExtraction.NestedInputField` whose access path's first segment is the immediately-enclosing `NestingField`'s SDL name; every non-nested leaf carries its original extraction unchanged. The emit-site helpers in step 4 add a paired `@DependsOnClassifierCheck` against this key. Without this pin, a future refactor could let a `NestingField`-derived leaf escape with its raw `Direct` extraction and the emitters would silently produce wrong wire access (`in.get("landkode")` instead of `in.get("lokalisering").get("landkode")`) with no compile-time signal.
