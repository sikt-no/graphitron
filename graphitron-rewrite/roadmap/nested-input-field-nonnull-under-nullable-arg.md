...
Wear the contractual form alongside the javadoc per "Classifier guarantees shape emitter assumptions" → "Enforcement (the contractual form)": tag the `BodyParam`-construction sites in `FieldBuilder.walkInputFieldConditions` (`implicitBodyParam` / `compositeImplicitBodyParam` callsites) and the scalar-arg arms of `FieldBuilder.projectFilters` (the `ColumnArg` / `CompositeColumnArg` / `ColumnReferenceArg` / `CompositeColumnReferenceArg` cases that pass the arg's `nonNull()` straight into `BodyParam`) with `@LoadBearingClassifierCheck(key = "body-param.nonnull-is-effective-runtime", description = ...)`, and tag `TypeConditionsGenerator.buildConditionMethod` with `@DependsOnClassifierCheck(key = "body-param.nonnull-is-effective-runtime", reliesOn = ...)`. Both sites already wear paired annotations for other invariants (e.g. the `nodeid-fk.identity-carrying-lift` pair on `projectFilters` / `walkInputFieldConditions`), so this is the established shape. `LoadBearingGuaranteeAuditTest` then surfaces any future producer regression (e.g. a new BodyParam construction site that passes raw `field.nonNull()`) as an orphaned consumer rather than as silent code drift.

## Scope notes for Spec

- **Where the AND happens.** Pushing it into `walkInputFieldConditions` keeps the conjunction local to the consumer and avoids touching the InputField records. The alternative (carry `effectiveNonNull` on each InputField at construction time in BuildContext) would require threading parent nullability through `inputFieldFromNodeIdResolved` and the `@reference` arm, both of which currently see only the field's own type. Consumer-side is simpler and matches the existing `enclosingOverride` precedent. **Migration trigger:** the "Generation-thinking" principle calls for lifting a predicate into the model once two consumers evaluate it; `walkInputFieldConditions` is the sole consumer today. If a second consumer (LSP, validator, a future emitter) ever needs effective nullability over the same chain, lift the conjunction to a slot on `InputField` computed once at construction time. The current consumer-side choice is justified only as long as the count stays at one.
...
- **No validator change.** This is a value-flow tightening on an existing slot, not a new classification. No shape is rejected, no new classifier invariant is introduced, so there's no validator-mirror obligation under "Validator mirrors classifier invariants". Stated explicitly so the next reader doesn't re-walk the question. The `@LoadBearingClassifierCheck` / `@DependsOnClassifierCheck` pair above is the producer-side contract, not a validator-side reject; `ValidateMojo` is unchanged.
