...
`FieldBuilder.java` is 2,787 lines after the directive-axis pass and the first two projection-resolver lifts: Phase 1 lifted `@service` into [`ServiceDirectiveResolver`](../graphitron/src/main/java/no/sikt/graphitron/rewrite/ServiceDirectiveResolver.java), Phase 2a lifted `@tableMethod` into [`TableMethodDirectiveResolver`](../graphitron/src/main/java/no/sikt/graphitron/rewrite/TableMethodDirectiveResolver.java), Phase 2b lifted `@externalField` into [`ExternalFieldDirectiveResolver`](../graphitron/src/main/java/no/sikt/graphitron/rewrite/ExternalFieldDirectiveResolver.java), Phase 2c lifted `@lookupKey` directive-level invariants into [`LookupKeyDirectiveResolver`](../graphitron/src/main/java/no/sikt/graphitron/rewrite/LookupKeyDirectiveResolver.java), Phase 5 lifted OrderBy projection (~230 lines across five tightly coupled helpers) into [`OrderByResolver`](../graphitron/src/main/java/no/sikt/graphitron/rewrite/OrderByResolver.java), and Phase 6a lifted lookup-mapping projection into [`LookupMappingResolver`](../graphitron/src/main/java/no/sikt/graphitron/rewrite/LookupMappingResolver.java). Argument-resolution unification has shipped (Phase 4 landed under Done), so this is no longer blocked.
...
7. The remaining projection resolvers (`Pagination`, `Condition`, `InputField`, `MutationInput`).
8. Final mop-up: helpers move with their nearest resolver, conflict detection extracts if the testability win justifies it.
