...
- **Documentation directory collision, resolved as a rename, not a pick.** Repo-root `docs/` (the `graphitron-docs` site module: `pom.xml`, `index.adoc`, `faq.adoc`, `README.adoc`, `manual/`, ...) and `graphitron-rewrite/docs/` (architecture reference: `README.adoc`, `workflow.adoc`, `rewrite-design-principles.adoc`, ...) are two different trees that both contain a `README.adoc`. `docs/pom.xml`'s `stage-architecture` resource execution already reads `../graphitron-rewrite/docs` as a source distinct from its own `${project.basedir}` (lines ~110-131), and its antrun step copies `../graphitron-rewrite/docs/README.adoc` to `staging/architecture/index.adoc` (line ~153) precisely because the two trees are not the same content. Unlike every other top-level entry, `graphitron-rewrite/docs/` is not a straight `git mv <x> <x>` in commit 2: root already owns `docs/`, so the move collides (git refuses same-target-exists, or a naive per-file merge silently clobbers the site's own `README.adoc`). Resolution: rename `graphitron-rewrite/docs/` to `architecture-docs/` in the unwrap commit, so root `docs/` stays exclusively the site module. Update `docs/pom.xml`'s two `../graphitron-rewrite/docs` references to `../architecture-docs` (the AsciiDoctor output path, `staging/architecture` → public `/architecture/` URL, is the site's own output directory name and is unaffected by the source-tree rename).
...
Two commits, not one. The deletion and the rename are separated so the rename commit gives `git log --follow` a clean boundary; we are **not** routing this through an R19-style history squash.
...
4. **Commit 2 — unwrap.** `git mv` the contents of `graphitron-rewrite/` up to the repo root; promote `graphitron-rewrite-parent` to the root POM (fix `relativePath`s; no two-way merge needed since the legacy parent is already deleted); the surviving `graphitron-javapoet` is now the only copy. `graphitron-rewrite/docs/` is the one exception to "same name, new location": `git mv graphitron-rewrite/docs architecture-docs`, since root already has an unrelated `docs/` (see End state). Keep this commit isolated.
5. Update workflows and CI to drop the `-f graphitron-rewrite/pom.xml` flag and `graphitron-rewrite/` path prefixes: `maven-publish.yml`, `rewrite-build.yml`, `preview-docs.yml`, `.gitlab-ci.yml`, and every workflow `paths:` filter. (`.gitlab-ci.yml` already targets the rewrite reactor, so this is a path-prefix edit, not a legacy-removal.) Also fix `.github/workflows/tree-sitter-natives-release.yml`: its `NATIVES_MODULE` and `GRAMMAR_DIR` env vars hardcode `graphitron-rewrite/graphitron-tree-sitter-natives/...`, which is not a `paths:` trigger filter and is easy to miss by grepping only for that key ; both need the `graphitron-rewrite/` prefix dropped or the release workflow silently breaks on the next natives release after the move.
6. Update docs and tooling paths: `CLAUDE.md` (scope section, common commands), `.claude/web-environment.md`, the `.claude/skills/*` and roadmap-tool path defaults (`graphitron-rewrite/roadmap` → `roadmap`), every `graphitron-rewrite/docs/...` reference across the roadmap and skills (→ `architecture-docs/...`, per the rename in commit 2), `docs/pom.xml`'s `../graphitron-rewrite/docs` staging-resource paths (→ `../architecture-docs`), and `preview-docs.yml`'s `graphitron-rewrite/docs/**` path filter (→ `architecture-docs/**`, a rename not a prefix-strip). Also reword `rewrite-design-principles.adoc`'s "Rewrite builds independently of legacy Graphitron modules" section: it names `verify-standalone-build.sh` and asserts an invariant against pulling in legacy artifacts that no longer exist once commit 1 lands ; per "Documentation names only live tests/code", don't leave it asserting an invariant about a script or a legacy tree that no longer exists at the path it names.
7. Cut a release tag (`v10.0.0` or the next planned RC) to verify the consolidated publish workflow end-to-end against Maven Central staging. Note the release-event publish workflow is governed by `main`'s copy of `maven-publish.yml`, so the hazard only closes once this lands on `main`.
...
- `docs/` (root) and `graphitron-rewrite/docs/` both contain a `README.adoc` today; `docs/pom.xml`'s `stage-architecture` execution and antrun copy step already treat them as distinct trees (see End state), confirming the unwrap needs a rename for `graphitron-rewrite/docs/`, not a same-named move.
