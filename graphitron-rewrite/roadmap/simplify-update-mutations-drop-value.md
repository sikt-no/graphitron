...
* `@condition`-method-shape — pins the reflection contract: every `@condition` method on a mutation takes `(Table t, <scope-appropriate scalars>)`, never a per-row jOOQ record. The bulk-emit path depends on this for v-table compatibility; the validator must guarantee it.
* `WhereFilters` well-formedness — at most one input-type layer, at most one mutation-field layer, override chain rule (at most one `override: true` across layers), no no-op overrides on non-PK input fields. The emitter walks `whereFilters` without re-checking; the validator owns the structural promise.
* `RowIdentity`-emit-arm correspondence — if the field has `RowIdentity.KeyLookup`, the field also has non-empty `lookupKeyFields`; if `RowIdentity.Predicate`, the field's `tableInputArg().whereFilters()` is non-empty and contains an `override: true` layer (or the variant is unreachable); if `RowIdentity.BulkKeyLookup`, the field's `@mutation` carries `multiRow: true`. This guarantees the variant identity matches the payload the consumer reads.
