...
Post-R190 every DataLoader name is `path` alone at five emission sites: `DataLoaderFetcherEmitter.buildDataLoaderName` (~`:155`), `TypeFetcherGenerator` (the batched-fetcher name composition, ×1), `MultiTablePolymorphicEmitter` (the batched connection / list fetchers, ~`:1113` / `:1201`), and `QueryNodeFetcherClassGenerator.dispatchNodes` (~`:156`). Each composes `String.join("/", env.getExecutionStepInfo().getPath().getKeysOnly())`. *(Line numbers as of 2026-06-26; anchor on the symbols, the files drift.)* R45 re-introduces the tenant prefix conditionally, driven by the field's `TenantIdSource`:
...
- Validator-mirror surface: `GraphitronSchemaValidator.validate` gains `validateTenantIdSources(schema, errors)`, draining the new rejection list into `ValidationError`s the same way R190's `validateContextArgumentTypeAgreement` does. The `@LoadBearingClassifierCheck` / `@DependsOnClassifierCheck` annotation pairing follows the R190 / R204 precedent so the classifier and emitter cannot drift.
