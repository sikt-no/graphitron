...
**Consumer-side annotation discipline (intended use of the audit infrastructure, not a loose end).** This subsection is the *intended use* of the load-bearing-check infrastructure for cross-item coordination, not a shipped-broken state. The reviewer should read this as "R141 declares what its emitters trust about the role permit and adds the audit machinery to hold R12's landing accountable to honouring that trust", not as "R141 ships annotations that will fail CI until R12 lands". The mechanism is concrete; see below.

`LoadBearingGuaranteeAuditTest` today rejects any consumer key with no producer (`ORPHANED_CONSUMER` violation; `LoadBearingGuaranteeAuditTest.java:48-54`). R141 extends the audit with a small **pending-producer allow-list**: a documented `Map<String, String> PENDING_PRODUCERS` keyed by consumer-side key, valued by the roadmap item expected to publish the producer (here, `R12 / error-handling-parity`). A key on this allow-list is *exempt* from the orphaned-consumer assertion; instead, the audit emits a `PENDING_PRODUCER` warning (logged but not a violation) so the gap is surfaced at every CI run rather than buried. When R12 lands its producer-side `@LoadBearingClassifierCheck` annotations, R12's commit removes the matching allow-list entry; if R12 ships and forgets, the audit *immediately* converts to a `DUPLICATE_TOLERATED` violation (producer exists but allow-list entry still present), forcing the cleanup. If R12 ships *with* the producer-side annotation but the key name diverges from R141's consumer expectation, the orphaned-consumer assertion fires loudly. CI stays green throughout the R141 → R12 window; the audit emits real failures only on genuinely mismatched contracts.

R141 lands the consumer-side `@DependsOnClassifierCheck` annotations on every emitter that reads `shape.errorChannel()` (the catch-arm builder) and on every emitter that constructs `new ErrorChannel(...)`-shaped values from the unified walk's `ErrorChannelRole` permit. The annotation keys against R12's planned producer keys for the `ErrorChannelRole` classifier rule (canonically the predicate body that produces a well-formed `ErrorChannel`: `error-channel.payload-class-canonical-constructor-shape`, `error-channel.errors-slot-positionally-resolved`, `error-channel.mappings-constant-deduped`; the final key names are R12's to set when it publishes producers). Each of these three keys lands on the R141 allow-list with the value `"R12 / error-handling-parity"`. R12's `Roadmap entries` section is updated as part of R141's commit sequence to call out the allow-list lift as part of R12's landing checklist. The consumer-side annotations are R141's contractual record of what R141's emitters trust about the role permit; the producer-side declarations and matching allow-list lift are R12's contractual record of what R12's classifier guarantees.
...
An error-channel execution test is **not** added in R141 — R12 owns the error-channel execution coverage, and R141's carrier-shape lift relocates R12's production site without changing the `ErrorChannel` record or the catch-arm runtime, so R12's L4 coverage stays load-bearing across the relocation. If R12 ships before R141, R141 inherits R12's coverage and the lift's role-permit classification subsumes R12's standalone walk in commit (1) of R141's sequence; if R141 lands first against R12's `Ready` plan body, the error-channel slot is plumbed via the `ErrorChannelRole` permit and the catch arm follows R12's eventual shape unchanged.
...
**Intra-R141 ordering.** The carrier-shape lift (sealed `CarrierFieldRole`, refactored `SingleRecordCarrierShape`, unified walk in `tryResolveSingleRecordCarrier`) lands **before** the new sealed leaf in R141's commit sequence: the leaf's classifier path consumes the lifted shape, the leaf's `Optional<ErrorChannel>` slot populates from `shape.errorChannel()`, and the no-permit-match rejection truth-table row exercises the lifted shape's rejection path. Commit-level order is (1) introduce `CarrierFieldRole` + refactor `SingleRecordCarrierShape` + consolidate the walks (existing `MutationDmlRecordField` and any other `SingleRecordCarrierShape` consumers migrate to the lifted shape and the unified walk in this commit), (2) add `MutationBulkDmlRecordField` + classifier routing + emitter, (3) add the truth-table rows and execution test.

**R12 coordination.** R141's carrier-shape lift relocates R12's planned `resolveErrorChannel` production site into the unified walk; the `ErrorChannel` record itself is unchanged. R12 is `Ready` (not yet shipped), which makes the coordination cheap: if R141 lands first, R12's plan body adjusts to consume the `ErrorChannelRole` permit when R12 lands (its catch-arm-emitter machinery is unaffected, only the resolver entry point); if R12 lands first, R12 ships the standalone walk and R141 inlines the predicate body into the unified walk in commit (1). Either ordering converges on the same end state: one carrier walk, R12's record produced inside the `ErrorChannelRole` classifier rule, the slot on the field record populated via `shape.errorChannel()`.

**R144 coordination: R141 lands first and absorbs subsequent R144 divergence.** R144 (`mutation-cardinality-safety-default`, Spec) inverts the cardinality-safety polarity for DELETE / UPDATE input fields and retires `@lookupKey` on `INPUT_FIELD_DEFINITION` in favour of a default-filter walk plus `@multiRow` opt-out and `@value` for UPDATE assignment partition. R141 and R144 are peers in the `mutations-errors` theme and have been coordinating fence-line edits in parallel, but **R141 is the lead item** and is expected to reach `In Review` first; R144 lands afterwards. The ordering choice is deliberate: R141's foundational moves (carrier-shape lift, new sealed leaf, classifier-tier coverage of the bulk-carrier matrix) need to be in trunk before R144's polarity flip rewrites the UPDATE filter-column source, so that R141's classifier and emitter paths exist for R144 to retarget rather than R144 having to forward-declare them. R141's design accommodates R144 by reading `tableInputArg.fieldBindings()` (R144's polarity-agnostic surface) rather than the legacy `@lookupKey` gate (§"Mutation-kind coverage" line 282 already notes this); when R144 lands, the binding production site changes but the consumer surface R141 reads does not.
...
The existing test method `productionAnnotationsAreConsistent()` (`LoadBearingGuaranteeAuditTest.java:58`) picks up new producer/consumer pairings automatically via classpath scan; R141 adds two new pairings plus one new audit feature:

- `single-record-carrier-shape.roles-exhaustively-classified` — producer `@LoadBearingClassifierCheck` on `BuildContext.tryResolveSingleRecordCarrier`; consumer `@DependsOnClassifierCheck` on every emitter reading `shape.data()` or `shape.errorChannel()` (data-channel projection, catch-arm builder).
- `mutation-bulk-dml-record-field.input-order-preserving-emit` — producer on `MutationBulkDmlRecordField`; consumer on `TypeFetcherGenerator.buildMutationBulkDmlRecordFetcher`.
- **New audit feature: pending-producer allow-list.** R141 extends `LoadBearingGuaranteeAuditTest` with a `PENDING_PRODUCERS` map (consumer-key → owning-roadmap-item) that exempts listed keys from the orphaned-consumer assertion and emits a `PENDING_PRODUCER` warning instead. The three R12-paired error-channel keys (`error-channel.payload-class-canonical-constructor-shape`, `error-channel.errors-slot-positionally-resolved`, `error-channel.mappings-constant-deduped`) populate this map with the value `"R12 / error-handling-parity"`. A new meta-test `metaTest_pendingProducerAllowlistEmitsWarningNotViolation()` (mirroring the existing `metaTest_orphanedConsumerIsDetected()` at `LoadBearingGuaranteeAuditTest.java:71`) pins this behaviour.
...
| Unified `tryResolveSingleRecordCarrier`; `resolveErrorChannel` removed | `single-record-carrier-shape.roles-exhaustively-classified` audit pair; compile-time (removed method) |
...
| `buildMutationBulkDmlRecordFetcher` emits per-row DML, preserves input order | `bulkInsertWithThreeRowsInNonPkOrderPreservesInputOrderInResponse` + `bulkUpdateWith...` |
...
| Two new load-bearing-check pairings | `LoadBearingGuaranteeAuditTest#productionAnnotationsAreConsistent` picks up automatically |
| Consumer-side R12 annotations + pending-producer allow-list | `LoadBearingGuaranteeAuditTest#metaTest_pendingProducerAllowlistEmitsWarningNotViolation` |
...
- R141 adds consumer-side `@DependsOnClassifierCheck` annotations on every emitter that reads an `ErrorChannelRole` permit (catch-arm builder and friends), keyed against R12's planned producer-side `error-channel.*` classifier checks for the `ErrorChannelRole` rule. R141 extends `LoadBearingGuaranteeAuditTest` with a `PENDING_PRODUCERS` allow-list (consumer-key → owning-roadmap-item) that exempts these three keys from the orphaned-consumer assertion and emits a `PENDING_PRODUCER` warning instead; CI stays green in the R141 → R12 window. R12's landing checklist removes the matching allow-list entries when it publishes the producer-side annotations; if R12 ships without doing so, the audit converts to a `DUPLICATE_TOLERATED` violation. The new meta-test `LoadBearingGuaranteeAuditTest#metaTest_pendingProducerAllowlistEmitsWarningNotViolation` pins this behaviour.
...
- **Coordinates with** [`error-handling-parity.md`](error-handling-parity.md) (R12, `Ready`): R141 reuses R12's `ErrorChannel` record unchanged but relocates its production site from R12's standalone `resolveErrorChannel` walk into the unified carrier walk introduced by the carrier-shape lift. The `Optional<ErrorChannel>` slot on field records keeps its current external API. Either ordering converges on the same end state; R12's plan body adjusts to consume the `ErrorChannelRole` permit when it ships, or R141 inlines the predicate body if R12 ships first. See §"Implementation order" for the convergence detail.
