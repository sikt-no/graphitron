...
The existing test method `productionAnnotationsAreConsistent()` (`LoadBearingGuaranteeAuditTest.java:58`) picks up new producer/consumer pairings automatically via classpath scan; R141 adds one new pairing plus one new audit feature:

- `single-record-carrier-shape.roles-exhaustively-classified` — producer `@LoadBearingClassifierCheck` on `BuildContext.tryResolveSingleRecordCarrier`; consumer `@DependsOnClassifierCheck` on every emitter reading `shape.data()` or `shape.errorChannel()` (data-channel projection, catch-arm builder). This is a real classifier-time shape contract: emitters rely on `shape.roles()` being exhaustively classified to dispatch over a closed set of permits, and `CarrierFieldRoleCoverageTest` (below) is the build-time signal that the contract holds for every permit.
