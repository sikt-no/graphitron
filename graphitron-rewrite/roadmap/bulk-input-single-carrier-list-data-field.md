...
- `CarrierFieldRoleCoverageTest` scans every consumer source file for an explicit dispatch arm against every `CarrierFieldRole` permit. The current implementation is grep-on-source-name; the self-review flagged that this passes on javadoc prose and no-op `case ErrorChannelRole ignored ->` arms. Hardening to reflect over `CarrierFieldRole.class.getPermittedSubclasses()` (mirroring `GeneratorCoverageTest.everyGraphitronFieldLeafHasAKnownDispatchStatus`) is tracked at R151 (`carrier-field-role-coverage-test-hardening`).
...
- **Follow-up tracked at** [`carrier-field-role-coverage-test-hardening.md`](carrier-field-role-coverage-test-hardening.md) (R151, Backlog). R141's self-review flagged that `CarrierFieldRoleCoverageTest`'s grep-based scan passes on javadoc prose mentions of permit names and on no-op `case ErrorChannelRole ignored ->` arms. R151 hardens the check to reflect over `CarrierFieldRole.class.getPermittedSubclasses()` so a missing dispatch arm fails at the audit, mirroring `GeneratorCoverageTest`'s exhaustive-by-construction shape.
