...
- The carrier-resolution classifier in `FieldBuilder` routes `(tia.list() == true, dataField.wrapper().isList() == true, kind ∈ {INSERT, UPDATE, UPSERT})` to `MutationBulkDmlRecordField`. The single-input + list-data-field cell (`tia.list() == false, dataField.wrapper().isList() == true`) is rejected via Invariant #16.
