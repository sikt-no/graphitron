...
Phases A through E, D-list, F (floor), and F (stretch) have shipped to trunk. Each phase preserves R53 wire-compat for non-dotted inputs.
...
| F (stretch) | `reflectServiceMethod` gained a 7-arg overload that accepts `Map<String, GraphQLInputType> slotTypes`; the 6-arg overload delegates with `Map.of()` so existing tests untouched. The new `unambiguousReachablePath` helper walks each slot's input-object structure, matches leaf scalars by canonical Java type (`Int → Integer`, `Float → Double`, `String/ID → String`, `Boolean → Boolean`, with `List<>` wraps propagated), and returns the dotted path iff exactly one matches across all slots. The single production call site (`ServiceDirectiveResolver`) passes the real slot types so the suggestion is concrete; ambiguous and no-match cases fall back to the floor's `<fieldName>` placeholder | (this commit) |
