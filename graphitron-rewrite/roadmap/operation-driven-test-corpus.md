...
*AsciiDoctor render is one extension that knows three things.* It can render an operation by name (loads `.graphql`, emits description + body + variables table), a coordinate's SDL excerpt with surrounding context, and a capability page (preamble from `capabilities/<slug>.adoc` + auto-rendered "Surface" section from exemplar coordinates + auto-rendered "Worked operations" section from operations whose static selection set hits any tagged coordinate). The author writes ~50 lines of capability prose; the rest of the page composes from the joins. The doc site grows three index axes: `/operations/...`, `/capabilities/...`, `/classifications/...`, all derived.
