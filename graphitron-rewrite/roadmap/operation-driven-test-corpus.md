...
*Examples are curated, distinct from exercising operations.* An exercising operation is anything in the corpus whose selection set touches a tagged coordinate; the report counts it for coverage. An *example* is a curated demonstration: a capability author has decided "this operation, in this scenario, with this summary, illustrates a specific facet." One capability can have many examples covering different facets (`simple-cursor-paging`, `filtered-pagination`, `paging-into-polymorphic-children` are all `pagination`); the same operation can be an example for several capabilities; not every exercising operation is an example (most are coverage noise). Examples live in the capability `.adoc`'s frontmatter as a structured list, one entry per (operation, facet, summary):
...
examples:
  - operation: pagination/films-by-year.graphql#FilmsByYear
    facet: simple-cursor-paging
    summary: Cursor + first/after over a flat list.
  - operation: pagination/films-by-actor.graphql#FilmsByActor
    facet: filtered-pagination
    summary: How filter args compose with the cursor.
---
```

The `operation` reference resolves to an `operation_key` and must point at an existing operation in the corpus; the integrity check fails the build on a dangling reference, the same way the slug-resolution check does. Frontmatter rather than body conventions because examples are a finite structured relation between (capability, operation, facet, summary); authoring inertia is lower with explicit fields than with a "follow this convention" body parser.
...
| `capability` | one row per durable capability slug | parsed from `graphitron-rewrite/capabilities/<slug>.adoc` |
...
*AsciiDoctor render is one extension that knows three things.* It can render an operation by name (loads `.graphql`, emits description + body + variables table), a coordinate's SDL excerpt with surrounding context, and a capability page composed of four sections in order: *preamble* (the `.adoc` body prose), *worked examples grouped by facet* (one block per frontmatter `examples:` entry, rendering the referenced operation + author summary), *surface* (auto-rendered exemplar coordinates with SDL excerpts; the participating coordinates via transitive closure live one click away as a "full surface" toggle), and *exercised by* (a coverage-diagnostic block listing operations in the corpus that touch the surface but aren't promoted to examples — useful as a candidate pool for future curation, kept distinct from worked examples so the doc page reads as authored prose, not as test telemetry). The author writes ~50 lines of capability prose plus a short examples list; the rest composes from the joins. The doc site grows three index axes: `/operations/...`, `/capabilities/...`, `/classifications/...`, all derived.
...
|---|---|---|
| A — schema & directive | Rename `leaves`→`classification`, `mentions`→`roadmap_mention`, `trace` view→`classifier_call` (R104 DDL and `ClassificationTrace.java` JSONL field names). Co-emit `generated_fetcher.jsonl` from the codegen visitor at every coordinate where codegen produces a fetcher; add the matching DuckDB table. Define `directive @capability(name: String!) repeatable on OBJECT \| FIELD_DEFINITION \| INTERFACE \| UNION` in graphitron's `directives.graphqls`. Assume `graphitron-rewrite/capabilities/` is populated by R115; add the `capability` table populated by a roadmap-tool scanner of those files; add `capability_coordinate` populated from an SDL parse (exemplars from directive scan; implicit rows from transitive-closure traversal). | Existing R104 tests pass post-rename; a known coordinate produces a `generated_fetcher` row in the codegen pipeline test; the R115 catalog parses cleanly into `capability` rows; a test SDL with one `@capability` directive produces one exemplar row and the expected implicit closure; the integrity-check view "every directive value resolves to an authored slug" returns zero violations against the R115 catalog plus the pilot SDL. |
...
| C — report and pilot | Add `classification_quadrant` view, capability-axis views (capability coverage, capability health, capability → classification, operation → capability static + dynamic), and the integrity-check views (every `@capability` directive value resolves to an authored `.adoc`; every authored slug appears on at least one coordinate; every `examples:` operation reference resolves to an existing operation in the corpus). Pilot: tag the films-paginated relation in `graphitron-sakila-example` with `@capability(name: "pagination")`; author `graphitron-rewrite/capabilities/pagination.adoc` with one `examples:` entry referencing the pilot operation, facet `simple-cursor-paging`; convert the films-paginated test method in `GraphQLQueryTest` to use `OperationRunner` against `src/test/graphql/pagination/films-by-year.graphql` (operation name `FilmsByYear`, plus a sibling for the empty-year case). | Running `mvn install -Plocal-db -Pleaf-coverage` produces non-empty `classification_quadrant` rows (at minimum: a healthy classification for the films `RootField` and the actor/language `ChildField` arms exercised); the `pagination` capability shows as exercised in the capability-coverage view; static and dynamic operation→capability sets agree for the pilot operation; the integrity-check view returns zero violations. |
| D — docs render | AsciiDoctor extension under `/docs/` that loads `.graphql` files, the `capabilities/*.adoc` files (preamble + frontmatter `examples:`), and the SDL, and emits per-operation, per-capability, and per-coordinate pages. | The rendered docs site has the pilot scenario as a worked example with description, body, and variables table; the pagination capability page renders preamble + worked examples (one entry, faceted) + surface (exemplar coordinates) + exercised-by (the coverage-diagnostic block, distinct from worked examples). |
