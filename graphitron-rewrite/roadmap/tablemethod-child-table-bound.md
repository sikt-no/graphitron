...
### Known gap before this item can move In Review

Runtime execution-tier coverage of the table-bound child site (the sakila fixture from commit 3) requires the parent fetcher to project the FK source column when the child `@tableMethod` field is in the selection set. Today `$fields` walks only user-requested SDL fields, so `parentRecord.get(DSL.name("film_id"), Integer.class)` throws `IllegalArgumentException: Field "film_id" is not contained in row type ("public"."inventory"."inventory_id")`. The fix is FK-column injection at classify time, analogous to the projection synthesis that `NodeIdReference` / `CompositeColumnReference` already do — a separable concern from the lift itself but a prerequisite for the execution-tier assertions the Tests section calls for. Land this as a sub-commit before requesting In Review, OR fold it into commit 5 if the DTO-parent emit lands first.
...
- **Sub-commit (preferably before In Review) — FK-projection injection** so commit 3's sakila fixture executes end-to-end, satisfying the table-bound-parent execution-tier assertions in the Tests section.
