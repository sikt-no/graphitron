...
The four new permits carry `JoinStep.LiftedHop hop` plus a new `AccessorTupleRef(ClassName parentBackingClass, String methodName)`. The two `AccessorTableRecord*` permits keep the existing `AccessorRef` (which carries `elementClass` for the `__elt.into(...)` cast). Two ref types, each narrow to its own shape family — the existing principle on narrow component types is preserved.
...
1. **Variants + ref carrier + classifier (model-first slice).**
   ...
   - Add `AccessorTupleRef(ClassName parentBackingClass, String methodName)` next to `AccessorRef`. Reuse `LifterRef` if the contract collapses cleanly; default to a separate record (the carrier role differs semantically — instance accessor vs. static lifter — and component-narrowness wins).
...
- **L4 (pipeline).** `RecordParentFetcherPipelineTest` (or the closest existing pipeline test for accessor-derived keys): assertions track variant shape — `RowN` for `AccessorRow*`, `RecordN` for `AccessorTableRecord*` and `AccessorRecord*`. Includes the parent VALUES emission (`k.field<N>()` vs `DSL.val(k.value<N>())`) for at least one of each shape.
...
- All `switch(batchKey)` sites — at minimum the four switches on `BatchKey` itself plus `GeneratorUtils.buildRecordParentKeyExtraction` and `SplitRowsMethodEmitter`'s parent VALUES emission — are walked exhaustively. The seal makes this `javac`-checked.
