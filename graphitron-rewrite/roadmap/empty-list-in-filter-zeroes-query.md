...
The current empty-list behaviour was **deliberate and test-locked**, not an accidental regression. `GraphQLQueryTest.films_filteredBySameTableNodeId_emptyListReturnsUnfilteredBaseline` (named `...emptyListReturnsNoRows` before this item inverted it) locked exactly the no-rows outcome at spec time, with a comment stating the legacy `empty → noCondition` short-circuit "is gone" on purpose, to align `@nodeId` filters with every other column-equality filter. R230 separately added the `null` guard for the sibling `.in(null) → false` footgun but intentionally left `[]` distinct from `null` (omitted/`null` = no filter → `noCondition`; `[]` = match the empty set → no rows).
...
- **Execution tier** (`GraphQLQueryTest`, the only tier that observes jOOQ's `IN () → false` rendering): invert the empty-list test (`films_filteredBySameTableNodeId_emptyListReturnsUnfilteredBaseline`, renamed from `...emptyListReturnsNoRows`) to expect the unfiltered baseline (all 5 films) and rewrite its comment to record the decision; add a regression test matching the bug report exactly (`filmsConnectionByOptionalIds_idsEmptyList_paginatesFullTableAndCountsAll`: an `@asConnection` query with a list `@nodeId` filter, empty list → unfiltered nodes, and the count path → full total); and add/confirm a lookup-field execution test that an empty input list still yields 0 rows (`inlineLookupTableField_emptyInput_returnsEmpty` covers this) so the fetch/lookup divergence is named by live tests on both sides.
