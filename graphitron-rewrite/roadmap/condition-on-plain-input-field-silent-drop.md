...
2. **Classifier-tier tests assert the type, not the projection.** `GraphitronSchemaBuilderTest`'s plain-input `@condition` cases (`ARG_CONDITION_OVERRIDE`, `FIELD_CONDITION_OVERRIDE`, `INPUT_FIELD_CONDITION_OVERRIDE` at `:4071, :4082, :4169`) assert `schema.type(...) isInstanceOf PojoInputType.class` and stop there. The `@table` siblings (`COLUMN_FIELD_WITH_CONDITION`, `COLUMN_REFERENCE_FIELD_WITH_CONDITION`, `NESTING_FIELD_WITH_CONDITION` at `:4106-4168`) assert `it.inputFields().get(0).condition()` is populated. The asymmetric assertion shape masked the asymmetric projection.
...
Precedence when both channels fire on the same call: a non-empty `condErrors` buffer *and* a single `Unresolved` with non-null `lookupColumn` lift jointly as `Rejection.structural`, joining the column-miss reason and the condErrors prose with `; ` under the same `"plain input type '<typeName>': "` prefix. The composite reason supersedes the column-hint shape; the schema author needs to see both failures, and a typed `UnknownName` that hid the reflection failure would mislead.
...
- `Rejected(rejection)` returns `ArgumentRef.UnclassifiedArg(name, typeName, nonNull, list, rejection)` — the typed `Rejection` rides through, not its `.message()` — mirroring the `@notGenerated` precedent at `FieldBuilder.java:985-1005`. The typed-rejection chain (`docs/typed-rejection.adoc` § "Sealed `Resolved` across the resolver siblings": "a typed value rides on the result without losing structure") runs end-to-end: an `AuthorError.UnknownName` produced by `InputFieldResolver.resolve` reaches `UnclassifiedField.rejection` as `AuthorError.UnknownName`, not as a `Structural` collapse of its `message()`. Acceptance tests 4 and 6 depend on this; threading the typed value is part of R205, not a follow-up.

To make that chain hold, four collateral edits land in the same change:

- **`ArgumentRef.UnclassifiedArg`** (`ArgumentRef.java:359-365`): the `String reason` component becomes `Rejection rejection`; a `public String reason() { return rejection.message(); }` convenience preserves existing String-only call sites without forcing them to refactor at the same time.
- **The four other `UnclassifiedArg` construction sites in `classifyArgument`** (`FieldBuilder.java:990, :998, :1023, :1034`): each wraps its current prose with `Rejection.structural(...)`. None of these sites has a typed-payload upstream to preserve, so `Structural` is the right arm.
- **`FieldBuilder.projectFilters`** (`:1312-1449`): the out-param `List<String> errors` becomes `List<Rejection>`. The two `errors.add("argument '" + u.name() + "': " + u.reason())` sites at `:1352, :1356` become `errors.add(u.rejection().prefixedWith("argument '" + u.name() + "': "))`; the `prefixedWith` plumbing on every `Rejection` arm (interface method at `Rejection.java:30`, exhaustive overrides at `:62, :74, :93, :127, :167, :210, :256, :312, :326, :353`) preserves the typed arm across the prefix step.
- **`FieldBuilder.projectForFilter`** (`:1223-1224`): the single-rejection case forwards the typed value directly; multi-rejection cases fold to `Structural`:
  ...
  if (filters == null) {
      return new TableFieldComponents.Rejected(
          errors.size() == 1
              ? errors.get(0)
              : Rejection.structural(errors.stream()
                    .map(Rejection::message)
                    .collect(Collectors.joining("; "))));
  }
  ```

  The sibling `errors.add(r.message())` sites at `:1230, :1242` (ConditionResolver / OrderByResolver rejection forwarding) become `errors.add(r.rejection())`; those resolvers already return typed rejections, so the prior `.message()` call was the only typed-payload loss point on those paths.
...
- `graphitron/src/main/java/no/sikt/graphitron/rewrite/FieldBuilder.java:985-1005` — `@notGenerated` precedent; `:990, :998, :1023, :1034` — the four other `UnclassifiedArg` construction sites updated to wrap typed `Rejection.structural(...)` (Step 2 plumbing); `:1006-1008` — `classifyArgument` PlainInputArg construction (switch on `Resolution`); `:1223-1224` — `projectForFilter`'s single-rejection / multi-rejection typed-forwarding (Step 2 plumbing); `:1337-1350` — `projectFilters` branches for both ArgumentRef variants; `:1351-1358` — `UnclassifiedArg` / `UnboundArg` propagation (Step 2 lifts the `errors` accumulator to `List<Rejection>` here, using `Rejection.prefixedWith` instead of String concat); `:1500-1568` — `walkInputFieldConditions` and `:1511, :1524, :1546` — the `if (implicitBodyParams != null && …)` guards that simplify after Step 1(a).
- `graphitron/src/main/java/no/sikt/graphitron/rewrite/ArgumentRef.java:359-365` — `UnclassifiedArg` record; Step 2 lifts the `String reason` component to `Rejection rejection` so the typed payload from `InputFieldResolver.Resolution.Rejected` rides through to `UnclassifiedField.rejection`.
- `graphitron/src/main/java/no/sikt/graphitron/rewrite/BuildContext.java:1489-1518` — `buildInputFieldCondition` (`condErrors`-appending path that Step 2 closes); `:1570-1656` — `classifyInputFieldInternal` Unresolved construction sites (`@reference` path miss, terminal column miss, NestingField failure aggregation).
...
- `graphitron/src/main/java/no/sikt/graphitron/rewrite/model/Rejection.java:48-65, :473-476` — `AuthorError.UnknownName` record and `Rejection.unknownColumn(summary, attempt, candidates)` factory used by Step 2; `:30, :62, :74, :93, :127, :167, :210, :256, :312, :326, :353` — `prefixedWith` interface method and exhaustive arm overrides used by Step 2's typed-payload prefix step.
...
- `graphitron/src/test/java/no/sikt/graphitron/rewrite/GraphitronSchemaBuilderTest.java:4071-4176` — plain-input classification cases to extend with projection-shape assertions; new bare-resolved-no-condition case.
