...
- R215 (`column-binding-at-classification-not-usage.md`) — the structural restructure that subsumes R211's placeholder. Under R215 the override:true gate moves above the column lookup, so the column-miss arm is unreachable on this branch by construction. R211's `doesNotContain("no column 'sakskode' found")` assertion stays as a regression guard; the placeholder code can be removed once R215 lands.
