...
2. **Each data field of the outcome type** ; a **`LightDataFetcher`**. `LightDataFetcher.get(GraphQLFieldDefinition, Object sourceObject, Supplier<DataFetchingEnvironment>)` hands the source straight in as a parameter, so the fetcher pattern-matches the `sourceObject` it is given: `sourceObject instanceof Outcome.Success<?> success ? <the field's own read off success.value()> : null`. On the `ErrorList` arm it resolves `null`; graphql-java renders the data field null and does not descend, while the non-null `Outcome` source keeps the sibling errors field reachable.
...
- **`@table`-bound DataLoader data fields** (`RecordTableField`, `RecordLookupTableField`, the consumer's case): the fetcher reads the correlation key off `success.value()`, dispatches the DataLoader, and returns the `CompletableFuture`; on the `ErrorList` arm it returns `CompletableFuture.completedFuture(null)` **without dispatching**. As a `LightDataFetcher` it reaches the loader via `environmentSupplier.get()`; or it stays a full `DataFetcher` reading the narrowed source. Either way the fetcher does its own unwrap, no env-rebuild.
