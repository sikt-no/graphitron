...
Paths live on the data-bearing leaves (`Scalar`, `ListOf`), not on composites. In default mapping every sibling field's `sdlPath` shares a prefix; in the forward-compatible scattered `@argMapping` syntax (`paramName: { fieldA: input.x, fieldB: input.other.y, ... }`) sibling paths are independent. The model represents both uniformly: only the path values differ. The scattered parser isn't part of R238's scope; the model just doesn't need changes when it lands.
...
4. **Derive `ValueShape` for each `FromArg` entry.** Starting from the SDL type at the path's leaf and the Java target type, build the value subtree:
   ...
   - Java-driven descent (forward-compatible with scattered `@argMapping` syntax `paramName: { fieldA: input.x, fieldB: input.other.y }`): iterate the Java target's record-components or JavaBean properties, look up each leaf's path from the explicit per-field entry, resolve it through a flat `GraphQLInputObjectType.getField` chain, and assemble the `ValueShape` bottom-up. R238 doesn't ship the scattered parser, but the model and walker accommodate it without restructuring.
