...
* `getting-started.adoc` — runtime wiring. The existing `## Federation` section grows the contributor-level rationale (`@link` opt-in semantics, the `<schemaInput tag>` federation-implicit decision, the `fetchEntities` seam, why `Graphitron.buildSchema` does the wrap rather than the consumer). D9 (federation flow) anchors the section. The existing `## Dev loop` section grows the runtime framing (LSP server, schema watcher, classpath watcher, generator dispatch in one JVM) as the contributor-side complement to the consumer-side "what you do / what you observe" prose. D7 (dev-loop runtime) anchors that section.
...
A `SealedHierarchyDocCoverageTest` lives next to `DirectiveDocCoverageTest` and walks `Resolved.permits()` (and any other sealed hierarchy the chapter enumerates), asserting each permit's class name appears in the documenting page. Same shape as the existing four `DocCoverageTest` suites: bidirectional, tied to a closed set the compiler already exhaustivity-checks. If the chapter prose decides to *not* enumerate (describe shape rather than variants and let javadoc carry the per-permit detail), the test scope shrinks accordingly; the spec leaves that call to the implementer once the prose drafts are concrete. The decision is reflected in the test scope, not in the choice of whether to test.
...
* Augment `getting-started.adoc § Federation` with the contributor-level rationale (`@link` semantics, the `<schemaInput tag>` decision logic, the `Graphitron.buildSchema` federation-wrap rule, the `fetchEntities` seam). Add D9 (federation flow).
* Augment `getting-started.adoc § Dev loop` with the runtime-framing prose (LSP + schema watcher + classpath watcher in one JVM, generator dispatch, idempotent-write coupling to IDE recompile). Add D7 (dev-loop runtime).
...
* Replace the two affected sections in `rewrite-design-principles.adoc` ("Builder-step results …", "Wire-format encoding …") with one-line forward pointers into `typed-rejection.adoc`.
