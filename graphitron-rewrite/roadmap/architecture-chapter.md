...
| D10 | Typed-rejection sealed hierarchy as a classDiagram: `Rejection.{AuthorError \| InvalidSchema \| Deferred}` as the spine, with `AuthorError.{UnknownName \| Structural \| AccessorMismatch}` and `InvalidSchema.{DirectiveConflict \| Structural}` sub-permits drawn out, plus `LookupKeyDirectiveResolver.Resolved.{Ok, Rejected}` overlaid as a worked example of how a resolver's typed-result wraps a `Rejection`. The chapter prose names the sealed-`Resolved` pattern across the thirteen sibling resolvers (per `rewrite-design-principles.adoc:63`) but does not enumerate per-resolver arms; javadoc on each `*DirectiveResolver.Resolved` carries that detail. | `typed-rejection.adoc`
|===
...
D10 and the `typed-rejection.adoc` prose name one closed set the test walks: `Rejection` and its sub-permits (`AuthorError.{UnknownName | Structural | AccessorMismatch}`, `InvalidSchema.{DirectiveConflict | Structural}`, `Deferred`). The "Documentation names only live tests/code" principle (`rewrite-design-principles.adoc`) makes unenforced enumerations a drift surface: a new permit lands, the prose silently goes stale, readers trust a false invariant.

A `SealedHierarchyDocCoverageTest` lives next to `DirectiveDocCoverageTest` and walks `Rejection.permits()` transitively, asserting each permit's class name appears in `typed-rejection.adoc`. Same shape as the existing four `DocCoverageTest` suites: bidirectional, tied to a closed set the compiler already exhaustivity-checks.

The rule is mechanical: *if the chapter prose names a permit, the test pins it; if the prose describes shape only ("rejection is a typed variant; see javadoc on each `*DirectiveResolver.Resolved` for permits") and lets javadoc carry the per-permit detail, no test entry for that hierarchy.* Test scope follows prose enumeration; the spec does not punt the decision to the implementer because the rule is what governs both halves. Applied to the chapter as currently sketched: `Rejection` and its sub-permits are pinned (D10 draws them; the prose enumerates them). The sealed-`Resolved` pattern is *not* pinned by this test — there is no single `Resolved` class with concrete permits; thirteen sibling resolvers each declare their own (e.g. `LookupKeyDirectiveResolver.Resolved.{Ok, Rejected}` shown as D10's worked example, `ServiceDirectiveResolver.Resolved.{TableBound | Result | Scalar | ErrorsLifted | Rejected}`, etc.), and the chapter describes the cross-resolver pattern shape-only with javadoc carrying per-resolver detail. `BatchKey` is referenced but not enumerated (the principles doc enumerates it; that doc is not in the test's scope), so it is also not pinned by this test.
