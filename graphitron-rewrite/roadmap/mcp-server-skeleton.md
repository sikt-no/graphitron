...
Coordinates, pinned in the parent pom's `dependencyManagement` (versions are the latest GA at implementation time; confirm exact patch then):

- MCP Java SDK: import `io.modelcontextprotocol.sdk:mcp-bom` (the 2.x line, which tracks the 2025-11-25 MCP spec; the BOM's most recent release as of this writing is 2026-06-12). Depend on `io.modelcontextprotocol.sdk:mcp` without a version. The core `mcp` artifact ships the Streamable HTTP server transport with no web-framework dependency.
...
One escape hatch the implementer should check before pinning: if the 2.x SDK ships a self-contained Streamable HTTP **server** provider over `com.sun.net.httpserver` (not the servlet one), prefer it; it is zero added container dependency and still real SDK transport code, and it proves an equally real seam for R118. Verify against the SDK's server-transport docs; absent that, Jetty is the default and the resolved choice.
...
- **`graphitron-mcp/.../GraphitronMcpServer.java`**: `public final class GraphitronMcpServer implements AutoCloseable`, mirroring `DevServer`'s shape. Constructor takes an `InetSocketAddress` (so production passes `127.0.0.1:8488` and the test binds an ephemeral port), builds the `HttpServletStreamableServerTransportProvider` on the `/mcp` endpoint path, builds an `McpSyncServer` registering `serverInfo` (name `graphitron` to match the client-config server key; version held to the graphitron artifact version, read from the jar manifest rather than a hardcoded literal so it does not drift; the dynamic project-identity stamp stays Deferred), the `instructions` string, and the `about` prompt, mounts the provider servlet in an embedded Jetty `Server` at `/mcp`, and starts it. Exposes `port()` (the bound local port, for ephemeral-port tests and the startup log). `close()` stops Jetty and closes the MCP server. A failed bind surfaces as an `IOException` (Jetty wraps the underlying `BindException`); the constructor translates/propagates it so the caller can produce the Mojo message, exactly as `DevServer` does.
- **`DevMojo.bindServer`** (`DevMojo.java:144-155`): after the LSP `DevServer` is constructed, construct `GraphitronMcpServer` on `127.0.0.1` and the MCP port. That port lives in a package-private `int mcpPort` field defaulting to a `DEFAULT_MCP_PORT = 8488` constant (mirroring `DEFAULT_PORT`/`port`), but **not** a `@Parameter`: it stays non-overridable for users (the configurable port is Deferred) while letting `DevMojoTest` inject an ephemeral taken port instead of binding the well-known 8488. On bind failure, translate to `MojoExecutionException` with a clear message ("graphitron:dev: MCP port 8488 is already in use ...") mirroring the existing LSP arm, and **close the already-constructed LSP `DevServer` before rethrowing** so a partial bind does not leak the LSP socket. Hold the server in a field and close it in **`DevMojo.cleanup`** (`DevMojo.java:303-309`) alongside `server.close()`. So the bind-failure test can assert no LSP-socket leak, relax `DevMojo.server` from `private` to package-private and add a package-private `DevServer.isClosed()` backed by the existing `closed` `AtomicBoolean` (`DevServer.java:47`).
- **`DevMojo.execute`**: extend the existing startup log line (`DevMojo.java:130-131`) with a second line naming the MCP URL and a copy-pasteable `claude mcp add --transport http graphitron http://127.0.0.1:8488/mcp`. No config file is generated or mutated at startup.
...
The contributor-facing "How this is wired" subsection (`getting-started.adoc:355-385`) is updated in this same item, not deferred to R118: "four cooperating components" becomes five, the MCP server gets its own bullet and a node in the D7 mermaid diagram, and the Ctrl+C cleanup sentences (`:352-353`, `:366`) name the MCP server alongside the LSP socket, watchers, and debounce. The fifth component lands with this skeleton, so that prose stops being accurate the moment R341 ships; the fix rides here rather than depending on a later item.
...
- `graphitron-rewrite/graphitron-maven-plugin/.../DevMojo.java:144-155` (`bindServer`, the `BindException → MojoExecutionException` arm to mirror), `:303-309` (`cleanup`), `:130-131` (the startup log line to extend).
...
- `graphitron-rewrite/graphitron-lsp/.../LspVocabulary.java:44-46`, `:78-88`: the "shape, not state, read once at startup" precedent for the static content.
- `graphitron-rewrite/docs/getting-started.adoc` § "Dev loop" (`:305-353` the LSP connection prose this item extends; `:355-385` the four-cooperating-components contributor section the MCP server becomes a fifth sibling of).
