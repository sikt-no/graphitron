...
- *`Wrap.TableRecord(XRecord)` / MANY* (new): cast `(List<XRecord>) env.getSource()`; `source.isEmpty()` short-circuit; PK-keyed map; iterate `source` and read PK columns via the typed `record.get(<XTable.<PK_FIELD>>)` accessors. Composite-PK keying uses `List.of(r.get(pk1), r.get(pk2), …)` paralleling the `Wrap.Record` MANY arm's map-key shape. Same input-order projection shape, typed against `XRecord` instead of `RecordN`.
...
**Carrier types are producer-kind-monomorphic.** A single-record carrier type returned by both a DML and a `@service` mutation would land both registrations at the same `(carrierType, dataFieldName)` coords with different `Wrap` shapes. New load-bearing key `carrier-data-field.single-producer-kind` pins this on the `FieldRegistry.reclassify` site: a second registration of `SingleRecordTableField` at the same coords with a different `Wrap` shape rejects with a typed `Rejection` naming both mutations and the carrier type. The validator surfaces the same predicate, tagged `@DependsOnClassifierCheck` against the same key, so the two sites stay aligned. The schema author's recourse is to split the carrier into two payload types (one per producer kind) or to converge the producers.

The rejection is order-dependent today: whichever mutation classifies second triggers the typed `Rejection`. This is an accepted trade-off for the simpler producer-side enforcement — the validator's mirror surfaces the rejection at the carrier level (order-independent) when it runs, which is sufficient for the schema author's diagnostic.
