...
No separate validator-mirror site. The producer-side rejection lands as `UnclassifiedField` + `Rejection.structural`, which the existing `validateUnclassifiedField` path (`GraphitronSchemaValidator.java:118, 895`) surfaces as a `ValidationError` at validate time. The "Validator mirrors classifier invariants" principle (`rewrite-design-principles.adoc:101-105`) targets *unimplemented generator branches* that would otherwise escape to runtime; a carrier-coord conflict is not that shape — the classifier produces a typed rejection that already fails the build, so a parallel validator walk would duplicate logic without adding a catchable case.

The producer-side rejection names both mutations. The first producer to register for a given `(carrierType, dataFieldName)` writes its `RegisteredProducer(mutationName, wrap)` to `ctx.carrierProducerRegistry` alongside the `fieldRegistry.reclassify` call (see helper contract above). The second producer, on mismatched wrap, reads the previously-registered mutation name back out of the map and builds a diagnostic of the shape "@service mutation '`opprettRegelverksamling`' classifies '`OpprettRegelverksamlingPayload`.regelverksamlinger' with `Wrap.TableRecord(RegelverksamlingRecord)`, conflicting with previously-registered DML mutation '`insertRegelverksamling`' which classifies with `Wrap.Record`; split the carrier into two payload types (one per producer kind) or converge the producers." Order-dependence is only about which mutation hosts the `UnclassifiedField`; both producer names appear in the diagnostic regardless of registration order.
