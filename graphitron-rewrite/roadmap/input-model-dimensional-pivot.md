...
`InputTypeGenerator` is unrelated — it emits the `<TypeName>Type.type()` schema-rebuild scaffolding for graphql-java's runtime schema, walks `schema.getInputObjectTypes()` directly, and never reads any carrier. It stays per-input and is touched only by R222's Phase 3 / 4 to drop `TableInputType`-permit discrimination, not by the validation pivot.
...
- `InputTypeGenerator`: iterate `schema.getInputObjectTypes()` directly via graphql-java runtime types; permit-identity dispatch (`TableInputType` discrimination) retires. `InputTypeGenerator` reads no carrier (schema-rebuild scaffolding is independent of classification). `InputRecordGenerator` migrated in Phase 1; nothing to do here.
...
- **No per-input model record, no per-input map.** Per-input walkers don't exist in R222. The validation POJO emit (`InputRecordGenerator`) is consumer-driven: it walks output fields and emits one POJO per input-type-typed arg. `InputTypeGenerator` (schema-rebuild scaffolding) walks `schema.getInputObjectTypes()` directly via graphql-java and reads no carrier.
