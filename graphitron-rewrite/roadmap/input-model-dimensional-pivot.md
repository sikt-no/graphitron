...
Acceptance: model additions compile; every existing input today produces an equivalent `Input` from the type map with `validationShape` populated by the walker rather than by classifier-side derivation; the per-input emit is unchanged; output of `InputRecordGenerator` is byte-identical to trunk.
