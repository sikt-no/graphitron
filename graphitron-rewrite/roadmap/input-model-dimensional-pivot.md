...
- **Nested input type *with* its own `BackingClass.JooqTableRecord` binding:** R122 (`compound-entity-mutations`) territory — the nested `InputUsage` carries its own `table` distinct from the parent's. The natural SDL declaration is `@reference(path:)` on the nested-input slot, naming the FK from the parent table to the child table; the path's terminal element is the child's `table`, and the FK column the parent's PK lands in is read from the path. The recursive `InputUsage` model already admits this shape — `Optional<TableRef> table` carries no constraint that the nested table must equal the parent's. R222 does not implement the visitor arm (the slot's `@reference(path:)` is read for `@nodeId` leaves today but not yet for nested-input slots) and does not implement the mutation emitter that consumes it; today's classifier produces `Unresolved("no column 'X' reachable")` at the parent's level, which lifts to `UnboundField` under R215, and the pivot inherits that outcome verbatim. R122 lands as a body change at the visitor and a new mutation-emitter arm; the model shape is in place.
...
- At each `argument` visit on an `INPUT_OBJECT`-typed arg, produce the `ArgumentOccurrence` with an `InputTypeArg` carrying a root `InputUsage`. The root's `table` is populated from the enclosing field's `@table` return when applicable. The root's `backingClass` resolves in this order: (1) if `table` is present, `backingClass = BackingClass.JooqTableRecord(<TableRecord-class-for-table>, table)` — the jOOQ `TableRecord` is the natural binding carrier for any table-bound site; (2) else if a user service method consumes the visited field, `backingClass` is the param-type-derived arm (`Pojo` or `JavaRecord`) classified by the class's runtime shape; a param typed as a jOOQ `Record` subclass that is *not* a `TableRecord` surfaces `Rejection.AuthorError("backing class %s is a jOOQ Record but not a TableRecord; supported non-table backings are Java record or POJO")` and the visitor does not construct an `InputUsage` for that arg; (3) else, empty (graphitron's SQL emission walks `classifiedFields` directly, no DTO target needed). When a service-method param is present and the use site is table-bound, the visitor verifies the param is itself a `JooqTableRecord` for the same table; mismatch surfaces as `Rejection.AuthorError` (invariant #2's pre-construction check). The visitor recurses into each nested-input field declaration in `Input.fields()`: it constructs a child `InputUsage` applying the same three-step resolution against the nested context. **By default**, the parent's table is forwarded into step (1) of the nested resolution (today's "nested fields inherit the parent's column-resolution scope" semantics), and parent-class reflection through the field's name drives step (2). **R122 (`compound-entity-mutations`) extends step (1)**: when the nested-input slot carries an `@reference(path:)` directive whose terminal element resolves to a table different from the parent's, the child's `InputUsage.table` is the terminal table, the child's `backingClass` is the corresponding `JooqTableRecord`, and the FK column the parent's PK lands in is read from the path. R222 does not implement that arm (the model slot is in place; the visitor body and the mutation emitter that consumes it are R122's), but the seam is named here so R122's spec phase does not reinvent it. The recursion is structural: `InputField.NestingField.nested` holds the child `InputUsage` directly.
