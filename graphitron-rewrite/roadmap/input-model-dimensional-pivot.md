...
- The orchestrator's per-field dispatch lights up in this phase (per-input dispatch lit up in Phase 1 with `ValidationShapeWalker`); the same `carrier-slot-always-populated-with-arm` key now covers both substrates' slots.
...
- **Identity keys.** Per-input carriers key by input type name (`String`), matching the existing rewrite-wide pattern: `GraphitronSchema.types`, `fieldsByType`, `entitiesByType`. For type+field identity (a `Diagnostic.location` companion, or a future per-input-field carrier map), graphql-java's `graphql.schema.FieldCoordinates` is the established key — already in use across `GraphitronSchema.fields`, `FieldRegistry`, `MappingsConstantNameDedup`.
- **No per-input model record.** Per-input walkers consume `GraphQLInputObjectType` directly as their substrate; per-input emitters (`InputRecordGenerator`, `InputTypeGenerator`) iterate `schema.getInputObjectTypes()` and look carriers up in per-family maps by `GraphQLInputObjectType.getName()`.
