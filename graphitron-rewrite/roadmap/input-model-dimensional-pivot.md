...
- **Substrate-parametric walkers.** Each walker is `Walker<S, C>` where `S` is the SDL substrate it walks (`GraphQLFieldDefinition` for per-output-field walkers, `GraphQLInputObjectType` for per-input walkers, additional substrates if a future axis needs them) and `C` is the carrier family it produces. The walker returns `WalkerResult<C>` — a sealed `Ok(carrier, diagnostics)` / `Err(errors, diagnostics)`. Walkers are pure functions over `graphql.schema.*` primitives; they do not consult a shared graphitron-internal classification model and do not depend on a shared mutable sink.
- **Carrier storage is asymmetric and honest.** Per-output-field carriers attach as non-Optional slots on `OutputField` — the sealed interface (`extends GraphitronField permits RootField, ChildField`) pre-exists the pivot, the carrier-getters land as abstract methods on the interface with each permit record (`RootField`, `ChildField`) carrying the storage as record components, and the existing carrier-independent state (`DomainReturnType`, the producer-of-source story) makes the permits the natural host. Per-input carriers live in per-family maps on the classification artifact, keyed by input type name (`String`, e.g. `"FilmInput"`); no per-input model record is introduced because per-input identity has no carrier-independent state worth a record. Where a walker or diagnostic needs to address a specific field on an input (e.g. a future per-input-field carrier, or a `Diagnostic`'s location attribute), graphql-java's `FieldCoordinates` is the existing rewrite-wide key for type+field identity (already in use across `GraphitronSchema.fields`, `FieldRegistry`, `MappingsConstantNameDedup`). Every slot and every map entry is populated with exactly one arm — a valid arm or the explicit absent arm (`No<Family>`). Consumers pattern-match the arm at use time; absence is a first-class domain state, not a present/missing flag. There is no `Invalid<Family>` arm: structural failure is the wrapper's concern, not the carrier's, and consumers never see one.
- **Each substrate elects its walkers.** During classification, the substrate's properties drive walker dispatch procedurally. An output field reads its directive set + return-type shape to elect per-field walkers (described in "Walker dispatch" below). Each `GraphQLInputObjectType` runs every per-input walker the orchestrator registers (today: `ValidationShapeWalker`; future: R98's `ConstraintSetWalker`). No top-level sealed family on either substrate; impossible combinations are excluded at dispatch. Slots and map entries whose walker was not elected default to the family's `No<Family>` arm.
- **Walkers are independently unit-testable.** Parse a small SDL fragment, pull the relevant substrate (a `GraphQLFieldDefinition` or `GraphQLInputObjectType`), run the walker, assert on the returned `WalkerResult` (the `Ok`/`Err` arm, the carrier on `Ok`, the errors on `Err`, the diagnostics list on both). Pure function in, pure sealed-result out; no mocks required. Pipeline coverage comes as a by-product when downstream emitters consume the carriers.
- **Schema diagnostics ride with the walker result.** The carrier shape is no longer 1-1 with the SDL. The orchestrator collects every `WalkerResult` post-walk across every substrate; non-error diagnostics (severity Warning, Information, Hint — from both `Ok` and `Err` results) flow to `BuildContext` and to the LSP wire surface; errors (only ever on `Err` results) accumulate and block downstream generation (validator cross-checks, generators, per-type emitters), surfacing all `AuthorError`s in one pass rather than aborting on the first. Classification itself runs to completion regardless — every walker is invoked on every substrate instance, and every `WalkerResult` is collected — so the LSP can consume populated carrier slots and diagnostics for the parts of the SDL that classified cleanly even when other parts errored. Downstream consumers run only when every `WalkerResult` was `Ok`, so they never observe a failure state. The wrapper's `Ok`/`Err` arms make this an enforced contract: an `Err` walker cannot produce a carrier to install, and the slot stays in `No<Family>` for the rest of the classification pass.
...
| `ValidationShape` | `GraphQLInputObjectType` | per-input Jakarta-validation type shape (R94, renamed from `InputRecordShape`); consumer is `InputRecordGenerator` | single valid arm (`ValidationShape.Of`), holds per-field validation-target metadata |
...
The substrate column matters because dispatch and orchestration are per-substrate: the orchestrator iterates each substrate kind (output fields, input types, ...) and runs every elected walker against each instance. The `WalkerResult` aggregation is uniform across substrates; consumers read the carrier either from `OutputField` slots (per-output-field) or from the classification artifact's per-family maps via the input type's name (per-input).
...
**Per-output-field dispatch.** The output field decides which per-field walkers to run from its own properties:
...
**Per-input dispatch.** Every `GraphQLInputObjectType` runs every registered per-input walker — there is no directive-driven election analogous to the output-field side, because the per-input substrate has no equivalent of "this directive on this field shape excludes that walker." This asymmetry is incidental to the substrates: `Walker<S, C>` / `WalkerResult<C>` / orchestrator aggregation behave uniformly across both sides; only the elect-walkers function differs in shape (a procedural table on the output-field side, an unconditional iteration on the input side). The substrate-parametric contract is not carved out for either substrate. Today the registered set is `{ValidationShapeWalker}`. Future additions are additive: R98 adds `ConstraintSetWalker`, R164's per-input concerns (if any) drop in as further entries, etc. The dispatch is a single iteration over `schema.getInputObjectTypes()` × `perInputWalkers`; carriers collect into the classification artifact's per-family maps, keyed by each substrate's name.

**Three options were considered for the dispatch rule itself.** *(a) A sealed type at the field level* — each combination of electable walkers as a permit. Rejected: this re-introduces the cross-product encoding the pivot exists to dissolve; adding a walker multiplies the permits. *(b) Capability-marker-driven dispatch* — read the field's existing capability markers (`SqlGeneratingField`, `MethodBackedField`) and map each capability to a `Set<Walker<?, ?>>`. Rejected: walker election isn't a pure capability mapping. It involves negation (`@service` *excludes* every other walker, totalising), conditional fanout (`@tableMethod` elects MethodArguments AND Pagination AND Ordering), and ordering semantics (PredicateCarrier sentinel directives trigger arm flips that no capability marker carries). A capability-to-walker-set map flattens these decisions; the procedural function keeps them readable in one place. *(c) Procedural dispatch* (chosen) — a single function per substrate reads the substrate's properties and decides which walkers to invoke. The constraint rules live in code that reads top-to-bottom, are individually testable, and don't pretend to be type-enforced when they're not.

Adding a new carrier (Aggregation, GroupBy, future axes) is additive: new walker, new storage (slot on `OutputField` for per-field carriers, new map on the classification artifact for per-input carriers), new entry in the dispatch table. No enclosing taxonomy churns.
...
R222 introduces no new model record on the per-input side and no new identity wrapper. Per-input walkers consume `GraphQLInputObjectType` directly as their substrate; the carriers they produce live in per-family maps on the classification artifact, keyed by input type name (`String`):

```java
record Classification(
    // per-output-field carriers attach as slots on OutputField (sealed interface; see below).
    // per-input carriers live in per-family maps keyed by input type name:
    Map<String, ValidationShape> inputValidationShapes
    // future per-input walkers add additional maps (R98's ConstraintSet, etc.)
) { }
```

The key shape matches the existing rewrite-wide pattern: `GraphitronSchema.types` (`Map<String, GraphitronType>`), `fieldsByType` (`Map<String, List<GraphitronField>>`), `entitiesByType` (`Map<String, EntityResolution>`) all key by type name. Per-input carrier maps slot into the same idiom. Keying by name rather than by `GraphQLInputObjectType` reference identity is honest to the SDL identity, survives schema rebuilds without reference-equality concerns, and aligns with the identity LSP clients address. The map invariant is "every input type in the schema appears as a key, with a valid or `No<Family>` arm as the value" — pinned by the same `carrier-slot-always-populated-with-arm` audit that covers `OutputField`'s slots.

Where a walker or diagnostic needs to address a *field* on an input — e.g. a future per-input-field carrier map, or the source location attached to a `Diagnostic` — graphql-java's `FieldCoordinates` is the established rewrite-wide key for type+field identity (`GraphitronSchema.fields`, `FieldRegistry`, `MappingsConstantNameDedup` all use it). R222 doesn't ship such a map, but the pattern is the one to reach for.

Per-input emitters (`InputRecordGenerator`, `InputTypeGenerator`) iterate `schema.getInputObjectTypes()` directly, read each substrate's name via `GraphQLInputObjectType.getName()`, and look the carrier(s) up in the relevant map(s). No per-input model record means no pass-through wrappers over graphql-java accessors and no sealed-interface-with-one-permit scaffolding.

Per-output-field carriers attach as abstract getter methods on the existing `OutputField` sealed interface; each permit record (`RootField`, `ChildField`) implements them as a record component. `OutputField` pre-exists the pivot (`public sealed interface OutputField extends GraphitronField permits RootField, ChildField`); the permits already host carrier-independent identity (`DomainReturnType`, the producer-of-source story). Consumers pattern-match through the interface (`field.predicate()`, etc.); the storage lives on each permit record:
...
Each slot is a sealed family; the record holds exactly one arm. Slots and map entries are always populated; the arm answers what the substrate actually does on that dimension:

- Walker was not elected for this substrate → the slot (or map entry) defaults to the family's `No<Family>` arm.
- Walker was elected and returned `Ok` with a valid result → the slot/entry holds the relevant valid arm (e.g. `Condition`, `MethodArguments.Of`).
- Walker was elected and returned `Ok` with nothing meaningful → the slot/entry holds `No<Family>` (the diagnostic stream may carry a `CarrierProducedNothing` event).
- Walker was elected and returned `Err` → the slot/entry stays at `No<Family>` for the rest of classification. The wrapper's errors join the build's accumulated error list; downstream generation is blocked, so no per-type generator or validator consumer observes the `No<Family>` placeholder. Introspection consumers (the LSP) may read the slot/entry directly and render the error diagnostic alongside.
...
sealed interface ValidationShape {              // per-input carrier; today's R94 shape
    record Of(...) implements ValidationShape {}
    record NoValidationShape() implements ValidationShape {}
}
```

Every family is two-arm: a valid arm (or two, for `PredicateCarrier`) and the explicit `No<Family>` arm. No family carries an `Invalid` arm. Structural failure is the `WalkerResult.Err` wrapper's concern, not the carrier's; the wrapper guarantees that downstream generators reading the slot only see valid or `No`. An earlier draft of this spec carried `InvalidPredicates(AuthorError cause)` and `InvalidMethodArguments(AuthorError cause)` on the two families whose consumers were thought to fork on structural failure, with the other four families surfacing failure through the diagnostic stream alone. Review pointed out that no downstream generator actually inspects a hypothetical `Invalid` arm — generation is blocked before any generator runs in either failure mode — so the asymmetry encoded an invariant the wrapper plus the classification/generation phase split already enforces. Removing the arms collapses the carrier vocabulary to one shape across all seven families and pulls the validity question out of the type the consumers pattern-match.
...
| `HasInputRecordShape` capability marker (5 permits) | retires. `ValidationShape` lives in `Classification.inputValidationShapes` keyed by input type name |
...
| 9 consumer-side discriminations on `TableInputType` (instanceof gates and switch arms) | retire; consumers read the relevant carrier slot or map entry directly |
...
R222 ships three walkers in tree as exemplars. Together they prove the abstraction across both substrates and the carrier-shape range:

- `ValidationShapeWalker` (Phase 1, per-input substrate) — the simplest case. Single-arm valid carrier, no bailout, no totalising directive. Demonstrates the substrate-parametric abstraction on `Walker<GraphQLInputObjectType, ValidationShape>` and lights up the orchestrator-aggregation contract on the per-input substrate before the more complex per-field walkers land. Replaces R94's classifier-side derivation; the per-input emit is byte-identical against trunk.
- `ConditionWalker` (Phase 2, per-output-field substrate) — demonstrates the bailout/restart pattern, on a `Walker<GraphQLFieldDefinition, PredicateCarrier>`.
- `MethodArgumentsWalker` (Phase 2, per-output-field substrate) — demonstrates the totalising directive, on a `Walker<GraphQLFieldDefinition, MethodArguments>`.

All three are independently unit-testable; all three have downstream consumers ready to migrate (Phase 3 for the per-field walkers; the per-input emit already reads `Classification.inputValidationShapes` keyed by input type name from Phase 1).
...
|---|---|
| R171 (sealed `InputLikeType` parent) | Dissolves: no per-input model record survives the pivot; per-input carriers live in coordinate-keyed maps. The parent-root fix becomes moot |
...
| R213 (rejections at consumer field) | Walker-time `SourceLocation` is the substrate's own SDL location (`GraphQLInputObjectField.getDefinition().getSourceLocation()` / arg's source location), paired with the substrate's identity (type name for input types, `FieldCoordinates` for input fields) |
...
- **R98** (multi-source input validation): once `ValidationShape` is a name-keyed map on the classification artifact, attaching `ConstraintSet` is a one-site change — add `Map<String, ConstraintSet>` alongside, populated by `ConstraintSetWalker`. Follow-up.
...
1. **`carrier-slot-always-populated-with-arm`**: every per-output-field carrier slot on `OutputField` is non-null and holds exactly one arm of its sealed family; every per-input carrier map on the classification artifact has an entry for every input coordinate in the schema, with each value holding exactly one arm. When a walker is not elected for the substrate, the slot/entry is initialized to the family's `No<Family>` arm. When an elected walker returns `Ok`, the slot/entry is set to `result.carrier()`. When an elected walker returns `Err`, the slot/entry stays at `No<Family>` for the rest of classification; downstream generation is blocked, so no per-type generator or validator consumer reads the placeholder. The LSP may read the slot/entry directly (it consumes classification output independent of whether generation ran). Pinned at the orchestrator; the audit walker catches null or uninitialized slots and missing map entries across both substrates.

2. **`substrate-emit-reads-only-own-substrate-carriers`**: per-output-field consumers (validator, fetcher generators, query-conditions emitters) read carrier slots on `OutputField` only; per-input consumers (`InputRecordGenerator`, `InputTypeGenerator`) read carrier entries from the per-input maps by input type name only. Neither side reaches across substrates, and neither side bypasses to walk the SDL itself. Pinned with `@DependsOnClassifierCheck` on each substrate's emit entry points.

**Carried by the type system, not the audit:**

- A walker `Walker<S, C>` returns `WalkerResult<C>`. The compiler enforces that an `Ok` result's carrier is one arm of `C`'s family — no separate `walker.produces-only-its-own-family` audit is needed. (The R222 draft listed this as a load-bearing key; it's a tautology under generics and adds no audit value beyond the type signature.)
...
- The legacy `input.record-shape-derived-from-backing` key retires entirely with the removal of the per-input backing-class slot (and of any per-input model record). `ValidationShape` is purely SDL-derived now and produced by `ValidationShapeWalker` like every other carrier.
- The earlier draft's `input-usage.*` keys retire with `InputUsage`'s removal. The walker model produces no shared recursive carrier; the orchestrator-level slot-population invariant plus the per-walker type-system guarantees (`Walker<S, C>` signature, `WalkerResult.Err` compact constructor) replace the recursive-tree invariant.
...
### Phase 1: introduce the per-input carrier maps, the carrier vocabulary, the substrate-parametric walker abstraction, `WalkerResult`, and the simplest example walker (`ValidationShapeWalker`)

- No new identity wrapper is introduced. Per-input carrier maps key by input type name (`String`), matching the existing rewrite-wide pattern (`GraphitronSchema.types`, `fieldsByType`, `entitiesByType`). Where a walker or diagnostic needs to address a field on an input, graphql-java's `FieldCoordinates` (already in use across `GraphitronSchema.fields`, `FieldRegistry`, `MappingsConstantNameDedup`) is the type+field key; R222 ships no map of that shape, but the pattern is the one to reach for if a future walker needs it.
...
- **R94's `InputRecordShape` becomes the `ValidationShape` carrier family.** Rename the type to `ValidationShape`. The R94 capability marker `HasInputRecordShape` stays in source during the Phase 1-3 additive migration window (it continues to mark the legacy `*InputType` permits while they exist); the new carrier lives in `Classification.inputValidationShapes` keyed by input type name, not through the marker. The marker class deletes in Phase 4 with the legacy permits. `ValidationShape` is sealed with `Of(...)` and `NoValidationShape` arms; today's walker always produces `Of(...)`.
- Implement `ValidationShapeWalker implements Walker<GraphQLInputObjectType, ValidationShape>` — the simplest concrete walker, demonstrating the substrate-parametric abstraction on a single-arm carrier. The walker reads the input type's field declarations and emits the same `ValidationShape.Of(...)` value R94's classifier currently computes. Unit tests are pure-graphql-java fragments parsed and walked; the orchestrator-aggregation contract is exercised on this walker before `ConditionWalker` / `MethodArgumentsWalker` ship in Phase 2.
- Add a `Map<String, ValidationShape> inputValidationShapes` slot on the classification artifact, populated post-walk from `ValidationShapeWalker`'s results. Every input type in the schema appears as a key; orchestrator initialises unwalked entries to `NoValidationShape`.
- Add a non-Optional getter for each per-output-field carrier family on the `OutputField` sealed interface; add the matching record component to each permit (`RootField`, `ChildField`), each defaulting to its `No<Family>` arm. Compiler-visible, no consumer behavior change.
- Land the orchestrator scaffolding that iterates substrates, dispatches walkers, aggregates `WalkerResult`s, and surfaces errors before blocking downstream generation. Classification runs to completion regardless of errors; the orchestrator's contract is the classification/generation phase split. Phase 1's only live producer is `ValidationShapeWalker`; the orchestrator's contract is the same once Phase 2's per-field walkers join.
...
- The first load-bearing key lands here: `carrier-slot-always-populated-with-arm` (parametric on `OutputField` slots and on per-input carrier maps), pinned on the orchestration site. The second (`substrate-emit-reads-only-own-substrate-carriers`) lands in Phase 3 once the per-type generators migrate.

Acceptance: model additions compile; every input coordinate in the schema appears as a key in `Classification.inputValidationShapes` with `Of(...)` populated by the walker rather than by classifier-side derivation; the per-input emit is unchanged; output of `InputRecordGenerator` is byte-identical to trunk; a fixture that triggers a `DirectiveDeprecated` walker diagnostic (e.g. `@table` on input) surfaces it in the LSP's `publishDiagnostics` stream with code `"graphitron.directive-deprecated"` and `tags: [Deprecated]`.
...
- The orchestrator's per-field dispatch lights up in this phase (per-input dispatch lit up in Phase 1 with `ValidationShapeWalker`); the same `carrier-slot-always-populated-with-arm` key now covers both substrates' slots. The previous draft's `walker.produces-only-its-own-family` and `walker-result.invalid-arm-paired-with-author-error` keys do not land — both are now structural (carried by `Walker<S, C>`'s signature and by `WalkerResult.Err`'s compact constructor, respectively).
...
- `InputRecordGenerator`, `InputTypeGenerator`: iterate `schema.getInputObjectTypes()` directly, read each substrate's name via `GraphQLInputObjectType.getName()`, and look the relevant carrier(s) up in the per-input maps (today: `Classification.inputValidationShapes`); permit-identity dispatch retires. These two consumers read only per-input carrier maps, never any per-output-field carrier; `substrate-emit-reads-only-own-substrate-carriers` pins the boundary.

Compiler exhaustiveness on each carrier's sealed family is the safety net for every consumer migration; the per-input substrate's graphql-java type is the iteration anchor.
...
- Delete the `HasInputRecordShape` capability marker class — orphaned once the legacy `*InputType` permits delete above (the new carrier lives in `Classification.inputValidationShapes` keyed by input type name, not via the marker).
...
- **R94** (shipped): `HasInputRecordShape` is the existing R94 marker; under this pivot the validation-shape carrier becomes a coordinate-keyed entry in `Classification.inputValidationShapes`, populated by `ValidationShapeWalker` via `WalkerResult` rather than through the marker. The R94 invariant (validationShape derived from backing class) retires because the per-input substrate carries no backing class; the shape is purely SDL-derived and now produced by the same walker pattern as every other carrier.
...
- **Identity keys.** No bespoke coordinate wrapper. Per-input carriers key by input type name (`String`), matching the existing rewrite-wide pattern: `GraphitronSchema.types` is `Map<String, GraphitronType>`, `fieldsByType` is `Map<String, List<GraphitronField>>`, `entitiesByType` is `Map<String, EntityResolution>`. For type+field identity (a `Diagnostic.location` companion, or a future per-input-field carrier map), graphql-java's `graphql.schema.FieldCoordinates` is the established key — already in use across `GraphitronSchema.fields`, `FieldRegistry`, `MappingsConstantNameDedup`. An earlier R222 draft introduced a `SchemaCoordinate(String)` wrapper that conflated both shapes; review surfaced that it added no type-safety over `String` for the type-name case and duplicated `FieldCoordinates` for the type+field case, so the wrapper retires.
- **No per-input model record.** Per-input walkers consume `GraphQLInputObjectType` directly as their substrate; per-input emitters (`InputRecordGenerator`, `InputTypeGenerator`) iterate `schema.getInputObjectTypes()` and look carriers up in per-family maps by `GraphQLInputObjectType.getName()`. Earlier drafts of R222 introduced `Input` / `InputFieldDecl` wrapper records; the pivot dropped them because per-input identity has no carrier-independent state worth a record, and graphql-java already provides every accessor those wrappers would have exposed.
- **`Walker<S, C>`** — a pure function over an SDL substrate `S` returning `WalkerResult<C>`. Built on graphql-java primitives. Independently unit-testable. Stateless across invocations; no shared mutable sink. Per-output-field walkers are `Walker<GraphQLFieldDefinition, ?>`; per-input walkers are `Walker<GraphQLInputObjectType, ?>`. Future substrates (e.g. types, enums) are additive.
- **`WalkerResult<C>`** — sealed `Ok<C>(C carrier, List<Diagnostic> diagnostics)` / `Err<C>(List<AuthorError> errors, List<Diagnostic> diagnostics)`. `Ok.carrier` is one arm of `C`'s family (valid or `No<C>`). `Ok` rejects Error-severity diagnostics by compact-ctor; `Err.errors` is non-empty by compact-ctor invariant. The orchestrator surfaces every error before blocking downstream generation. Classification runs to completion across every substrate regardless of how many walkers returned `Err`. Non-error diagnostics ride on either arm. Independent of the substrate the walker walked.
- **Carriers** — `Pagination`, `Ordering`, `PredicateCarrier`, `MethodArguments`, `InsertRows`, `UpdateRows` (per-output-field, attached as slots on `OutputField`); `ValidationShape` (per-input, attached as a value in `Classification.inputValidationShapes` keyed by input type name). Each is a two-arm sealed family with valid arm(s) plus the explicit absent arm (`No<Family>`). No family carries an `Invalid` arm; structural failure rides on `WalkerResult.Err`, not inside the carrier. Each carrier is produced by exactly one walker; every slot and every map entry holds a valid arm or `No<Family>`.
...
- **Unit-tier (new, primary):** `ValidationShapeWalker` tests. One test per SDL shape variation that R94's existing classifier-side derivation covers, asserted now against the walker's `WalkerResult.Ok(ValidationShape.Of(...))`. Exercises the substrate-parametric abstraction on the simplest carrier — single-arm valid carrier, no bailout, no totalising directive — and pins the orchestrator-aggregation contract on the per-input substrate before the more complex per-field walkers land in Phase 2.
...
- **Unit-tier (new):** orchestrator aggregation across substrates. Build with one per-input walker (`ValidationShapeWalker`) and two per-field walkers (`ConditionWalker`, `MethodArgumentsWalker`), one returning `Err`, the others returning `Ok` → orchestrator collects every walker's result across both substrates, surfaces every `AuthorError`, blocks downstream generation; carrier slots remain at their `No<Family>` defaults on the `Err` branches but other walkers' `Ok` slots are populated (introspection consumers like the LSP can read them). Build with N `Err` walkers spanning both substrates → all N error sets are surfaced in one pass, not just the first; classification completes through every walker regardless. Pins the "classification runs to completion; generation blocks" contract and the substrate-parametric aggregation invariant.
...
- **Walker-per-output-field multiplies work for inputs reused across many consumers.** Each output field that elects a walker pays per-field walker invocation cost; an input reused across ten fields runs each elected walker ten times. Mitigation 1: the walker walk is over graphql-java's reflective accessors on `GraphQLInputObjectField` lists; typically low tens of fields, microseconds per invocation. Profile before optimising. Mitigation 2: if profiling shows a hotspot, walkers can deduplicate per `(GraphQLInputObjectType, Consumer)` pair via a side cache keyed by input type name. The cache lives outside the model; structural equality is the contract.
...
The cure this draft commits to: separate the *consumer concerns* into independent *carriers*; produce each by an independent *walker* over an SDL substrate; store the carriers where the substrate's own emit can read them, with arms — valid or the explicit `No<Family>` — that are part of each family's domain model. No top-level sealed hierarchy on any substrate; impossible combinations are excluded by dispatch-time election rather than by type. Walkers are pure functions over graphql-java primitives returning a sealed `WalkerResult<C>`: `Ok(carrier, diagnostics)` on success, `Err(errors, diagnostics)` on structural failure. Diagnostics are LSP-aligned by construction (severity, code, source, message, tags, relatedInformation), so the editor experience — hover, inlay hints, decorations, jump-to-cause — falls out of the walker's structured output without a parallel translation layer. Classification runs to completion across every substrate regardless of how many walkers errored; the orchestrator collects every walker's result, surfaces every error, and blocks the downstream generation phase from running when any `Err` is present. Downstream generators never observe a failure state — the wrapper plus the phase split handles it. Introspection consumers (the LSP) read classification output directly and so see populated carriers for the parts that classified cleanly even when others didn't. Each axis is independently unit-testable.

The walker abstraction is substrate-parametric, not output-field-keyed. Per-output-field consumer concerns attach as slots on the existing `OutputField` sealed interface (whose permit records `RootField` / `ChildField` already host carrier-independent identity); per-input consumer concerns live in per-family maps on the classification artifact, keyed by input type name (no per-input model record, because per-input identity has no carrier-independent state worth a record); future substrates (types, enums, ...) follow whichever shape is honest for that substrate. The asymmetry between the two storage shapes is the honest one — symmetric structure would either force a vestigial record onto the per-input side or strip identity-carrying state from the per-output-field side. Drawing the abstraction at a single substrate would recreate the asymmetry the pivot exists to dissolve, with some derivations being walker outputs and others being stored slots computed by ad-hoc classifier paths. The uniform shape — same `Walker<S, C>`, same `WalkerResult` aggregation, same orchestrator halt rule — is the property that makes adding R98's `ConstraintSetWalker` or a hypothetical `EnumValidationWalker` a drop-in extension rather than a structural decision.
