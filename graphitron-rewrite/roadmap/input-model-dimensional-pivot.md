...
The `Input` record carries what's true of the SDL declaration in isolation: name, location, the assembled-schema form, the SDL-derived per-input-type emit shape (R94's `InputRecordShape`), and the pre-binding SDL field declarations. `Input` carries no backing-class slot: the active backing class is a use-site fact that lives on `InputUsage`, resolved by the visitor at construction time. At the root, the resolution is: (1) when the use site has an effective table, the backing class is `JooqTableRecord(<TableRecord-for-table>, table)` — the jOOQ-generated `TableRecord` is the natural carrier whether the table came from a `@table`-returning consumer or from a service-method param typed as a `TableRecord`; (2) when there is no table binding but a user service method consumes the input, the backing class is the param's runtime-shape arm (`Pojo` / `JavaRecord` / `JooqRecord`); (3) otherwise, empty. For nested use sites, the backing class is derived by reflection through the parent's class. The legacy SDL hints `@record(class:)` and `@table` on input types are dropped at this pivot; see §"`@table` and `@record(class:)` on inputs are dropped" below. `BackingClass` is a four-arm sealed family that carries what the four `*InputType` permits used to encode in their identity. `InputFieldDecl` is the SDL declaration (name, type, location, directives) before any table binding is applied. `InputUsage` is the recursive classified-state carrier: it pairs an `Input` with a resolved `(backingClass, table)` and the classified fields produced for that pair.
...
|---|---|
| `GraphitronType.InputType` 4-arm sealed root | `Input` (no backing-class slot); 4 arms move onto `BackingClass` with the arm chosen by the consumer-derived class's runtime shape; the active backing class at a use site lives on `InputUsage.backingClass()` |
...
- Implement an adapter `Classifier.adaptLegacy(GraphitronType.InputType)` that constructs an `Input` plus, given a use-site `(BackingClass, TableRef)`, an `InputUsage`. The `Input` adapter is lossless on the SDL-side facts (name, location, recordShape, fields); the legacy `@record(class:)` directive that today drove the permit choice is read by the adapter only to seed the Phase 1-2 `InputUsage.backingClass` (preserving binding behavior until Phase 3 ships the visitor's resolver-param-derived path). Phase 3 emits the `BuildWarning` and retires the SDL-side read. The `InputUsage` adapter is lossless against the legacy classified-field shape (the body lifts `BuildContext.classifyInputFieldInternal` onto the new classifier entry point).
