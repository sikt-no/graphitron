 ...
 * unbound), produced when an {@link Input} is wrapped in a {@link InputUsage} for a
...
The `Input` record carries what's true of the SDL declaration in isolation: name, location, the assembled-schema form, the SDL-derived per-input-type emit shape (R94's `InputRecordShape`), and the pre-binding SDL field declarations. `Input` carries no backing-class slot: the active backing class is a binding-site fact that lives on `InputUsage`, resolved by the visitor at construction time. At the root, the resolution is: (1) when the binding site has an effective table, the backing class is `JooqTableRecord(<TableRecord-for-table>, table)` — the jOOQ-generated `TableRecord` is the natural carrier whether the table came from a `@table`-returning consumer or from a service-method param typed as a `TableRecord`; (2) when there is no table binding but a user service method consumes the input, the backing class is the param's runtime-shape arm (`Pojo` / `JavaRecord` / `JooqRecord`); (3) otherwise, empty. For nested binding sites, the backing class is derived by reflection through the parent's class. The legacy SDL hints `@record(class:)` and `@table` on input types are dropped at this pivot; see §"`@table` and `@record(class:)` on inputs are dropped" below. `BackingClass` is a four-arm sealed family that carries what the four `*InputType` permits used to encode in their identity. `InputFieldDecl` is the SDL declaration (name, type, location, directives) before any table binding is applied. `InputUsage` is the recursive classified-state carrier: it pairs an `Input` with a resolved `(backingClass, table)` and the classified fields produced for that pair.

`InputField.NestingField` flips from `List<InputField> fields` to `InputUsage nested` (see Phase 1 below). The recursion that was implicit in the legacy "nested fields inherit parent table" prose becomes a structural property of the model: a nested input's binding context is whatever the parent's classifier produced for it, carried explicitly on the nested `InputUsage`.

```java
record InputTypeArg(
    String name,
    InputUsage bound,                             // the classified-state carrier, root of the recursive tree
    boolean nonNull,
    boolean list,
...
// The consumer-side coordinate that drove the table binding at the root InputUsage.
// Recorded for diagnostics and LSP hover only; the binding fact itself lives on
// InputUsage.table(). The origin is empty when the binding came from the input's
...
`ConsumerBinding` retires as a slot on `InputTypeArg`; the table fact it used to carry now lives on `InputUsage.table()`. The producer-coordinate it carried (for diagnostics) lives either inside a per-arg diagnostic event keyed on `ArgumentOccurrence.consumer()` or, for cross-walk lookups, on a side map keyed by `InputUsage` identity. The simplification: there is one slot for "the table at this binding site" (`InputUsage.table`), not two halves to compose.

The two-source table model collapses cleanly because the `@table` directive on input types is dropped at this pivot (see §"`@table` on input is dropped" below). With the directive gone, only two sources of binding remain: the input's backing class (consumer-independent; lives on `BackingClass.JooqTableRecord`) and the consumer's `@table` return (consumer-dependent; resolved at the root `InputUsage`'s construction). The visitor composes them once at construction time; downstream consumers read `InputUsage.table()` without reproducing the predicate.
...
|---|---|
| `GraphitronType.InputType` 4-arm sealed root | `Input` (no backing-class slot); 4 arms move onto `BackingClass` with the arm chosen by the consumer-derived class's runtime shape; the active backing class at a binding site lives on `InputUsage.backingClass()` |
...
| `JooqTableRecordInputType.table` slot, `TableInputType.table` slot | Both collapse to `InputUsage.table` (the binding-site fact) + `InputUsage.backingClass = JooqTableRecord(<TableRecord-for-table>, table)`. The legacy distinction (was the table sourced from `@record(class:)` reflection or from `@table`/consumer chain) erases at the binding-site model: both legacy carriers produce a `JooqTableRecord`-backed `InputUsage` for the same table. The legacy directive-driven path is dropped, surfacing as `BuildWarning`s for the duration of Phase 6's sweep |
...
| `InputField.NestingField.fields` (`List<InputField>`, parent-table-inherited) | `InputField.NestingField.nested` (`InputUsage`): the nested binding context is structural rather than inherited; future R122 nested `@table` lands as `nested.table` distinct from the parent's |
...
- Implement an adapter `Classifier.adaptLegacy(GraphitronType.InputType)` that constructs an `Input` plus, given a binding-site `(BackingClass, TableRef)`, an `InputUsage`. The `Input` adapter is lossless on the SDL-side facts (name, location, recordShape, fields); the legacy `@record(class:)` directive that today drove the permit choice is read by the adapter only to seed the Phase 1-2 `InputUsage.backingClass` (preserving binding behavior until Phase 3 ships the visitor's resolver-param-derived path). Phase 3 emits the `BuildWarning` and retires the SDL-side read. The `InputUsage` adapter is lossless against the legacy classified-field shape (the body lifts `BuildContext.classifyInputFieldInternal` onto the new classifier entry point).
...
- At each `argument` visit on an `INPUT_OBJECT`-typed arg, produce the `ArgumentOccurrence` with an `InputTypeArg` carrying a root `InputUsage`. The root's `table` is populated from the enclosing field's `@table` return when applicable. The root's `backingClass` resolves in this order: (1) if `table` is present, `backingClass = BackingClass.JooqTableRecord(<TableRecord-class-for-table>, table)` — the jOOQ `TableRecord` is the natural binding carrier for any table-bound site; (2) else if a user service method consumes the visited field, `backingClass` is the param-type-derived arm (`Pojo` / `JavaRecord` / `JooqRecord`) classified by the class's runtime shape; (3) else, empty (graphitron's SQL emission walks `classifiedFields` directly, no DTO target needed). When a service-method param is present and the binding site is table-bound, the visitor verifies the param is itself a `JooqTableRecord` for the same table; mismatch surfaces as `Rejection.AuthorError` (invariant #2's pre-construction check). The visitor recurses into each nested-input field declaration in `Input.fields()`: it constructs a child `InputUsage` applying the same three-step resolution against the nested context — parent's table forwarded into step (1), parent-class reflection through the field's name into step (2). The recursion is structural: `InputField.NestingField.nested` holds the child `InputUsage` directly.
...
- `CatalogBuilder` four sites: each reads `arg.usage()` for binding-site facts; `arg.input()` only when an SDL-side fact is needed (name, source location, record shape).
- `InputRecordGenerator`, `InputTypeGenerator`: read `Input.recordShape()` and `Input.schemaType()` directly; permit-identity dispatch retires. These two consumers do *not* touch `InputUsage`; the `per-type-emit-ignores-binding-context` invariant pins that boundary.
...
- **`BackingClass`** replaces the four `*InputType` permit names. Reading `boundInput.backingClass()` directly answers "which Java class materialises this input at this binding site?", which is what consumers of that signal always wanted. The active fact lives on `InputUsage`; `Input` carries no backing-class slot. The arm is selected by a three-step resolution at the binding site: table-bound → `JooqTableRecord(<TableRecord-for-table>, table)`; non-table-bound with a service-method consumer → param-type runtime-shape arm (`Pojo` / `JavaRecord` / `JooqRecord`); otherwise empty. Nested binding sites inherit step (1) via the parent's table and reach step (2) via parent-class reflection.
...
- **`ConsumerBinding`** retires as a slot on `InputTypeArg`; the table fact lives on `InputUsage.table()`. The visit-site coordinate that drove the inference lives on the enclosing `ArgumentOccurrence` (for the root) and is recoverable from the parent walk for nested nodes.
...
- **Pipeline-tier (new):** cross-consumer divergence. One input used by two consumers with different return tables produces two `InputTypeArg` occurrences whose root `InputUsage.table()` slots hold the distinct tables. Today's `InputType` collapse becomes per-binding-site success.
