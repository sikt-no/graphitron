...
| `JooqTableRecordInputType.table` slot, `TableInputType.table` slot | Both collapse to `BoundInput.table` (the binding-site fact) + `BoundInput.backingClass = JooqTableRecord(<TableRecord-for-table>, table)`. The legacy distinction (was the table sourced from `@record(class:)` reflection or from `@table`/consumer chain) erases at the binding-site model: both legacy carriers produce a `JooqTableRecord`-backed `BoundInput` for the same table. The legacy directive-driven path is dropped, surfacing as `BuildWarning`s for the duration of Phase 6's sweep |
...
- **Pipeline-tier (new):** consumer-derived path. Input with no `@table` directive used by a single `@table`-returning consumer produces an `InputTypeArg` whose root `BoundInput.table().isPresent()` carries the consumer's table; the consumer's `SchemaCoordinate` is recoverable from the enclosing `ArgumentOccurrence`. The root `BoundInput.backingClass()` is `JooqTableRecord(<TableRecord-for-table>, table)` (the table-bound arm of the three-step resolution). R97 Phase 2's acceptance fixture lands here. Companion: a non-table-bound input consumed by a user service method whose param is a Java record produces a `BoundInput.backingClass()` of `JavaRecord(<class>)`; a non-table-bound input reached only by graphitron-emitted resolvers produces empty.
