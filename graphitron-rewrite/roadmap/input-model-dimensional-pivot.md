...
- **Carrier slots live on the model record whose per-instance emit reads them.** Per-output-field consumer concerns (predicate emission, method-args, pagination, ordering, INSERT row plans, UPDATE row plans) attach as carrier slots on `OutputField`. Per-input consumer concerns (Jakarta-validation type emit, future R98 multi-source `ConstraintSet`) attach as carrier slots on `Input`. Each slot is non-Optional and always populated with exactly one arm — a valid arm or the explicit absent arm (`No<Family>`). Consumers pattern-match the arm at use time; absence is a first-class domain state, not a present/missing flag. There is no `Invalid<Family>` arm: structural failure is the wrapper's concern, not the carrier's, and consumers never see one.
- **Each substrate elects its walkers.** During classification, the substrate's properties drive walker dispatch procedurally. An output field reads its directive set + return-type shape to elect per-field walkers (described in "Walker dispatch" below). An `Input` runs every per-input walker the orchestrator registers (today: `ValidationShapeWalker`; future: R98's `ConstraintSetWalker`). No top-level sealed family on either substrate; impossible combinations are excluded at dispatch. Slots whose walker was not elected default to the family's `No<Family>` arm.
...
The substrate column matters because dispatch and orchestration are per-substrate: the orchestrator iterates each substrate kind (output fields, input types, ...) and runs every elected walker against each instance. The `WalkerResult` aggregation is uniform across substrates; consumers never see substrate identity, only the carrier slot on the relevant model record.
...
**Per-input dispatch.** Every `Input` instance runs every registered per-input walker — there is no directive-driven election analogous to the output-field side, because the per-input substrate has no equivalent of "this directive on this field shape excludes that walker." Today the registered set is `{ValidationShapeWalker}`. Future additions are additive: R98 adds `ConstraintSetWalker`, R164's per-input concerns (if any) drop in as further entries, etc. The dispatch is a single iteration over `schema.inputs()` × `perInputWalkers`; carriers collect onto `Input`'s slots.
...
Adding a new carrier (Aggregation, GroupBy, future axes) is additive: new walker, new slot on the relevant substrate's model record, new entry in the dispatch table. No enclosing taxonomy churns.
...
`Input` carries SDL declaration fields plus per-input carrier slots, the same pattern `OutputField` follows on the per-field side:
...
sealed interface Input extends GraphitronType, EmitsPerTypeFile permits Input.Of {
    ...
    ValidationShape validationShape();   // carrier slot, populated by ValidationShapeWalker
}
...
`Input` carries the SDL declaration (name, location, the assembled-schema form, the pre-binding SDL field declarations) plus one carrier slot per registered per-input walker. Today that's `validationShape: ValidationShape`; future per-input walkers (R98's `ConstraintSet`) add slots additively. No backing class. No table. No classified field list. The per-input emit (`InputRecordGenerator`) reads `Input.validationShape()` for Jakarta-validation type generation; the slot is a carrier the same way `OutputField.predicate()` is.
...
Each slot is a sealed family; the record holds exactly one arm. Slots are always populated; the arm answers what the substrate actually does on that dimension:
...
All three are independently unit-testable; all three have downstream consumers ready to migrate (Phase 3 for the per-field walkers; the per-input emit already reads `Input.validationShape()` from Phase 1).
...
1. **`carrier-slot-always-populated-with-arm`**: every carrier slot on every model record (`OutputField`, `Input`) is non-null and holds exactly one arm of its sealed family. When a walker is not elected for the substrate, the slot is initialized to the family's `No<Family>` arm. When an elected walker returns `Ok`, the slot is set to `result.carrier()`. When an elected walker returns `Err`, the slot stays at `No<Family>` for the rest of classification; downstream generation is blocked, so no per-type generator or validator consumer reads the placeholder. The LSP may read the slot directly (it consumes classification output independent of whether generation ran). Pinned at the orchestrator; the audit walker catches null or uninitialized slots across both substrates.

2. **`substrate-emit-reads-only-own-substrate-carriers`**: per-output-field consumers (validator, fetcher generators, query-conditions emitters) read carrier slots on `OutputField` only; per-input consumers (`InputRecordGenerator`, `InputTypeGenerator`) read carrier slots on `Input` only. Neither side reaches across substrates, and neither side bypasses to walk the SDL itself. Pinned with `@DependsOnClassifierCheck` on each substrate's emit entry points.
...
- The legacy `input.record-shape-derived-from-backing` key retires entirely with `Input`'s backing-class-slot removal. `ValidationShape` is purely SDL-derived now and produced by `ValidationShapeWalker` like every other carrier.
...
### Phase 1: introduce `Input`, `InputFieldDecl`, the carrier vocabulary, the substrate-parametric walker abstraction, `WalkerResult`, and the simplest example walker (`ValidationShapeWalker`)
...
- **R94's `InputRecordShape` becomes the `ValidationShape` carrier family.** Rename the type and the slot (`recordShape` → `validationShape`) on every input variant. The R94 capability marker `HasInputRecordShape` renames to `HasValidationShape` in lockstep (it retires entirely in Phase 5). `ValidationShape` is sealed with `Of(...)` and `NoValidationShape` arms; today's walker always produces `Of(...)`.
...
- Add a `Map<String, Input>` slot on `GraphitronSchema` populated from the existing types map; each `Input` carries the slot populated post-walk from `ValidationShapeWalker`'s `Ok` result.
- Add a non-Optional slot for each per-output-field carrier family on the existing Output record, each defaulting to its `No<Family>` arm. Compiler-visible, no consumer behavior change.
...
- The first load-bearing key lands here: `carrier-slot-always-populated-with-arm` (parametric on the substrate's model record), pinned on the orchestration site. The second (`per-type-emit-ignores-walker-output`) lands in Phase 3 once the per-type generators migrate.

Acceptance: model additions compile; every existing input today produces an equivalent `Input` from the type map with `validationShape` populated by the walker rather than by classifier-side derivation; the per-input emit is unchanged; output of `InputRecordGenerator` is byte-identical to trunk; a fixture that triggers a `DirectiveDeprecated` walker diagnostic (e.g. `@table` on input) surfaces it in the LSP's `publishDiagnostics` stream with code `"graphitron.directive-deprecated"` and `tags: [Deprecated]`.
...
- `InputRecordGenerator`, `InputTypeGenerator`: read `Input.validationShape()` (now a walker-populated carrier slot) and `Input.schemaType()` directly; permit-identity dispatch retires. These two consumers read only `Input`'s carrier slots, never any per-output-field carrier; `substrate-emit-reads-only-own-substrate-carriers` pins the boundary.
...
- **R94** (shipped): `HasValidationShape` becomes a carrier slot on `Input`, populated by `ValidationShapeWalker` via `WalkerResult`. The R94 invariant (validationShape derived from backing class) retires because `Input` no longer carries a backing class; the shape is purely SDL-derived and now produced by the same walker pattern as every other carrier.
...
- **`Input`** — the SDL declaration: name, location, assembled-schema form, `List<InputFieldDecl>`, and one carrier slot per registered per-input walker (today: `validationShape: ValidationShape`). Carries no backing class, no table, no classified state. Read by per-input emitters (Jakarta validation) as their model record; read by walkers as their substrate.
...
- **Carriers** — `Pagination`, `Ordering`, `PredicateCarrier`, `MethodArguments`, `InsertRows`, `UpdateRows` (per-output-field); `ValidationShape` (per-input). Each is a two-arm sealed family with valid arm(s) plus the explicit absent arm (`No<Family>`). No family carries an `Invalid` arm; structural failure rides on `WalkerResult.Err`, not inside the carrier. Each carrier is produced by exactly one walker and lives on its substrate's model record as a non-Optional slot whose value is always a valid arm or `No<Family>`.
...
The cure this draft commits to: separate the *consumer concerns* into independent *carriers*; produce each by an independent *walker* over an SDL substrate; let the substrate's model record carry the carriers as fixed-meaning non-Optional slots whose arms — valid or the explicit `No<Family>` — are part of each family's domain model. No top-level sealed hierarchy on any substrate; impossible combinations are excluded by dispatch-time election rather than by type. Walkers are pure functions over graphql-java primitives returning a sealed `WalkerResult<C>`: `Ok(carrier, diagnostics)` on success, `Err(errors, diagnostics)` on structural failure. Diagnostics are LSP-aligned by construction (severity, code, source, message, tags, relatedInformation), so the editor experience — hover, inlay hints, decorations, jump-to-cause — falls out of the walker's structured output without a parallel translation layer. Classification runs to completion across every substrate regardless of how many walkers errored; the orchestrator collects every walker's result, surfaces every error, and blocks the downstream generation phase from running when any `Err` is present. Downstream generators never observe a failure state — the wrapper plus the phase split handles it. Introspection consumers (the LSP) read classification output directly and so see populated slots for the parts that classified cleanly even when others didn't. Each axis is independently unit-testable.

The walker abstraction is substrate-parametric, not output-field-keyed. Per-output-field consumer concerns attach as carriers on `OutputField`; per-input consumer concerns attach as carriers on `Input`; future substrates (types, enums, ...) follow the same pattern. Drawing the abstraction at a single substrate would recreate the asymmetry the pivot exists to dissolve, with some derivations being walker outputs and others being stored slots computed by ad-hoc classifier paths. The uniform shape — same `Walker<S, C>`, same `WalkerResult` aggregation, same orchestrator halt rule — is the property that makes adding R98's `ConstraintSetWalker` or a hypothetical `EnumValidationWalker` a drop-in extension rather than a structural decision.
