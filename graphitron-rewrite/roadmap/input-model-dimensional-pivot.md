...
```java
class PredicateWalker implements Walker<GraphQLFieldDefinition, PredicateCarrier> {
...
record Classification(
    // per-output-field carriers attach as slots on the existing OutputField record (below).
...
Per-output-field carriers attach as slots on the existing `OutputField` record. `OutputField` pre-exists the pivot, carries carrier-independent state (`DomainReturnType`, the producer-of-source story, the `RootField` / `ChildField` sub-hierarchy), and is the natural host for per-field walker outputs:
...
| `HasValidationShape` capability marker (5 permits) | retires. `ValidationShape` lives in `Classification.inputValidationShapes` keyed by `SchemaCoordinate` |
...
- **R94's `InputRecordShape` becomes the `ValidationShape` carrier family.** Rename the type to `ValidationShape`. The R94 capability marker `HasInputRecordShape` retires; the carrier no longer lives on a per-input model record but in `Classification.inputValidationShapes` keyed by `SchemaCoordinate`. `ValidationShape` is sealed with `Of(...)` and `NoValidationShape` arms; today's walker always produces `Of(...)`.
...
- Add a non-Optional slot for each per-output-field carrier family on the existing `OutputField` record, each defaulting to its `No<Family>` arm. Compiler-visible, no consumer behavior change.
...
- The first load-bearing key lands here: `carrier-slot-always-populated-with-arm` (parametric on `OutputField` slots and on per-input carrier maps), pinned on the orchestration site. The second (`per-type-emit-ignores-walker-output`) lands in Phase 3 once the per-type generators migrate.
...
- Delete `HasValidationShape` capability marker (carrier lives in `Classification.inputValidationShapes` keyed by `SchemaCoordinate`).
...
- **R94** (shipped): `HasValidationShape` becomes a coordinate-keyed entry in `Classification.inputValidationShapes`, populated by `ValidationShapeWalker` via `WalkerResult`. The R94 invariant (validationShape derived from backing class) retires because the per-input substrate carries no backing class; the shape is purely SDL-derived and now produced by the same walker pattern as every other carrier.
