...
R222 introduces no new model record on the per-input side. Per-input walkers consume `GraphQLInputObjectType` directly as their substrate; the carriers they produce live in per-family maps on the classification artifact, keyed by `SchemaCoordinate`:

```java
record SchemaCoordinate(String value) { }   // thin wrapper over the GraphQL-spec coordinate
                                            // ("FilmInput", "FilmInput.title", ...)

record Classification(
    // per-output-field carriers attach as slots on OutputField (sealed interface; see below).
    // per-input carriers live in per-family maps keyed by SchemaCoordinate:
    Map<SchemaCoordinate, ValidationShape> inputValidationShapes
    // future per-input walkers add additional maps (R98's ConstraintSet, etc.)
) { }
```

Keying by `SchemaCoordinate` rather than by `GraphQLInputObjectType` reference identity is honest to the SDL identity, survives schema rebuilds without reference-equality concerns, and aligns with the same identity LSP clients address. The map invariant is "every input coordinate in the schema appears as a key, with a valid or `No<Family>` arm as the value" — pinned by the same `carrier-slot-always-populated-with-arm` audit that covers `OutputField`'s slots.

Per-input emitters (`InputRecordGenerator`, `InputTypeGenerator`) iterate `schema.getInputObjectTypes()` directly, derive each substrate's `SchemaCoordinate`, and look the carrier(s) up in the relevant map(s). No per-input model record means no pass-through wrappers over graphql-java accessors and no sealed-interface-with-one-permit scaffolding.
...
| `HasInputRecordShape` capability marker (5 permits) | retires. `ValidationShape` lives in `Classification.inputValidationShapes` keyed by `SchemaCoordinate` |
...
All three are independently unit-testable; all three have downstream consumers ready to migrate (Phase 3 for the per-field walkers; the per-input emit already reads `Classification.inputValidationShapes` keyed by `SchemaCoordinate` from Phase 1).
...
| R213 (rejections at consumer field) | Walker-time `SourceLocation` is the substrate's own SDL location (`GraphQLInputObjectField.getDefinition().getSourceLocation()` / arg's source location), paired with its `SchemaCoordinate` |
...
- **R98** (multi-source input validation): once `ValidationShape` is a coordinate-keyed map on the classification artifact, attaching `ConstraintSet` is a one-site change — add `Map<SchemaCoordinate, ConstraintSet>` alongside, populated by `ConstraintSetWalker`. Follow-up.
...
2. **`substrate-emit-reads-only-own-substrate-carriers`**: per-output-field consumers (validator, fetcher generators, query-conditions emitters) read carrier slots on `OutputField` only; per-input consumers (`InputRecordGenerator`, `InputTypeGenerator`) read carrier entries from the per-input maps via the substrate's `SchemaCoordinate` only. Neither side reaches across substrates, and neither side bypasses to walk the SDL itself. Pinned with `@DependsOnClassifierCheck` on each substrate's emit entry points.
...
### Phase 1: introduce `SchemaCoordinate`, the per-input carrier maps, the carrier vocabulary, the substrate-parametric walker abstraction, `WalkerResult`, and the simplest example walker (`ValidationShapeWalker`)

- Add `record SchemaCoordinate(String value)` to `model/` — a thin typed wrapper over the GraphQL-spec coordinate string. Provide a helper that derives the coordinate from a `GraphQLInputObjectType` (the type's name) and from a `GraphQLInputObjectField` on a parent type (`Parent.field`).
...
- **R94's `InputRecordShape` becomes the `ValidationShape` carrier family.** Rename the type to `ValidationShape`. The R94 capability marker `HasInputRecordShape` stays in source during the Phase 1-3 additive migration window (it continues to mark the legacy `*InputType` permits while they exist); the new carrier lives in `Classification.inputValidationShapes` keyed by `SchemaCoordinate`, not through the marker. The marker class deletes in Phase 4 with the legacy permits. `ValidationShape` is sealed with `Of(...)` and `NoValidationShape` arms; today's walker always produces `Of(...)`.
...
- Add a `Map<SchemaCoordinate, ValidationShape> inputValidationShapes` slot on the classification artifact, populated post-walk from `ValidationShapeWalker`'s results. Every input coordinate in the schema appears as a key; orchestrator initialises unwalked entries to `NoValidationShape`.
...
- Delete the `HasInputRecordShape` capability marker class — orphaned once the legacy `*InputType` permits delete above (the new carrier lives in `Classification.inputValidationShapes` keyed by `SchemaCoordinate`, not via the marker).
...
- **Carriers** — `Pagination`, `Ordering`, `PredicateCarrier`, `MethodArguments`, `InsertRows`, `UpdateRows` (per-output-field, attached as slots on `OutputField`); `ValidationShape` (per-input, attached as a value in `Classification.inputValidationShapes` keyed by `SchemaCoordinate`). Each is a two-arm sealed family with valid arm(s) plus the explicit absent arm (`No<Family>`). No family carries an `Invalid` arm; structural failure rides on `WalkerResult.Err`, not inside the carrier. Each carrier is produced by exactly one walker; every slot and every map entry holds a valid arm or `No<Family>`.
