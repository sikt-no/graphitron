...
This item *deliberately* doesn't take that on. R187 is a bug fix with a small, surgical diff; the unification is its own architectural item and the right move is to file it as a follow-up rather than expand R187's scope. A new Backlog stub (slug TBD, e.g. `service-param-classification-sealed-hierarchy`) gets created when this Spec moves to Ready, referencing R185, R187, and the predicate-chain smell.
