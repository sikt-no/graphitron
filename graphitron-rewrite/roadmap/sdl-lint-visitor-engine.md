...
8. **no-deprecated-directive-usage**: a deprecated graphitron directive or directive argument is used in the consumer SDL. Detection is two-sourced, because GraphQL's `@deprecated` is valid only on field / argument / input-field / enum-value, not on a directive *definition*: (a) the genuinely `@deprecated`-tagged argument and input-field cases, `@asConnection(connectionName:)` (`directives.graphqls:237`) and `ExternalCodeReference.name` (`:309`, use `className`), are read straight from the bundled `directives.graphqls` marker, drift-free; (b) whole-directive deprecations the language cannot tag, currently `@index` (`:244`, use `@order`), come from a small curated set of deprecated directive names held in the rule and pinned by test. Usage detection is then an AST walk, no classification needed. `@record` is deliberately excluded here: the redundant-record advisory below owns it, so the coordinate is warned once. Severity warning. Subsumes the retired R296.
