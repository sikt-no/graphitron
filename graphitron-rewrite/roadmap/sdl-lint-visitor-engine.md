...
- Projecting the typed `LintRule` id onto the MCP `diagnostics` wire mapping, null-guarded like the tool's existing `putIfNotNull` fields since `BuildWarning.rule` is null for non-lint warnings (the field itself lands in `graphitron` per the bullet above; only this projection is in `graphitron-mcp`). `GraphitronMcpServer` already surfaces `ValidationReport` warnings via that tool over the shared `Workspace`, so an MCP-aware agent sees which rule fired with no new tool or seam; findings reach the MCP for free.
...
The two classification advisories whose directive is ignored anyway, `redundant-record-directive` (remove the ignored `@record`) and `splitquery-redundant-on-record-parent` (remove the redundant `@splitQuery`), can carry a safe deletion fix. Because they are classifier-emitted, the classifier attaches the `LintFix` to the `BuildWarning` it already produces; the engine does not re-derive them.
