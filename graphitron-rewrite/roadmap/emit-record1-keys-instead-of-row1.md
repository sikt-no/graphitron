...
| `AccessorRowKeyedSingle` / `AccessorRowKeyedMany` | `@record` parent w/ typed `TableRecord` accessor | `Record<N>` (auto-derived; no developer-facing source on this arm — flipped to gain `value<N>()` access for the auto-emitted rows-method) | `__elt.into(table.col, ...)` |
...
1. **`BatchKey.keyElementType()` per-variant shape.** `RowKeyed` / `MappedRowKeyed` / `LifterRowKeyed` produce `RowN<...>`; `RecordKeyed` / `MappedRecordKeyed` / `AccessorRowKeyedSingle` / `AccessorRowKeyedMany` produce `RecordN<...>`. The `keyElementType()` switch in `BatchKey` is the single source of truth.
...
6. **`GeneratorUtils.buildAccessorRowKeySingle` / `Many`.** Auto-derived from a typed `TableRecord` accessor; no developer-facing source. Emits `__elt.into(table.col1, ...)` to produce `RecordN<...>` keys, giving the auto-emitted rows-method's `value<N>()` access for the parent VALUES table emission.
...
8. **`@DependsOnClassifierCheck` annotations.** The two checks under `buildAccessorRowKeySingle` / `Many` describe the `Field`-typed `into(...)` projection (R60-introduced; this scope keeps them on `RecordN`).

## Tests

- **L1 (`BatchKeyTest`).** New parameterised test pins `keyElementType()` and `javaTypeName()` per variant: `RowKeyed`, `MappedRowKeyed`, `LifterRowKeyed` → `RowN<...>`; `RecordKeyed`, `MappedRecordKeyed`, `AccessorRowKeyedSingle`, `AccessorRowKeyedMany` → `RecordN<...>`. Captures the seven-variant shape map directly.
