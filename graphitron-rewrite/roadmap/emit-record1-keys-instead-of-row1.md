...
4. **`GeneratorUtils.buildKeyExtractionWithNullCheck` (`RowKeyed` only).** Keep per-column scalar extraction for the null check; replace the `DSL.row(...)` tail with `Record1<T> key = ((Record) env.getSource()).into(table.col1)` (re-reading from the record by `Field` reference; the redundant read is O(1) and keeps the null-check shape uniform).
...
6. **Classifier rejection for the deferred combo.** In `FieldBuilder.classifyChildFieldOnResultType` (or whichever entry point produces a `RowKeyed` for a `RecordParentBatchKey` parent), reject the `JavaRecordType` / `PojoResultType` parent combination with a structured `Rejection` whose message names the deferred follow-up slug. Sibling unit-test coverage in the existing `*ValidationTest` set.
