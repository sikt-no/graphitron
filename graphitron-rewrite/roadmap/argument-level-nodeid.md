...
- **Carrier narrowing** (review cleanup). `ColumnReferenceArg` / `CompositeColumnReferenceArg` declare `JoinStep.FkJoin fkJoin` directly (not `List<JoinStep>`). The single-hop direct-FK shape that R40 ships is now structural rather than a constructor invariant. The conceptual mirror with `InputField.ColumnReferenceField` / `CompositeColumnReferenceField` is preserved at the wire-shape level (refTypeName, columns, extraction) but diverges on the join shape — input-field carriers keep `List<JoinStep>` for now and align with the arg-side narrowing as part of R57's carrier rework.
- **Load-bearing classifier check pairing** (review cleanup). `classifyArgument` carries `@LoadBearingClassifierCheck("r40.nodeid-fk-target.direct-fk")` for the FK-target rejection arm; `projectFilters` carries the matching `@DependsOnClassifierCheck`. Audited by `LoadBearingGuaranteeAuditTest`. R57 supersedes this pairing by lifting direct-vs-translated into a sealed `Resolved.FkTarget` sub-variant — the type system carries the certainty after the split, and the annotations come off both sites.
- **Follow-on items filed**: `R57 nodeid-fk-target-arg-join-translation` for the pathological FK-target case (with model-shape note: lift direct-vs-translated into sealed sub-variants of `Resolved.FkTarget`); `classification-vocabulary-followups.md` items 7 (same-table `@nodeId` SDL walked three times per field) and 8 (Java-17 decode emission duplicated across `ArgCallEmitter` + `LookupValuesJoinEmitter`).
