...
- `src/main/resources/application.yaml` configures the HTTP port and a JDBC datasource (`quarkus.datasource.jdbc.url` / `username` / `password`) pointing at an existing Postgres. The example does **not** use Quarkus dev-services and does **not** ship database init: the app does not own the database (a principle the rewrite holds module-wide), so a developer running `mvn quarkus:dev` points the URL at whatever Postgres they have running, with `init.sql` from `graphitron-sakila-db` already loaded. For local exploration this is typically the same Postgres the `local-db` profile already targets (`jdbc:postgresql://localhost:5432/rewrite_test`); environment-variable defaults in `application.yaml` make this the path of least resistance without hard-coding it. No `quarkus.datasource.devservices.*` keys appear anywhere.
...
- One smoke test (`@QuarkusTest`) hits `/graphql` with a basic query and asserts a non-empty response. It reuses the same Postgres infrastructure the in-process query-to-database tests use: the existing Testcontainers harness (or the pre-existing local Postgres under `-Plocal-db`) starts before Quarkus boots and sets `quarkus.datasource.jdbc.url` as a system property so the booting app picks it up. This is the only HTTP-shaped test in the module; everything else stays at the schema/engine level (Stage 2).
