...
- §3+§5 helper-registry layer. `CompositeDecodeHelperRegistry` collects composite-key NodeId decode helpers per emitted `<Root>Conditions` class, deduplicating by `(encoderClass, methodName, mode, list)`; `ArgCallEmitter.buildCallArgs`/`buildArgExtraction` gained nullable-registry overloads that thread through `buildNestedInputFieldExtraction` and `buildNodeIdDecodeExtraction`; `QueryConditionsGenerator` instantiates one registry per class, threads it through each `buildConditionMethod`, and adds the collected helpers as private static methods alongside the public condition methods. Other call sites (Inline*, SplitRows*, TypeFetcher) keep the inline form by passing no registry. Helper naming is pinned: `decode<NodeType>Row` / `decode<NodeType>Rows` for SKIP, `decode<NodeType>RowOrThrow` / `decode<NodeType>RowsOrThrow` for THROW. Verified end-to-end: the regenerated `filmActorsByCompositeNodeIdsCondition` in sakila-example reduces to `decodeFilmActorRows(env.getArgument("filter") instanceof Map<?, ?> _m1 ? _m1.get("ids") : null)` plus a single private `decodeFilmActorRows(Object wire)` helper. (`R79 §3+§5 helper registry` commit)
- §6 registry unit test (`CompositeDecodeHelperRegistryTest`): six tests covering same-key dedup, SKIP↔THROW separation, scalar↔list separation, and per-mode body-shape assertions on the emitted `MethodSpec`. (same commit)
- §5 sub-step 1: shared outer-arg lift. `QueryConditionsGenerator.computeLiftedOuters` scans every `NestedInputField` callParam across a method's filters; outer args referenced ≥2 times are lifted to one `Map<?, ?> <camelCaseArg>Map = env.getArgument("...") instanceof Map<?, ?> _m ? _m : null;` local at the top of the emitted method. `ArgCallEmitter.buildCallArgs`/`buildArgExtraction` gain a 6-arg overload that threads the lift map; `buildNestedInputFieldExtraction` and `buildMapChain` swap depth-0's `instanceof Map<?, ?> _m1` rebind for a `<local> != null` guard against the lifted local when the outer arg is in the map. Local naming is `<camelCaseArg>Map` to avoid colliding with JooqConvert lifts (`<name>Keys`) or method parameters (`table`, `env`). Single-use outer args keep the inline rebind. Verified end-to-end: `filmsWithMultiImplicitCondition` (the only ≥2-occurrence case in sakila-example today) reduces from two `_m1` rebinds to one `filterMap` declaration plus two `filterMap != null ? ... : null` references. (`R79 §5 outer-arg lift` commit)
- §5 lift unit test (`QueryConditionsGeneratorLiftTest`): five tests pinning the lift contract — two callParams sharing an outer lifts; single callParam does not; distinct outer args do not; cross-filter occurrence counts; outer-arg names are camelCased. (same commit)
- §6 pipeline-tier helper-dedup test (`QueryConditionsPipelineTest`): two end-to-end assertions on `QueryConditionsGenerator.generate(schema, ...)` output using the `nodeidfixture` `Bar` composite-PK NodeType. (1) Two query fields consuming the same `[ID!] @nodeId(typeName: "Bar")` arg share exactly one private static `decodeBarRows` helper, with both condition methods invoking it; (2) one scalar + one list arg on the same NodeId type produce two distinct helpers (`decodeBarRow`, `decodeBarRows`), confirming the registry's `(encoderClass, methodName, mode, list)` key separation works at the pipeline level. (`R79 §6 helper-dedup + arity > 22 tests` commit)
- §6 arity > 22 unit test on `NodeIdLeafResolverTest`. Extends `nodeidfixture` with a `too_wide` table (23-column composite PK, `varchar(20)` columns) plus a matching `NodeIdFixtureGenerator.METADATA` entry that registers the synthetic `__NODE_TYPE_ID = "TooWide"` and `__NODE_KEY_COLUMNS` array. The test points an `@nodeId(typeName: "TooWide")` arg at the resolver and asserts a `Resolved.Rejected` with a structural `Rejection` whose message names the type, the leaf, the arity (`23 key columns`), and the cap (`Row22 cap`). Bumped `jooq.codegen.schema.version` to force jOOQ regeneration on incremental builds. (same commit)

R79 sub-steps now all shipped. Status flips to In Review at the next handoff.
