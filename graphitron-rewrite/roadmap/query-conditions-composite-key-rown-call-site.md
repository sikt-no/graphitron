...
- §6 pipeline-tier helper-dedup test (two distinct condition methods on the same class consuming the same NodeId type emit one shared helper). The unit test on `CompositeDecodeHelperRegistry` covers the dedup contract; an end-to-end pipeline assertion would additionally confirm that `QueryConditionsGenerator` threads one registry across all condition methods on a class. Sakila has only one composite-key NodeId field today (`filmActorsByCompositeNodeIds`), so this needs a second fixture method on the same root type. Low marginal value over the unit test; leaving to follow-up.
