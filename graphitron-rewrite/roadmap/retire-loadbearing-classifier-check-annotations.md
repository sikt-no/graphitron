...
4. `.claude/skills/reviewer-prompt/SKILL.md` lines 84–85 ("Load-bearing classifier checks. A classifier guarantee an emitter relies on that lacks `@LoadBearingClassifierCheck` / `@DependsOnClassifierCheck` annotations."): remove the rubric line and replace with a "Missing type-system lift" entry under the new framing. `.claude/skills/srp/SKILL.md` line 148 carries only an example `grep` command (`grep -rn LoadBearingClassifierCheck`) in the spot-check guidance, not a rubric pushing the pattern; the grep example is the FQN-aware-grep teaching and survives the retirement, but swap the example identifier to a non-retired one (e.g. `everyGraphitronFieldLeafHasAKnownDispatchStatus`) so the spot-check doesn't pin on a vanishing symbol.
...
6. Resolve in-flight items that propagate the pattern — handled together in Phase 1, not Phase 5. Two items qualify; both are in `Spec` / `Backlog` today and both would create a multi-week roadmap-state contradiction if left under their current framing during the retirement window. Decide each in the Phase 1 commit so the regenerated `roadmap/README.md` doesn't ship the contradiction.
   - `graphitron-rewrite/roadmap/loadbearing-classifier-checks-multischema-fqn.md` (R125, `Backlog`). The item's coherent argument ("pipeline tests pin locally, annotation pair pins globally for the audit and find-usages navigation") is exactly the argument R237 is retiring; the whole item is about adding more sites. Two options: (i) Discard with reason "superseded by R237"; or (ii) rewrite the body around the type-system equivalent (audit conformance via pipeline tests + sakila-example compile against `JooqCatalog.TableEntry.tableClass()` payload faithfulness across the multi-schema codegen execution). Default: Discard, unless the FQN audit has independent value the rewrite surfaces.
   - `graphitron-rewrite/roadmap/nested-input-types-in-mutation-fields.md` (R186, `Spec`). The Phase 5-step-7 section proposes a *new* audit key `mutation-input.nesting-leaf-carries-path-extraction` and a paired `@DependsOnClassifierCheck` site (`nested-input-types-in-mutation-fields.md:108-110`). Unlike R125, the annotation isn't R186's whole reason for existing — R186's main work is admitting nested input types and rewrapping `CallSiteExtraction`; the annotation proposal is one bullet inside Phase-5-step-7. Two options: (i) drop that bullet (delete the "New audit key" subbullet and the paired-annotation language; the surrounding access-path invariant still holds, mechanically pinned by `CallSiteExtraction.NestedInputField`'s sealed-variant carrier and the pipeline-tier coverage step 4 already adds); or (ii) reframe to the type-system equivalent (a pipeline-tier test that fails if a `NestingField`-derived leaf ships with a non-`NestedInputField` extraction). Default: (i), since the carrier shape is already a sealed sub-variant. The rest of R186 is unaffected.
...
- R125 and R186 are resolved (R125 Discarded or rewritten; R186 has its annotation bullet dropped or reframed); the regenerated `roadmap/README.md` reflects the new direction.
