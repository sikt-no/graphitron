...
- **Without `@reference`.** Leaf target table's PK columns, read via `TableRef.primaryKeyColumns()` after resolving the leaf type's `TableRef`. Mirrors the existing leaf-PK lifter use at `FieldBuilder.java:3102` (`expectedTable.primaryKeyColumns()`). Single degenerate hop, parent_values → leaf, no FK involved.

Both cases collapse to: the lifter is a deterministic function of the parent (a static method), materialised into the parent_values CTE; the first hop's source-side columns become the column-equality join target. DataLoader alignment of child rows to parent batch keys falls out of that column equality plus the lifter's per-parent determinism. **Uniqueness of the lifter tuple on the source table is not a precondition** — the leaf-PK case is trivially unique by definition; the `@reference` case takes whatever columns the FK orientation places on the first hop's source side, and the directive does not depend on those being unique on that table. Non-PK unique-key lookups stay out of scope (see "Out of scope" below); any future extension would have to prove its derived tuple represents an actual unique key.
...
3. Derive the expected parent-side column tuple by directive shape:
   ...
   - **`@reference` absent.** Bypass `parsePath`; resolve the leaf type's `TableRef` and read the PK columns directly via `TableRef.primaryKeyColumns()` (matching the existing call site at `FieldBuilder.java:3102`).
...
- `BuildContext.parsePath`'s gate is **unchanged**. The leaf-PK case bypasses `parsePath` and reads `TableRef.primaryKeyColumns()` directly. The `@reference` case calls `parsePath` with `startSqlTableName = null` (the supported no-parent-table path).
- `BatchKey.LifterRowKeyed` is split into `LifterLeafKeyed` and `LifterPathKeyed` permits. The `RecordParentBatchKey` permits clause (`BatchKey.java:170`) is updated in the same edit (`permits RowKeyed, LifterLeafKeyed, LifterPathKeyed, AccessorKeyedSingle, AccessorKeyedMany`); every existing exhaustive `switch(batchKey)` site is walked downstream (the seal makes this `javac`-checked).
