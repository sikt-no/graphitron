...
| `R84` | Path expressions in argMapping | In Review | [plan](argmapping-path-expressions.md) |
...
- `R90` [**LSP Javadoc surfacing + per-line definitions + @externalField completion**](lsp-javaparser-javadoc-and-definitions.md): R18 Phase 5 originally folded three JavaParser-driven capabilities in alongside `@service` / `@condition` / `@record` autocomplete:
...
- `R84` [**Path expressions in argMapping**](argmapping-path-expressions.md) — In Review, feature
...
- `R90` [**LSP Javadoc surfacing + per-line definitions + @externalField completion**](lsp-javaparser-javadoc-and-definitions.md) — Backlog, Backlog
