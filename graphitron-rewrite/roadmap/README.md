...
- [**New-developer on-ramp pass**](developer-onramp.md): A consolidating sweep over docs, READMEs, and a few high-touch source files so a new contributor can go from "I cloned the repo" to "I can locate where to make my change" without bouncing between four files. The rewrite is stabilising and we have no external users yet, so this is the right window to fix the documentation drift that has accumulated and to add the orientation pieces we have been deferring.
