...
| Same-table `[ID!] @nodeId` filter → primary-key IN predicate | In Review | [plan](self-table-nodeid-filter.md) |
...
- [**Decompose `FieldBuilder`**](decompose-fieldbuilder.md): Split the 2,217-line / 56-private-method builder along the field taxonomy. Blocked on argument-resolution unification. Proposed split: `QueryFieldBuilder`, `MutationFieldBuilder`, `ChildFieldBuilder` plus a shared argument-classification module.
