...
*Input side.* `CallSiteExtraction.NodeIdDecodeKeys` is a first-class sealed sub-taxonomy on the existing `CallSiteExtraction` hierarchy (two arms by failure mode — `SkipMismatchedElement` and `ThrowOnMismatch`; see "Failure-mode contract"), with full arms in `ArgCallEmitter.buildArgExtraction`. Each arm calls the per-NodeType `decode<TypeName>` helper (see "`NodeIdEncoder` API") to turn a `String` / `List<String>` into a typed value (a single column's Java type for arity-1 carriers, a `Record<N>` of typed key values for arity > 1), returning `null` on malformed input or typeId mismatch; the arms differ only in how that null surfaces. The third failure mode (`NullOnMismatch`) is dispatcher-driven, not carrier-driven; it covers the `Query.node` / `Query.nodes` / federated `_entities` paths uniformly via `EntityFetcherDispatchClassGenerator`, which reads `NodeType.decodeMethod` directly and hand-rolls null-per-id semantics rather than going through a carrier extraction arm. Coverage extends to the top-level-argument case: today `NestedInputField` is the only nested-Map traversal that runs extractions, but a top-level `[ID!] @nodeId` arg needs the same extraction at the call-site root, so `ArgCallEmitter`'s top-level path gains the same dispatch the nested path already has.
...
- `InputField.NodeIdField` → arity-1: `InputField.ColumnField` with `extraction = NodeIdDecodeKeys.SkipMismatchedElement(decodeMethod)`, body is `ColumnPredicate.Eq`. Arity > 1: `InputField.CompositeColumnField` with the same extraction, body is `ColumnPredicate.RowEq`. (Single-id filters surface a malformed id as `falseCondition()`, the arity-1 degenerate of "skip the bad element"; never throw — input filters are not contract-violation surfaces.)
- `InputField.NodeIdReferenceField` → arity-1: `InputField.ColumnReferenceField` (FK joinPath retained) with `NodeIdDecodeKeys.SkipMismatchedElement`, body `ColumnPredicate.Eq`. Arity > 1: `InputField.CompositeColumnReferenceField`, body `ColumnPredicate.RowEq`.
...
- `ArgumentRef.ScalarArg.NodeIdArg` retires; the post-R50 successor depends on the call-site role:
    - *Lookup-key arg* (today's `LookupMapping.NodeIdMapping` path): folds onto `LookupMapping.ColumnMapping`'s `ScalarLookupArg` (single-key target) or `DecodedRecord` (composite-key target) with `NodeIdDecodeKeys.ThrowOnMismatch`; see "Lookup arg restructure".
    - *Mutation-key arg* (`@nodeId` on a top-level scalar argument used as a mutation key): arity-1: `ScalarArg.ColumnArg` with `NodeIdDecodeKeys.ThrowOnMismatch`. Arity > 1: `ScalarArg.CompositeColumnArg` with the same arm.
    - *Top-level filter arg* `[ID!] @nodeId(typeName: T)` (the field-argument-level analogue of `InputField.NodeIdInFilterField`): arity-1: `ScalarArg.ColumnArg` with `NodeIdDecodeKeys.SkipMismatchedElement`. Arity > 1: `ScalarArg.CompositeColumnArg` with the same arm. R50 builds the model and emitter infrastructure for this shape; the classifier wiring (`FieldBuilder.classifyArgument` recognising `@nodeId(typeName: T)` on a field argument) is R40's small follow-on, per "Coupling".
    - *Dispatcher-consumed arg* on `Query.node(id:)` / `Query.nodes(ids:)`: produces no carrier in the model — the dispatcher consumes the raw arg value (a base64 id string or list of them, polymorphic across NodeTypes) and resolves the helper via `NodeType.decodeMethod` per id at the dispatcher level. See "Query.nodes folds onto the lookup pipeline" for how the per-typeId batch flows through `LookupValuesJoinEmitter`.
...
*Query.nodes folds onto the lookup pipeline.* `Query.nodes(ids:)` is a lookup wearing query-field syntax: input is a set of decoded NodeId tuples, output is one row per input id, ordered by input position. After R50, it is structurally equivalent to the federated `_entities` resolver — both decode upstream, both group by typeId, and both fan out per-typeId batches through the same lookup emitter. `EntityFetcherDispatchClassGenerator` is the single dispatcher shared by `Query.node`, `Query.nodes`, and `_entities` (today `QueryNodeFetcherClassGenerator.fetchById` already routes through `EntityFetcherDispatchClassGenerator.RESOLVE_BY_REPS_METHOD`); R50 keeps that arrangement and only re-points the per-typeId emission.

- Cross-typeId fanout dispatcher stays in `EntityFetcherDispatchClassGenerator` (with `QueryNodeFetcherClassGenerator` adapting `Query.node` / `Query.nodes` into representations): it peeks each id's typeId via `peekTypeId`, decodes via the per-NodeType `decode<TypeName>` helper looked up through `NodeType.decodeMethod`, and groups decoded tuples by typeId. A null return at any stage (typeId-not-recognised, decode-mismatch) resolves to null at that input position; the failure mode is the dispatcher-driven `NullOnMismatch` of "Failure-mode contract".
- Per-typeId batch routes through `LookupValuesJoinEmitter` with a `LookupMapping.ColumnMapping` carrying one `ScalarLookupArg` (single-key target) or `DecodedRecord` (composite-key target). The batch has already decoded; the emitter receives typed key tuples directly, so each `LookupArg`'s `extraction = Direct`. VALUES rows carry `idx` + decoded keys; JOIN to the target table; ORDER BY `idx`.
- Cross-typeId merge scatters per-typeId results back to original input positions by global `idx`.

`EntityFetcherDispatchClassGenerator`'s bespoke per-typeId VALUES emission (`select<TypeName>Alt<N>`) re-points onto `LookupValuesJoinEmitter`: today the dispatcher hand-rolls its own `VALUES (idx, col1, ...)` derived table joined to the type's jOOQ table (per the class javadoc at `EntityFetcherDispatchClassGenerator.java:35`), parallel to the VALUES+JOIN pipeline `@lookupKey` lookups already use; post-R50 the two pipelines collapse into one. The dispatcher's idx-driven scatter retains its role for cross-typeId merging, and `QueryNodeFetcher.rowsNodes` keeps its current rep-synthesis-and-dispatch shape (per `QueryNodeFetcherClassGenerator.java:187`); only the per-typeId SQL emission changes hands. `Query.node(id:)` (arity-1, single result) skips the VALUES form and degenerates to a single WHERE on decoded keys; ordering is moot for one row.

> **Scope adjustment (post-review).** Phase (f-E) shipped a regression test pinning the emitted SQL *shape* (`VALUES + JOIN + ORDER BY idx`) so the dispatcher's hand-rolled emission can't silently regress to the legacy `WHERE row-IN`. The actual collapse onto `LookupValuesJoinEmitter` does not ship in R50 — the two pipelines remain parallel. Tracked separately. See "Post-review cleanup → Deferred."
...
> **Scope adjustment (post-review).** R50 ships only the *classifier* side of rooted-at-parent: phase (g-B) resolves the FK chain and produces `ChildField.ColumnReferenceField` (arity-1) or `ChildField.CompositeColumnReferenceField` (arity > 1) with `compaction = NodeIdEncodeKeys`. The matching emitter side does not ship in R50; both arms surface as `FetcherEmitter` runtime stubs. R24's scope expands to absorb rooted-at-parent single-hop emission alongside its existing multi-hop and condition-join coverage. See "Post-review cleanup → Deferred."
...
- `decode<TypeName>(String) → Record<N><...>` per `@node` type. Returns `null` uniformly for malformed input or typeId mismatch; carrier consumers wrap that null through the `CallSiteExtraction.NodeIdDecodeKeys.*` arms (Skip, Throw), and dispatcher consumers wrap it directly in `EntityFetcherDispatchClassGenerator`'s null-per-id scatter (see "Failure-mode contract").
...
- `CallSiteExtraction.NodeIdDecodeKeys.SkipMismatchedElement`, `.ThrowOnMismatch` — one validator-coverage arm each, mirroring the per-variant pattern the existing `CallSiteExtraction` arms (`Direct`, `EnumValueOf`, `TextMapLookup`, `ContextArg`, `JooqConvert`, and the Map-traversal `NestedInputField`) already follow. The dispatcher-driven `NullOnMismatch` mode is not a carrier arm and gets no `CallSiteExtraction` coverage; its behaviour is exercised in execution-tier (see "Test surface" → Failure-mode parity).
...
- `@LoadBearingClassifierCheck(key = "nodeid.decode.failure-mode")` on the classifier sites that pick the carrier `NodeIdDecodeKeys` arm — `BuildContext.classifyInputField` (`SkipMismatchedElement` for input-object-field filters) and `FieldBuilder.classifyArgument` (`SkipMismatchedElement` for top-level `[ID!] @nodeId` filter args; `ThrowOnMismatch` for lookup-key and mutation-key args). `@DependsOnClassifierCheck(key = "nodeid.decode.failure-mode", reliesOn = "...")` on each emitter site that switches on it (the body emitter for predicate construction, `LookupValuesJoinEmitter` for `LookupArg`-driven extraction, `ArgCallEmitter.buildArgExtraction` for nested + top-level extraction). Relaxing the classifier's per-call-site selection breaks emitted failure handling.
- `@LoadBearingClassifierCheck(key = "columnpredicate.column-arity")` on the classifier sites that pick `Eq` / `In` vs `RowEq` / `RowIn` (and the parallel single-column-carrier vs `Composite*`-carrier routing decision): `FieldBuilder.projectFilters` and `FieldBuilder.implicitBodyParam` for the predicate-arm choice in `BodyParam.ColumnPredicate`; `BuildContext.classifyInputField`, `FieldBuilder.classifyArgument`, and `FieldBuilder.classifyChildFieldOnTableType` for the carrier-shape choice (single-column vs `Composite*`) keyed off `NodeType.keyColumns().size()`. `@DependsOnClassifierCheck` on the body emitter and on every projection / call-site emitter that consumes the carriers. The arm's column shape (single `ColumnRef` vs `List<ColumnRef>` with size ≥ 2) is the carrier the emitter relies on with no defensive size check.
- `@LoadBearingClassifierCheck(key = "compaction.encode-keys")` on `FieldBuilder.classifyChildFieldOnTableType` — the `compaction` slot on `ChildField.ColumnField` / `ColumnReferenceField` is `NodeIdEncodeKeys` only when the GraphQL type is `ID` and the parent table participates in a `NodeType` whose `keyColumns.size() == 1`; otherwise `Direct`. `Composite*` output carriers narrow the slot to `NodeIdEncodeKeys` at the type level, so no separate classifier check is needed for them. `@DependsOnClassifierCheck` on the projection emitter that switches on the slot.

## Failure-mode contract

Three call-site categories want three different responses to a `decode<TypeName>` returning `null` (malformed input or typeId mismatch). Two are carrier-driven and surface in the model as sealed arms on `CallSiteExtraction.NodeIdDecodeKeys`; the third is dispatcher-driven and lives in `EntityFetcherDispatchClassGenerator` rather than as a carrier arm, because the dispatcher consumes a polymorphic id stream that has no single column to bind to. Folding the mode into a single `NodeIdDecodeKeys(decodeMethod, mode)` record would still put the same predicate (`extraction.mode() == ...`) into every emitter that consumes the arm; the carrier-driven cases instead split into sealed arms so the classifier picks the arm at build time, the emitter switches exhaustively, and the validator gets a coverage arm per arm for free.

*Carrier-driven (sealed arms on `CallSiteExtraction.NodeIdDecodeKeys`):*

- `SkipMismatchedElement(HelperRef.Decode decodeMethod)` — `[ID!] @nodeId(typeName: T)` filter, both as an input-object field (`InputField.NodeIdInFilterField`'s post-R50 successor) and as a top-level field-argument (`ScalarArg.ColumnArg` / `CompositeColumnArg`). A `null` return on any element short-circuits the bad element to "no row matches"; never throws. Empty list and null arg follow existing column-equality behaviour (predicate omitted when the arg is absent, `falseCondition()` when the list is present-but-empty, matching today's `NodeIdInFilterField` body). *Classifier home:* `BuildContext.classifyInputField` for input-object-field carriers — R50 territory, where `InputField.NodeIdField` / `NodeIdReferenceField` / `NodeIdInFilterField` / `IdReferenceField` are constructed today and re-routed to single-column / `Composite*` carriers post-R50. The top-level field-argument shape is wired by R40's small follow-on to `FieldBuilder.classifyArgument`; R50 provides only the carrier and emitter infrastructure.
- `ThrowOnMismatch(HelperRef.Decode decodeMethod)` — `@nodeId` on a top-level scalar/list argument used as a lookup or mutation key, including the lookup-side `ScalarLookupArg` / `DecodedRecord` arms of `LookupMapping.ColumnMapping`. A `null` return is an authored-input error and surfaces as a `GraphqlErrorException`-shaped error, not silent null; a wrong-type id at a lookup key is a contract violation rather than "no match." *Classifier home:* `FieldBuilder.classifyArgument` (lookup-key and mutation-key arms of the `@nodeId` scalar-arg branch); the lookup-key arm continues to feed `FieldBuilder.projectForLookup`, which consumes the `ScalarLookupArg` / `DecodedRecord` shape post-restructure.

*Dispatcher-driven (no carrier arm):*

- *NullOnMismatch* — `Query.node(id:)`, `Query.nodes(ids:)`, and the federated `_entities` resolver, plus any future signature-classified node-by-id fetcher (a Query field whose element type is the `Node` interface). All three share `EntityFetcherDispatchClassGenerator`, which `peekTypeId`s each id, looks up the helper through `NodeType.decodeMethod`, and resolves a `null` decode to `null` at that input position (or `null` for the whole field on a single-id field). Parity with the existing Relay-spec behaviour `_entities` already implements; R50 keeps that flow and only re-points per-typeId emission onto `LookupValuesJoinEmitter`. Because the dispatcher reads `NodeType.decodeMethod` directly rather than switching on a carrier slot, there is no `NodeIdDecodeKeys.NullOnMismatch` arm in the model. *Generator home:* `EntityFetcherDispatchClassGenerator` for the shared decode-and-scatter loop; `QueryNodeFetcherClassGenerator.fetchById` for the `Query.node` / `Query.nodes` adapter into the dispatcher; `EntityResolutionBuilder.build` for the federation `_entities` sidecar that the dispatcher consumes.

The shared decode primitive is the per-Node-type `decode<TypeName>` helper (see "`NodeIdEncoder` API"); each call-site category composes it with the appropriate response to a `null` return. Adding a future carrier-driven failure mode is a new sealed arm plus a validator-coverage arm plus an emitter switch arm; adding a dispatcher-driven mode is a change to the dispatcher generator. No shared flag to misset, no per-emitter mode-predicate to keep in sync.
...
- **Composite-PK Node.** The existing `nodeidfixture.bar` table (composite PK `(id_1, id_2)`, registered with `__NODE_TYPE_ID = "Bar"` and `__NODE_KEY_COLUMNS = [ID_1, ID_2]` by `NodeIdFixtureGenerator`). The fixture infrastructure is already in place; R50 grows SDL coverage on top of it — a `Bar` GraphQL type declared `@node`, plus pipeline cases for `[ID!] @nodeId(typeName: "Bar")` filters (driving `ColumnPredicate.RowIn` body emission) and composite-key encode-on-output (driving the `CallSiteCompaction.NodeIdEncodeKeys` slot on `ChildField.CompositeColumnField`).
- **Rooted-at-parent single-hop reference.** Two new tables in `nodeidfixture`: a parent NodeType `parent_node(pk_id, alt_key UNIQUE, ...)` registered with `__NODE_KEY_COLUMNS = [PK_ID]`, plus a child table `child_ref(child_id PK, parent_alt_key REFERENCES parent_node(alt_key), ...)` whose FK targets the parent's *non-PK* unique column. SDL-side `ChildRef.parent: ParentNode` with the resulting `@nodeId`-shaped projection forces a JOIN from child to parent to read `parent_node.pk_id` (the keyColumn the FK doesn't reach), driving the rooted-at-parent encode path without pulling in correlated-subquery emission. The implementer pins exact column types and `__NODE_TYPE_ID` value; the load-bearing shape is "FK column does not positionally match parent NodeType's `keyColumns`."
...
- **R24 (`NodeIdReferenceField` JOIN-projection form)** absorbs more than first scoped. R50 ships the classifier side of rooted-at-parent (phase g-B produces `ChildField.ColumnReferenceField` / `CompositeColumnReferenceField` with `compaction = NodeIdEncodeKeys`); the matching emitter does not ship in R50. R24's scope therefore covers rooted-at-parent single-hop emission *and* the multi-hop / condition-join correlated-subquery cases it originally targeted. R24 retains its file and stays in Backlog; re-spec it once a real schema reaches one of those shapes.
...
- **Execution-tier.** Every existing `@nodeId` execution test continues to round-trip end-to-end: `Query.node(id:)`, `Query.nodes(ids:)`, federated `_entities`, same-table `[ID!] @nodeId` filter, rooted-at-child reference, and the new composite-PK / rooted-at-parent cases. SQL inspection via `ExecuteListener` confirms emitted bodies use `c.eq(...)` / `c.in(...)` for `ColumnPredicate.Eq` / `In` and `DSL.row(c1, ..., cN).eq(...)` / `.in(...)` for `RowEq` / `RowIn`, with decoded key tuples flowing as bind values rather than encoded `String` ids. `Query.nodes(ids:)` SQL is asserted to be the lookup shape (`VALUES + JOIN + ORDER BY idx`) rather than the legacy `WHERE row-IN`, catching regressions where the dispatcher falls back to the old flow. Failure-mode parity with "Failure-mode contract": one typeId-mismatch test per failure mode — dispatcher-driven `NullOnMismatch` against `Query.node(id:)`, `Query.nodes(ids:)`, and federated `_entities` (asserting the same null-per-id semantics across all three, since they share `EntityFetcherDispatchClassGenerator`); carrier-driven `SkipMismatchedElement` against an `[ID!] @nodeId` filter; carrier-driven `ThrowOnMismatch` against a lookup-key argument.

## Post-review cleanup

In-Review architectural review surfaced two retirements the plan listed but didn't ship, plus two scope deferrals worth recording so the prose stops claiming work that lives elsewhere. The retirements land here in R50; the deferrals get amended notes on the existing plan sections and (where applicable) follow-on Backlog items.

### Retirements (in scope, ship before R50 → Done)

- **Delete unreachable `InputField.NodeIdField`.** "Wire-shape leaks to retire" listed it for retirement; its body emitter (`BodyParam.NodeIdIn`) is gone; no live classifier path constructs it. The record, the permits clause, the validator's no-op arm, the `FieldBuilder` stub arms, the `TypeFetcherGenerator.NOT_DISPATCHED_LEAVES` entry, and the `NOT_IMPLEMENTED_REASONS` entry all delete in lockstep. Pure deletion — no behaviour change.

- **Retire `InputField.NodeIdReferenceField` compat fallbacks → `UnclassifiedField`.** `BuildContext` constructs `NodeIdReferenceField` in two edge cases the post-R50 column-shaped successors do not cover: a plain `@table` target with no NodeType, and an orphan target with no resolvable decode helper. Both call sites preserve "classified-but-inert" behaviour at runtime — but the body emitter for that path was deleted, so reaching emission produces a no-op filter the author can't see at validate time. Per *Validator mirrors classifier invariants*, both fallbacks reroute to `UnclassifiedField` with a pointed `AUTHOR_ERROR` reason naming the missing `@node` annotation. Once both reroute, drop the variant + permits + validator arm + `FieldBuilder` stub arm + `NOT_DISPATCHED_LEAVES` + `NOT_IMPLEMENTED_REASONS` entry alongside the `NodeIdField` retirement.

### Deferred (out of scope, recorded so the plan matches reality)

- **Rooted-at-parent JOIN-with-projection emission.** "Variant-by-variant collapse → Single-hop emission, two shapes" describes both rooted-at-child (no JOIN) and rooted-at-parent (single-hop JOIN) emission. Rooted-at-child shipped; rooted-at-parent's classifier-side resolution shipped in phase (g-B) as `ChildField.ColumnReferenceField` with `compaction = NodeIdEncodeKeys` (and `CompositeColumnReferenceField` for arity > 1), but no live emitter handles either case — the runtime gets a `FetcherEmitter` stub instead of a build-time `NOT_IMPLEMENTED` rejection. The "Coupling → R24" entry already names R24 as the home for multi-hop and condition-join correlated-subquery emission; expanding R24 to also cover the rooted-at-parent single-hop is the natural placement. Plan amendment: "Variant-by-variant collapse → Single-hop emission, two shapes" gains a deferral note pointing at R24, and "What we're NOT doing" picks up the rooted-at-parent emission alongside multi-hop.

- **`EntityFetcherDispatch` per-typeId VALUES collapse onto `LookupValuesJoinEmitter`.** "Query.nodes folds onto the lookup pipeline" claims the dispatcher's bespoke `select<TypeName>Alt<N>` emission re-points onto `LookupValuesJoinEmitter`. Phase (f-E) shipped a regression test pinning the emitted SQL *shape* (`VALUES + JOIN + ORDER BY idx`), but the actual dispatcher VALUES emission stays hand-rolled — the two pipelines remain parallel. A new Backlog item tracks the residual collapse. Plan amendment: the "Query.nodes folds onto the lookup pipeline" prose flags the gap and points at the Backlog item.
