...
The rowsNodes lambda's bespoke `WHERE row-IN` body deletes; the dispatcher's idx-driven scatter retains its role for cross-typeId merging, but the per-typeId emission is the same VALUES + JOIN pipeline `@lookupKey` and post-R50 `@nodeId` lookups use, identical between `Query.nodes(ids:)` and `_entities`. `Query.node(id:)` (arity-1, single result) skips the VALUES form and degenerates to a single WHERE on decoded keys; ordering is moot for one row.
