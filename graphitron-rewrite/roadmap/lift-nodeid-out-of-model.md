...
- `ProjectionTransform.Direct` — column projects as-is. Default for column projections that don't go through a transform; carried on the projection-emission carrier so the emitter switches exhaustively rather than null-checking an `Optional<ProjectionTransform>`.
- `ProjectionTransform.NodeIdEncodeKeys(String typeId, List<ColumnRef> keyColumns)` — wraps the projected key columns in a `NodeIdEncoder.encode(typeId, c1, ..., cN)` call. `keyColumns` is the resolved tuple, classifier-decided once; the emitter does not re-resolve from `NodeType`.

Dispatch is symmetric with the input side: `CallSiteExtraction` arms route through `ArgCallEmitter` for call-site input shaping, `ProjectionTransform` arms route through the existing scalar-projection emitter for output column shaping. The validator gets a coverage-test arm per `ProjectionTransform` variant just as it does today per `CallSiteExtraction` variant; "Validator + dispatch coverage" below names both.
...
- `InputField.NodeIdField` (output side, scalar) → column projection with `ProjectionTransform.NodeIdEncodeKeys`.
- `InputField.NodeIdReferenceField` → `ColumnReferenceField` with FK path; encoding lifts to projection with `ProjectionTransform.NodeIdEncodeKeys`.
- `InputField.NodeIdInFilterField` → multi-column `ColumnField` with `NodeIdDecodeKeys.SkipMismatchedElement` extraction; body is `ColumnPredicate.RowIn` (or `In` for arity-1).
- `InputField.IdReferenceField` → `ColumnReferenceField` over the FK columns with `NodeIdDecodeKeys.SkipMismatchedElement` extraction; body is `ColumnPredicate.Eq` (single FK), `ColumnPredicate.In` (single FK list arg), or `ColumnPredicate.RowIn` (composite FK list). Subsumes R20.
- `ChildField.NodeIdField` / `NodeIdReferenceField` → column projection forms with `ProjectionTransform.NodeIdEncodeKeys`.
...
- `ArgumentRef.ScalarArg.NodeIdArg` → multi-column `ColumnArg` with `NodeIdDecodeKeys.NullOnMismatch` extraction for `Query.node(id:)` / `Query.nodes(ids:)` shapes, `ThrowOnMismatch` for lookup/mutation key shapes. Stale "Step 5 (the reference variant)" docstring and the matching `FieldBuilder.projectFilters` line 1211 "Step 4 follow-up" comment delete with the variant.
