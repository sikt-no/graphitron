...
- `CallSiteExtraction.NodeIdDecodeKeys.NullOnMismatch(String typeId)` — every Query field classified as a node-by-id fetcher (any Query field whose element type is the `Node` interface, per the signature-based classifier; not just literal `node` / `nodes`), plus the federated `_entities` resolver. typeId mismatch on a single `id` resolves to `null` for the whole field; on one element of `ids`, to `null` for that element only. Parity with the existing Relay-spec behaviour `_entities` already implements.
...
- **Mutation return-encode unification.** `MutationField.DmlTableField.nodeIdMeta` (see "What stays") emits `NodeIdEncoder.encode` at the mutation's return boundary inline in the DML emitter, not via `ProjectionTransform.NodeIdEncodeKeys`. Aligning the two encode paths (so child-field projections and mutation returns share one `ProjectionTransform` dispatch) is principled but a separate change; R50 leaves the mutation emitter alone and confines `ProjectionTransform` to `ChildField` carriers. A future item can lift mutation returns onto the same hook once the `ChildField` side has settled.
