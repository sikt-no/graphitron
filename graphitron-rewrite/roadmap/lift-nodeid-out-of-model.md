...
- **`columnName` redundancy cleanup (in scope).** `ChildField.ColumnField.columnName` and `ChildField.ColumnReferenceField.columnName` retire in the same change as the carrier-slot additions (`compaction`); see "Composite-key column carriers". `ChildField.PropertyField` and `ChildField.RecordField` keep their `columnName` slots — their `column: ColumnRef` is nullable for non-table-backed parents, so `columnName` is the only carrier of the SDL string in those cases.
