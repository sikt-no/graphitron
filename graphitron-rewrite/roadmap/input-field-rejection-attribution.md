...
- R214 (`column-binding-at-classification-not-usage.md`) — the cascade-aware classification restructure. R214's direction §2 (defer `@table` column-coverage to consumption) makes attribution natural: rejections fire at the field's source location by construction. R213 may merge into R214 if both land together, or stay as the sibling that handles the plain-input override:false-on-non-binding-field case the classifier still rejects directly.
