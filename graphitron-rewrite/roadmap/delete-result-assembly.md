...
A post-ship self-review pass surfaced one more stale-description site the spec's grep-style enumeration would not have caught (no deleted symbol named literally; the staleness is a coverage claim that the description outlived). The `@LoadBearingClassifierCheck(key = "service-catalog-strict-service-return")` on `ServiceCatalog.reflectServiceMethod` claimed coverage of "ResultReturnType-with-backing-class paths", but that coverage moved out of `ServiceCatalog` when `ServiceDirectiveResolver.computeExpectedServiceReturnType` started returning `null` for `ResultReturnType` and the equality check landed in `FieldBuilder.checkServiceReturnMatchesPayload`. The `ServiceCatalog` description now names `checkServiceReturnMatchesPayload` as the sibling producer (with a forward pointer to `computeExpectedServiceReturnType` for the routing), and the sibling-reference paragraph on `service-resolver-root-list-record-return-pair` updates in lockstep. Both rewrites are description-only; the producer key, consumer wiring, and audit balance are unchanged.
...
The same commit deletes `graphitron-rewrite/roadmap/service-domain-object-execute-coverage.md` (R169). R169 is a Backlog item whose entire scope is execute-tier sakila coverage for the `ResultAssembly.Assembly` arm; once that arm is gone, the scope no longer exists. Per workflow.adoc's *Discarded* pattern (supersession before shipping), the file deletes outright. The `roadmap/README.md` regeneration in the same commit drops R169 from the rendered roll-up.
...
- `FieldBuilder.classifyServiceCarrierProducer` javadoc (R178-step-3 NoBacking-only structural strict-return rationale, introduced by fe270f9) : names `{@link #resolveServiceResultAssembly}` as ClassBacked carriers' diagnostic path and `{@code resolveServiceResultAssembly}'s payload-class diagnostic` as the routing target the NoBacking-only restriction preserves. Rewrite to name `buildServiceField`'s surviving legacy-equality check as the ClassBacked diagnostic source (single "must return `<PayloadClass>`" reject); the NoBacking-only restriction itself survives unchanged, only the named routing target changes.
- `FieldBuilder.buildMutationServiceField`'s `MutationServiceRecordField` arm comment (R178-step-3 split rationale, inline at the carrier-error yield) : names `{@link #resolveServiceResultAssembly}` as the per-constructor-diagnostic site for ClassBacked payloads. Rewrite to name `buildServiceField`'s legacy-equality check as the diagnostic site; the per-constructor walk is gone post-R179, so the rationale becomes "ClassBacked payloads route through the surviving legacy-equality check, which produces the payload-class diagnostic" instead of "the per-constructor diagnostic citing the payload class".
...
- `SettKvotesporsmalShapeRegressionTest.java` : the class javadoc's "Diagnostic-wording pin" bullet names `resolveServiceResultAssembly`'s `"must return <PayloadClass>"` wording as the migration target; the `classBacked_returnMismatch_diagnosticDoesNotCiteInnerTableRecord` test javadoc names `resolveServiceResultAssembly` as the rejection-routing site. Both rewrites reframe to the surviving site: the legacy-equality check in `FieldBuilder.buildServiceField` (the legacy passthrough check that survives the deletion). The test's actual assertion (`.contains("must return")` against a payload-class-naming diagnostic) is unaffected since the surviving reject reuses today's payload-class-citing wording (see Delete bullet 3).
- `MutationInputResolver.java` `ResultReturnType` arm comment (R178-step-3 narrative around `validateReturnType`) : names `resolveServiceResultAssembly` as the unified-path classification site. Rewrite to name `buildServiceField`'s legacy-equality check as the site, with no change to the validator behaviour described.
