...
* Resolve D1/D2 per review and apply.
...
* **Unit (`@UnitTier`).** Per D1: a type carrying `@table` + `@record` (and `@error` + `@record`) produces the ignored-directive warning (or the conflict rejection, whichever the reviewer chooses), with the chosen behavior pinned.
