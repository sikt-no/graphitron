...
`FieldBuilder.classifyChildFieldOnResultType` (`FieldBuilder.java:2693`) is the one classifier path that produces the result-mapped `RecordTableField` / `RecordLookupTableField` arms; it does not read `DIR_SPLIT_QUERY` today. Two seams within that method head into those arms: the `@sourceRow` branch (after `SourceRowDirectiveResolver.resolve`, with rejection guards on bad lifter signature, parent shape, etc.) and the regular `@record`-parent branch under `case ReturnTypeRef.TableBoundReturnType` (with rejection guards on `parsePath`, `TableFieldComponents`, and `resolveRecordParentBatchKey`). Both classify into DataLoader-keyed arms ; lifter-keyed for `@sourceRow`, parent-record-keyed for the regular path ; so `@splitQuery` is structurally redundant on either. The warning fires on both seams. When the field carries `@splitQuery`, call `ctx.addWarning(new BuildWarning(...))` with a message that names the field's coordinate and explains the redundancy:
...
**Holistic surfacing.** The advisory fires *before* each seam's rejection guards, not after them. A developer who put `@splitQuery` on a `@record`-parent field carries the misconception about scope regardless of whether some unrelated invariant on the same field also rejects (a bad lifter signature, an unresolvable `@reference`, a `TableFieldComponents.Rejected` from a column miss, a `RecordBatchKeyResolution.Rejected` from FK ambiguity, a `parsePath` error). Gating the warning on "classification succeeded" recreates exactly the silent-acceptance failure the spec was written to retire ; the developer fixes the rejection, rebuilds, and *now* learns about the redundant directive. So the placement is: at the top of the `@sourceRow` block (the directive's parent-shape preconditions guarantee a table-targeting field at this point), and immediately after `resolveReturnType` confirms `ReturnTypeRef.TableBoundReturnType` in the regular branch (before the path-error / table-field-components / batch-key rejections).
...
Pipeline-tier tests cover both warning-producing seams across both classification outcomes (Ok and Rejected). Five fixtures, each with a child field carrying `@splitQuery`:

1. A `@record`-parent + `@table`-returning child (regular path, classifies to `RecordTableField`).
2. The same shape with `@lookupKey` on the child (regular path, classifies to `RecordLookupTableField`).
3. A `@sourceRow` field on a `@record`-parent (DTO-parent path, classifies to `RecordTableField` ; one happy-path fixture is enough since both lookup/non-lookup arms within `@sourceRow` share the same emit-warning seam).
4. A regular `@record`-parent field whose `@reference` is unresolvable (`UnclassifiedField` outcome) ; locks the holistic-surfacing rule on the regular seam.
5. A `@sourceRow` field with a bad lifter signature (`UnclassifiedField` outcome) ; locks the holistic-surfacing rule on the `@sourceRow` seam.

Each fixture asserts the classification arm (`isInstanceOf(...)` for the expected outcome, including `UnclassifiedField` for fixtures 4 and 5) and that `schema.warnings()` carries a `BuildWarning` whose `message()` contains the field's coordinate and the substring `"@splitQuery is redundant on a @record-parent field"`.
...
- `FieldBuilder.classifyChildFieldOnResultType` emits a `BuildWarning` when the field carries `@splitQuery` at *both* construction seams (the `@sourceRow` branch and the regular `@record`-parent branch under `TableBoundReturnType`). Emission happens before each seam's rejection guards, so an unrelated rejection on the same field does not suppress the advisory. Successful classifications still produce `RecordTableField` / `RecordLookupTableField`; rejected classifications produce `UnclassifiedField` and *also* surface the warning.
...
- Pipeline-tier tests pin the warning's presence + the structural classification outcome on five fixtures: three happy-path (`RecordTableField`, `RecordLookupTableField`, `@sourceRow` Ok) and two rejection-coexistence (regular path's unresolvable `@reference`, `@sourceRow`'s bad lifter signature).
...
A second reviewer pass on the same day surfaced a subtler asymmetry: the spec's seam placement was *after* each seam's rejection guards (post-`SourceRowDirectiveResolver.Ok`, post-`RecordBatchKeyResolution.Resolved`), which silently suppresses the redundancy advisory on any field that has `@splitQuery` *and* an unrelated rejection (bad lifter signature, unresolvable `@reference`, FK ambiguity, etc.). This recreates the exact silent-acceptance failure R3 was written to retire ; the developer fixes the unrelated issue, rebuilds, and *only then* learns about the redundant directive. The fix is principled rather than narrow: a developer who put `@splitQuery` on a `@record`-parent field carries the misconception about scope regardless of whether some other invariant on the same field also rejects. Diagnostics on a single field should surface holistically, not get gated on whichever check fires first. Tightened to fire each seam's warning *before* its rejection guards, with two new pipeline-tier fixtures that lock the holistic-surfacing rule (one per seam: a regular-path field with an unresolvable `@reference`, a `@sourceRow` field with a bad lifter signature; both assert `UnclassifiedField` *and* the warning's presence).
