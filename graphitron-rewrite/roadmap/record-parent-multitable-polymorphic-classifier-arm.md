...
**Shipped on this branch.** Reachable permits on `ChildField.{InterfaceField, UnionField}` from `@record`-backed parents: `RowKeyed` on `JooqTableRecordType` parents (any cardinality), `AccessorKeyedMany` on Pojo / JavaRecord parents with a typed list / set TableRecord accessor. `AccessorKeyedSingle` on Pojo / JavaRecord parents is **deferred** in this R105 — the single-cardinality multi-table polymorphic emitter (`MultiTablePolymorphicEmitter.buildScalarPerParentFetcher`, `MultiTablePolymorphicEmitter.java:316-366`) reads parent context as `Record parentRecord = (Record) env.getSource()` and has no `@record`-Pojo arm; producing the permit there would generate code that ClassCastExceptions on a Pojo source. `resolvePolymorphicRecordParent` returns `Rejection.deferred("polymorphic-child-record-parent-single-cardinality")` for that shape. `LifterRowKeyed` for polymorphic returns remains deferred per Out of scope.

R105 follow-ups (carried forward):
- Widen `MultiTablePolymorphicEmitter.buildScalarPerParentFetcher` to consume `parentKey` + `parentResultType` analogously to the list arm; the classifier defer above lifts when this lands.
- Per-producer `LoadBearingClassifierCheck` split was correct under today's audit; future work could lift the audit to a *contract*-on-the-field-record form so consumers cite one key rather than the producer-key disjunction.

The canonical-constructor on `ChildField.{InterfaceField, UnionField}` now enforces non-null `parentKey` + `parentResultType`, lifting the validator's de-facto contract into the type system. The full build (`mvn -f graphitron-rewrite/pom.xml install -Plocal-db`) passes.
