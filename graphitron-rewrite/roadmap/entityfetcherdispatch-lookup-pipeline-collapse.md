...
Implementation landed 2026-05-01. Reviewer pass on 2026-05-01 reshaped the helper API and reverted the dispatcher's join syntax — see *Reviewer pass deltas* below.

- New `ValuesJoinRowBuilder` helper at `graphitron/src/main/java/no/sikt/graphitron/rewrite/generators/util/ValuesJoinRowBuilder.java` carries the typed `Row<N+1>` array declaration, per-cell typed value construction, alias-args list, and USING-args list. Unit tests at `graphitron/src/test/java/no/sikt/graphitron/rewrite/generators/util/ValuesJoinRowBuilderTest.java` pin arity behaviour, the arity-22 cap, the empty-slots guard, alias args, USING args, the directive-context cap message, and accept both lookup-shape (`DSL.inline(i)`) and dispatcher-shape (`DSL.val(idx, Integer.class)`) idx cells.
- Helper API is generic over the caller's slot type (`<S>`) plus a `Function<S, ColumnRef>` projection. Lookup callers pass `Slot::targetColumn`; the dispatcher passes `c -> c` against `List<ColumnRef>`. The helper's earlier `Slot` record was removed — there's no parallel-list bridge any more, and the value-expression callback receives the caller's rich slot directly.
...
- `SelectMethodBody` (federated `_entities` + `Query.nodes`) collapsed onto the helper for the row-array, cells, and alias-args pieces. Join syntax kept on the original `.join(input).on(t.COL.eq(input.field("COL", T.class))…)` shape — see *Reviewer pass deltas*. `Condition condition = DSL.noCondition()` is declared before the join so the chain stays symmetric with the lookup site and gives R36 Track B (per-typename interface filters) a hook to AND into; jOOQ folds `noCondition()` away at render time.
- f-E SQL-shape regression test (`GraphQLQueryTest.nodes_perTypeIdBatch_emitsValuesJoinOrderByIdxShape`) continues to pass; matches `join ` plus `values (` plus `order by` substrings.

### Reviewer pass deltas

- **Dispatcher kept on `.on(...)`, not switched to `.using(...)`.** The original spec called for symmetry, but the dispatcher's SELECT projection comes from `<TypeName>.$fields(env.getSelectionSet(), t, env)` which references `t.<col>` directly. `USING` collapses joined columns at render time, which can interact poorly with `$fields`-emitted projections that include the joined key columns themselves. Keeping `.on(...)` matches the legacy shape and avoids that risk; the helper still exposes `usingArgs` for the lookup root path.
- **Helper API is generic.** The original implementation introduced a `ValuesJoinRowBuilder.Slot(ColumnRef)` record and required callers to keep a parallel list aligned with their richer slot type. Reviewer flagged the parallel-list invariant and the dead `Slot` parameter in the value-expr callback as one issue with one principled fix: drop the helper's `Slot`, take `List<S>` plus `Function<S, ColumnRef>`, pass `S` back through the callback. Lookup callers now pass `Slot::targetColumn`; the dispatcher passes `c -> c`.
- **Cap-violation message includes directive context.** `rowTypeArgs` (and the methods that delegate to it) take a `String directiveContext` parameter. Lookup passes `"@lookupKey"`, dispatcher passes `"@key"`, restoring the schema-author UX of the pre-collapse error messages.
- **Empty-slots guard.** `rowTypeArgs` rejects `slots.isEmpty()` with the same directive-context-aware message. Both upstream classifiers already enforce non-empty key columns, so this is defensive only.
- **`MAX_ARITY` is package-private.** Tests live in the same package; no caller needs the constant exposed publicly.

### Earlier implementation deviations (preserved)
