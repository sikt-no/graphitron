...
See link:../manual/reference/directives/scalarType.html[the `@scalarType` directive reference] for the full surface, including the per-arm rejection reasons (`FieldNotFound`, `CoercingErased`, etc.) and the LSP completion behavior on `@scalarType(scalar: |)`.
...
* `.additionalType(...)` for additional object or input types beyond the
  reachable set. Custom scalars go through `@scalarType(scalar:)` on the SDL
  declaration; Graphitron registers the constant for you, and a manual
  `additionalType` of the same scalar is rejected as a duplicate.
...
*Agent users (MCP).* Alongside the LSP, `mvn graphitron:dev` runs an MCP
server on `http://127.0.0.1:8488/mcp` (Streamable HTTP, loopback only). An
MCP-aware agent (Claude Code, Cursor) connected to it gets ambient context
about your graphitron project and a `+/mcp__graphitron__about+` slash command
that explains the dev loop.

Among the tools it exposes, `docs.search` answers natural-language questions
over the bundled manual ("which directive joins two tables?") and returns the
matching passages with a link into this site, so you can find the right pattern
without already knowing the vocabulary to grep for. Its index loads
asynchronously at startup; while it warms, `docs.search` returns a short notice
and the structured tools (catalog, schema, services, conditions, records,
diagnostics, edges) answer in the meantime.

The `graphitron-sakila-example` project ships a committed `.mcp.json` pointing
at this URL, so cloning it needs zero MCP configuration. For your own project,
...
The port is fixed at `8488` (the LSP's is `8487`). If `8488` is already taken
(a second `graphitron:dev` for another project), the dev server fails fast with
a message naming the conflict rather than silently rebinding, so a stale
`.mcp.json` never points an agent at the wrong project's server.

[quote]
____
*`catalog.search`*, semantic, fuzzy discovery over your database catalog. Ask
in natural language ("where are customer addresses stored?") and get back the
most relevant tables ranked by similarity, by their schema-qualified SQL name.
Feed a hit straight into `catalog.describe` for the full column / key / FK
detail. The index warms in the background when `graphitron:dev` starts and
refreshes after a schema change; the first search during a refresh reports that
it is warming and points you at the structured `catalog.*` tools meanwhile.

Semantic search is much stronger when your jOOQ-generated catalog carries table
and column comments. Enable comment generation in your jOOQ codegen
configuration so the database's own documentation feeds the search; without
comments, search still works on table and column names alone.
____
...
startup and scoped logs) now picks up the `@service` / `@condition` /
...
Beyond diagnostics, the editor connection surfaces the Java side of the
schema. Cmd/Ctrl-click (or your editor's go-to-definition) on a `@service`,
`@externalField`, `@condition`, or `@enum` class or method name jumps to its
Java source, and hover shows its Javadoc; the same works for `@table` and
`@field`, jumping to the generated jOOQ table class and column. This needs
the referenced sources to be on the build, which they are for the project's
own source roots (hand-written and generated); references into dependency
jars without attached sources fall back to no jump. Completion narrows to the
right candidates per slot: `@externalField` offers the Field-returning lifter
methods, and the `argMapping` string completes Java parameter names on the
left of each `:` and the field's GraphQL arguments on the right, flagging
unknown or duplicated entries.

Stop the loop with Ctrl+C (a JVM shutdown hook closes the LSP socket,
the MCP server, the watch service, and the debounce executor cleanly).
...
* *MCP server* binds a second loopback port (`8488`) and speaks the Model Context Protocol over Streamable HTTP to an MCP-aware agent. Today it serves static content only, the handshake `instructions` string plus an `about` prompt, so it does not read the warm `Workspace` or feed the generator dispatch; it is the transport-and-lifecycle seam the live catalog/schema discovery tools (R118) build on. It lives in its own `graphitron-mcp` module so that item's heavy native dependencies stay off the plugin's own compile surface. Hosted in embedded Jetty; the literal `8488` is pinned by a `DevMojoTest` assertion on `DevMojo.DEFAULT_MCP_PORT`.
...
flowchart TD
    subgraph JVM["dev JVM (one process)"]
        ...
        Editor -. MCP/HTTP :8488 .-> MCP["MCP server<br/>(static context +<br/>about prompt)"]
...
| `brew install tree-sitter`. graphitron-lsp probes `/opt/homebrew/lib` and
`/usr/local/lib` automatically, so no env-var wiring is needed if Homebrew
installed to its standard location.

| Windows (vcpkg)
| `vcpkg install tree-sitter:x64-windows`. graphitron-lsp picks up the DLL
from `$VCPKG_ROOT/installed/x64-windows/bin/tree-sitter.dll` (or the legacy
`$VCPKG_INSTALLATION_ROOT` variable, or the default `C:\vcpkg\...` layout)
automatically. If vcpkg's `bin` directory is on `PATH`, that also works
through jtreesitter's normal loader chain.
...
<<nixos-shell-nix,NixOS shell.nix>> below. Nix-store paths are content-hashed
so probing isn't tractable; the shellHook is the standard NixOS pattern.
Nixpkgs `tree-sitter` is `0.25.x` (within jtreesitter 0.26's compat window).
...
| If you installed `libtree-sitter` somewhere the probe doesn't cover, set
`JAVA_TOOL_OPTIONS="-Djava.library.path=/your/lib/dir"` in your shell rc.
This is the most robust env-var-only escape hatch across platforms because
the JVM reads it at startup before any native-lookup work runs — unlike
`DYLD_LIBRARY_PATH` it doesn't trip macOS's SIP stripping, and unlike
`LD_LIBRARY_PATH` it survives `sudo` boundaries.
|===
