...
The chapter prose above enumerates `Rejection`'s permits: `AuthorError.UnknownName`, `AuthorError.Structural`, `AuthorError.AccessorMismatch`, `AuthorError.RecordBindingMultiProducer`, `AuthorError.TypeConflict`, `InvalidSchema.DirectiveConflict`, `InvalidSchema.CaseFoldCollision`, `InvalidSchema.Structural`, and `Deferred`. A new permit on the sealed hierarchy must land with a corresponding mention in this page; otherwise the prose silently goes stale. `SealedHierarchyDocCoverageTest` walks `Rejection.permits()` transitively and asserts each permit name appears in `typed-rejection.adoc`: bidirectional, tied to a closed set the compiler already exhaustivity-checks. A new permit added without a paragraph here fails the test; a permit removed without removing its mention fails too.
