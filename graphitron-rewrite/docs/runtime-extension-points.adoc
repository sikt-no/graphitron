...
*Why per-app emission rather than a shared runtime jar.* Graphitron emits `GraphitronContext` into the consumer's source tree on every generator run, instead of shipping it as an interface in a `graphitron-runtime` library that the consumer would depend on. The choice is deliberate. A shared runtime jar would force every consumer onto the same `DataFetchingEnvironment` import (graphql-java's), the same `DSLContext` import (jOOQ's), and the same method signatures across versions; the emitted form lets each app's interface live in its own package, share its own type aliases, and evolve in lockstep with the schema the generator was run against. The runtime "extension point" is not an inheritance contract that ships independently; it is a typed shape the generator emits, and the consumer implements. No Graphitron runtime classes leak into the consumer's classpath, and the generator stays independent of any jar publication cadence.
...
public interface GraphitronContext {
    ...
    default <T> T getContextArgument(DataFetchingEnvironment env, String name) {
...
`getDslContext` is the only abstract method; the others carry defaults so apps
that only need to thread a `DSLContext` can pass a lambda
(`(GraphitronContext) env -> dsl`) or rely on the `Graphitron.newExecutionInput(dsl)`
factory overload.

=== Registration

Apps register an implementation per request via `Graphitron.newExecutionInput(...)`,
which puts the context under the typed key every emitted fetcher reads from:
...
xref:getting-started.adoc#hello-world[Getting Started > Hello world]. The rest of
...
sequenceDiagram
    ...
    Client->>Engine: ExecutionInput<br/>(query, GraphitronContext, DataLoaderRegistry)
    Engine->>Fetcher: invoke (DataFetchingEnvironment env)
    Fetcher->>Ctx: graphitronContext(env).getDslContext(env)
    Ctx-->>Fetcher: per-request DSLContext<br/>(tenant routing, RLS session vars)
...
xref:getting-started.adoc#tenant-scoped-dslcontext[Getting Started > Tenant-scoped DSLContext].
...
entry. See
xref:getting-started.adoc#context-arguments-from-a-jwt-claim[Getting Started > Context arguments from a JWT claim]
