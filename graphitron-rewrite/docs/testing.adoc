...
For build commands and database setup, see xref:../../../.claude/web-environment.md[web-environment.md].
...
. *The behaviour is "this generated source must compile against the real jOOQ catalog"* → **Compilation.**
  No test class to write; the fixture-driven `mvn compile -pl :graphitron-test -Plocal-db` is the assertion.
...
`graphitron-test` hosts tests at every tier; its module dependency on post-generator artifacts is the reason those tests live there, not a tier signal.

== Tier annotations

Each tier has a JUnit 5 meta-annotation in `graphitron-fixtures` main scope (`no.sikt.graphitron.rewrite.test.tier`), reachable from every test class in `graphitron` and `graphitron-test`:
...
An enforcement test in each in-scope module (`graphitron`, `graphitron-test`) walks that module's own test classpath and fails the build if any `@Test`-bearing class lacks a tier identity, or carries more than one.
...
*Where:* `graphitron-test`, run with `mvn compile -f graphitron-rewrite/pom.xml -pl :graphitron-test -Plocal-db`.
...
*Where:* `graphitron-test`, run with `mvn test -f graphitron-rewrite/pom.xml -pl :graphitron-test -Plocal-db`.
...
== Module location vs. tier (`graphitron-test`)

Several tests live in `graphitron-test` for module-dependency reasons but classify by assertion, not module.
...
* `ScatterSingleByIdxTest` -- `@UnitTier` (direct unit coverage, fully in-memory; lives in `graphitron-test` because it reflects against a generated `*Fetchers` class).
...
See xref:../../../.claude/web-environment.md[web-environment.md] for database setup prerequisites and the fixtures-jar footgun recovery.
