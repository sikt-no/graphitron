...
Three pairings are structurally illegal; `SourceKey`'s compact constructor rejects them. Each is documented as a `@LoadBearingClassifierCheck` and paired with a `@DependsOnClassifierCheck` on the emit site that depends on it.
...
Each invariant is one paragraph at a single place (the compact constructor) but governs the emit shape at multiple consumers downstream. Relaxing one without auditing the consumer side surfaces as an orphaned `@DependsOnClassifierCheck` in `LoadBearingGuaranteeAuditTest`, not as a runtime surprise.
