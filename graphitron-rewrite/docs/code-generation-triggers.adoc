...
A field's classification factors into two asserted dimensions. The worked example below renders this
dimensional form from the corpus; the leaf-name tables that follow are the transitional reference until
the field taxonomy is fully migrated.
...
| `@mutation(typeName: INSERT)`, returning `ID` or a `@table` type | `MutationInsertTableField` | Field method stub
| `@mutation(typeName: UPDATE)`, returning `ID` or a `@table` type | `MutationUpdateTableField` | Field method stub
| `@mutation(typeName: DELETE)`, returning `ID` or a `@table` type | `MutationDeleteTableField` | Field method stub
| `@mutation(typeName: UPSERT)`, returning `ID` or a `@table` type | `MutationUpsertTableField` | Field method stub
