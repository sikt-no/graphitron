...
[NOTE]
====
*Pre-pivot vocabulary.* This section describes the field axes (source context, target type, scope) in their original form. They fold into the three-axis `carrier x intent x mapping` model in <<field-classification,Field Classification>>, under the R222 dimensional-model umbrella; materialising those axes as slots on the field itself is R290 (`datafetcher-field-dimensional-slots`). Treat <<field-classification,Field Classification>> as canonical for how a field classifies; the terms below survive as the historical framing and as properties derived from the three axes, not as independent axes.
====
...
| Producer's reflected return is a Java/jOOQ class (a `@service` return, a `@tableMethod` return, or a parent-accessor chain) | `ResultType`* | `*Fetchers` class only (no SQL scope of its own). The backing class is reflection-derived, not declared; a reachable type still carrying `@record` warns to remove it.
...
| Input type with `@table` | `TableInputType` | Used in mutation generation. If the declaration also carries `@record`, `@table` wins; `@record` is ignored and a build warning names it for removal (shadowed by `@table`).
| Input type without `@table` (the `@service` / `@condition` / `@tableMethod` parameter the input flows into is reflected as a developer class) | `InputType`* | No generation (developer-provided class). The input's backing class is reflected from the parameter type it flows into; a residual `@record` is ignored and warned.
...
* *`carrier`*, the GraphQL parent-type category, which *is* the field type: `Query` (the root query),
  `Mutation` (the root mutation), or `Source` (every other, non-Subscription type). The carrier is
  position, and it is also the legality gate: write intents only on `Mutation`, `NodeResolve` only on
  `Query`, `Nesting` only on `Source`.
* *`intent`*, the operation kind: reads (`Fetch`, `Lookup`, `NodeResolve`, `Nesting`, and the
  modeled-but-unpopulated `EntityResolve` / `Count` / `Facet`), writes (`Insert`, `Upsert`, `Update`,
  `Delete`, and the unpopulated `UpdateMatching` / `DeleteMatching`), and the coarse service polarity
  (`QueryService`, `MutationService`).
* *`mapping`*, what domain object the value is, carrying *build-vs-consume*: `Table` / `TableConnection` /
  `Column` are catalog shapes graphitron *builds* the SQL for; `Record` / `Field` are domain shapes
  graphitron *consumes* without having built. `Table:Column :: Record:Field` (mirror : reflect).
...
intent family). The governing principle is *assert what nothing else carries; derive what another axis or
slot already forces*. The subsections below sound out the derived layer and each axis in turn.

=== The derived layer: same verdict, different mechanism

Two fields can share an identical `(carrier, intent, mapping)` verdict yet emit different SQL, because
...
through a DataLoader. Both classify identically (`carrier = Source`, `intent = Fetch`, `mapping = Table`);
only the derived new-query layer differs, forced by the `@splitQuery` slot.
...
key and carry the same `Source` / `Fetch` / `Table` verdict; `@splitQuery` flips only the derived
...
over the same `film_language_id_fkey` foreign key and carry the same `Source` / `Fetch` / `Table` verdict;
the record handoff forces a derived keyed re-query under `FilmDetails`, not a different asserted axis.
Asserted by the `record-table` corpus example.

=== `mapping`: build-vs-consume (Column vs. Field)

The `mapping` axis records what domain object the value is, and with it whether graphitron *builds* the
SQL (catalog: `Table` / `Column`) or *consumes* a value it did not build (domain: `Record` / `Field`).
Two scalar flavors hinge on the parent's table-ness: a scalar under a `@table` parent maps to `Column`
(a real database column graphitron projects), while under a record-backed parent (a plain object with no
`@table`, here produced as a service method's return type) a scalar maps to `Field` (a POJO property read
off a record graphitron only reflects). A nested non-table object under a record parent is the object
flavor of the same `Field` mapping. All three are `Source` / `Fetch`; only the parent's table-ness moves
the `mapping` axis across the build-vs-consume line.
...
`Film.title` classifies with `mapping = Column` (`title` is a column of the `film` table graphitron
builds the projection for). `FilmStats.count`, a scalar under the record-backed `FilmStats`, classifies
with `mapping = Field` (a property graphitron reflects). `FilmDetails.stats`, a nested non-table object
under the record-backed `FilmDetails`, also classifies with `mapping = Field` (its object flavor). All
hold `carrier = Source`, `intent = Fetch`. Asserted by the `mapping` corpus example.
...
than as a distinct mapping value. The intent is `Fetch` for interface and union fields (root or child)
and `NodeResolve` for the Relay `node` / `nodes` roots; the new keyed query a plain-interface or union
field opens (`InterfaceField` / `UnionField`, and the polymorphic roots `QueryInterfaceField` /
`QueryUnionField` / `QueryNodeField` / `QueryNodesField`) is a *derived* new-query, not an asserted axis.
The one structural difference is a `@table`+`@discriminate` interface child (`TableInterfaceField`): it
is foreign-key-correlatable from the parent, so it inlines rather than opening a new query, but its
asserted `Source` / `Fetch` / `Table` verdict is the same.
...
so it classifies with `carrier = Source`, `intent = Fetch`, `mapping = Table` (`InterfaceField`): a
derived new keyed query projects the participant table, with `Address` recorded as the participant slot.
...
`FilmActor.related` returns the union `FilmOrActor`, so it classifies with `carrier = Source`,
`intent = Fetch`, `mapping = Table` (`UnionField`): a derived new keyed query projects whichever
...
The `@table`+`@discriminate` interface child (`TableInterfaceField`, inline) and the Relay `Node` root
(`QueryNodeField`, `intent = NodeResolve`) are further leaves on this same polymorphic rule, each holding
`mapping = Table`; they are asserted corpus-only (`table-interface`, `relay-node`).
...
`carrier = Mutation`, the write verb as its `intent` (`Insert` / `Update` / `Delete`), and
`mapping = Table`. The write produces the affected row, then a follow-up `SELECT` re-projects it through
the catalog; that read-back is the *derived re-fetch* (a write producer yielding a `Table` shape), not a
separate asserted axis. The mutation's input object is part of the rendered closure, so the excerpt
...
type `Film`: `carrier = Mutation`, `intent = Insert`, `mapping = Table` (`MutationInsertTableField`), the
read-back being the derived re-fetch. Asserted by the `dml` corpus example.
...
| `@mutation(typeName: DELETE)`, returning `ID` | `MutationDeleteTableField` | Fetcher method ; write + encoded-PK off `RETURNING` (no read-back ; the row is gone, `mapping = Column`)
