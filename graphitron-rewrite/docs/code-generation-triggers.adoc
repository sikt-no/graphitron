...
`interface` corpus example. The union and Relay-`Node` shapes follow the same dimensional rule and are
asserted corpus-only (`union`, `table-interface`, `relay-node`); rendering their selections as doc
excerpts needs the fragment and argument support tracked in R281's pre-migration hardening.
