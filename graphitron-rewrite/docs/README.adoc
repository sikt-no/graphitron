...
You want to *extend the runtime*, wire per-request values into `Graphitron.newExecutionInput(...)`, register custom scalars, hook in jOOQ listeners. → xref:runtime-extension-points.adoc[Runtime Extension Points].
...
You're looking for a *deeper reference*, the architectural principles that govern both classifier and emitter, the test-tier rubric, the unified argument-resolution lift, the dispatch-axis model behind DataLoader-backed source-side fields. → see xref:rewrite-design-principles.adoc[Rewrite Design Principles], xref:testing.adoc[Test-tier guide], xref:argument-resolution.adoc[Argument Resolution], xref:dispatch-axes.adoc[Dispatch axes]. The ongoing-work view lives at xref:../roadmap/index.adoc[Rewrite Roadmap].
...
`graphitron-rewrite/` is a nested monorepo with its own parent pom (`graphitron-rewrite-parent`, `10-SNAPSHOT`). Eleven modules cover the generator, its inputs, its consumers, the reusable runtime surface, and the tooling that wraps both ends:
...
| `graphitron-mcp`
| MCP server embedded in the `dev` Mojo: serves ambient context and an `about` prompt to an MCP-aware agent over loopback Streamable HTTP. The dependency-quarantine seam for the live discovery tools (R118).

| `graphitron-jakarta-rest`
| Reusable, spec-conformant GraphQL-over-HTTP serving library (vendor-neutral Jakarta REST + CDI) over a Graphitron schema: the `/graphql` resource, status-code semantics, SDL endpoint, and GraphiQL page, behind a `GraphitronApplication` SPI the consumer's adapter implements (R399). The first hand-written *runtime* artifact consumers depend on; compiles at `<release>17</release>`.
