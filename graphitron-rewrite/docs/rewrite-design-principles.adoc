...
NOTE: The `@LoadBearingClassifierCheck` / `@DependsOnClassifierCheck` annotation pair is in retirement under roadmap item R237 (`graphitron-rewrite/roadmap/retire-loadbearing-classifier-check-annotations.md`). Existing sites remain audit-clean until they retire under the chosen Phase 3 branch; no new producer keys should be filed during the retirement window.
