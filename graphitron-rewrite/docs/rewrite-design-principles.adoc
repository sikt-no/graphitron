...
Every builder-step lift returns a sealed `Resolved` result; rejection is a variant, not a return-string-or-null. R6's lift of cross-cutting concerns out of `FieldBuilder` produced ten siblings on the same shape: `ServiceDirectiveResolver`, `TableMethodDirectiveResolver`, `ExternalFieldDirectiveResolver`, `LookupKeyDirectiveResolver`, `OrderByResolver`, `ConditionResolver`, `MutationInputResolver`, `EnumMappingResolver`, `BatchKeyLifterDirectiveResolver`, `NodeIdLeafResolver`. Each entry point returns a sealed `Resolved` (typically `{None | Ok | Rejected}`, or domain-specific arms like `ServiceDirectiveResolver.Resolved.{Success.{TableBound | Result | Scalar} | ErrorsLifted | Rejected}`), and the caller's switch is exhaustive across success and failure modes both.
...
table + key columns, single-hop by construction) and a `LifterRef` (declaring class + method name).
The emitter feeds that into the existing `SplitRowsMethodEmitter.buildListMethod` path with no
identity branching: target accessors come from the `JoinStep.WithTarget` capability, key extraction
