...
The smell to watch for: an emitter building a `CodeBlock` whose template spans several lines of nested `? :`, casts to `Object` to dodge a parameterised `instanceof`, and binds `_x`-style locals. That block is being written for the emitter author's convenience, not the consumer's; the fix is a named helper with a statement body. `ArgCallEmitter.buildNodeIdDecodeExtraction` is the current worked counter-example and the R259 cleanup target.
