...
The typed-rejection narrative ("Builder-step results are sealed, not strings or out-params"), the sealed-hierarchies guidance, and the wire-format-boundary principle are slated to consolidate into the public user-manual architecture chapter once it lands (tracked under R86, `graphitron-rewrite/roadmap/architecture-chapter.md`); until then, this contributor-facing reference is the canonical source.
...
Reading the reflection `java.lang.reflect.Type` tree is permitted only at builder-side classifiers that convert reflection output into the typed model. Today four files cross that boundary: `ServiceCatalog` (for `@service` and `@tableMethod` parameter classification, including the post-R7 `classifySourcesType` for DataLoader source parameters); `ServiceDirectiveResolver` (for `@externalField` reference methods); `BatchKeyLifterDirectiveResolver` (R1, for the developer-supplied `@batchKeyLifter` lifter signature); and `FieldBuilder` (for `@batchKeyLifter` argument-mapping and accessor reflection on `@record` parents whose backing class carries typed FK accessors). Each converts raw reflection output into `MethodRef.Param`, `BatchKey`, or `AccessorRef` values (each carrying a `ParamSource`). Everything downstream ; validator, generator ; switches on the pre-classified values and never touches reflection types.
