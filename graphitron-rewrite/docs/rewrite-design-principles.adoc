...
`BatchKey` illustrates the pattern: `RowKeyed` / `RecordKeyed` / `MappedRowKeyed` / `MappedRecordKeyed` carry `parentKeyColumns: List<ColumnRef>`, while `LifterLeafKeyed` carries a `JoinStep.LiftedHop` plus a `LifterRef` and `LifterPathKeyed` carries the resolved `List<JoinStep>` FK chain plus a `LifterRef`. The variants split across two sealed sub-interfaces (`ParentKeyed` for the catalog-PK arms, `RecordParentBatchKey` for the `@record`-parent arms) plus a `LifterKeyed` sub-seal of the latter so side-aware accessors live exactly where they apply. None carry fields they don't use. The compiler enforces exhaustive switches ; when a new variant is added, every switch that doesn't handle it becomes a compile error.

When variants split on independent axes (container × key-shape × backing-class for `BatchKey`; encode-direction for `HelperRef`'s `Encode` / `Decode`; etc.), use sealed sub-interfaces per axis rather than inventing a god accessor whose meaning depends on the variant. The R1 split removed `BatchKey.keyColumns()` ; a shared accessor whose semantics differed per arm ; in favour of `ParentKeyed.parentKeyColumns()` and `LifterKeyed.parentSideColumns()` (delegated through `LiftedHop` on `LifterLeafKeyed`, through the first FK hop on `LifterPathKeyed`). The smell to watch for: a single shared accessor that means "FK source columns" in one arm and "child target columns" in another. The split pushes the per-axis meaning into the type system.
...
The remaining keys cover NodeId FK invariants (`nodeid-fk.direct-fk-keys-match`), error-channel structure (`error-type.path-message-fields`, `error-channel.mappings-constant`), declared-exception coverage (`service-method.declared-exceptions-covered`), and `@sourceRow` routing (`sourcerow-classifies-as-record-table-field`, `sourcerow-leafkey-batchkey-is-lifterleafkeyed`, `sourcerow-pathkey-batchkey-is-lifterpathkeyed`).
