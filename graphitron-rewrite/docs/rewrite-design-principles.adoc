...
`SourceKey.Reader` illustrates the pattern at its current shape: `ColumnRead` carries no payload (catalog-FK columns live on the surrounding `SourceKey`, not the reader), `AccessorCall(AccessorRef)` carries the typed instance accessor on a `@record` parent's backing class, `SourceRowsCall(LifterRef)` carries the developer-supplied `@sourceRow` static lifter, `ServiceTableRecord(ClassName)` carries the developer-declared typed jOOQ `TableRecord` subclass the `@service` method returns, and `ServiceUntypedRecord` carries no payload (untyped `Record`/scalar service return). Each variant holds exactly the data its rows-method body needs to read parent-side input; none carry fields they don't use. `SourceKey.Wrap` is a sibling sealed: `Row` / `Record` / `TableRecord(ClassName)`, where the `TableRecord` arm carries the developer-declared subclass and the other two arms carry nothing. The compiler enforces exhaustive switches ; when a new variant is added (e.g. R75's `ResultRowWalk` Reader permit for rooted-DML payload data fields), every switch that doesn't handle it becomes a compile error.

When variants split on independent axes, use sealed sub-interfaces per axis (or separate records orthogonal to the variant axis) rather than inventing a god accessor whose meaning depends on the variant. R38 is the canonical worked example: the pre-reshape `BatchKey` taxonomy conflated three axes ; container choice (positional list vs mapped set), source-side reader contract (catalog FK, typed accessor, `@sourceRow` lifter, service record), key-shape wrap (`Row`, `Record`, typed `TableRecord`) ; into a 10-permit sealed hierarchy whose downstream dispatch sites all reconstructed the underlying axes via `instanceof BatchKey.X` chains. The reshape pulled each axis into its own first-class type: `SourceKey.Wrap` for key shape, `SourceKey.Reader` for body input contract, `SourceKey.Cardinality` (ONE / MANY) for the per-source row count, and a sibling `LoaderRegistration` record carrying `Container` (POSITIONAL_LIST / MAPPED_SET) and `Dispatch` (LOAD_ONE / LOAD_MANY) for DataLoader identity. Each dispatch site reads off whichever axis it actually cares about, instead of reconstructing the axis from a conflated permit. Net type-identity count fell from 10 to 1 + 5 = 6, and three of the dispatch axes became orthogonal at the type level rather than tangled inside one taxonomy. The smell to watch for: a single shared accessor that means "FK source columns" in one arm and "child target columns" in another, or a sealed permit name that splices two axes together (`MappedRowKeyed` was both "mapped container" and "RowN key shape" in one identity). The split pushes the per-axis meaning into the type system.
...
Find the live set via `LoadBearingGuaranteeAuditTest`'s scan, or grep for `@LoadBearingClassifierCheck` under `graphitron-rewrite/`. The two worked examples:
...
The remaining keys cover NodeId FK invariants (`nodeid-fk.direct-fk-keys-match`), error-channel structure (`error-type.path-message-fields`, `error-channel.mappings-constant`), declared-exception coverage (`service-method.declared-exceptions-covered`), `@sourceRow` routing (`sourcerow-classifies-as-record-table-field`, `sourcerow-leafkey-sourcerows-singlehop`, `sourcerow-pathkey-sourcerows-fkchain`), and `SourceKey` cross-axis invariants (`source-key.source-rows-call-wraps-row`, `source-key.accessor-call-wraps-record`, `source-key.service-table-record-target-aligned-empty-path`).
