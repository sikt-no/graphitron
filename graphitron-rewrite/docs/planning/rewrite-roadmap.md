...
| Implicit column conditions for `@table` input types | Ready | [plan](plan-implicit-input-conditions.md) |
...
- **Graphitron emits a prebuilt programmatic `GraphQLSchema`** **[Spec]** — replace the emitted `Graphitron.java` facade's SDL + `RuntimeWiring` assembly with a prebuilt `GraphQLSchema`; apps call `Graphitron.getSchema()` and start serving. SDL becomes a build-time derivative for federation composition and client docs. Single breaking change, no parallel path. Reshapes umbrella items *directive stripping* (collapses into `SchemaPrinter.Options`), *client SDL emission* (supplies the mechanism), and *federation SDL integration* (simplifies to `Federation.transform(builtSchema)`). Depends on *schema loading* landing first. `@notGenerated` drops to a validator-level rejection in the same commit (directive declaration stays for parse compatibility). ([plan](plan-graphitron-prebuilt-schema.md))
