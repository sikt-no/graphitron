...
| Fact | Value |
|---|---|
| Commits ahead of `origin/main` | 566 (as of sync on 2026-04-24) |
...
| New commits since plan drafting | `ac3df0b7` + `f7d2be49` deleted legacy integration code; `76754b33` added standalone `graphitron-rewrite-maven` inside `graphitron-rewrite/` |
...
- Root `pom.xml`: only the `<module>graphitron-rewrite</module>` addition
  (not the Java 21 version bump; see §Surgical pom.xml edit)
...
| Root `pom.xml` Java 21 version bump | Legacy modules are not ours to change here |
...
### Step 4: surgical pom.xml edit

The root `pom.xml` in the working tree has two changes relative to
main: the module entry we want and the Java version bump we don't want.

1. Open `pom.xml` and find the `<modules>` block.
2. Verify `<module>graphitron-rewrite</module>` is present.
3. Find the `maven-compiler-plugin` configuration and revert the
   `<release>` value back to `17` (main's value). The line reads:
   `<release>21</release>` after the squash; change it to `<release>17</release>`.

Alternatively use git to restore main's version and re-apply just the
module entry:

```bash
git diff HEAD -- pom.xml   # shows both changes
# manually revert the <release>21</release> change, keeping the module entry
```

### Step 5: commit the rewrite module
...
# Should list: pom.xml plus every file under graphitron-rewrite/
# Should NOT list anything outside graphitron-rewrite/ except pom.xml
```
...
### Step 7: drop the remaining legacy changes
...
### Step 9: push and fast-forward trunk
...
branch touched `pom.xml`. The rewrite branch's copy has
`<module>graphitron-rewrite</module>` plus `<release>21</release>`.
Main's current tip has neither. After `merge --squash` the conflict
markers will be in the `<modules>` block and in the
`maven-compiler-plugin` configuration. Accept the rewrite branch's
`<modules>` block (it has the module entry) and revert the release
value to `17`.
...
**`plan-rewrite-maven-plugin.md` is still `[Spec]` in the roadmap**:
the implementation of that plan landed on the trunk (`76754b33`) but the
plan was not promoted to Done before this squash. That is fine: once
the squash lands, delete the plan file and move its roadmap entry to
Done, citing the squash commit sha, as a follow-up commit on trunk.
