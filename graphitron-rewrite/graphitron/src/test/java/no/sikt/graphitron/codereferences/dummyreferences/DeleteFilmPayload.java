...
/**
 * Test fixture record used by the DML payload-assembly classifier (R12 DML chunk). The
 * canonical (all-fields) constructor exposes both a row slot ({@link FilmRecord}, the jOOQ
 * record class for the {@code film} table) and an errors slot. Used as
 * {@code @record(record: {className: "...DeleteFilmPayload"})} on a payload returned by a
 ...
 */
public record DeleteFilmPayload(FilmRecord film, List<? extends GraphitronError> errors) {}
