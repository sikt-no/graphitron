 ...
 *       through to {@code redact} on no match.</li>
 *   <li>The nested {@code Mapping} taxonomy ({@code ExceptionMapping},
 *       {@code SqlStateMapping}, {@code VendorCodeMapping}) carries the criteria the
 *       dispatch arm matches against plus the {@code @error} handler's optional
 *       description. There is no per-mapping factory: the matched exception itself
 *       goes into the errors list, and graphql-java's {@code PropertyDataFetcher}
 *       reads each declared {@code @error} field directly off whatever source object
 *       happens to be in the slot. Each per-channel {@code Mapping[]} constant lives
 *       on the per-package {@code ErrorMappings} helper. {@code ValidationHandler}
 *       entries produce no {@code Mapping}; the wrapper invokes
 *       {@code jakarta.validation.Validator} as a pre-execution step (§5) and the
 *       resulting {@code GraphQLError}s go directly into the errors slot, never
 *       reaching the dispatcher.</li>
 * </ul>
 *
