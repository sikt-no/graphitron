...
/**
 * R370 compilation-tier fixture: the photo-negative of {@link AddressOccupantCarrier}. Where that
 ...
 * the {@code AccessorRef}'s parent-backing {@code ClassName}. Before R370 that {@code ClassName} was
 * {@code ClassName.bestGuess(binaryName)}, which does not split on {@code $}, so the emitted cast
 * spelled {@code ((pkg.NestedOccupantCarrierHolder$Carrier) env.getSource()).address()} and failed
 * {@code javac}. After R370 the boundary resolves the enclosing structure via
