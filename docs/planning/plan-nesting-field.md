...
>
> Post-implementation amendment (review round on branch `claude/review-nestingfield-plan-VijHA`):
> - `buildWiringEntry` with `className=null` is only safe for a closed set of variants — validator gains a nested-depth whitelist rejecting the rest (`PlatformIdField`, `ServiceTableField`, …).
> - Shape-compat check recurses into nested `NestingField` entries; two-level shared-nested types are no longer silent on divergent inner columns.
> - Generated projection's `default` arm uses silent skip (not `AssertionError`); validator is the fail-fast layer. Plan snippet updated to match.
...
`NestingField` only arises under `@table` parents — `classifyObjectReturnChildField` is reached via `classifyChildFieldOnTableType` (`FieldBuilder.java:1217`), which is gated on `TableBackedType`. `@record` parents route through `classifyChildFieldOnResultType` (`FieldBuilder.java:1510`) with no path to `NestingField`, so no additional rejection rule is needed.
...
private static void emitSwitch(CodeBlock.Builder code, int depth,
    ...
    for (var f : fields) {
        switch (f) {
            ...
            // catches drift in the PROJECTED_LEAVES partition. Silent skip matches the style of
            // the existing top-level switch — the validator is the fail-fast layer, not this.
            default -> { /* validator rejects non-PROJECTED_LEAVES variants */ }
        }
    }
...
**`buildWiringEntry` contract at nested depth.** The aggregator calls `buildWiringEntry(nf, className=null, representativeParentTable, resultType=null)`. Any arm depending on `className` or `resultType` falls through to the `$L::$L` fallback and emits `null::fieldName` — invalid Java. Only the arms guarded by `field.name()` alone or by `parentTable != null` (i.e. the `ColumnField`, `NodeIdField`, `TableField`, `LookupTableField`, `NestingField`, `ConstructorField` arms as currently implemented) are safe at nested depth. Everything else is rejected by the validator's nested-depth whitelist (see below); keeping the set explicit in one place is the invariant that lets the aggregator pass `className=null` safely.

**Aggregator.** Each nested type is wired exactly once regardless of how many outer parents reference it, so nested wiring belongs at the global aggregator rather than on any parent's Fetchers class. Collection happens in `GraphQLRewriteGenerator.generate()` alongside the existing `connectionWirings` producer (`GraphQLRewriteGenerator.java:70–84`): iterate `schema.fields()` for every `NestingField` instance across all classified types, dedupe by nested GraphQL type name, pick the first outer `@table` parent in SDL order as representative (SDL order is preserved by `TypeBuilder.buildTypes()`'s `LinkedHashMap` over `ctx.schema.getAllTypesAsList()`), project to `List<NestedTypeWiring>`, pass as a new argument to `GraphitronWiringClassGenerator.generate(...)`. Each `NestedTypeWiring` carries the nested type name, the representative parent's `TableRef`, and the classified `nestedFields`. The aggregator emits one `.type(TypeRuntimeWiring.newTypeWiring(ntw.typeName()) … )` block per entry, calling a static helper exposed from `TypeFetcherGenerator` (refactor `buildWiringEntry` into a package-visible static so the aggregator can reuse it without duplicating per-leaf logic). `ColumnFetcher`'s `((Record) source).get(column)` (`ColumnFetcherClassGenerator.java:56`) relies on jOOQ's name-based fallback in `Record.get(Field)` — representative parent's `Tables.FILM.TITLE` works against any incoming `Record` whose metadata carries a compatibly-typed `TITLE` column, which the parent-compatibility check has guaranteed. Lock the fallback behaviour in with an execution test.
...
- **Parent compatibility.** A plain-object nested type may be referenced from any number of outer `@table` parents. For each nested type, pick a representative parent (first outer `@table` parent in SDL order; ties not possible since type names are unique), classify `nestedFields` against it, then re-classify against every other parent and assert each ColumnField's `columnName` + `ColumnRef.columnClass` (Java type) matches the representative's, field-by-field. Mismatches reject with a pointed error: "nested field `FilmDetails.releaseYear` requires column `RELEASE_YEAR` on `ADVERTISEMENT`, not found" / "… resolves to `Short` on `FILM` but `Integer` on `ADVERTISEMENT`". Non-column leaves (ColumnReferenceField, ComputedField, …) reject at nested depth via the existing stubbed-variant check; multi-parent support for those lands with their own roadmap arms. Nested `NestingField` entries recurse — the shape check descends into `nestedFields()` pairwise so a two-level nested type with divergent inner columns doesn't slip past.
...
- **Nested-depth whitelist.** In the same recursive walk, reject any leaf that is neither in `NOT_IMPLEMENTED_REASONS` (handled above) nor in the closed set of variants wireable at nested depth: `ColumnField`, `NodeIdField`, `TableField`, `LookupTableField`, `ConstructorField`, `NestingField`. Variants outside this set (`PlatformIdField`, `ServiceTableField`, etc.) classify successfully at nested depth but have no `buildWiringEntry` arm that works without a fetcher class name, so the aggregator would emit `null::fieldName`. Reject at parse time with `"<Variant> is not yet supported under NestingField — see rewrite-roadmap.md #8"`. Expanding the whitelist is a one-line edit paired with adding a `className`-independent arm to `buildWiringEntry`.
...
- **`PlatformIdField` / `ServiceTableField` / other whitelist-excluded variants under `NestingField`.** Rejected at validate time by the nested-depth whitelist — their `buildWiringEntry` arms require a fetcher class name and there's none at nested depth. Support lands by adding a `className`-independent arm in `buildWiringEntry` and extending the whitelist.
