...
1. **Rows-method signature is 2-param, not 3-param.** Plan pinned `(List<K>, DataFetchingEnvironment, SelectedField) → List<List<Record>>`. Actual is `(List<K>, DataFetchingEnvironment)`. `DataFetchingFieldSelectionSet.getField(String)` cited by the plan as the service-stub template's `dfe.getSelectionSet().getField(fieldName)` source does not exist on graphql-java 25.0 — only the list-returning `getFields(String, String...)` does. The emitter uses `env.getSelectionSet()` directly for projection, which is semantically identical to what `sel.getSelectionSet()` would have returned when `sel` is the field being fetched. Pre-existing `buildServiceDataFetcher` (`:970-972`) still emits the non-existent `getField(String)` call — a latent issue but pre-dates this plan and out of scope.

2. **Parent-VALUES rows use typed `Row<N+1>`, accessed via `fieldJ()` not `valueJ()`.** Plan Decision 7 said unpack via `k.value1()`…`k.valueN()`. Those methods live on `Record1/Record2/…`, not on `Row1/Row2/…` — `Row` is a schema construct and exposes its cells as typed `Field<T>` via `fieldJ()`. The spirit of Decision 7 (codegen-time arity, no runtime introspection) is preserved. One unavoidable `@SuppressWarnings("unchecked")` for generic array creation per emitted method. Arity capped at 22 (jOOQ's typed Row/Record hierarchy).

3. **DataLoader batch-lambda parameters explicitly typed.** Target-typed inference picks `List<Object>`, breaking the `rowsMethod(keys, dfe)` call. Typing one lambda parameter forces typing both per Java syntax.

4. **DataLoader name construction via `getTenantId + "/" + path`, not `getDataLoaderName`.** Substantive architectural change landed in `0c27eb8` after reviewer pushback. The pre-existing `GraphitronContext.getDataLoaderName(env)` contract is unsafe: the implementer controls the whole name string, so they must encode aliases, list-index stripping, and tenant scoping correctly. The `DefaultGraphitronContext.getDataLoaderName` (legacy) emitted `{FieldName}For{TypeName}`, getting all three wrong — two tenants issuing the same query would silently share a DataLoader cache, and aliased field uses (`heroes: actors` vs `villains: actors`) would collide. Fix splits the name: Graphitron-controlled path via `env.getExecutionStepInfo().getPath().getKeysOnly()` (named segments only; list indices stripped) + user-controlled prefix via the new `GraphitronContext.getTenantId(env)` default method. Legacy `getDataLoaderName` is Javadoc'd as legacy-only; rewrite paths (`buildSplitQueryDataFetcher` + `buildServiceDataFetcher`) now use the new `buildDataLoaderName()` helper. This is a candidate design-principle entry: "user-provided DataLoader name strings let users encode the wrong scope — Graphitron owns path + alias handling, users control only the tenant prefix."
