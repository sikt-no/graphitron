...
### Platform-id as synthesized NodeId **[Approved]** — [legacy-platform-id.md](legacy-platform-id.md)
...
- **Replace body-substring assertions on generated `CodeBlock`s** **[Unplanned]** — CLAUDE.md bans `assertThat(code.toString()).contains(...)`-style tests (they're implementation tests that break on every refactor), but 28 violations live across 6 test files (`TypeFetcherGeneratorTest`, `TypeClassGeneratorTest`, `TablePipelineTest`, `FetcherPipelineTest`, `TableFieldPipelineTest`, `LookupTableFieldPipelineTest`). Pattern tends to spread because no better primitive exists in the repo today. Scope: (1) audit each assertion and classify as "structural fact retrievable from `TypeSpec`/`MethodSpec` API" vs "body-content — compile/execution-tier only"; (2) for the former, introduce a small AssertJ extension (or helpers) that walks JavaPoet's object graph — e.g. `assertThat(method).hasParameter("env", DataFetchingEnvironment.class)`, `assertThat(typeSpec).hasMethod("actorsInputRows")`; (3) delete the body-content assertions entirely and lean on `graphitron-rewrite-test-spec` compile + execution for that coverage. Not a blocker for any active plan but compounds faster the longer it's deferred.
