...
- **#1 (prod: 68) — Lift `@asConnection` rejection on `@splitQuery` fields** **[Ready]** — emit `ROW_NUMBER() OVER (PARTITION BY fk)` envelope to support per-parent Relay pagination inside DataLoader batches; scope: `SplitTableField` and `SplitLookupTableField` ([plan-split-query-connection.md](plan-split-query-connection.md)).
