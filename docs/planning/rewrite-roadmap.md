...
### `DSLContext` params on `@service` methods **[Pending Review]** — [plan-service-dsl-context-param.md](plan-service-dsl-context-param.md)
...
New `@facet` directive for filter-input fields. `MakeConnections` synthesizes a `facets: XFacets` field on the Connection type plus reusable `*FacetValue` types; `FieldWrapper.Connection` carries a `FacetSpec` list; `TypeFetcherGenerator.buildQueryConnectionFetcher` emits one `UNION ALL` aggregate per Connection request — one arm per selected facet, each with its own filter-minus-self `WHERE`. Phase 1 spike (complete; see [spike-faceted-search-sql.md](spike-faceted-search-sql.md)) benchmarked five SQL shapes against 200× Sakila and redirected v1 from `GROUPING SETS` to `UNION ALL`, resolving the NULL-bucket and count-desc ordering Open Questions along the way. Covers Jira GG-335; resolves SOPP-141 (closed in favour of this).
...
### Implicit `@reference` path inference **[In Progress]** — [plan-implicit-reference-inference.md](plan-implicit-reference-inference.md)
...
Five legacy-parity gaps surfaced by running the rewrite validator against a real `sis-graphql-spec` schema. Each corresponds to a feature the legacy generator supports; the rewrite rejects at classify/validate time. User-flagged as important.
...
- **`DSLContext` on `@condition` / `@tableMethod` methods** **[Unplanned]** — sibling of the shipped `@service` classifier fix. `ServiceCatalog.reflectTableMethod` still rejects `org.jooq.DSLContext` parameters with the existing "not a Table<?>, not a GraphQL argument, and not a context key" message. Lifting the gate requires `ArgCallEmitter.buildCallArgs` (condition / `@tableMethod` call-arg emission) to walk `params()` rather than `callParams()` so it can inject the framework-supplied DSLContext at its declaration-index slot; today the emitter iterates `callParams()` only and would drop the structural param, producing positionally-wrong calls. Deferred from the 2026-04-21 `@service`-only landing because the `@service` emitters are still stubs (the new `DslContext` variant is invisible to them by construction), whereas `@condition` emission is live and would mis-emit immediately. File when a real schema needs it; the `@service` classifier fix already covers the ten errors surfaced by the 2026-04-20 audit.
- **`Set<T>` parent-keys on `@service` methods** **[Unplanned]** — the tenth error from the 2026-04-20 validator audit: `navnAlleSprak` declares `Set<StudieprogramRecord>` as its `parentKeys` / `sources` parameter, but `ServiceCatalog.classifySourcesType` only accepts `List<T>`. Batch-key design question with two paths: (a) narrow the rule — require `List<T>` on the service signature (predictable batching order, the current direction for every other `BatchKey` consumer), with migration guidance for the one known offender; (b) broaden `classifySourcesType` + `BatchKey` to accept `Set` (loses element ordering, which `BatchKey` currently relies on for result alignment). Pick the rule before writing the plan — the implementation is small either way, but the choice is schema-author-visible. One known site today.
...
- `0b2e4e9` + `49d7879` — Nesting-field emission (first arm of #8). `ChildField.NestingField` out of `NOT_IMPLEMENTED_REASONS`: classifier recurses against the outer parent's table (`FieldBuilder.classifyObjectReturnChildField`), `TypeClassGenerator.emitSelectionSwitch` projects nested columns through a recursive switch with depth-suffixed loop vars, and `GraphitronWiringClassGenerator` emits one `TypeRuntimeWiring` per nested type (fed by `GraphQLRewriteGenerator.collectNestedTypes`). Validator enforces list rejection, nested-depth whitelist, stubbed-leaf walk, and multi-parent shape compatibility with recursion into nested NestingField entries. Multi-parent sharing relies on jOOQ's name-based `Record.get(Field)` fallback. 8 execution tests on `Film.summary` / `Film.info.meta` / `Film.inlineBundle` fixtures.
