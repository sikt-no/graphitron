...
| Argument-resolution unification, Phase 4 | Spec | [plan](argument-resolution.md) |
...
- **Rewrite emitter + classifier hygiene sweep** **[Spec]** — three independent cleanups surfaced during `plan-split-query-connection.md` §1+§2 implementation: (1) deduplicate `SplitRowsMethodEmitter.buildListMethod/buildSingleMethod/buildConnectionMethod` by extracting the shared VALUES + FK-chain prelude; (2) audit every helper emitter for the "aliased Table is a parameter, never a local declaration" convention and document it; (3) introduce a machine-readable rejection-category enum so "invalid schema" / "author error" / "deferred feature" / "internal invariant" stop collapsing into free-form message strings ([plan-rewrite-hygiene-sweep.md](plan-rewrite-hygiene-sweep.md)).
