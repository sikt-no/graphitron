...
**Null-source-with-table-target is an existing edge case.** When the parent is a non-`JooqTableRecordType` result (`PojoResultType`, `JavaRecordType`, `JooqRecordType`), `parentSqlTableName` is `null` (line 1587-1588). If the child still has a table-bound return, we pass `(null, targetTable)` into `parsePath`; the new inference guard requires both endpoints non-null, so it skips and the path stays empty. The caller's existing `deriveBatchKeyForResultType`-returns-null → `UnclassifiedField` arm keeps handling this case with the current error wording ("…requires a FK join path and a typed backing class for batch key extraction"). The inference work does not change that path; see §4 for why the compound guard in `deriveBatchKeyForResultType` is preserved.
...
The CARDINALITY and CONDITION_JOIN branches stay. Post-change, `path.get(0)` in `buildListMethod` (line 343) is guaranteed to be a non-null `FkJoin` by the classifier contract: empty paths never reach `buildListMethod` — either `parsePath`'s inference branch rejects them (zero/multi FK → `UnclassifiedField`) or the caller's `deriveBatchKeyForResultType` null-return produces an `UnclassifiedField` before a `RecordTableField`/etc. is constructed — and `CONDITION_JOIN`-first paths are rejected upstream via `JoinPathEmitter.hasConditionJoin` in the same `unsupportedReason` method. Add a one-line invariant comment where `path.get(0)` is dereferenced so a future refactor that introduces a new `JoinStep` subtype doesn't silently break the cast.

**Leave `FieldBuilder.deriveBatchKeyForResultType`'s compound null-return check intact (line 1656).** It might look like the empty-path arm can be narrowed to `IllegalStateException` once inference guarantees a non-empty path, but empty is still legitimately reachable: when the parent is a `PojoResultType`/`JavaRecordType`/`JooqRecordType` (non-`JooqTableRecordType`) with a table-bound child return, `parentSqlTableName` is `null` and inference is skipped. The current compound guard (`joinPath.isEmpty() || !(joinPath.get(0) instanceof FkJoin ...)`) correctly routes both that case and the condition-first explicit path (`@reference(path: [{condition: ...}, ...])`) to the caller's `UnclassifiedField` arm. Splitting the guard would either crash the build on a legitimate classifier-error input, or add dead branches for a micro-optimisation with no real payoff.
...
- Running the generator against `sis-graphql-spec` surfaces two kinds of change vs. today's build. (1) Fields with a unique source → target FK that previously hit the EMPTY_PATH stub-validator error at build time start classifying and compiling successfully (the common case — user wrote `@splitQuery` without `@reference` because they thought it should "just work"). (2) Fields with zero or multiple FKs still fail the build, but the error now surfaces at classify time with the precise "no foreign key found…" / "multiple foreign keys found…; add a @reference directive…" wording instead of the generic "requires a @reference path" emit-time stub. No schemas move from "builds today" to "errors after this plan" — the validator already rejects every empty-path case; this plan shifts when and how that rejection appears.
