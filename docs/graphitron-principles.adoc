...
Graphitron is designed to work _with_ the database, not around it. The generated code respects database constraints and assumes the database is a trustworthy partner in keeping data safe, not a dumb storage layer to be abstracted away.
