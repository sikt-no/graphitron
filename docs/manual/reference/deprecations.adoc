...
| Rewrite v1
| Remove it. The backing Java class is now inferred from the producing field's reflected type: an xref:directives/service.adoc[`@service`] method return (or parameter, for inputs), a xref:directives/table.adoc[`@table`] resolution, a xref:directives/tableMethod.adoc[`@tableMethod`] return, or a parent-accessor chain. A reachable type still carrying `@record` emits a build warning (redundant, shadowed-by-`@table`, or disagrees-with-reflection). The directive stays declared so existing schemas keep parsing.
|===
