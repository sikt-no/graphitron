...
| Phased out
| Rewrite v1
| Use `className:` instead. `name:` was a Norwegian-language alias kept for backward compatibility with legacy schemas; `className:` is the canonical key on `ExternalCodeReference` (used by xref:directives/service.adoc[`@service`], xref:directives/error.adoc[`@error`], and other directive sites that reference external Java code).
|===
...
| Removed; `OBJECT` / `INTERFACE` unaffected
| Rewrite v1
| Remove `@table` from input types. The location went through the deprecation cycle (deprecated with a per-usage build warning, then removed); the classifier now rejects any input application at build time with a migration message. An input's fields resolve against each consuming field's table, so filters and lookup arguments need no directive. For a `@mutation(typeName: INSERT)` or `UPDATE` field, the write target is derived from the return (a `@table` return, or a carrier payload's `@table`-element data field); for an encoded-`ID` / scalar return, name it with xref:directives/mutation.adoc[`@mutation(table: "…")`] on the field. For a `@mutation(typeName: DELETE)` field, name the write target with `@mutation(table: "…")` on the field.
...
* xref:directives/table.adoc[`@table`] on an `INPUT_OBJECT` is rejected (the row above records the completed deprecation cycle). The declaration keeps the location so old schemas parse to the migration message rather than a generic parse error.
