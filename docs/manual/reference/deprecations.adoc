...
[NOTE]
====
*Honest deviation from the R68 plan.* The plan called for a "target removal version" column. The rewrite's `@deprecated(reason:)` markers do not carry a structured removal version, and the project does not currently version directive surface as a separate cadence. Until the rewrite ships a versioned directive contract, "Status" carries the practical signal (discouraged, transitional, removed) and "Migration" names the replacement.
====
...
| xref:directives/index.adoc[`@index`]
...
* xref:directives/index.adoc[`@index`] and xref:directives/asConnection.adoc[`@asConnection`] carry their own deprecation prose at the canonical site.
