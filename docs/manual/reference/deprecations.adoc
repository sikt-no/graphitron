...
| Rewrite v1
| Remove `@table` from input types. The mutation's write/read target is derived from the consuming field's resolved target table, so the directive on the input is redundant. For a `@mutation(typeName: INSERT)` field, the write target is derived from the return (a `@table` return, or a carrier payload's `@table`-element data field); for the encoded-`ID` / scalar-return `INSERT`, name it with xref:directives/mutation.adoc[`@mutation(table: "…")`] on the field. For a `@mutation(typeName: DELETE)` field, name the write target with `@mutation(table: "…")` on the field instead. `UPDATE` and `UPSERT` still read the write target from the input's `@table` for now. A build warning fires per `@table`-on-input usage (INSERT-consumed and DELETE-consumed inputs no longer carved out).
...
| Rewrite v1
| Remove it. The backing Java class is now inferred from the producing field's reflected type: an xref:directives/service.adoc[`@service`] method return (or parameter, for inputs), a xref:directives/table.adoc[`@table`] resolution, a `@tableMethod` return, or a parent-accessor chain. A reachable type still carrying `@record` emits a build warning (redundant, shadowed-by-`@table`, or disagrees-with-reflection). The directive stays declared so existing schemas keep parsing.
|===
