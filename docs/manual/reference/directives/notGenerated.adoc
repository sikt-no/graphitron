...
| `field: T @notGenerated` on a class-backed parent | xref:sourceRow.adoc[`@sourceRow`] or a typed Java accessor | A class-backed parent (Java record / POJO) must expose the data either through a typed Java-record accessor or through an explicit lifter.
...
| `interface T @notGenerated` / `union U @notGenerated` | (no replacement needed) | The legacy "skip interface/union wiring" axis was always tied to `RuntimeWiring`-side type resolvers, the rewrite always generates these from xref:discriminate.adoc[`@discriminate`] / xref:discriminator.adoc[`@discriminator`]. Remove the directive.
|===
...
* xref:sourceRow.adoc[`@sourceRow`] is the replacement when a class-backed parent needs a custom path to a child `@table`'s batch key.
