...
| `field: T @notGenerated` on a `@record` parent | xref:sourceRow.adoc[`@sourceRow`] or a typed Java accessor | The `@record` parent must expose the data either through a typed Java-record accessor or through an explicit lifter.
...
* xref:sourceRow.adoc[`@sourceRow`] is the replacement when a `@record` parent needs a custom path to a child `@table`'s batch key.
