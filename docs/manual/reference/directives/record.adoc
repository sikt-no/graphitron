...
* *Output / result types* take their backing from the producing field's *return* type: an xref:service.adoc[`@service`] method return, a xref:table.adoc[`@table`] resolution, or a parent-accessor chain that threads the class down to child types.
* *Input types* take their backing from the Java *parameter* type the input flows into (the xref:service.adoc[`@service`] / xref:condition.adoc[`@condition`] method parameter). An input with no such parameter carries no backing class; its fields resolve against each consuming field's table.
...
* *Shadowed by `@table`*: the `OBJECT` type also carries xref:table.adoc[`@table`], which supplies the backing class; `@record` is ignored. (This case only arises on object types. `@table` on an input is itself a deprecated, ignored location, so it supplies no backing class and shadows nothing; an input carrying both falls through to the redundant / disagrees-with-reflection cases on its producer's reflected class alone.)
