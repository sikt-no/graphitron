...
* In an `argMapping` path, a `@nodeId` leaf must be opened with one of its node type's key columns and must name `typeName:` explicitly; opening a list of node ids is reported as deferred, that shape naming the list of a key column across the decoded ids and parameter binding not emitting it yet. Binding such a leaf without opening it sends the encoded id to the database verbatim and is a build error. See xref:routine.adoc#node-id-key-projection[Binding a parameter to a node id's key column].
