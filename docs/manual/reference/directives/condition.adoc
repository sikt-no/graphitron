...
FK-target `@nodeId` filter. When an input field carries both `@nodeId(typeName: "X")` and `@condition`, the field decodes an opaque id for a *foreign-key-linked* table `X` (reached from the input's own table through a catalog foreign key). The reflected method's table parameter is then `X`, not the input's own table:

[source,graphql]
----
input SoknadsmangeltypeFilterInput @table(name: "soknadsmangeltype") {
...
* An input field carrying both `@nodeId(typeName: "X")` and `@condition` binds the condition method's table parameter to the FK-*target* table `X` (the table the decoded id identifies), reached through the catalog foreign key from the input's own table. The predicate is emitted as a correlated `EXISTS` over that foreign key, so the method receives an alias for `X` rather than the input's own table. Single-column and composite-key NodeType targets are both supported (the composite correlation ANDs every foreign-key column pair). This holds at every emission site: top-level query conditions, inline child-field fetchers, and `@splitQuery` batched fetchers.
* Nested input types must be carrier-mapped to flow through `@condition`. A `@table` input maps to a jOOQ record (only at the leaf, jOOQ records can not contain other records); a class-backed input maps to its Java record (and any input above it must also be a Java record). An input is class-backed when the Java parameter it flows into is that record type; leaving it without a carrier routes through the `PojoInputType` path, which still works for plain scalar paths.
...
* An input type's backing class determines the carrier the condition method reads from: xref:table.adoc[`@table`] supplies a jOOQ record, otherwise the carrier is the Java record the method parameter is typed as (inferred, not declared).
