...
A query selecting any facet field issues exactly one extra SQL statement per request: a `UNION ALL` of per-facet `GROUP BY` arms, one arm per *selected* facet, each under its filter-minus-self predicate. When no facet field is selected, the aggregate is skipped entirely. Results are ordered facet, count descending, value ascending.
