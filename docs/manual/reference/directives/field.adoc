...
Names the underlying-binding target for a GraphQL slot. The axis is determined by the directive site: column on a field or input field, column on a filter argument, database string (or Java enum constant) on an enum value, and, on an xref:error.adoc#_extra_fields[`@error` extra field], the source-class accessor matched against each handler's exception (or `GraphQLError`) class.
