...
* When auto-discovery cannot derive a participant's join, xref:referenceFor.adoc[`@referenceFor`] is the successor to `@multitableReference`'s per-route paths: one application per participant, stating that participant's complete path (multi-FK disambiguation, same-table self-FK, condition, and multi-hop routes). Unlike the retired `routes:` input-wrapper list, `@referenceFor` states one fact at its natural grain (`type:` + `path:`).
