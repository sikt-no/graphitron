...
A `@routine` puts a function call in the FROM clause. `WHERE` and `ORDER BY` are different clauses of the same statement, so a routine-backed field filters and sorts like any other field: `@condition`, filter arguments, `@defaultOrder` and `@orderBy` all compose, at root and at child positions alike, and they resolve against the *terminus* of the field's table chain (the last node, which is the table the field's element type is bound to).

=== What the call spends, and what filters its result

An `argMapping` entry spends the input it names, and only that input. A leaf it binds feeds the routine parameter and never reaches the WHERE clause; every other input field of the same argument filters the result exactly as it would on a table-backed field, binding through `@field(name:)` or, absent that, by a case-insensitive match on its own name. So one input object can carry both halves, which is usually how an author wants to spell it:

[source,graphql]
----
input ActorFilmFilter {
    actorRef:  ID! @nodeId(typeName: "Actor")   # spent: projects actor_id into pActorId
    minLength: Int!                             # spent: feeds pMinLength
    title:     String                           # filters the result: WHERE title = ?
}
...
type Query {
    actorFilms(filter: ActorFilmFilter!): [ActorFilm!]
        @routine(
            ...
            argMapping: "pActorId: filter.actorRef.actor_id, pMinLength: filter.minLength"
        )
        @defaultOrder(fields: [{name: "film_id"}])
}
----

The two spent leaves above are spent in two different ways: `actorRef` through the key projection, which hands `pActorId` a decoded `actor_id`, and `minLength` as a plain scalar read. `title` is left over, and what it resolves against is the chain terminus, here the function result itself.

Two consequences follow, and both are build errors rather than silence:

* *A surviving leaf that binds nothing fails the build*, with the message it already gets on a table-backed field: it names no column of the terminus and carries no `@condition`. Nothing about being inside a routine argument exempts it.
* *A spent leaf carrying `@field` or `@condition` is a directive conflict.* It asks for a predicate it will never get, the routine parameter having consumed it first. `@nodeId` on a spent leaf is not a conflict: there the directive is a decode instruction the key projection carries out, not a filter.
...
Where the node type's key is a single column you may leave the leaf closed:
`pOrganisasjonskode: input.organisasjonId` names that column, there being no other it could name, and
the parameter receives it exactly as the spelled-out form does. That holds wherever the `@nodeId`
sits, an argument of the field (`pOrganisasjonskode: organisasjonId`) as much as an input field below
one. Naming the column is then a matter of saying out loud what the build would infer, which is worth
doing where the reader benefits.

Three things are build errors rather than surprises at runtime:

* *Binding a `@nodeId` whose key is more than one column without saying which.* One binding carries
  one value, and nothing in `pOrganisasjonskode: input.organisasjonId` says which of the two it is.
  The build stops, states the count, and lists the columns you could have named. It says the same
  where the node type resolves no key columns at all, the remedy there being
  xref:node.adoc[`@node(keyColumns:)`] on that type.
* *`@nodeId` without `typeName:` at this position.* A bare `@nodeId` infers its target from the
  ...
  from. Name the type.
* *A key column the parameter cannot take.* Projecting an `Integer` column into a `String` parameter
  is a build error naming the column, its type and the parameter's, rather than a compile error in
  generated code you did not write. Bind a parameter of the column's own type, or project a column the
  parameter can take. The check needs both types: where the parameter's type cannot be resolved, a
  routine whose call surface was not captured, the build lets it through and your compiler is the
  backstop.

An omitted node id is not one of them. A `@nodeId` the client leaves out, or sends as `null`, has
nothing to decode, so the projection is `null` and the parameter receives `null`. That holds wherever
the absence sits on the path: a nullable `@nodeId` leaf, and a non-null one inside an input object the
client omitted, project the same `null`. Express an optional filter with a function that tolerates a
NULL parameter, since what an absent value means is the function's to define and not the generator's;
a function whose parameter is not NULL-tolerant fails inside the database, which is that function's
own declared contract. Only absence is affected: a malformed node id, and one encoded for another node
type, still fail the request as client errors before the routine runs.
...
=== Every input the field advertises must feed a parameter

A `Mutation` `@routine` field resolves no filter surface at all. The call is the write, and the post-commit re-read below is keyed by the captured columns rather than filtered, so nothing on a write seat reads input except the routine's own parameter list. An input the `argMapping` does not bind therefore reaches nothing, and that is a build error naming the input, leaf by leaf and argument by argument: a field of an input object the bindings never name, and an argument slot the bindings never name, fail alike. It is the same verdict a DML mutation's unbound input field already gets, and the remedy is the same: bind it, or take it off the schema.

There is no spelling yet for an input deliberately left unread, the Relay `clientMutationId` pattern or an idempotency key among them. Neither mutation kind has one, and nothing echoes such a value back into a payload.
