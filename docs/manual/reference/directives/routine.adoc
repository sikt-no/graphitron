...
* *A key column the parameter cannot take.* Projecting an `Integer` column into a `String` parameter
  ...
  routine whose call surface was not captured or a parameter declared `int` rather than `Integer`,
  the build lets it through and your compiler is the backstop.
