...
Backs a field with a database *routine* rather than a catalog table. Supported today is the table-valued function (PostgreSQL `RETURNS TABLE` / `SETOF`): jOOQ generates such a function as a first-class catalog `Table<R>`, so the field's `@table`-bound element type resolves the routine-result columns exactly as a plain table type does, and the rewrite projects `Type.$project(...)` against it with the same selection narrowing. The difference from a plain catalog read is the row source: instead of the bare `Tables.X` singleton, the rewrite calls the schema's generated `Routines` convenience method with the routine's IN parameters bound from GraphQL field arguments (`argMapping`) or from columns of the previous table in the chain (`columnMapping`).
...
[[read-surface]]
== The read surface: filtering and ordering

A `@routine` puts a function call in the FROM clause. `WHERE` and `ORDER BY` are different clauses of the same statement, so a routine-backed field filters and sorts like any other field: `@condition`, filter arguments, `@defaultOrder` and `@orderBy` all compose, at root and at child positions alike, and they resolve against the *terminus* of the field's table chain (the last node, which is the `@table` type the field projects). The field's remaining arguments are unaffected: an argument bound to a routine IN parameter is spent on the call and is never read as a filter.
...
* With `@reference` on the field, the return type must be `@table`-bound, and the chain's last node must be that table. A non-`@table`, non-carrier return is rejected with `@routine requires a @table-annotated return type`; a routine-terminus mismatch is rejected with `@routine could not be resolved — the field's @table type ... does not match the routine's result table ...`. Without `@reference` on a Mutation field, the return may instead be a payload carrier (exactly one nullable `@table`-element data field plus an optional errors-shaped field); the carrier + `@reference` combination is rejected as a directive conflict, and `@reference` on the carrier's data field is deferred (see <<write,Writes on Mutation>>).
...
* Connection pagination does not compose with routine-terminus chains (`@asConnection` there is a build error: keyset pagination needs an ordering contract the routine result does not carry; use `[T]` or `T`). Over a catalog-terminus chain that merely contains a routine node it is reported as deferred, as is `@lookupKey` composition. `@orderBy` and `@condition` compose normally on a read (see <<read-surface,The read surface>>); on a `Mutation` field they are reported as deferred, the write's result shape having no filter or order surface to compose onto.
