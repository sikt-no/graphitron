...
`ReferenceElement` fields are identical to xref:reference.adoc[`@reference`]'s: `table` (jOOQ table identifier for the hop's destination), `key` (foreign-key constraint name, optionally schema-qualified as `schema.constraint`; see xref:reference.adoc#schema-qualified-keys[Schema-qualified keys]), `condition` (external join condition).
