...
The directive is repeatable, one application per participant. Applications are keyed by `type` and are independent; each `path` is that participant's complete path. This is the opposite of repeated xref:reference.adoc[`@reference`] applications, which concatenate into one running chain. Declaring the same `type` twice is a build error. Participants you do not name keep automatic discovery (override-merge): `@referenceFor` on the ambiguous participant is enough, its siblings still auto-discover.
...
| `type` | `String!` | (required) | Name of the participant type this path applies to. Case sensitive. Must be a table-bound participant of the consuming field's return type.
| `path` | `[ReferenceElement!]!` | (required) | Join path in the direction the generated join runs: from the parent type's table to this participant's table on an output field, from this participant's table to the `@nodeId` target's table on a decode leaf. Same element grammar as `@reference`: `{table:}`, `{key:}`, `{condition:}`.
|===
...
Where no generated route fits any participant, xref:condition.adoc[`@condition(override: true)`] on the leaf hands the whole `WHERE` predicate to your method. See xref:../../how-to/global-id.adoc#multitable-filter-inputs[Multitable filter inputs] for that ladder.
...
| Placement | Field / argument / input field | Multi-table interface/union child field, and `@nodeId` input fields / arguments of a multi-table consumer
|===
