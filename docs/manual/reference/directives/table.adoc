...
* For multi-schema catalogs, an unqualified `name:` must be unique across schemas. When the same bare name exists in two or more schemas (`event` in `public` and `archive`, say), the lookup returns empty and the build fails with the standard `table 'event' could not be resolved in the jOOQ catalog` rejection. Disambiguate by qualifying explicitly: `@table(name: "public.event")`. Single-schema consumers see no change; only collision sites are forced to qualify.
