...
An extra field carrying xref:nodeId.adoc[`@nodeId`]`(typeName:)` publishes a global object ID built from what the read yielded, so the exception carries the key and the client sees the ID:
...
* An extra field carrying xref:nodeId.adoc[`@nodeId`] needs `typeName:` and a node type keyed on a single column; see xref:#nodeid-extra-field[Publishing a node id from an extra field].
