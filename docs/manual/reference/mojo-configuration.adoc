...
:description: Every parameter of every goal on Graphitron's Maven plugin (graphitron-maven-plugin). Covers the three goals (generate, validate, dev), the eleven shared <configuration> parameters, the dev-only watch parameters, the <schemaInput> POM binding, and the CLI -D properties.
:!toc:
