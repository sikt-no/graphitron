...
:description: Every parameter of every goal on Graphitron's Maven plugin (graphitron-maven-plugin). Covers the four goals (generate, validate, capture, dev), the ten shared <configuration> parameters, the eight dev-only parameters, the <schemaInput> POM binding, and the CLI -D properties.
:!toc:

`graphitron-maven-plugin` exposes four Maven goals: `generate` (the build-time codegen pipeline), `validate` (a faster schema-only run that emits no Java), `capture` (fills the fact store and stops), and `dev` (the editor-loop server). Each goal accepts the same handful of `<configuration>` parameters; `dev` adds eight of its own. One of the parameters (`<schemaInputs>`) is a list-of-complex-objects with its own POM binding.
...
All four goals read and write a fact store, which is what lets a run start from the previous run's
rows instead of re-reading every jar on your classpath. Where it lives depends on which kind of run
it is, and on the three build goals it is not a choice you make.

`generate`, `validate` and `capture` put it under the module's build directory, at
`target/graphitron-model/`, with everything else a run produces. One store per module holding that
module's one graph, owned outright by the build that wrote it and removed by `mvn clean`. There is
nothing to configure and nowhere else to want it: a hermetic CI job already gets a store inside the
workspace, and a container that discards `$HOME` never goes near one.

`dev` puts it in your per-user cache instead, because a session's store is shared. Its language
server and its query tools answer from the store between builds, so it has to survive a `mvn clean`
and be reachable from a module other than the one being built. That is the one case worth a
parameter, and xref:#dev-goal-parameters[`<storeDirectory>`] on the `dev` goal is it.

The store never holds anything that cannot be rebuilt from your sources, so deleting it is always
safe.
...
Eight parameters unique to the `dev` goal: five watch-loop knobs, the store home, and two POM blocks
configuring tools only a session runs. The knobs and the store home take a CLI property; the two
blocks are overridden by environment variables instead, named in their own rows.
...
| `String`
| `observed`
| `graphitron.dev.rediscover`
| How a round decides what to re-read. `observed`, the default, lets the session skip re-reading an input it has watched continuously since the store last read it, so a save costs work proportional to the edit rather than to the workspace. `always` turns that off: nothing is trusted and every round rediscovers what changed by reading bytes, exactly as the loop behaved before the mechanism existed. Reach for `always` when the session appears not to notice edits, which happens where a filesystem watcher under-reports: bind mounts, some network shares, and container-mounted working trees. It costs the saving and nothing else, an unobserved round being a correct round, so it is also the one-line bisect for a bug report.

| `storeDirectory`
| `File` (path)
| a per-user cache directory
| `graphitron.store.directory`
| Where this session's fact store is kept, so the session and the readers it serves start from the
  previous round's rows instead of re-reading every jar on the classpath. The default is your platform's cache location
...
`disabledRules` silences a rule everywhere it would fire: the build log, your editor's squiggles, and the MCP diagnostics tool. `excludedTypes` skips the matching types for every rule that has a type name to match against (glob syntax: `*` matches any run of characters, `?` one character), which is the schema linter's own checks and the rules derived from your database's keys, such as xref:directives/reference.adoc#duplicate-rows-through-a-junction[`reference-path-fans-out`]. A handful of advisories come from graphitron's schema classifier rather than the linter (for example `redundant-record-directive`); these are not tied to a single type name, so they are silenced by rule id in `disabledRules`, not by `excludedTypes`.
...
| Type-name globs whose matching types are skipped by every rule that reports at a type or field coordinate. A classifier advisory carries no such coordinate, so one on an excluded type still fires and must be silenced through `disabledRules`.
|===
...
In a build that configures `<tenantColumn>`, your `mount` can also declare a parameter typed `Optional` of the tenant column's Java type, anywhere in its signature, and graphitron hands it the tenant the connection is being mounted for: the routed tenant's key, or `Optional.empty()` for the default source that serves global tables. Like the seam, it is recognised by type, so it is neither payload nor a factory parameter. Two such parameters, or any other `Optional` parameter on a multi-tenant mount, fail the build; a plain parameter of the tenant type stays ordinary payload. Graphitron takes one connection and runs one mount per tenant key, even when several keys share a `DataSource`, so the identity you mount is per connection and tenant. `unmount` does not receive the tenant; put it in the handle your `mount` returns if you need it there. Which tenants a request may touch is a separate, request-level set, checked before any connection is taken; see xref:../how-to/tenant-scoping.adoc[How-to: Tenant scoping].
...
The same test also pins which heading each row sits under against the goals that declare the parameter, because presence in the wrong table is still wrong: this page says the shared parameters apply to every goal, so a dev-only parameter listed among them tells you to configure something three goals ignore. Moving a parameter between Mojos therefore forces its row to move too, and a parameter table added under a heading the test does not know the goal scope of fails rather than going unchecked.
