...
The `validate` and `capture` goals exist so consumers can run `mvn graphitron:validate` or `mvn graphitron:capture` from the command line without editing their POM's `<execution>` block to thread `<outputPackage>` / `<jooqPackage>`. The mojo substitutes an inert sentinel (`validation.unused`) for both packages when they are not configured, so the classifier stage still type-checks against `RewriteContext`'s non-null contract. The sentinel never reaches generated code; neither pipeline emits anything.
