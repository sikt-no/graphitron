...
`buildSchema` returns a fully wired `GraphQLSchema`: every type, field, fetcher, and directive Graphitron understands is attached before the schema is built. Custom scalars are wired automatically from the `@scalarType(scalar:)` directive on each `scalar` declaration (see xref:directives/scalarType.adoc[`@scalarType` directive]); the `customizer` lambda receives the underlying `GraphQLSchema.Builder` so consumer code can layer in additional non-scalar types, hand-rolled type resolvers, or custom directives before the build.
...
The factory IS the registration point; the parameter list is schema-driven (`DSLContext` first, then one parameter per `contextArgument` named in `@service` / `@condition` directives, alphabetically):
...
Reads the per-request `DSLContext` from `env.getGraphQlContext().get(DSLContext.class)`, populated by the factory's `defaultDsl` parameter. The default impl is the only impl; this is not an override point. On the escape-hatch path, tenant-specific routing means building the right `DSLContext` at request entry and passing it to `Graphitron.newExecutionInput(dsl, ...)`; for generator-owned database-per-tenant routing (divined bindings, fan-out), see xref:../how-to/tenant-scoping.adoc[How-to: Tenant scoping].
...
`DefaultValidatorHolder` is a holder-class idiom inside the same emitted file; its `INSTANCE` is initialised on the first call to `getValidator` (`Validation.buildDefaultValidatorFactory().getValidator()`) and shared by every subsequent default-using request. The method is no longer an override point on the sealed interface, and there is no custom-validator-factory configuration today.

== `NodeIdEncoder` utility

Emitted by `NodeIdEncoderClassGenerator` into `<outputPackage>.util`, unconditionally: the class ships on every run, and a schema with no `@node` type simply gets no per-type helpers on it. Static, non-extendable encode/decode helpers for Relay global IDs:
...
public final class NodeIdEncoder {
    ...
    public static String[] decodeValues(String expectedTypeId, String base64Id);

    public static void requireColumnAgreement(
        String conflictLabel, DataType<?> type, Object a, Object b);
...
Two more helpers are public because generated code in other packages calls them, and they are documented here because a consumer reading generated sources will meet them:

* `decodeValues(expectedTypeId, base64Id)` -- the raw unpack behind every per-Node `decode<TypeName>`: the key values as a `String[]`, or `null` when the input is malformed or carries another typeId. Use it only where you genuinely have no typed target; the per-Node helper gives you the same values already coerced to the column types.
* `requireColumnAgreement(conflictLabel, type, a, b)` -- the value-agreement check the write path emits when two input fields land on one column. It coerces both values through the destination column's `DataType` and throws a `GraphqlErrorException` naming `conflictLabel` when they disagree.
...
* xref:../how-to/custom-scalars.adoc[How-to: Custom scalars] covers declaring a `scalar` with `@scalarType(scalar:)` and authoring the `GraphQLScalarType` constant the directive points at.
* xref:../how-to/tenant-scoping.adoc[How-to: Tenant scoping] covers database-per-tenant wiring: the tenant column, per-tenant DataSources, and fanning a field out across tenants with `@tenantFanOut`.
