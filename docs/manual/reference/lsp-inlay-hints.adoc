...
| `false`
| Renders a compact classification label at every field declaration and every object / interface / input / union / scalar / enum type declaration. Labels include `"column"`, `"table field"`, `"query node field"`, `"insert mutation"`, `"node type"`, and the rest of the closed vocabulary.
