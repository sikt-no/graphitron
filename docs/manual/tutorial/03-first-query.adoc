...
With the server from xref:01-prerequisites.adoc[page 1] still running on `http://localhost:8080`, open a second terminal and send a query. (If you prefer the in-browser GraphiQL playground, open `http://localhost:8080/graphql` in a browser and paste the query body into the left pane instead of running curl.)
...
|===
| xref:02-first-schema.adoc[← Previous: The starter schema]
| xref:index.adoc[↑ Tutorial overview]
| xref:04-joining-tables.adoc[Next: Joining tables →]
|===
