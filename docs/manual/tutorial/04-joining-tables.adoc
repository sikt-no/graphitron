...
Mary, Patricia and Barbara belong to store 1 (address `47 MySakila Drive`); Linda and Elizabeth belong to store 2 (address `28 MySQL Boulevard`).
...
|===
| xref:03-first-query.adoc[← Previous: Running your first query]
| xref:index.adoc[↑ Tutorial overview]
| xref:05-mutations.adoc[Next: A first mutation →]
|===
