...
* *JDK 25.* The generator runs on Java 25 (`pom.xml` sets `<release>25</release>` and the parent's `requireJavaVersion` enforcer fails the build on anything older). Run `java -version` to confirm.
...
The example you build against lives at `graphitron-sakila-example/`. Everything in this tutorial happens from the repository root unless stated otherwise.

== Start PostgreSQL with the example data

The example reads from a database named `rewrite_test` on `localhost:5432` with `postgres` / `postgres` credentials. The DDL and seed data live in `graphitron-sakila-db/src/main/resources/init.sql`; load that file into a fresh database and you are done.
...
docker run -d --name graphitron-pg \
  ...
  -p 5432:5432 \
  -v "$PWD/graphitron-sakila-db/src/main/resources/init.sql:/docker-entrypoint-initdb.d/init.sql:ro" \
  postgres:16
----
...
psql -d rewrite_test -f graphitron-sakila-db/src/main/resources/init.sql
----
...
The example serves the GraphiQL playground: an in-browser editor with auto-complete, query history, and a one-click run button against the same `/graphql` endpoint. Open `http://localhost:8080/graphql` in a browser (any request with `Accept: text/html`, which is what a browser sends) and you get the playground; curl and other POST traffic to the same URL route to the engine.
