...
Two directive families do the work here:
...
The `@reference` directive on `address` is a foreign-key join. You will use it on xref:04-joining-tables.adoc[page 4]; ignore it for now.

`@nodeId` and `@node` are the Relay Global Object Identification machinery. They are out of scope for this tutorial; the xref:../how-to/global-node-identification.adoc[corresponding how-to] covers them.
