...
The example's GraphQL schema lives at `graphitron-sakila-example/src/main/resources/graphql/schema.graphqls`. Open that file in an editor; this page walks you through three types and the directives that wire them to PostgreSQL.
...
If you are curious what Graphitron produced for these types, the generated sources live under `graphitron-sakila-example/target/generated-sources/graphitron/`. The interesting files for `Customer` are `CustomerType.java` (the GraphQL type wrapper) and `QueryCustomersFetcher.java` (the resolver for `Query.customers`). You do not need to read them to follow the rest of the tutorial; they are there when you want to see what the directives translated into.
