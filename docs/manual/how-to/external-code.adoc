...
The slot names are historical (each directive was named before the shape was unified). Mechanically all six slots accept the same `ExternalCodeReference` input.
...
* xref:source-row.adoc[Source-row directive] (`@sourceRow`) uses a static method that supplies the per-parent batch key for a `@record`-typed parent. Sibling pattern with flat `(className, method)` args instead of the `ExternalCodeReference` wrapper documented above.
