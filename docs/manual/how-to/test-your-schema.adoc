...
* *`Graphitron.newExecutionInput(...)` is the per-request wiring point.* Its parameter list is schema-driven: a `DSLContext` first, then one typed parameter per `contextArgument` declared in your `@service` / `@condition` / `@tableMethod` directives. The example wires the same `dsl` for every test; on this escape-hatch path a per-tenant `DSLContext` is built at request entry and passed as `defaultDsl`. For generator-owned database-per-tenant routing, see xref:tenant-scoping.adoc[How-to: Tenant scoping].
