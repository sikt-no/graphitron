...
GraphQL input types classify into one of five carriers in the rewrite. For class-backed carriers, the backing class is reflected from the `@service` / `@condition` / `@tableMethod` method parameter the input flows into:
...
* *Plain* (`PojoInputType`): the fallback when the input is neither `@table`-bound nor reflected to a typed class. The carrier is `Map<String, Object>` at runtime.
...
* *Java-record carriers cascade upward.* If a leaf input is class-backed by a Java record, every input *above it* in the nesting chain must also be Java-record-backed. The rewrite needs a stable Java type to hold the nested record. Mixing carriers (e.g. a `@table` outer with a class-backed inner) does not compose: the outer `@table` input has no slot for the inner record.
...
The conservative default: when in doubt, use `@table` on the outermost input (so the rewrite can drive implicit predicates) and plain inputs on inner nesting layers (so the rewrite's `Map`-traversal handles the descent). Only reach for a class-backed carrier (a service method whose parameter is a typed Java class) when the input shape demands a typed Java carrier, or when listed nested inputs are involved.
...
* xref:../reference/directives/table.adoc[`@table`] (or, for class-backed inputs, the producer's reflected parameter type) determines the input carrier the condition method reads from.
