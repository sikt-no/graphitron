...
| `condition:` alone | Treated as the join `ON` predicate: the generator joins the hop's target table and applies the returned `org.jooq.Condition` as the join condition. See <<condition-form>> for how the target table is worked out, which differs by position.
| `condition:` + `key:`/`table:` | `condition:` acts as an *extra* constraint on the hop joined via the FK, not as the `ON` predicate. The predicate is emitted wherever the hop is: inside the projecting subselect for an output field, inside the filter's `EXISTS` for a filter.
| none of the three | Not a valid hop. Each element must pin its hop somehow. (An `@reference(path: [])` with no elements at all is a different case; see <<pitfalls>>.)
|===
...
The full set of input-field directives (`@field`, `@condition`, `@reference`) interoperate; see xref:condition-cascade.adoc[How-to: Stacking and overriding conditions] for the input-carrier rules and how the override cascade interacts with the per-field directives.
...
* If the hop *ends the field's reference chain* and the field's return type carries `@table`, that table is the target. The method's parameters are then free to be `Table<?>`.
* Otherwise, including every hop in a filter path and every hop that has further hops after it, the target is the class of the method's *second* parameter. It must be a concrete generated jOOQ table class; `Table<?>` resolves nothing and the build rejects it, naming the parameter and saying what it is being read for.
...
* *A `condition:` hop needs concrete parameter types wherever no return type declares its target.* That is every hop of a filter path and every non-final hop of an output field's path. A `Table<?>` second parameter resolves no target table and the build rejects it. See <<condition-form>>.
* *Prefer a declared foreign key when the relationship is one.* If a `{condition:}` hop is really a key equality the database does not declare, declare it as a jOOQ synthetic foreign key and switch to `{key:}`. The predicate form restates itself at every field and forgoes auto-discovery, `{table:}` short forms, `@nodeId` chains and completions.
