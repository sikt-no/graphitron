...
You have an existing Graphitron subgraph and you want a tight edit-save-see-it loop. The short version: run two long-lived terminals, one for `mvn graphitron:dev` (regenerates Java from your `.graphqls` and serves an LSP), one for `mvn quarkus:dev` (hot-reloads the running app). Edit the schema, save, and the change flows through both without restarting anything.
