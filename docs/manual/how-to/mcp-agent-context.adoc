...
| `code`
| Lists the consumer Java classes on the compile classpath, selected by `kind`: `service`, `condition` or `record`. One class per entry, carrying its public methods and its record components, so a class that is more than one of those is one answer rather than two. Each class and method also carries where a parsed `.java` declares it and the doc comment it was written with.
