...
[TIP]
====
This is the companion to xref:dev-loop.adoc[the dev loop]. The same `mvn graphitron:dev` process serves both the LSP (for your editor) and the MCP server (for your agent) off one warm store. If you have the dev loop running, the MCP server is already up.
====
...
| `schema`
| Describes what graphitron made of your schema, one entry per type and per `Type.field` coordinate on it. Five questions each: which classifier claims it and whether a directive or a structural reading decided that; what it binds (the catalog table, the column, the backing class and the member names it offers, the join path a `@reference` walks, the consumer method a `@service`, `@externalField` or `@condition` names, an abstract type's participants); whether a classification was `DEMANDED` of it or it is `EXEMPT`, and under which rule; which directives conflict on it, with the message to fix; and every site it is declared or extended at. A slot appears when there is something to report and is absent otherwise, so an absent slot reads as "nothing claims or binds this". Four bindings are authored but not resolved to a catalog column yet and so report nothing: a composite `@nodeId` field's key columns, an interface's `@discriminate` column, a `@pivot`'s two columns, and a participant's cross-table column.

| `code`
| Lists the consumer Java classes on the compile classpath, selected by `kind`: `service` or `condition`. Each kind is what an author may name at that directive rather than what the classpath happens to contain. One class per entry, carrying its public methods and its record components, so a class that is both is one answer rather than two. Each class and method also carries where a parsed `.java` declares it and the doc comment it was written with.

| `diagnostics`
| The current schema validation diagnostics, the same `(file, location, message)` triples the LSP delivers to your editor.

| `diagnostics.aggregate`
| Counts those same diagnostics grouped by dimensions you choose (what kind of error, which attempted name, which file or directory) instead of listing them. On a schema mid-migration this answers "what is broken, in what proportion" in one small result, and each group's key feeds back into `diagnostics` to read just that group.
...
| `store.console`
| The connection coordinates of this session's read-only SQL console onto its own fact store, for a question none of the tools above asks. Fields, not prose, plus a ready-to-run command. Always present: without a console it answers `disabled` and names the command that starts one. See xref:#store-console[Query the fact store from an agent] below.

| `execute`
| Runs a GraphQL query or mutation against your generated resolvers in-process, on the dev database you configure, and returns exactly what the query returns. Mutations always roll back. Appears only when a dev database is configured; see xref:#execute-tool[Run a query without an app server] below.
|===
...
* xref:../../architecture/how-to/dev-loop-internals.adoc#dev-loop-detail[Dev loop reference] (architecture docs): how the `dev` goal wires the LSP, MCP server, watchers, and generator dispatch into one JVM.
