...
* Declaring an ordering on a multi-table interface or union field is rejected at build time as a deferred capability: the read is one statement per participant and neither xref:../reference/directives/defaultOrder.adoc[`@defaultOrder`] nor xref:../reference/directives/orderBy.adoc[`@orderBy`] is lowered onto the branches, so the rows arrive in participant primary-key order whatever the declaration asked for. See xref:sort-results.adoc[How-to: Sort results].
