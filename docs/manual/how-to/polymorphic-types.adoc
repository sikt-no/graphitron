...
=== Filtering a multi-table field

Filter arguments on a multi-table field are lowered once per participant, each against that participant's own table, so a plain xref:../reference/directives/field.adoc[`@field(name:)`] filter needs the column to exist on every branch. To hand the predicate to your own Java method instead, the directive is xref:../reference/directives/condition.adoc[`@condition`] (the SQL-predicate directive, not the `{condition:}` element of a `@reference` path used later on this page), and the multi-table coordinate is the one place its method signature has a choice worth making deliberately: one declaration per participant table keeps each branch's generated typed columns, while a single `Table<?>` declaration serves every branch and resolves columns by name. xref:add-custom-conditions.adoc#filter-a-multitable-interface[Filter a multitable interface or union] covers both forms, the mixed set, and what each one checks at compile time.
...
* Subtype-specific fields on a single-table interface may pull from another table via xref:../reference/directives/reference.adoc[`@reference`]; the generator projects a one-row-capped correlated subquery gated by the discriminator value.
* Auto-discovery is the default multi-table-child idiom (the `AddressOccupant` shape); xref:../reference/directives/referenceFor.adoc[`@referenceFor`] is the explicit per-participant surface for the four cases it cannot serve, all supported: multi-FK disambiguation, same-table self-FK participants, condition joins, and multi-hop chains. The legacy `@multitableReference` directive is rejected; see xref:../reference/directives/multitableReference.adoc[`@multitableReference`], whose successor for per-route paths is `@referenceFor`.
...
* An ordering declared on a root `Query` field returning a list or connection of a multi-table interface or union is lowered onto every participant branch: xref:../reference/directives/defaultOrder.adoc[`@defaultOrder`] and xref:../reference/directives/orderBy.adoc[`@orderBy`] both apply, provided every participant carries each order column at the same SQL type and bound Java type. On a child field, or a single-valued root field, the read lowers no ordering and declaring one is rejected at build time as a deferred capability. See xref:sort-results.adoc[How-to: Sort results].
...
* xref:../reference/directives/reference.adoc[`@reference`] for cross-table access on subtype-specific fields and for FK-chain access to single-table polymorphic interfaces.
