...
The Edge type is named after the connection type, whether or not you override it: the override above mints `SharedMoviesConnectionEdge`, exactly as legacy did. So a client holding a fragment on the edge type keeps compiling across the upgrade even while the override is still in place.
