...
Step 1's two rejections are non-optional (the build fails); its `@table`-on-input half warns only. Steps 2-3 and that half of step 1 will become non-optional when the corresponding retirement gates fire. Steps 4-6 produce deprecation notices but no failures. Step 7 only matters for schemas that relied on the divergence; most schemas don't.
