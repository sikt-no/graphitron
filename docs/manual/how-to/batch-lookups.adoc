...
Every `@lookupKey` shape compiles to the same generator pattern: a `VALUES(idx, key1, key2, ...)` derived table joined against the target table on the key columns, carrying `idx` out in the select list. The output list is the same length as the input key list; unmatched keys produce `null` at their input position. Each key is answered on its own, so nothing is deduplicated or reordered: `filmById(film_id: ["1", "1", "1"])` returns three copies of film 1. The `VALUES` table is what makes that true, carrying one row per input position rather than a set of distinct keys. That slot-per-key shape is the join result scattered by `idx`, not the join result itself, which is why the element type has to be nullable: a lookup field declaring `[Film!]!` is rejected at build time, because one unmatched key would otherwise null the entire field. xref:sort-results.adoc[Sorts] do not apply: the order is the input order, full stop. Adding xref:../reference/directives/asConnection.adoc[`@asConnection`] on top of a `@lookupKey` field is rejected by the classifier (`@asConnection on @lookupKey fields is invalid`); pagination would shift positions and break the positional contract.
...
* A lookup field's list elements must be nullable. `[Film]!` and `[Film]` are accepted, `[Film!]!` and `[Film!]` are rejected with a build error, since a miss has to be able to occupy its output position.
