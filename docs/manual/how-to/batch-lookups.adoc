...
* `@asConnection` on a `@lookupKey` field is rejected, root and child alike, but by different checks: a child hits the classifier's `@asConnection on @lookupKey fields is invalid`, while a root field is rejected earlier by `@lookupKey`'s `@table`-annotated-return-type invariant, since the connection type is not `@table`-annotated. Use `@splitQuery` + `@asConnection` for a paginated child list.
