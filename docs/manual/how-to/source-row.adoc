...
The directive is rejected on `@table` parents (use xref:join-with-references.adoc[`@reference`] there) and on jOOQ-record parents (the catalog already drives batching). It applies only to plain Java records and POJOs whose backing class is declared via `@record(record: {className: "..."})`.
...
* xref:result-types.adoc[Result-type variants] covers the four `@record` backing-class shapes and when the classifier auto-derives a batch key from a typed accessor instead.
