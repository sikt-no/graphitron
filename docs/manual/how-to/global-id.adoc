...
* `id: ID!` is required on every node type, but xref:../reference/directives/nodeId.adoc[`@nodeId`] on it is not: the field satisfying the `Node` interface is a node ID by construction. When the bound table also has a column literally named `id`, the node ID wins and the build warns that the column is shadowed; write `@nodeId` to confirm, or xref:../reference/directives/field.adoc[`@field(name: "id")`] to expose the raw column instead.
