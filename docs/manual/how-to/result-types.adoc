...
* *Table-bound and class-backed are exclusive on an output type.* The type is either jOOQ-table-bound (`TableType`, via `@table`) or backed by a Java class reflected from its producer (`ResultType`). *Input* types are never table-bound (`@table` on an input is a retired location and is rejected): an input is class-backed when a consuming method parameter reflects to a class, otherwise its fields resolve against each consuming field's table.
