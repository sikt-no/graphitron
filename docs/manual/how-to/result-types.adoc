...
* *Is the child resolved by a `@service` rather than by a join?* Leave it class-backed and let the service signature name the key: the `Sources` element type (`Set<XRecord>`) says which table the batch keys on, and the parent must either *be* an `XRecord` or expose exactly one zero-arg accessor returning one. Neither accessor inference (which matches by the child field's name) nor `@sourceRow` (which needs a catalog anchor for its columns) applies on this path; see xref:handle-services.adoc#batching-class-backed[How-to: Handle services].
