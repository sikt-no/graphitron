...
* `X extends TableRecord`. The framework supplies fully-populated parent records (every column on the parent table), rebuilt column by column from the parent row (which the SELECT projects under reserved aliases so client-selected fields can never collide with it). Use this when the body needs columns beyond the parent's PK; `titleTitlecase` reads `film.getTitle()` straight off the record without a SELECT. Note the cost: choosing the typed-record shape opts the parent SELECT into projecting the full parent row whenever the field is selected, regardless of how narrow the client's selection is.

The typed-record arm is the most powerful, with one caveat: on a table-backed parent the framework projects the full parent row whenever the field is selected, so every column is present; but if the parent is itself a `@service` returning a hand-rolled record, the framework has no SELECT to widen and the record is only as populated as the producing service made it. Plan accordingly.
