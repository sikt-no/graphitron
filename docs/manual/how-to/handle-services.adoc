...
. The framework runs the parent query first. The parent's selection set is collected into a `Set<KeyType>` where `KeyType` is one of `Row<N>` / `Record<N>` / a typed `TableRecord`. Every shape carries the same thing: the parent's key columns, which for a `@table` parent are its primary key. The shapes differ in how you read them, not in what they contain.
...
* `X extends TableRecord`. The framework supplies a typed record carrying the parent's primary key. Use this when a typed record is the shape your SQL wants to work with; read the key off it with the generated accessor (`film.getFilmId()`).

[IMPORTANT]
====
*The keys carry the parent's primary key, and nothing else.* This is the contract in every shape, typed record included: a `FilmRecord` the framework hands you has `FILM_ID` populated and its other columns unset. If the body needs another column, fetch it, in one batched query, through the injected `DSLContext`.
...
* *A `@table` parent with no primary key cannot host a batched child `@service`.* The batch key is the parent's primary key, so there is nothing to key on; the classifier rejects the coordinate at build time and names the table.
