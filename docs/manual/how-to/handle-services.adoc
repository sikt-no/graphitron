...
The accepted source shapes are `Set<Row<N>>`, `Set<Record<N>>`, and `Set<X extends TableRecord>`. `ServiceCatalog.classifySourcesType` picks the matching batch shape from the parameter's reflected element type. `Row<N>` works (use `DSL.row(value)` to construct keys back); `Record<N>` lets you call `.value1()`/`.value2()` to extract column values directly; the typed-record arm hands you fully-populated parent records when the parent has a `@table`.
...
. The framework runs the parent query first. The parent's selection set is collected into a `Set<KeyType>` where `KeyType` is one of `Row<N>` / `Record<N>` / a typed `TableRecord`. On the `Row<N>` / `Record<N>` shapes the `Set` carries only the parent's key columns; on the typed-`TableRecord` shape it carries fully-populated parent records, because declaring that shape makes the parent SELECT project the full parent row (see below).
...
* `X extends TableRecord`. The framework supplies a parent record carrying every column on the parent table, and the key extraction reconstructs it per column from whichever shape the parent arrived in. Use this when the body needs columns beyond the parent's PK; `titleTitlecase` reads `film.getTitle()` straight off the record.

The typed-record arm serves two parent kinds and reads each on its own terms:

* *SQL-projected parent* (the parent came from a framework SELECT, e.g. a `@table` query or a split-query parent). The SELECT projects the full parent row under reserved `__src_<col>__` aliases (so a client-selected field can never collide with it), and the framework rebuilds the record column by column from those aliases. Every column is present. The cost: selecting a typed-record `@service` child opts the parent SELECT into projecting the full parent row whenever the field is selected, regardless of how narrow the client's selection is.
* *Service-returned parent* (the parent is itself a `@service` handing back a hand-rolled record, e.g. `selectFrom(FILM)` returning `Result<FilmRecord>`). There is no framework SELECT to widen, so the record is copied as the producing service populated it, and is only as complete as that service made it. Plan accordingly.

The framework picks the read at runtime from the parent record's own type, so the same typed-record `@service` child works under both parent kinds without any change to its Java signature.
