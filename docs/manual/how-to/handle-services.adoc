...
* *SQL-projected parent* (the parent came from a framework SELECT, e.g. a `@table` query or a split-query parent). The framework force-includes the parent's key columns in that SELECT even when the client's selection contains no field mapping to them, and builds the key record from them.
