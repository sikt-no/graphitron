...
One statement for the whole batch, which is what the DataLoader dispatch exists to make possible. Reading `film.getTitle()` off the key instead would work only when the client's own selection happened to include `title`, and return `null` the rest of the time.
====
