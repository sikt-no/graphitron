...
**Problems:**
- The file is monolithic. 2,176 lines in one class. Hard to navigate, slow to find the section you need. Should be split by concern (one file per type/field family, like the validation tests already are).
...
~3,150 lines across 40+ test classes. Each test class covers one field or type variant. Uses `ValidatorCase` interface + enum parameterization: each enum constant builds a `GraphitronField` directly (no SDL parsing), declares expected error messages, and a one-line parameterized test asserts them.
...
~420 + ~65 lines. Tests structural properties of generated `TypeSpec` objects: method names, return types, parameter signatures, whether a method is a stub. Does **not** assert on generated code body strings (per CLAUDE.md instructions), except for checking method-reference names in wiring code.
...
~297 + ~100 lines. End-to-end SDL → `GraphitronSchema` → generated `TypeSpec` list. Verifies that the full pipeline produces the expected class names, method names, and wiring entries for a given schema.
...
264 lines, 15 test methods. Starts a PostgreSQL container, loads schema + seed data, builds a GraphQL engine with the generated wiring, and runs actual queries.
...
### 1. Split `GraphitronSchemaBuilderTest`

The 2,176-line monolith should be split into focused files mirroring the validation test structure: `ColumnFieldClassificationTest`, `TableFieldClassificationTest`, `ServiceFieldClassificationTest`, `TypeClassificationTest`, etc. Each file owns one section of the current truth table.

Keep the shared `buildSchema()` helper in a `ClassificationTestHelper` class alongside the existing `FieldValidationTestHelper`.
...
The two init.sql files should be one. The fixtures module defines the schema for jOOQ code generation; the test-spec module should reuse the same SQL at test time. Options:

- **Option A:** Move `init.sql` to a shared resource directory in the parent module, reference it from both.
- **Option B:** Have `graphitron-rewrite-test-spec` depend on `graphitron-rewrite-test-fixtures` for its test resources too (it already depends on it for compile). Copy or symlink the init.sql.

Also align PostgreSQL versions (both should use the same major version).

### 5. Add a compilation smoke-test assertion
...
### 6. Expand test-spec schema coverage
...
### 8. Add `TypeConditionsGenerator` unit tests
...
|---|---|---|---|
| 1 | Unify `init.sql` | Eliminates drift risk, simplifies maintenance | Small |
| 2 | Shared `buildSchema()` via classpath | Removes fragile path, enables module relocation | Small |
| 3 | Shared model fixture factories | Reduces constructor noise across all tests | Medium |
| 4 | Split `GraphitronSchemaBuilderTest` | Makes the 2,176-line file navigable | Medium |
| 5 | Expand test-spec schema as stubs are implemented | Ensures new generators are end-to-end tested | Ongoing |
| 6 | Add compilation smoke-test assertion | Catches silently dropped classes | Small |
| 7 | Add `TypeConditionsGenerator` unit tests | Fills the gap at the structural test level | Small |
| 8 | Test context arguments end-to-end | Covers untested extraction path | Small |
