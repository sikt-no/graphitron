...
- **`TypeFetcherGenerator.NOT_IMPLEMENTED_REASONS`**
  ...
  `Map<Class<? extends GraphitronField>, String>` with 29 entries across
  `QueryField`/`MutationField`/`ChildField`. Consumed by `stub(f)` (line
  914) which throws `UnsupportedOperationException` with the map-registered
  reason.
- **Sealed `switch` in `TypeFetcherGenerator.generateTypeSpec`**
  (lines 210-285). Exhaustive over `GraphitronField`; six real arms, ~29
