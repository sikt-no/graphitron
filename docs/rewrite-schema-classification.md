...
Both lookup and lift use the same structural pattern: a set of input rows is transformed into a derived table, and the target scope JOINs against it. The cases differ in what controls the join.
...
**Lift table via `@splitQuery`** — Graphitron controls both sides:
...
**Lift table from user-provided returns** (service fields, `@tableMethod` fields):

When a `@service` or `@tableMethod` field returns a result that needs to lift into a table-mapped scope, the return value must be a jOOQ `TableRecord` with its PK fields populated. Graphitron reads those PK values to construct the lift table and JOIN the target scope against them. This is a hard requirement — Graphitron cannot construct the lift table otherwise.
