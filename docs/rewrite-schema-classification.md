...
| **Derived source** | A `TableField` on a result-mapped type — new scope via DataLoader, connected via a derived source table |
...
Both lookup and derived source use the same structural pattern: a set of input rows is transformed into a derived table, and the target scope JOINs against it. The cases differ in what controls the join.

**Lookup table** — input comes from GraphQL arguments (`@lookupKey`):

The critical invariant is that output cardinality and ordering match the input keys exactly. Which index the database hits to satisfy the query is an infrastructure concern — Graphitron does not require a PK lookup. The positional VALUES join enforces the invariant regardless.

**Derived source table via `@splitQuery`** — Graphitron controls both sides:

The parent scope and child scope are both generated by Graphitron, so the join columns and structure can be designed however makes sense. No external constraint.

**Derived source table from user-provided returns** (service fields, `@tableMethod` fields):

When a `@service` or `@tableMethod` field returns a result that needs to connect to a table-mapped scope, the return value must be a jOOQ `TableRecord` with its PK fields populated. Graphitron reads those PK values to construct the derived source table and JOIN the target scope against them. This is a hard requirement — Graphitron cannot construct the derived source table otherwise.
