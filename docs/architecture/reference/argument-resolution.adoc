...
- `TypeBuilder.buildInputType` rejects any input carrying `@table` (a retired
  directive location; the rejection message carries the per-verb migration
  guidance) and classifies every other input on the plain path. An input carries
