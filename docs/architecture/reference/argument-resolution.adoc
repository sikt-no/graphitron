...
- `TypeBuilder.buildInputType` is directive-driven: an explicit `@table` on the
  input produces the deprecated `TableInputType` bridge, and every other input is
  plain. A plain input carries no table of its own; its fields resolve against the
  consuming field's target table at the call site (`InputFieldResolver` on the
  filter path, `TypeBuilder.resolveInputFields` on the arg-level `@lookupKey`
  path). The `override` axis is threaded field-relative there too, as
...
`ColumnBackedField` / `ColumnBackedReferenceField` on a `@table` input contributes
...
1. *Override is threaded field-relative, not gated whole-type.* There is no
   ...
   `@condition` override flag. The per-call-site classifier handles plain-input
   column resolution directly against the outer field's table with the existing
   `catalog.findColumn` + `@field(name:)` path; a column-miss under an active
   override is admitted as an `InputField.UnboundField` rather than rejected.
