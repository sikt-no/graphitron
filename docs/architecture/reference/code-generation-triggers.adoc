...
[NOTE]
====
*Transitional.* This section describes the classification walk and the sealed model it produces, a
surface being drained rather than one to extend. What it states is accurate today. For the
architecture the walk drains into, the fact store and the command relations rendered from it, see
...
The walk and the `GraphitronSchema` model it produces are the *transitional producer surface* of the
strangler migration described in the xref:../explanation/pipeline-overview.adoc[Pipeline overview]:
directives are also decoded by capture into the fact store's `graphitron_` relations, consumers
re-source from the walk's model onto the store one at a time, and new facts land only in the store,
never as new leaf types.

----
GraphQL Schema
      ↓
GraphitronSchemaBuilder  (classification walk ; transitional producer surface)
      ↓
GraphitronSchema
...
Generators
  ...
  ├── ProjectionUnitRenderer   →  types.* (command-driven)
  └── ConditionGlueRenderer    →  conditions.*Conditions (rendered from ConditionCommand rows)
----
...
[NOTE]
====
*Transitional.* This section describes the field axes (source context, target type, scope) in their
original form, superseded by the three-axis `(source, operation, target)` model the field now carries
directly through its `source()` / `operation()` / `target()` accessors. The terms below survive as
properties derived from those three axes, not as independent axes. For how a field classifies today,
see <<field-classification,Field Classification>>; for the architecture both are being drained into,
see xref:../explanation/pipeline-overview.adoc[Pipeline overview].
====
...
| *Result-mapped* | producer's reflected return is a Java/jOOQ class | Runtime wiring only. The type's backing class is derived by reflection from its producing field (a `@service` return or a parent-accessor chain). Graphitron validates types and wires data fetchers, but generates no SQL until a new scope starts.
|===
...
stateDiagram-v2
    ...
    InScope --> InScope : record handoff<br/>(target-table field on result-mapped source<br/>or @service returning target-table type<br/>→ new scope via DataLoader, parent PK or custom batch key)
...
| *Record handoff* | A target-table field on a result-mapped source, or any user-provided return (`@service`) reaching a target-table type ; new scope via DataLoader, keyed by the parent's PK or a custom batch key
...
| *`@splitQuery`* | On a table-mapped source, forces a new scope via DataLoader: `TableField` → Table-sourced `BatchedTableField` (R432 merged the former Split/Record leaf pairs onto these source-gated leaves); a co-present `@lookupKey` rides along as the leaf's keyed lookup arm. On a result-mapped source it is redundant ; the record handoff already opens a new scope ; and should produce a build *warning*, not an error.
| *`@lookupKey`* | Argument values become the derived target table (see <<derived-tables,Derived tables>>). Blocks pagination (preserves the N × M result invariant). The correspondence rides as the fetch leaf's keyed `LookupResolution` arm: `TableField` without `@splitQuery`, Table-sourced `BatchedTableField` with it, `QueryTableField` at the root. Orthogonal to scope ; see <<scope,Scope>>.
|===
...
| `@table` + `@node`, or `@table` + `implements Node` over a table publishing node metadata | `NodeType` | `*Fetchers` class + `*` class (with Relay ID handling)
...
inherited references are projected off the base under the unqualified alias, which its qualified read
cannot address, so that coordinate is a deferred rejection rather than a silent miss.
...
| `@mutation(typeName: UPSERT)`, returning `ID` or a `@table` type | `UnclassifiedField` (typed deferral; the Upsert write arm has no live producer) | Rejected at classification ; UPSERT generation gated pending R145
...
| `@reference` on scalar | `ColumnBackedReferenceField` (arity 1, `compaction = Direct`) | Inline projection in `$project` + `ColumnFetcher` wiring value
| `@reference` on scalar declared on a `TableInterfaceType` participant | `ParticipantColumnReferenceField` | Wiring value ; materialised by the enclosing `TableInterfaceField` fetcher's discriminator-gated correlated subselect (the same capped shape a plain scalar `@reference` takes) and read back via `FetcherEmitter` (participant-side FK, not the interface-side resolver)
| `@nodeId`, no typeName | `ColumnBackedField` (`compaction = NodeIdEncodeKeys`; arity = the node key's column count) | Wiring value (`ColumnFetcher`); the projected column(s) are wrapped positionally in the per-Node `encode<TypeName>` helper
...
| `@externalField(reference: {className, method})` | `ComputedField` | Inlined call in `$project()` (`<Class>.<method>(table).as("<name>")`) + `ColumnFetcher` registered by alias in `wiring()`. Method must be `public static Field<X> name(<ParentTable> table)`.
...
[NOTE]
====
*Transitional.* This section describes the schema patterns the classification walk reads without a
directive, a surface being drained rather than one to extend. What it states is accurate today. For
the architecture the walk drains into, see
xref:../explanation/pipeline-overview.adoc[Pipeline overview].
====
...
| Field name matches column name (on `@table` type) | `ColumnBackedField` ; direct column mapping
...
[WARNING]
====
*Incomplete.* This map covers the draining `no.sikt.graphitron.rewrite` tree only. Its opening claim
that all source lives there is false: `no.sikt.graphitron.facts`,
`no.sikt.graphitron.plan`, `no.sikt.graphitron.command` and `no.sikt.graphitron.render` are
top-level siblings of `rewrite`, and this map names none of them, so it does not route a
contributor to where a change belongs. The per-file lists below are hand-maintained and have
drifted from the tree in both directions. For the packages that hold the destination, see
xref:../explanation/pipeline-overview.adoc[Pipeline overview] and
xref:../explanation/fact-model.adoc[The fact model].
====
...
| Join path | `JoinStep.java` | `Hop` (a `TableExpr` target × an `On`: `ColumnPairs` / `Predicate`); R431 retired the transitional `LiftedHop` onto `ParentCorrelation.OnLiftedSlots`
...
| DataLoader keys | `SourceKey.java`, `KeyLift.java`, `LoaderRegistration.java` | `SourceKey` is the batch-key residue (`columns` plus sealed `Wrap`: `Row` / `Record` / `TableRecord`). `KeyLift` is the record-parent lift provenance (`FkColumns` / `Lifter` / `Accessor` / `ProducedRecords`, the arity-bearing arms carrying an `Arity`). `LoaderRegistration` carries DataLoader identity (`Container`: `POSITIONAL_LIST` / `MAPPED_SET`; `Dispatch`: `LOAD_ONE` / `LOAD_MANY`).
| Ordering | `OrderBySpec.java` | `Fixed` / `Argument` / `None`
| Pagination | `PaginationSpec.java` | Relay cursor arguments
| Argument extraction | `CallSiteExtraction.java` | Direct strategies (`Direct` / `EnumValueOf` / `ContextArg` / `JooqConvert`) plus sealed sub-shapes: `NestedInputField` (descends an input-object graph for `@condition` on `INPUT_FIELD_DEFINITION`), `NodeIdDecodeKeys` (`ThrowOnMismatch`; arity-N NodeId decode at the call site), `NodeIdDecodeRecord` (NodeId decode into a jOOQ-record member), `InputBean` (service input-bean instantiation), and `JooqRecord` (jOOQ record construction from input fields)
| Input field hierarchy | `InputField.java` | Sealed interface ; `ColumnBackedField` / `ColumnBackedReferenceField` (each arity 1..N) / `NestingField` for mutation inputs
...
| Service reflection | `ServiceCatalog.java` | Reflects `@service` / `@condition` Java methods into `MethodRef`
|===

=== Generators (`generators/`)

Four families, by what they emit. The Generators package holds emitter helpers (`SplitRowsMethodEmitter`, `JoinPathEmitter`, `MultiTablePolymorphicEmitter`, `ArgCallEmitter`, `FetcherEmitter`, `HandleMethodBody`, `SelectMethodBody`, etc.) alongside the entry-point generators below ; helpers are reached from inside a generator and don't appear in this table. The lookup VALUES + JOIN emission lives outside the package, in `render/LookupRows`.

|===
| Family | Generators | What lands

| Fetcher emission | `TypeFetcherGenerator`; `ProjectionUnitRenderer` and `ConditionGlueRenderer` (command-driven) | `fetchers.*Fetchers` (wiring + field fetcher/rows methods); `types.*` (`$project(grouped, table, env)` projection units: anchors, anchor-prefixed nesting units, per-coordinate pivot units); `conditions.*Conditions` (per-parent condition glue, one method per filtered coordinate)
...
`GeneratorUtils` (root of `generators/`) holds shared building blocks reached from any family ; `ResolvedTableNames`, key-type construction, constants. Condition glue is command-driven: the plan's `ConditionCommands` producer mints one row per filtered coordinate and `render/ConditionGlueRenderer` emits the per-parent `*Conditions` classes whose methods take the jOOQ table and the argument map (the typed adapter / composer pairing now lives inside one glue body; see the "Wire boundaries are typed adapter / composer pairs" principle in xref:../principles/development-principles.adoc[Graphitron Development Principles]).
...
*See also:*
- xref:../principles/development-principles.adoc[Graphitron Development Principles] ; the six axioms, their corollaries, and the named enforcement behind each
