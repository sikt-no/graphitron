...
`GeneratorUtils` (root of `generators/`) holds shared building blocks reached from any family ; `ResolvedTableNames`, key-type construction, constants. Condition glue is command-driven: the plan's `ConditionCommands` producer mints one row per filtered coordinate and `render/ConditionGlueRenderer` emits the per-parent `*Conditions` classes whose methods take the jOOQ table and the argument map (the typed adapter / composer pairing now lives inside one glue body; see the "Wire boundaries are typed adapter / composer pairs" principle in xref:../explanation/development-principles.adoc[Graphitron Development Principles]).
