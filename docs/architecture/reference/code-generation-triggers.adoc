...
| `@mutation(typeName: INSERT)`, returning a `@table` type (write target derived from the return) | `MutationInsertTableField` | Fetcher method ; write + read-back SELECT (the derived re-fetch). The return names the write target
| `@mutation(typeName: INSERT, table: "…")`, returning `ID` | `MutationInsertTableField` | As above; an encoded-`ID` return names no table, so `@mutation(table:)` names it on the field. Byte-identical carrier across the two rungs
| `@mutation(typeName: UPDATE)`, returning `ID` or a `@table` type | `MutationUpdateTableField` | Fetcher method ; write + read-back SELECT (the derived re-fetch). Same two-rung write-target ladder as `INSERT` (return-derived, else `@mutation(table:)`)
| `@mutation(typeName: DELETE, table: "…")`, returning `ID` | `MutationDeleteTableField` | Fetcher method ; write + encoded-PK off `RETURNING` (no read-back ; the row is gone, target shape `Column`). The write target is named on the field; a `DELETE` return can never name it
