...
| Input type with `@table` (deprecated on input; see `docs/manual/reference/directives/table.adoc`) | `TableInputType` | Used in mutation generation. A build warning fires per `@table`-on-input usage (no carve-out). An `INSERT`-consumed input warns and names the return-derived fix and `@mutation(table:)`; a `DELETE`-consumed input warns and names `@mutation(table:)`; `UPDATE`/`UPSERT`-consumed inputs earn the base warning (no field-relative replacement yet).
| Input type without `@table` (the `@service` / `@condition` parameter the input flows into is reflected as a developer class) | `InputType`* | No generation (developer-provided class). The input's backing class is reflected from the parameter type it flows into.
...
| `@mutation(typeName: INSERT)`, returning a `@table` type (write target derived from the return) | `MutationInsertTableField` | Fetcher method ; write + read-back SELECT (the derived re-fetch). The input needs no `@table`; the return names the write target
| `@mutation(typeName: INSERT)`, returning `ID`, write target from `@mutation(table: "…")` or the input's `@table` | `MutationInsertTableField` | As above; an encoded-`ID` return names no table, so it comes from `@mutation(table:)` (preferred) or the deprecated input `@table`. Byte-identical carrier across the rungs
...
| `@mutation(typeName: DELETE)`, returning `ID`, write target from the input's `@table` | `MutationDeleteTableField` | Fetcher method ; write + encoded-PK off `RETURNING` (no read-back ; the row is gone, target shape `Column`)
| `@mutation(typeName: DELETE, table: "…")`, returning `ID`, input carrying no `@table` | `MutationDeleteTableField` | As above; the write target is named on the field rather than on the input type. Byte-identical carrier to the `@table`-on-input form
| `@mutation(typeName: UPDATE\|UPSERT, table: "…")` (`table:` on a verb that reads its table from the input `@table`) | `UnclassifiedField` (typed `MutationTableArgError.UnsupportedVerb`) | Validation error ; build fails. `table:` is wired for `DELETE` and `INSERT` only
...
| `@mutation(typeName: INSERT\|UPDATE)` with bulk input, returning a single-record carrier with a list-shaped `@table`-element data field | `MutationBulkDmlRecordField` | Per-row DML inside one `transactionResult`, accumulating a typed `Result<RecordN<PK>>` in input order. For `INSERT` the write target is derived from that `@table`-element data field, so the input needs no `@table`
...
| `@nodeId` on input scalar | `InputField.ColumnBackedField` (`NodeIdDecodeKeys` extraction; arity = the node key's column count) | Decodes a NodeId into the input's key columns
