...
The contract is positional: one output element per key passed, with `null` at the position of a key
that matched nothing. That is what forces the return type's elements to be nullable, and a `[Film!]!`
here is an author error whose message says so. `Film.filmId` inside the looked-up rows is an ordinary
inferred column read, unaffected by how its rows were reached. Asserted by the `lookup` corpus example.

Only a root query field is claimed this way. `@lookupKey` on a child field's argument keys that child's
correlated subquery by the values passed and claims nothing, so the child coordinate keeps whatever
verdict its return type gives it. The same corpus example pins that child form beside the root one.
