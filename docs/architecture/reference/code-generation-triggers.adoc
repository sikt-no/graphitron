...
[#error-type]
=== `@error` over an object type → `ErrorType`
...
The payload's own two fields are plain passthroughs over the record the service returned, which is
the reflection-derived backing of <<backing-class,the previous section>> and not an error concern.
...
What is left here is the surface a worked example cannot show. The corpus renders patterns that
classify and generate, so a deprecation warning and two rejections have no example to be pulled into;
they stay stated rather than demonstrated.
...
Four of the five rows left below are rejections, which the corpus does not render: it demonstrates
patterns that classify and generate, so a build failure has no worked example to be pulled into. The
first row is the exception, a live pattern still stated rather than shown.

|===
| Schema Pattern | `MutationField` Variant | `*Fetchers` Generates

| `@mutation(typeName: INSERT, table: "…")`, returning `ID` | `DmlTableField` (Insert write arm) | As the direct-`@table` INSERT above; an encoded-`ID` return names no table, so `@mutation(table:)` names it on the field. Byte-identical carrier across the two rungs
