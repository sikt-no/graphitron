...
| *`@splitQuery`* | On a table-mapped source, forces a new scope via DataLoader: `TableField` (no `@lookupKey`) → Table-sourced `BatchedTableField`; field with `@lookupKey` → Table-sourced `BatchedLookupTableField` (R432 merged the former Split/Record leaf pairs onto these source-gated leaves). On a result-mapped source it is redundant ; the record handoff already opens a new scope ; and should produce a build *warning*, not an error.
| *`@lookupKey`* | Argument values become the derived target table (see <<derived-tables,Derived tables>>). Blocks pagination (preserves the N × M result invariant). Without `@splitQuery` → `LookupTableField`; with `@splitQuery` → Table-sourced `BatchedLookupTableField`. Orthogonal to scope ; see <<scope,Scope>>.
|===
...
| Return `@table`, `@splitQuery` + `@lookupKey` | `BatchedLookupTableField` (Table-sourced) | Async DataLoader fetcher + `rows*()` method
| Return `@table`, `@lookupKey` (no split) | `LookupTableField` | Inline projection in `$project` ; correlated subquery via `DSL.multiset` + VALUES/JOIN lookup keyset
...
| Return `@table`, `@lookupKey` (catalog FK from parent's jOOQ TableRecord) | `BatchedLookupTableField` (Record-sourced, `KeyLift.FkColumns`) | Async DataLoader fetcher + `rows*()` method
...
| Return `@table` with `@sourceRow` (parent is `Backed` or `JavaRecordType`; no catalog FK required) | `BatchedTableField` / `BatchedLookupTableField` (Record-sourced, `KeyLift.Lifter`) | Async DataLoader fetcher + `rows*()` method; the developer-supplied static lifter populates the batched VALUES rows
...
**\*\* `UnclassifiedField`** is a direct permit of `GraphitronField`; it is not nested under `QueryField`, `MutationField`, or `ChildField`. It is listed in the tables above for completeness, but structurally it sits at the top level of the sealed hierarchy. `TableTargetField` is an intermediate sealed sub-interface of `ChildField` grouping all 6 SQL-generating child field variants (`TableField`, `BatchedTableField`, `LookupTableField`, `BatchedLookupTableField`, `TableInterfaceField`, `ServiceTableField`; R432 collapsed the former Split/Record pairs onto the two source-gated batched leaves).
...
| *`BatchedLookupTableField`* (`@splitQuery` + `@lookupKey`) | Yes | Both | Allowed ; must preserve N × M contract† | Never ; result count = N × M
|===
