...
| `intent_bound_table`, resolving `@table`'s reference through `graphitron_spelled_table`: the `name`
...
include::_example-node-type.adoc[]
...
include::_example-composite-node-key.adoc[]
...
include::_example-scalar-type.adoc[]
...
include::_example-result-backing.adoc[]
...
include::_example-input-backing.adoc[]
...
include::_example-error-type.adoc[]
...
include::_example-child-table.adoc[]
...
include::_example-record-table.adoc[]
...
include::_example-interface.adoc[]
...
include::_example-paginated-joined-table-interface.adoc[]
...
include::_example-table-interface.adoc[]
...
include::_example-participant-reference.adoc[]
...
include::_example-split-lookup.adoc[]
...
include::_example-paginated-child.adoc[]
...
`CountryCitiesConnection` and `CountryCitiesConnectionEdge` are synthesised for the coordinate and named after it,
...
include::_example-relay-node.adoc[]
...
include::_example-mutation-roots.adoc[]
...
include::_example-dml-payloads.adoc[]
...
include::_example-mutation-service.adoc[]
...
include::_example-service-table-child.adoc[]
...
include::_example-service-scalar-child.adoc[]
...
include::_example-service-child-class-backed-parent.adoc[]
...
include::_example-reference-and-computed.adoc[]
...
include::_example-routine-table-valued-read.adoc[]
...
include::_example-faceted-connection.adoc[]

The coordinates named `QueryFilmsConnection` and `QueryFilmsConnectionEdge` are the synthesised types, and they
...
==== The arguments the expansion appends

`@asConnection` appends `first` and `after` to the carrier, after whatever arguments the author
wrote. `first` carries the connection's page size, which is the application's `defaultFirstValue`
where it states one.

An author who paginates on their own terms keeps what they wrote. The forward pair lands beside a
backward one rather than in place of it, so a field declaring `last` and `before` emits all four.

include::_example-backward-paginated-connection.adoc[]

Where the author declares one of the two appended names themselves, theirs stands and only the
missing one is appended. What they wrote is emitted as written, so an argument declared bare stays
bare rather than acquiring the connection's page size. Asserted by the `authored-pagination-argument`
corpus example.

include::_example-authored-pagination-argument.adoc[]
...
include::_example-enum-column.adoc[]
