...
| `@reference` on scalar | `ColumnBackedReferenceField` (arity 1, `compaction = Direct`) | Inline projection in `$fields` + `ColumnFetcher` wiring value
...
| Fetcher emission | `TypeFetcherGenerator`, `TypeClassGenerator`; `ConditionGlueRenderer` (command-driven) | `fetchers.*Fetchers` (wiring + field fetcher/rows methods); `types.*` (`$fields(sel, table, env)` projection); `conditions.*Conditions` (per-parent condition glue, one method per filtered coordinate)
