...
| `@nodeId(typeName:)` on input scalar | `InputField.ColumnBackedReferenceField` (`NodeIdDecodeKeys` extraction; arity = the target key's column count) | Decodes a NodeId into FK columns referencing the typeName target
...
**\*\* `UnclassifiedField`** is a direct permit of `GraphitronField`; it is not nested under `QueryField`, `MutationField`, or `ChildField`. It is listed in the tables above for completeness, but structurally it sits at the top level of the sealed hierarchy. `TableTargetField` is an intermediate sealed sub-interface of `ChildField` grouping all 4 SQL-generating child field variants (`TableField`, `BatchedTableField`, `TableInterfaceField`, `ServiceTableField`; R432 collapsed the former Split/Record pairs onto the source-gated batched leaf, and the lookup leaves later folded onto their fetch siblings with the `@lookupKey` correspondence riding as the seal's `LookupResolution` arm).
