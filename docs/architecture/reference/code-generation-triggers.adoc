...
The four spellings of a plain `@table` child are worked through above and are not repeated here:
`@splitQuery` over an unkeyed child in the <<derived-layer,derived layer example>>, and both lookup-keyed
spellings in the <<lookup-child,child lookup pair>>. What is left is the polymorphic return types, the
plain-object form, and the three rows the build refuses:
