...
| Input type with `@table` (retired location; see `docs/manual/reference/directives/table.adoc`) | `UnclassifiedType` | Validation error; build fails. The rejection message carries the per-verb migration guidance (drop the directive for filters and lookup args; `@mutation(table:)` for DELETE and encoded-ID / scalar returns; return-derivation for INSERT/UPDATE). The SDL declaration keeps `INPUT_OBJECT` only so the parser reaches this rejection instead of a generic "unknown directive location" error.
