...
(`graphitron/src/test/resources/corpus`, one GraphQL document per example): the SDL shown is the live fixture the classifier
...
| Conflicting type directives (`@table` with `@error`) | Arm-order winner | Validation error from the store-backed conflict detection (`AuthoredClaimConflicts`) ; build fails
|===
...
is the plain `@table` return of the <<table-type,`@table` type example>>, which emits a full
...
all; that declaration is the <<node-type,`@node` example>> above. Asserted by the `relay-node`
corpus example.
...
| Both `@service` and `@mutation` | Arm-order winner | Validation error from the store-backed conflict detection (`AuthoredClaimConflicts`) ; build fails. The LSP/MCP projection reports `FieldClassification.Conflicted` carrying both claims' decoded slot facts (verb, table, method)
|===
...
worked through above: the column matched by name in the <<table-type,`@table` type example>>, and the
column renamed by `@field(name:)` in the <<derived-layer,derived layer example>>. Either way the
...
| `@nodeId(typeName:)`, non-FK-mirror target | `ColumnBackedReferenceField` (`compaction = NodeIdEncodeKeys`; arity = the target key's column count) | Nothing yet. A rooted-at-parent NodeId reference is rejected at validate time as deferred, at every arity, because no emitter brings the target's key columns into scope. The rejection is keyed on the variant class itself (`Rejection.StubKey.VariantClass`), so what the deferral names is the shape that has to grow an emitter
|===
...
is the <<nesting-type,unbound object type example>>. What is left is the three rows the build refuses:

|===
| Schema Pattern | `ChildField` Variant | `*Fetchers` Generates

| Conflicting directives | Arm-order winner | Validation error from the store-backed conflict detection (`AuthoredClaimConflicts`); build fails. The LSP/MCP projection reports `FieldClassification.Conflicted` carrying the claims
...
<<error-type,`@error` example>>. What is left is the two forms no example shows, one of which does
