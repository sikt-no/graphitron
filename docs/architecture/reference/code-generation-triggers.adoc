...
reflection-derived backing of <<backing-class,the previous section>>. `errors` is not. It is the
...
<<class-backed-service,class-backed service pair>>, and the `errors:` slot in the
<<error-type,`ErrorType` example>>. What is left is the two forms no example shows, one of which does
