...
One inferred reading is not yet rendered here: a field returning a `*Connection` type, which the
`@asConnection` transform produces, carries the pagination shape without a directive on the field
itself. Its verdict is gated by the `connection` and `faceted-connection` corpus examples until a
worked example for it lands on this page.
