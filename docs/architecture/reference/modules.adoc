...
The repository is a single Maven reactor rooted at the `graphitron-rewrite-parent` parent pom (`10-SNAPSHOT`). Twelve modules cover the generator, its inputs, its consumers, the reusable runtime surface, the tooling that wraps both ends, and the documentation site. The roadmap-tool `check-module-enumeration` step fails the build if this table stops naming a module the root pom declares, so the set below cannot fall behind the reactor:
