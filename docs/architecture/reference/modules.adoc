...
| Maven plugin: `generate`, `validate`, `capture` and `dev` Mojos (the dev mojo provides the watch loop). Also the home of the cross-tier tests: the dev mojo is what wires the generator to the two clients, so a test whose subject is the build and a client agreeing lives here, above both, rather than inside a client reaching up.
...
| `graphitron-lsp`
| Editor LSP server for `.graphqls` authoring. Reads the fact store and the buffer under the cursor; no dependency on `graphitron` at any scope.
