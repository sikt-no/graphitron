...
* *The owned-connection path (recommended).* Graphitron owns the
...
so you register nothing else. Per request you pass only your mount method's
...
For that operation Graphitron takes one connection when a field first needs it,
runs your mount method on it to mount identity, executes (each top-level
mutation field in its own transaction that commits or rolls back independently,
queries in autocommit), then runs your unmount method and releases the
connection. A connection whose unmount failed is evicted, never returned to the
pool.

=== Session identity through `<sessionState>`

Identity goes to the database, not to Java. The plugin's `<sessionState>`
configuration names a static Java `<mount>` method (typically the jOOQ-generated
executing method of your identity routine, or a hand-written facade of the same
shape) and an optional `<unmount>`. Graphitron reads the signatures at build
time: the mount's payload parameters become contextArguments on the generated
factory, handed through untouched, and its return type is the handle later
passed to `unmount`. Graphitron parses nothing about the payload; your method
and the database do. See
xref:../../manual/reference/mojo-configuration.adoc#session-identity[Session
identity] for the configuration and the signature contract.

[#hook-state-contract]
==== The hook state contract: session-scoped, never transactional

Session identity is connection-scoped state, and your methods must treat it
that way. Graphitron cannot see inside the mount or unmount you author, so the
contract is stated here and holds for every `<sessionState>`:
...
* *Set and reset session-scoped state only.* On Postgres that means
  ...
  which are transaction-scoped and vanish at the end of the statement that set
  them. Oracle RAS attach/detach and VPD context calls are session operations
  and already comply.
* *Do not rely on a surrounding transaction.* Neither mount nor unmount may
  ...
  persist.
* *You may assume no transaction is open.* Graphitron invokes both methods
  outside any open transaction, structurally: acquisition asserts the
  connection arrives in autocommit and fails closed if it does not (a pool
  configured `autoCommit=false` is a deployment error surfaced at the first
  acquisition, not silently worked around), and release settles any transaction
  the operation left open before unmount runs (so your clears take effect
  immediately rather than being reverted by the pool's return-rollback).

A mount that violates the first two points can leave identity mounted for the
pool's next borrower or drop it mid-operation; both are security defects, not
performance defects. The structure is what makes the contract cheap to hold:
`mount` runs once per taken connection with autocommit on, so what it sets is
committed session state, and a mutation field's transaction settling later
cannot revert it. Graphitron therefore never re-runs your methods around
...
per distinct divined tenant within an operation and mounts identity on each,
handing a mount that declares the `Optional<K>` tenant slot the key it is mounting
for. Only tenants in the request tenant set (the factories' leading parameter in a
`<tenantColumn>` build) are ever acquired: the generated per-operation connection
carrier refuses any other key with a client error before a connection is taken. The schema-side
inference that divines the tenant key is the tenant-column work; the runtime seam
it rides is part of this path. The consumer-facing recipe is
xref:../../manual/how-to/tenant-scoping.adoc[How-to: Tenant scoping].
...
* *Custom validator factory*: not configurable today. The default reads `Validation.buildDefaultValidatorFactory().getValidator()`; the single existing override point is currently unused, so no migration is required.
...
on the `@condition` and `@service` directives.
...
* *The `DSLContext` you pass into `Graphitron.newExecutionInput`* for per-request
  ...
  factory.
* *Schema directives* (`@condition`, `@reference`) for
...
The gradient is read off the routine your `<mount>` line names, not off
graphitron configuration: on the headline path there is no consumer Java between
the Mojo line and the database, so what the routine's own definition says is the
whole story. Only when `<mount>` names a hand-written facade does "visible in
the consumer's own code" hold literally, and then the facade is one more place
to read.

* *Enforced fence* (strongest). The mounted state lives behind a privilege
  fence: an Oracle package-bound context or RAS session attached by a
  definer-rights package, so code on the connection cannot set identity state
  except through the trusted package. `<mount>` names the package's executing
  method; the fence is the package's own definition.
* *Convention fence.* Plain PostgreSQL GUCs written by your mount (directly,
  or via a `SECURITY DEFINER` routine): any SQL on the connection can overwrite
  them, so the real guarantee is that Graphitron generates every statement and
  any `@service` code on the connection behaves. The build cannot see which
  fence a reflected method reference implements, so no warning grades it; the
  choice is documented here and in xref:../../security.adoc[the security guide]
  rather than policed at build time.
* *Cryptographic fence.* Pass the raw signed token as the mount's payload and
  have the routine verify the signature in-database, reading claims only from
  verified state, so even arbitrary SQL on the connection cannot forge another
  user's identity. The fence is the routine's verification logic; `<mount>`
  merely names it.

=== On the owned-connection path

Identity is mounted by your configured `<mount>` method (see
<<owned-connection-path,above>>); your RLS policies read whatever it set. For a
mount that writes a session GUC per request (and an unmount that clears it to
the empty string, since a touched GUC cannot return to unset), a fail-closed
policy reads:
...
On the owned-connection path the generated factory takes one typed parameter
per payload parameter of your mount method; no auth-framework type enters
generated code. For the common single-`String` payload you pass an opaque
claims string to `Graphitron.newOwnedExecutionInput(claims, ...)`. From
MicroProfile JWT (or any JWS bearer) the two forms come straight off the
injected `JsonWebToken`, and which you pick is the integrity-gradient decision:

* *Signed compact token* (cryptographic fence): pass `jwt.getRawToken()`. Your
  mount routine verifies the signature and reads claims in-database, so the
  identity holds even against distrusted SQL on the connection.
* *Bare claims JSON* (for a mount that parses claims without verifying): the
  payload segment of a JWS already is the claims JSON, so decode it with the
  JDK alone, no JSON parser:
...
passing it where claims JSON is expected fails inside your mount (for example
at a `jsonb` cast), with the mount's own error. If your platform hands you an
encrypted token (JWE), the payload segment is ciphertext; rebuild the claims
JSON from the platform's claim accessors instead.
...
* xref:code-generation-triggers.adoc[Code Generation Triggers]: Schema patterns
  to sealed variants, including the `@condition` directive
