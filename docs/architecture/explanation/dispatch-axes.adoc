...
`KeyLift`, carried by `BatchedTableField` / `BatchedLookupTableField` (both source shapes), and (as `parentKeyLift`) by the polymorphic `InterfaceField` / `UnionField`, names how the key tuple is lifted off the parent's held object:
