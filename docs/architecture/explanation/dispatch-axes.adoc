...
* `RootLauncherRenderer.render` frames every rows-method and root-launcher body, from a launcher command row. The per-shape skeleton that used to dispatch body framing on a permit projected from the field variant retired with the service fold, so the seam between body construction and outer-method framing is the command row itself; `UnifiedEmissionPinsTest` counts the render call sites that keep it there.
