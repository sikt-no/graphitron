...
projection seam (`CatalogBuilder.projectFieldClassification` is the transitional exemplar: an
exhaustive switch a new permit breaks until the view covers it). The drift smell at model scale
is a consumer-side shadow taxonomy; the re-sourcing invariant is
xref:fact-model.adoc[The fact model].
