...
seam (`CatalogBuilder.projectFieldClassification` for a projection reader,
`TriggerDispatchMatrixTest` for a relation reader: a new variant breaks the build until every view
covers it). The drift smell at model scale is a consumer-side shadow taxonomy; the re-sourcing
