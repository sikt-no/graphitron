...
`Class<?>` crosses into the store. The graphql-java schema types carry the same rule: a
`TypeDefinitionRegistry`, a `GraphQLSchema` or a traversal's elements may be held only by the
capture collaborators (`SdlFactCapture`, `GraphitronFactCapture`, `ClassificationDomainCapture`,
`CatalogFactCapture`) and, on the walk, by `ServiceCatalog` / `JooqCatalog` / `TypeBuilder` /
`FieldBuilder`.

Off capture it is a convention: reflection `Type`-tree reads live only at builder-side classifiers.
Everything downstream switches on captured or pre-classified values. If a consumer needs
information not yet decoded, add a column or a component and extract it at the boundary; never
reach past it.

*Enforced by:* the store's shape at capture (relations hold values, never live handles); review
only off capture, against the named holders above.
