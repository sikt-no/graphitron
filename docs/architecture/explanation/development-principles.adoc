...
discipline behind the store is xref:fact-model.adoc[The fact model].
...
the containment is structural: a relation stores values, so no live `Table<?>`, `ForeignKey` or
`Class<?>` crosses into the store. On the transitional
walk it is a convention: reflection `Type`-tree reads live only at builder-side classifiers,
discoverable by searching for the reads themselves (`java.lang.reflect.Type`,
`getGenericReturnType`, `getGenericParameterTypes`); `JooqCatalog` is the canonical holder of raw
jOOQ types, the deliberate exceptions importing `org.jooq` at their own boundary. Everything
downstream switches on captured or pre-classified values and never touches the raw types. If a
consumer needs information not yet decoded, add a column or a component and extract it at the
boundary ; never reach past it.

*Enforced by:* the store's shape at capture (relations hold values, never live handles); review
only on the walk, via the grep-able recipes above.
...
views over the one base; none owns a private model, and a view's coverage guarantee lives at its
projection seam (`CatalogBuilder.projectFieldClassification` is the exemplar: an exhaustive switch a
new permit breaks until the view covers it). Its readers are the generator's own now: the language
server used to dispatch on the same projection and reads relations instead, so what guarantees its
coverage is a gate over its own sealed vocabulary, `TriggerDispatchMatrixTest` over request triggers
and surfaces. The drift smell at model scale is a consumer-side shadow taxonomy; the re-sourcing
invariant is xref:fact-model.adoc[The fact model].

*Enforced by:* the compile-checked projection switch at each view's seam, or an exhaustive-partition
meta-test over the consumer's own sealed vocabulary where the consumer reads relations.
...
carry current prose contracts structurally are R240 (method type-token threading) and
R239 (`ColumnBackedField.parentTable`). To record a producer-consumer linkage the type system can't
