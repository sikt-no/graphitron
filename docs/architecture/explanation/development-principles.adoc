...
== Decide once, at the capture boundary; carry the decision as a fact

*Before implementing a feature, ensure the model carries what the feature needs ; pre-resolved,
ready to consume.* Capture decodes every external input once into the fact store's keyed base
relations, and the derived strata carry each conclusion as views over that base. During the
strangler window the classification walk (`GraphitronSchemaBuilder`) is the same boundary in its
transitional form, reading directives once and resolving everything downstream consumes. Every
consumer receives "what to do", never "what to interpret"; the modeling discipline behind the
store is xref:fact-model.adoc[The fact model].
