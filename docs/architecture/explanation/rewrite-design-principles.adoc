...
Reading the reflection `java.lang.reflect.Type` tree is permitted only at builder-side classifiers that convert reflection output into the typed model ; a deliberately small set of files, discoverable by their `java.lang.reflect` imports rather than enumerated here. Each converts raw reflection output into typed carrier values (`MethodRef.Param`, `AccessorRef`, `AccessorResolution`, and kin, each carrying a `ParamSource` where applicable). Everything downstream ; validator, generator ; switches on the pre-classified values and never touches reflection types.
