...
`RewriteSchemaLoader` merges the consumer's `.graphqls` files and auto-injects `directives.graphqls` from the `graphitron` jar before parse, so consumer schemas never re-declare the canonical directives and every later stage can treat directive presence as ground truth. The attribution pipeline then applies the schema-level rewrites (federation `@link` handling, tag and description notes) and cuts a read-only pre-synthesis snapshot of the registry before `KeyNodeSynthesiser` rewrites federation keys. Capture reads that snapshot and then runs the expansions itself (`MacroCapture`) rather than inheriting the rewrite's output, so an expansion's rows go through the same doors a written row does and the `graphitron_*_synthesis` relations record what an expansion contributed. What that leaves recoverable splits by kind. For the two relations marking rows an expansion *added* (`graphitron_type_declaration_synthesis`, `graphitron_type_directive_synthesis`) the transcription is the anti-join against the provenance. For the one marking a row an expansion *rewrote* (`graphitron_field_synthesis`) it is not: the written type expression survives only in that relation's own text column, and no anti-join recovers it.
