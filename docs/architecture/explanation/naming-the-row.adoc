...
xref:fact-model.adoc[The fact model]. Once a row's sentence is settled, the next question is
where its facts come from, which is xref:modeling-discipline.adoc[The modeling discipline]. The rendered
