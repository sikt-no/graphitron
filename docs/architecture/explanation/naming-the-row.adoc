...
Relations work the same way. `graphitron_table_entry` says "the author put `@table` on this type,
...
== One suffix, because the sentence has two tenses

The sentence a relation finishes comes in two tenses, and the difference is load-bearing enough to
be visible in the name. "The author put `@table` on this type" is a fact about a document: it joins
nothing, resolves nothing, and is true or false against one file you can open. "This type is bound
to this catalog table" is a fact about a resolution: it is a function of the document *and* the
catalog, and it can be true today and false tomorrow because somebody regenerated their jOOQ
classes.

Relations of the first kind carry the `_entry` suffix. `graphitron_table_entry` holds what was
written; `graphitron_tabletype` holds what it resolved to. That is not decoration. The most useful
question anybody asks of the pair is the anti-join between them, the applications that resolved to
nothing, which is exactly where an author's mistake lives and is the only place a diagnostic can
find it. A reader who cannot tell from a relation's name which tense it is in cannot find that
join.

The suffix also decides who writes the row, and it decides it the same way. A relation whose rows
are a function of one document is written by the walk over that document, while it is standing on
the coordinate; a relation whose rows are a function of the store is written by a gatherer that runs
after every walk has finished, because that is the earliest moment its inputs all exist. So the name
says the tense, the tense says the owner, and none of the three is a separate decision.
...
* `graphitron_table_entry` holds what the author wrote: `@table` on a type, with the name they spelled.
* `sql_table` holds what the catalog actually declares.
* `graphitron_spelled_table` answers a small, general question: how does a written
...
xref:fact-model.adoc[The fact model]. Once a row's sentence is settled, the next decision is
whether the rule that produces it is a view or something stored, which is
xref:writing-a-rule.adoc[Writing a rule]. The rendered
