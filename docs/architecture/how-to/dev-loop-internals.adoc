...
A console showing the first line and not the second is inside the refresh. That
alone is worth having, because a refresh runs inside the capture transaction and
the phases either side of it print nothing that would distinguish it.
...
[source,text]
----
[DEBUG] graphitron:  4/20 intent_field_reference_step_hop_keyed_live -> intent_field_reference_step_hop_keyed, graph 'orders'
...
return, so the last `n/20` line with no `done in` line under it is the relation the
run is stuck in. That is the whole recipe: no thread dump, no profiler, and no
