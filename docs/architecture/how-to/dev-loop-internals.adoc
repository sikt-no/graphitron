...
== A build or a dev start that goes quiet in the derivation stratum

The derivation stratum, the stages the capture runs after its gatherers have
flushed, prints its own pass boundary, so a run stuck inside it says so without
any instrument being turned on. Two lines per pass, on a capture and on a
`graphitron:dev` start alike:

[source,text]
----
[INFO] graphitron: deriving N stages for graph 'orders'
[INFO] graphitron: derivation stratum done in 3.4 s
----

A console showing the first line and not the second is inside the stratum. That
alone is worth having, because the phases either side of it print nothing that
would distinguish them. On a store none of whose stage tables holds a row yet, the
first line ends `, committing and analysing each`: that capture commits each stage
on its own and analyses what it wrote before the next one plans, which is the
cadence xref:../explanation/fact-model.adoc[the fact model] explains.

To name the stage, re-run with `-X` and grep for the per-stage lines, which the
same pass prints at debug:
...
[source,text]
----
[DEBUG] graphitron: 17/27 stage NodeIdDecodeHops
[DEBUG] graphitron: 17/27 done in 3.1 s, 812 rows
----

The name is printed before the stage's statements are issued and the duration
after they return, so the last `n/N` line with no `done in` line under it is the
stage the run is stuck in. `N` is how many steps the stratum runs, and it moves as
stages are added and retired, so read the denominator off the run rather than off
this page. That is the whole recipe: no thread dump, no profiler, and no
reconstruction of the stage order. Two lines per stage per graph is why this tier
is not on by default and not a reason to raise it.

From there the question is why that stage is expensive, which is a database
question and has its own procedure: see the `store-performance` skill, and
xref:../explanation/fact-model.adoc[the fact model] for what storing a rule costs.
