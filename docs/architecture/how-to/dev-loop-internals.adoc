...
a named phase. It is off by default and allocates nothing while off, so it sits
...
Output goes to stderr, or to a file with
`-Dgraphitron.lsp.trace.file=/tmp/lsp-trace.log`. It never goes to stdout: the
stdio launcher speaks JSON-RPC there, so a stray byte would desynchronise the
framing. Phases slower than `graphitron.lsp.trace.slowMs` (default 100) are
tagged `SLOW`.
...
"slow everywhere", which a duration-only format cannot.

[source,text]
----
lsp-trace > 6 workspace.mutate thread=graphitron-dev-conn-1
lsp-trace > 7   file.reparse thread=graphitron-dev-conn-1
lsp-trace < 7   file.reparse 1.1ms thread=graphitron-dev-conn-1 bytes=23781
lsp-trace > 8   file.typeIndex thread=graphitron-dev-conn-1
lsp-trace < 8   file.typeIndex 13.8ms thread=graphitron-dev-conn-1 bytes=23781 declared=400
lsp-trace < 6 workspace.mutate 21.9ms thread=graphitron-dev-conn-1 open=1 queued=1
----
