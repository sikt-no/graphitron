...
The `dev` goal runs six cooperating components in one JVM:

* *LSP server* binds the TCP port (default `8487`) and speaks LSP to whatever editor or agent connects. It serves diagnostics, hover, completion, go-to-definition, and find-references off the most recent classified `GraphitronSchema` and the validator's report on it. State is in-memory only; no LSP cache lives on disk. Its `didSave` notification is the primary fast path into the generator dispatch when an editor is attached.
...
* *Observation* is the session's memory of what it is watching and what it has seen move, and the reason a round need not rediscover that by reading bytes. Each watcher above marks the resolved path before it schedules its trigger, and says so when it cannot (an `OVERFLOW`, a subtree registered mid-session, a document URI that resolves to no file). Against that the session holds one instant per corpus, when it began watching, and one per instance it has seen move; the store holds the other half, a `read_at` beside each stamped row saying when the content behind that stamp began being read. An input is re-read unless its `read_at` is above its corpus's floor and above every mark against it, and every answer the comparison can get wrong is wrong in the direction of reading. `JavaSourceFacts` is the first reader of it: the `.java` cadence hashes the files the session cannot vouch for instead of the whole source tree, and dates every file it did hash, verification being the read rather than the rewrite. `-Dgraphitron.dev.rediscover=always` registers no corpus at all, which turns every answer back into "read it".
* *Generator dispatch* is the lifecycle thread that consumes wake-up events from either watcher, loads the (possibly-updated) classpath, runs the generator over the (possibly-updated) `.graphqls` sources, and feeds the resulting `GraphitronSchema` and emit results to the LSP server's in-memory state and to disk via the idempotent writer. A pass is *one* generator run producing both halves at once: the emitted tree and the editor-facing products (the completion catalog and the round's diagnostics). Reaching for a second entry point per pass captures the graph's partition twice and refreshes the materialization register twice, so a surface that wants something the pass does not yet return should widen `GraphQLRewriteGenerator.Pass` rather than call the generator again.
...
hover, go-to-definition, find-references, and the unknown-class diagnostics all resolve
...
== A build or a dev start that goes quiet in the materialization refresh

The refresh that refills the materialized fact-store targets prints its own pass
boundary, so a run stuck inside it says so without any instrument being turned on.
Two lines per pass, on a capture and on a `graphitron:dev` start alike:

[source,text]
----
[INFO] graphitron: refreshing N materializations for graph 'orders'
[INFO] graphitron: materialization refresh done in 3.4 s
----

A console showing the first line and not the second is inside the refresh, or in
one of the capture stages that run just ahead of it. That alone is worth having,
because both run inside the capture transaction and the phases either side print
nothing that would distinguish them. A stage prints no line of its own yet, so
rule out the refresh first with the grep below: a run stuck in a stage reaches
this tier with no per-registration line at all.

To name the registration, re-run with `-X` and grep for the per-registration
lines, which the same pass prints at debug:
...
[source,text]
----
[DEBUG] graphitron:  4/19 graphitron_node_id_decode_hop_rule -> graphitron_node_id_decode_hop, graph 'orders'
[DEBUG] graphitron:  4/19 done in 3.1 s, deleted 812 rows in 4 ms, inserted 812 rows in 3.1 s
----

The name is printed before the statements are issued and the durations after they
return, so the last `n/N` line with no `done in` line under it is the relation the
run is stuck in. `N` is how many registrations the register holds, and it falls as
rules move into capture stages, so read the denominator off the run rather than
off this page. That is the whole recipe: no thread dump, no profiler, and no
reconstruction of the refresh order. Two lines per registration per graph is why
this tier is not on by default and not a reason to raise it.

From there the question is why that relation is expensive, which is a database
question and has its own procedure: see the `store-performance` skill, and
xref:../explanation/fact-model.adoc[the fact model] for what a registration is and
what it costs.
