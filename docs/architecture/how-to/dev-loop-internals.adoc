...
| Both ran on the same lsp4j thread. While a notification's phases run, the
  ...
  out rather than merely being slow.

| `workspace.notify` much larger than the `workspace.mutate` before it
| The mutation's real cost is the diagnostic recalculation it triggers, not the
  edit. Check `queued=` on the `mutate` line and `files=` on
  `publishDiagnostics.drain`.
