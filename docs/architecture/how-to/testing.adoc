...
| `FactWriters` (`graphitron-model` test-jar)
...
| `CapturedStore` (`graphitron-model` test-jar)
| A real capture over a fixture document, with the fixture file, the caller-supplied graph identity, and named arms saying what each capture's inputs are. Below the line with the capture it drives, so a client can take it without taking the generator.

| A capture behind the generator's own attribution pipeline
| `PipelineCapturedStore` (`graphitron` test-jar)
| The same capture, with the pipeline's rewrites run first and both registries handed back, so a test can compare what capture wrote against what the rewrite put in front of it. The one capture arm that needs both tiers, which is why it is the one that stayed above the line.
...
| Whatever reader-side surface is local to you. `graphitron-mcp`'s `StoreFixture` is the worked example: every factory on it is one arm of `CapturedStore` or `FactWriters` with the module's own vocabulary in front of it, a generated jOOQ package name instead of a catalog and the module's own class census instead of a list the caller assembles. `graphitron-lsp`'s `StoreFixture` is the same shape.
|===

Where a build fixture goes is decided by the same question, one level up.
A client's fixture that runs a real build spans two tiers, so it lives above both rather than inside the client: `graphitron-maven-plugin` holds `StoreBackedBuild` and `BuiltStoreFixture`, and the tests that read them, because the dev mojo is what wires the generator to the two clients and that wiring is what those tests are about.
The clients publish test-jars so a relocated test keeps its own module's fixture vocabulary instead of a second copy that drifts, and neither client depends on `graphitron` at any scope; `StoreClientBoundaryTest` in each of them fails the build if one reacquires the edge, in main sources or in tests.
