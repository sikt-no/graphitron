...
discovered and extended, find a concept's grain and natural key first ; and keeps the
model a normalized fact base: each fact captured once at the boundary, everything downstream
...
xref:../explanation/typed-rejection.adoc[Typed rejection]; additive-then-cutover change discipline in
`roadmap/workflow.adoc`.

'''

== Decide once, at capture; carry the decision and its provenance as facts

*Before implementing a feature, ensure the model carries what the feature needs ; pre-resolved,
ready to consume.* Capture transcribes every external input once into the fact store's keyed base
relations ; provenance kept, not overwritten ; and the derived strata carry each decision as views
over that base, so the resolved answer never replaces the authored fact beneath it. During the
strangler window the classification walk (`GraphitronSchemaBuilder`) is the same boundary in
transitional form. Every consumer receives "what to do", never "what to interpret"; the modeling
discipline behind the store is xref:../explanation/fact-model.adoc[The fact model].
...
- A generator branches on a predicate over pre-resolved data ; the decision was not resolved,
  ...
  field, the branch belongs in the model.
...
=== The capture boundary is a containment invariant

"Decide once" is also a statement about *where* raw external types may live at all. At capture
the containment is structural: a relation stores values, so no live `Table<?>`, `ForeignKey`,
`Class<?>` or graphql-java schema object crosses into the store. `SdlFactCapture` and its
collaborators read each handle down to rows.

Off capture it is a convention: the rewrite and emit stages legitimately hold a registry or schema,
so the rule is direction, not a holder roster. A consumer of a *classification* switches on captured
values, and reaching back to a live object or a reflection `Type` tree for a fact the store could
hold is the smell. An undecoded fact earns a column or a component, extracted at the boundary.

*Enforced by:* the store's shape at capture (relations hold values, never live handles); off
capture, review against the reads themselves (`TypeDefinitionRegistry`, `GraphQLSchema`,
...
xref:../explanation/typed-rejection.adoc[Typed rejection]. In fact-base terms, rejections are facts too: located
...
hand-maintenance rather than derivation. The model's durable statement is
...
axis it forks on (xref:../explanation/dispatch-axes.adoc[Dispatch axes] narrates the split). The smell to watch
...
*Code generation is the narrowest view of the model, not the model itself.* Every consumer reads
views over the one base; none owns a private model, and each view's coverage guarantee is an
exhaustive gate over its own sealed vocabulary (`GeneratorCoverageTest`,
`TriggerDispatchMatrixTest`): a new variant breaks the build until every view covers it. The drift smell at model scale is a consumer-side shadow taxonomy; the re-sourcing
invariant is xref:../explanation/fact-model.adoc[The fact model].

*Enforced by:* the coverage gate at each seam.
...
vs decode) at the boundary, never below it. The worked example: nine "the model says this
is a NodeId" markers were retired in favour of decode and encode facts carried at the boundary
slots where each happens (see `roadmap/changelog.md` for the carrier-by-carrier story); cursor and
...
*Enforced by:* review only; the regression surface is pipeline-tier (a wire-shape carrier
...
arity-erased `RowN` where the decoder produced `Row<N><T1, ..., TN>`; the fix has shipped),
...
unguarded census) drifts silently. The worked example is an emit-side allow-list that drifted
...
`GraphitronField` sealed leaf: `TypeFetcherGenerator`'s `IMPLEMENTED_LEAVES`,
`NOT_DISPATCHED_LEAVES` and `STUBBED_VARIANTS.keySet()`, plus the projected bucket derived from
the projection producer's own membership declaration (`ProjectionCommands.CONTRIBUTION_MINTING_LEAVES`
minus the dual-arm kinds; safe to derive because the test below pins the partition and the
projection census test binds the declaration to observed minting), and `ValidateMojo` fails the
build on stubbed variants. The rule extends to every new
...
carry current prose contracts structurally are method type-token
threading and `ColumnBackedField.parentTable`. To record a producer-consumer linkage the type system can't
carry, use a javadoc `{@link}` from consumer to producer ; build-checked by the javadoc
reference gate. Rule: if you relax a producer's check body, audit every emitter site that
consumes the corresponding shape, in the same commit.

*Enforced by:* the type system where the contract is lifted; the javadoc reference gate for
the `{@link}` itself; the pipeline tier and the `graphitron-sakila-example` compile otherwise.
...
dispatch partition under "Rejections" is the pattern). Anything enumerable is first a materialized
view, regenerated and `--verify`-gated on every build (`supported-directives.adoc` is the
precedent); hand prose is the fallback for rationale only. Exemplars are chosen on
...
Every prose claim about code takes one of three honest forms: anchored on a checked reference
(a `{@link}` the reference gate verifies), pinned by a named live test or type, or written at
intent altitude ; the why and the shape ; so it survives mechanical refactors. Orientation
javadoc is the blessed intent-altitude case: a class-level on-ramp (scope, entry points, model
output, doc pointer; `GraphitronSchemaBuilder`'s top comment is the exemplar) orients without
narrating mechanics. The smell is narrow: line-by-line narration of a body, or prose
paraphrasing a mechanism ; a hand-maintained caller census ; true when written, silently
falsified as the code moves. Prefer deletion over rewrite.
...
`RetiredVocabularyGuardTest` for prose naming retired vocabulary.
...
written for the emitter author's convenience, not the consumer's. (Neither is cleaned up: the
NodeId-decode locals and the `@condition` arg-extraction descent are both live; see
xref:../reference/argument-resolution.adoc#nested-input-arg-extraction[Nested input-field arg
extraction].)
