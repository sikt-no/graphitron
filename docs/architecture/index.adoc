...
You want to *read the classification taxonomy*, every variant the transitional classification walk produces and what each drives. → xref:reference/code-generation-triggers.adoc[Code Generation Triggers]. For where that walk sits in the strangler migration, start at xref:explanation/pipeline-overview.adoc[Pipeline overview].
