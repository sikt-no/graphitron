...
The practical approach is PostgreSQL Row-Level Security with session variables set per request. See xref:architecture/runtime-extension-points.adoc[Runtime Extension Points] for how that wires through the per-request `DSLContext` you pass to `Graphitron.newExecutionInput(dsl, ...)`.
