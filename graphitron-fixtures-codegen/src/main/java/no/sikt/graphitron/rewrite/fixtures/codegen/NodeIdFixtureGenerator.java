...
public class NodeIdFixtureGenerator extends JavaGenerator {
     ...
     */
    private static final Map<String, Metadata> METADATA = Map.ofEntries(
        Map.entry("bar", new Metadata("Bar", List.of("ID_1", "ID_2"))),
        ...
        // naming a value — the shape the typeId-uniqueness reduction must catch by itself, and the
        // shape a mass promotion over a table family would produce. Every other typeId here is
        // distinct, so without this pair an inferred collision cannot be written at all.
        Map.entry("collide_a", new Metadata("195", List.of("KEY_A"))),
        Map.entry("collide_b", new Metadata("195", List.of("KEY_B"))),
        // Metadata keyed on a column other than the table's own `id` column, so the shadowed
        // column is not a key column: the encoded id is over KEY_X and `id` is reachable only
        // through @field.
        Map.entry("keyed_elsewhere", new Metadata("KeyedElsewhere", List.of("KEY_X"))),
        Map.entry("studieprogram", new Metadata("Studieprogram", List.of("STUDIEPROGRAM_ID"))),
        ...
        Map.entry("too_wide", new Metadata("TooWide", List.of(
        ...
        Map.entry("reordered_pk_parent", new Metadata("ReorderedPkParent", List.of("PK_A", "PK_B", "PK_C")))
    );
