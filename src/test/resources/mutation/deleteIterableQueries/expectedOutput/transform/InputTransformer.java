...
import fake.code.generated.mappers.DeleteInputMapper;
...
public class InputTransformer {
    ...
    public List<CustomerRecord> deleteInputToJOOQRecord(List<DeleteInput> input, String path) {
        var records = DeleteInputMapper.toJOOQRecord(input, path, arguments, ctx);
        return records;
    }

    public CustomerRecord deleteInputToJOOQRecord(DeleteInput input, String path) {
        return deleteInputToJOOQRecord(List.of(input), path).stream().findFirst().orElse(new CustomerRecord());
    }
