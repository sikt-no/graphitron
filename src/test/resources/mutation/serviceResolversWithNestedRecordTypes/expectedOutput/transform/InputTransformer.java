...
public class InputTransformer {
    ...
    public List<TestCustomerInnerInputRecord> editInputLevel2AToJavaRecord(
            List<EditInputLevel2A> input, String path) {
        var records = EditInputLevel2AJavaMapper.toJavaRecord(input, path, arguments, ctx, this);
        return records;
    }

    public List<TestCustomerInputRecord> editInputLevel2CToJavaRecord(List<EditInputLevel2C> input,
                                                                String path) {
        var records = EditInputLevel2CJavaMapper.toJavaRecord(input, path, arguments, ctx, this);
        return records;
    }
    ...
    public TestCustomerInnerInputRecord editInputLevel2AToJavaRecord(EditInputLevel2A input,
                                                                     String path) {
        return editInputLevel2AToJavaRecord(List.of(input), path).stream().findFirst().orElse(new TestCustomerInnerInputRecord());
    }

    public TestCustomerInputRecord editInputLevel2CToJavaRecord(EditInputLevel2C input, String path) {
        return editInputLevel2CToJavaRecord(List.of(input), path).stream().findFirst().orElse(new TestCustomerInputRecord());
    }
