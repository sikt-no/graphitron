...
public class AddressGeneratedResolver implements AddressResolver {
    ...
    public CompletableFuture<Store> store(Address address, String id, DataFetchingEnvironment env)
        ...
        DataLoader<String, Store> loader = env.getDataLoaderRegistry().computeIfAbsent("storeForAddress", name -> {
            var batchLoader = (MappedBatchLoaderWithContext<String, Store>) (keys, batchEnvLoader) -> {
                ...
                var dbResult = addressDBQueries.storeForAddress(ctx, idSet, id, selectionSet);
