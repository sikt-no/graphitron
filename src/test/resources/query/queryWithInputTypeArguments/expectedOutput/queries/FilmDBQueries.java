...
public class FilmDBQueries {
    public static Map<String, List<FilmCategory>> categoriesForFilm(DSLContext ctx,
        ...
        var film_film_film_category = FILM_CATEGORY.as("film_24625995");
        return ctx
                .select(
                        FILM.getId(),
                        DSL.row(
                                film_film_film_category.getId(),
                                select.optional("name", film_film_film_category.category().NAME)
                        ).mapping(Functions.nullOnAllNull(FilmCategory::new))
                )
                .from(FILM)
                .join(film_film_film_category)
                .onKey(FILM_CATEGORY__FILM_CATEGORY_FILM_ID_FKEY)
                .where(FILM.hasIds(filmIds))
                .and(film_film_film_category.LAST_UPDATED.eq(categoryIn.getLastUpdated()))
                .and(film_film_film_category.category().NAME.eq(categoryIn.getName()))
                .orderBy(film_film_film_category.getIdFields())
        ...
        return ctx
                ...
                .and(
                        categoryInList != null && categoryInList.size() > 0 ?
                                DSL.row(
                                        film_film_film_category.LAST_UPDATED,
                                        film_film_film_category.category().NAME
                                ).in(
        ...
        return ctx
                ...
                .and(
                        categoryInListOneField != null && categoryInListOneField.size() > 0 ?
                                DSL.row(film_film_film_category.LAST_UPDATED).in(
