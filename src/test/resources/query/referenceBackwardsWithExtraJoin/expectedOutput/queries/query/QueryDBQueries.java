...
public class QueryDBQueries {
    public static City cityForQuery(DSLContext ctx, String id, SelectionSet select) {
        ...
        var city_3312333495_address_left = city_customerstore_store_left.address().as("address_1503902379");
        return ctx
                .select(
                        DSL.row(
                                CITY.getId(),
                                DSL.row(
                                        city_customerstore_store_left.getId(),
                                        DSL.row(city_3312333495_address_left.getId()).mapping(Functions.nullOnAllNull(Address::new)),
                                        DSL.row(city_3312333495_address_left.getId()).mapping(Functions.nullOnAllNull(Address::new))
                ...
                .leftJoin(city_3312333495_address_left)
                .where(CITY.ID.eq(id))
