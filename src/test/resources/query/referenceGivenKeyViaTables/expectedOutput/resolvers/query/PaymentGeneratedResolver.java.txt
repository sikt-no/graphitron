...
public class PaymentGeneratedResolver implements PaymentResolver {
    ...
    public CompletableFuture<Language> originalLanguage(Payment payment,
        ...
        DataLoader<String, Language> loader = env.getDataLoaderRegistry().computeIfAbsent("originalLanguageForPayment", name -> {
            var batchLoader = (MappedBatchLoaderWithContext<String, Language>) (keys, batchEnvLoader) -> {
                ...
                var dbResult = paymentDBQueries.originalLanguageForPayment(ctx, idSet, selectionSet);
