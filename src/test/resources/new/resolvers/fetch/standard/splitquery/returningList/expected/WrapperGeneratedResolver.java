...
public class WrapperGeneratedResolver implements WrapperResolver {
    ...
    @Override
    public CompletableFuture<List<DummyType>> query1(Wrapper wrapper, DataFetchingEnvironment env)
            throws Exception {
    ...
    @Override
    public CompletableFuture<List<DummyType>> query2(Wrapper wrapper, DataFetchingEnvironment env)
            throws Exception {
        return new DataFetcher(env, this.ctx).loadNonNullable("query2ForWrapper", wrapper.getId(), (ctx, ids, selectionSet) -> WrapperDBQueries.query2ForWrapper(ctx, ids, selectionSet));
    }
}
