...
public class QueryGeneratedResolver implements QueryResolver {
    ...
    @Override
    public CompletableFuture<DummyConnection> query(Integer first, String after, String other,
                                                     DataFetchingEnvironment env) throws Exception {
        int pageSize = ResolverHelpers.getPageSize(first, 1000, 100);
        return new DataFetcher(env, this.ctx).loadPaginated(
                pageSize, 1000,
                (ctx, selectionSet) -> QueryDBQueries.queryForQuery(ctx, other, pageSize,after, selectionSet),
                (ctx, ids) -> QueryDBQueries.countQueryForQuery(ctx, other),
                (it) -> it.getId(),
                (connection) ->  {
