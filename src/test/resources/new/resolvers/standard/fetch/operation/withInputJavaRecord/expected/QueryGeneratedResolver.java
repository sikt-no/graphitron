...
public class QueryGeneratedResolver implements QueryResolver {
    ...
    @Override
    public CompletableFuture<Target> query(Input in, DataFetchingEnvironment env) throws Exception {
        var transform = new RecordTransformer(env, this.ctx);

        var inRecord = transform.inputToJavaRecord(in, "in");

        return new DataFetcher(env, this.ctx).load((ctx, selectionSet) -> QueryDBQueries.queryForQuery(ctx, inRecord, selectionSet));
    }

    @Override
    public CompletableFuture<List<Target>> queryListed(List<Input> in, DataFetchingEnvironment env)
            throws Exception {
        var transform = new RecordTransformer(env, this.ctx);

        var inRecordList = transform.inputToJavaRecord(in, "in");
