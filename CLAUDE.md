...
The `mvn install -Plocal-db` command above runs the full pipeline (build-fixtures → test → compile-spec → execute-spec). That full install is the **verification build**: the check a tree passes before it is pushed. Prefer it over targeted `-pl` builds for anything you intend to trust as verification, and never push a tree it did not cover. Coverage survives a rebase by judgment, not by ritual: every commit already on trunk passed its own verification build, so when a rebase brings in only changes that plainly cannot interact with yours (disjoint files, no shared build surface: say another item's roadmap metadata against your generator change), the two green builds jointly cover the merged tree and yours carries forward; where a cheap targeted check exists for the one touching point (a `mvn -pl roadmap-tool exec:java -q` regenerate after concurrent roadmap edits), run it instead of a full rebuild. The criterion is symmetric interaction, not which side carries code: incoming generator code against your markdown-only commit carries forward just as cleanly as the reverse. Rebuild before pushing when either side touches build configuration or anything the other side reads, or when in doubt. The judgment covers incoming rebased changes only: a new commit of your own always needs a verification build that covers it. The session flow in "Git Workflow" below and the `publish` skill both lean on this definition rather than restating it.

**A roadmap-only diff owes a scoped build, not the whole reactor.** A tree whose own commits are entirely under `roadmap/` is verified by `mvn verify -pl roadmap-tool,docs`, which runs every build step that reads `roadmap/` and nothing else. One file outside `roadmap/` in your own commits and you owe the full build. Three precisions, or the rule gets misread:

- `verify`, not `install`, and no `-Plocal-db`: neither module declares that profile, so passing it earns a Maven warning and nothing else. The catalog-jar footgun below lives in `graphitron-sakila-db`, which this scope does not touch.
- Like every scoped `-pl` command here it reads the *installed* artifacts of upstream modules, so it assumes a prior full install in the session. For a roadmap-only diff that is sound rather than a caveat: nothing you changed is upstream of these two modules, so a stale upstream cannot mask your change.
- The predicate is on **your own commits**, not on the tree, so it composes with the carry-forward judgment above rather than replacing it.

This is not "skip the build for markdown": both halves of the scope catch faults that happen. A stale `roadmap/README.md` fails `verify-roadmap-readme`, and an item body is rendered into the published site, so its prose can fail `check-adoc-xrefs` in `graphitron-docs`. What holds the rule up is that no build step and no test outside those two modules reads roadmap content a roadmap-only diff can change; `check-roadmap-consumers` and the roadmap-consumer guard in `graphitron`'s test tier fail the build when a third module starts naming a path into `roadmap/`.
