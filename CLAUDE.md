...
Every change moves Backlog → Spec → Ready → In Progress → In Review → Done. Each item has its own file in [`graphitron-rewrite/roadmap/`](graphitron-rewrite/roadmap/) carrying YAML front-matter (`status:`, `bucket:`, etc.); [`graphitron-rewrite/roadmap/README.md`](graphitron-rewrite/roadmap/README.md) is the rendered roll-up, regenerated by `mvn -pl :graphitron-rewrite-roadmap-tool exec:java`. Reviewer must be a different party than the author (for Spec → Ready) and the implementer (for In Review → Done). Any session can add Backlog items.
