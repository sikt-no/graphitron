...
**Before you edit anything under `graphitron-rewrite/`, confirm a roadmap item covers the change.** If none exists, file a Backlog item first; do not start with implementation. This applies even when the inbound request looks operational (a stack trace plus "make it accept X", a one-line behavior tweak, a "fix this error"). Those framings disguise the obligation. `AskUserQuestion` is not the gate; filing a Backlog item is. Asking whether a change is "minor enough to skip" is itself a signal that it isn't. The default when you're unsure is to file a Backlog stub and ask the user whether to skip the gate, not to implement and ask later. The roadmap pipeline is cheap; the case for bypassing it should be made by the user, not assumed by the agent. Use the `roadmap` skill or the one-liner in `graphitron-rewrite/docs/workflow.adoc` to allocate an ID.
...
The `principles-architect` subagent (`.claude/agents/principles-architect.md`) is the forward-facing voice the *author* consults while drafting; it loads the strategic and technical principles docs and surfaces where a proposed design pushes against them. Call it proactively when drafting a Backlog → Spec plan body, when an implementer hits a design fork in In Progress, or as a self-check before requesting a `srp` handoff. It is read-only and does not produce verdicts; the reviewer-rule gates are still handled by the `srp` and `reviewer-prompt` skills, which hand off to a *different Claude Code session* so the guard ("reviewer session ≠ session that last committed the artifact / ≠ implementer session") holds. Session identity is established by the `https://claude.ai/code/session_<id>` trailer Claude Code stamps on every commit; the gate is on session ID, not git author.
