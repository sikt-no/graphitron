...
- Shared header, in order: a `crumb` breadcrumb back-link to `../README.md`, a `kicker` line ("Concept explainer · theme: &lt;theme&gt;"), then the `<h1>`.
...
1. **Resolve the entry point.** An item ID: read that item plus everything it links (sibling items, changelog entries, architecture and manual pages). A concept slug: additionally grep `roadmap/*.md` for items referencing the concept.
