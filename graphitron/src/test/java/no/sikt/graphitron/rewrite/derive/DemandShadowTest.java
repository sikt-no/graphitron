...
class DemandShadowTest {
    ...
    private static int domainFieldCoordinateCount(DSLContext dsl, String graphName) {
        return dsl.fetchCount(dsl
            .select(GRAPHITRON_FIELD.TYPE_NAME, GRAPHITRON_FIELD.FIELD_NAME)
            .from(GRAPHITRON_FIELD)
            .join(INTENT_TYPE_DOMAIN)
            .on(INTENT_TYPE_DOMAIN.GRAPH_NAME.eq(GRAPHITRON_FIELD.GRAPH_NAME)
                .and(INTENT_TYPE_DOMAIN.TYPE_NAME.eq(GRAPHITRON_FIELD.TYPE_NAME)))
            ...
            .and(GRAPHITRON_TYPE.KIND.in(FIELD_BEARING_KINDS)));
    }
