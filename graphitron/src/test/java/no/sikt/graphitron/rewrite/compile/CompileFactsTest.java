...
class CompileFactsTest {
    ...
    void roundTripsThroughAStoreHandle(@TempDir Path tmp) {
        try (var store = GraphitronModelStore.open()) {
            ...
            writer(store, "dev", tmp).write(new CompileRound(false, List.of(ERROR, noPosition)));
    ...
    void aSecondRoundReplacesTheFirstWholesale(@TempDir Path tmp) {
        try (var store = GraphitronModelStore.open()) {
            var writer = writer(store, "dev", tmp);
    ...
    void aRoundLeavesASecondGraphsRowsAlone(@TempDir Path tmp) {
        try (var store = GraphitronModelStore.open()) {
            writer(store, "sibling", tmp.resolve("sibling")).write(
            ...
            var writer = writer(store, "dev", tmp.resolve("dev"));
            writer.write(new CompileRound(false, List.of(ERROR)));
            writer.write(new CompileRound(true, List.of()));
    ...
    void ordinalsDisambiguateIdenticalPositions(@TempDir Path tmp) {
        try (var store = GraphitronModelStore.open()) {
            writer(store, "dev", tmp).write(new CompileRound(false, List.of(ERROR, ERROR, ERROR)));
    ...
    void anotherCheckoutsPartitionIsLeftAlone(@TempDir Path tmp) {
        try (var store = GraphitronModelStore.open()) {
            ...
            writer(store, "dev", tmp.resolve("theirs")).write(new CompileRound(false, List.of(ERROR)));
            var before = store.dsl().selectFrom(JAVAC_DIAGNOSTIC).fetch().map(Object::toString);

            writer(store, "dev", tmp.resolve("ours")).write(new CompileRound(true, List.of()));
    ...
    void theWriterAnchorsItsGraphWhenNoCaptureHas(@TempDir Path tmp) {
        try (var store = GraphitronModelStore.open()) {
            writer(store, "dev", tmp).write(new CompileRound(false, List.of(ERROR)));
    ...
    private static CompileFacts writer(GraphitronModelStore store, String graphName, Path baseDir) {
        return new CompileFacts(store.dsl(), new FactCapture.GraphIdentity(graphName, baseDir));
    }
}
