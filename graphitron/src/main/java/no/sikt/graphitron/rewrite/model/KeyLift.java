 ...
 * DataLoader batch key off the parent's held object. The mechanism only, saying nothing about
 ...
 * the enclosing {@link GraphitronType.ResultType}). The axis is total on the batched leaves:
 * a table-parent {@code @splitQuery} field lifts by column projection ({@link FkColumns}), the
 ...
 * Table-sourced emit path is wrap-driven ({@code GeneratorUtils.buildKeyExtraction} against
 * the parent's own table row; re-deriving the read through the lift would be a second spelling
 * of the same projection), so the Table arm's stored {@link FkColumns} is a constructor-checked
 * totality fact ({@link #checkResidueAgreement}), not an emit input. {@code @service}-backed
 * fields never carry a lift: their key extraction is authored by the {@code Sources} signature.
 *
 ...
 * {@link SourceKey}s on lift-carrying leaves are constructed through this derivation; the leaf
 * constructors pin the rule via {@link #checkResidueAgreement}.
 */
public sealed interface KeyLift {
     ...
     * lift arm's {@link #wrap()} derivation, so a hand-built instance cannot pair a lift arm
     * with a mismatched wrap. Called from the compact constructors of the carrying
     * {@link ChildField} variants; a violation is a classifier-side bug.
     */
