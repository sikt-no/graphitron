...
/**
 * Build-time synthesis of {@code @key(fields: "id", resolvable: true)} on every node
 ...
 * composer sees the entity declaration. Node types are entities by definition (they
 ...
 *
 * <p>Nodehood comes from {@link NodeDeclaration}, so a node inferred from {@code implements Node}
 * plus catalog metadata is an entity on the same terms as a declared {@code @node}. This stage runs
 * on the raw {@link TypeDefinitionRegistry} while the attributed registry is being assembled, long
 ...
 *
 * <h3>Opt-out</h3>
 * <p>A consumer who writes {@code @key(fields: "id", resolvable: false)} on a node
...
public final class KeyNodeSynthesiser {
     ...
     * {@code @key(fields: "id", resolvable: true)} directive to every node
     ...
     */
    public static void apply(TypeDefinitionRegistry registry, NodeDeclaration nodes) {
     ...
     * needs the {@code "id"} alternative present for a node's implicit contract, and
