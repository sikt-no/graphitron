 ...
 * {@link no.sikt.graphitron.rewrite.model.GraphitronType.NodeType} membership were the same set, so
 ...
 * ({@link SchemaReachability}), arrival folding ({@link ArrivalIndex}), federation entity membership
 * ({@link no.sikt.graphitron.rewrite.schema.federation.KeyNodeSynthesiser}), and the LSP node view
...
public final class NodeDeclaration {
     ...
     *                context that has none (the same nullable-catalog {@link BuildContext} accepts
     ...
     * otherwise the lowercased GraphQL type name. Shared with {@link TypeBuilder} so the gate and
