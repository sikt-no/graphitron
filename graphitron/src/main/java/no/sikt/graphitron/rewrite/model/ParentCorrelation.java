 ...
 * {@code LookupTableField} / {@code ColumnBackedReferenceField} step-0; the batched launcher
 * renderer's correlation switch) read
 ...
 * arm pairs only with an empty {@code joinPath}, so the model can never carry a correlation that
 * disagrees with the path it sits on. The hop-anchored arms are denormalised views of data already
 * on {@code joinPath.get(0)} plus the carrier field's parent type's {@code @table} binding,
...
public sealed interface ParentCorrelation
     ...
     * arm (the pre-keyed lifter / accessor / re-fetch shape). Each ChildField variant compact
    ...
    /**
     * The pre-keyed correlation: the DataLoader key tuple <em>is</em> the target-column tuple;
    ...
    /**
     * The parent-anchor arm: the batch anchors the parent table and hop 0 attaches off it. In the
     * split-rows shape, {@code parentInput} joins the parent table on its PK, then hop 0 attaches
     * to {@code firstHop} off {@code parentAlias}; the parent PK grain (see
     * {@link #parentKeyColumns()}) is what makes that anchoring correct. This arm carries only the
     * topology payload ({@code firstHop}, {@code parentTable}) and deliberately exposes <em>no</em>
     * {@code condition()} accessor: a partial accessor whose meaning depends on the hop's occupant
     * would re-couple the two axes. Consumers dispatch the hop-0 attach on {@code firstHop().on()}
     * per {@link JoinStep}'s own two-axis model:
     *
     * <ul>
     ...
     *       ({@code method(parentAlias, firstAlias)}).</li>
     * </ul>
     *
