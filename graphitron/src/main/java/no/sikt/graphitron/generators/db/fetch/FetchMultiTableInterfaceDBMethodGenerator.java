...
public class FetchMultiTableInterfaceDBMethodGenerator extends FetchDBMethodGenerator {
    ...
    private CodeBlock createMapping(ObjectField target, List<ObjectDefinition> implementations) {
        var code = CodeBlock.builder();
        var mapping = createMappingContent(processedSchema.getInterface(target).getGraphClassName(), implementations);
    ...
    private CodeBlock createMappingContent(ClassName interfaceClassName, List<ObjectDefinition> implementations) {
        var code = CodeBlock.builder()
                .indent()
