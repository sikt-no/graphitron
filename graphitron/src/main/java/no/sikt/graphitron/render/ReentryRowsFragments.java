...
/**
 * The DML reentry companion's body fragments: the follow-up SELECT a mutation runs against its
 * {@code RETURNING}-captured keys, rendered from the row's {@link LaunchSource.Reentry} arm.
 * The companion resolves its correlation (the carried
 ...
 * ordered by {@code idx} so the payload aligns one-to-one and in order with the
 * {@code RETURNING} result); the single arm renders the legible degenerate, plain key equality,
 * with no VALUES table and no ORDER BY.
 *
 * <p>Deliberately no empty-input gate: the write emitter owns the no-match guard (a single-row
 * write with no match short-circuits before the call; a list write's empty-input contract
 * short-circuits before the transaction), so the companion is only ever called with captured
 * keys, and adding a gate here would duplicate a contract another emission already owns.
 */
public final class ReentryRowsFragments {
     ...
     */
    private static final String ROW_CONTEXT = "@mutation @table-return reentry key";

    /**
     * The companion's {@code keys} parameter type, derived from the row's one correlation fact
     * (the same fact the write emitter derives its {@code RETURNING} column list from, so the
     * assignment compatibility across the generated call boundary is structural): the typed
     * {@code RecordN} key row, lifted to {@code Result} for the list shape.
     */
