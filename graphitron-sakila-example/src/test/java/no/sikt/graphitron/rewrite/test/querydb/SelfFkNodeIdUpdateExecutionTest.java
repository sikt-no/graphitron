...
class SelfFkNodeIdUpdateExecutionTest {
     ...
     */
    private static final int MESSAGE_NO_BAND_START = 200;
    private static final int MESSAGE_NO_BAND_END = 299;
    ...
    void cleanUpUpdatedRows() {
        // Each test owns a distinct message_no in this class's band, and the cleanup names both
        // ends of it. Four classes in this module write `email`, each owning a hundred-wide band;
        // an open-ended `>= 100` would reach into SelfFkNodeIdInsertExecutionTest's band and delete
        // its in-flight rows whenever the two ran at the same time.
