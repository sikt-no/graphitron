 ...
 * {@code GraphitronSchemaValidator} writes. Each
...
public sealed interface Rejection permits Rejection.AuthorError, Rejection.InvalidSchema, Rejection.Deferred {
    ...
    sealed interface AuthorError extends Rejection permits AuthorError.UnknownName, AuthorError.Structural, AuthorError.AccessorMismatch, AuthorError.RecordBindingMultiProducer, AuthorError.TypeConflict, AuthorError.MultiProducerDomainTypeDisagreement, AuthorError.SortEnumMissingOrder, AuthorError.TenantColumnTypeDisagreement, AuthorError.NoTenantBinding, ServiceMethodCallError, ReflectionError, UpdateRowsError, DeleteRowsError, MutationTableArgError, ErrorChannelWalkerError, WireCoercionError, ServiceCarrierShapeError, PivotError, JooqRecordInputError {
         ...
         * {@code ClassAccessorResolver}; the {@code reason} carries the
         ...
         * {@code RecordBindingResolver}'s per-SDL-type fold when the
        ...
        record RecordBindingMultiProducer(String sdlTypeName, List<Binding> bindings)
             ...
             *
             * <p>Named rather than carrying the {@code ProducerBinding} it came from, on the rule
             ...
             * live {@link Class} that {@code ProducerBinding#reflectedClass()} holds could not
         ...
         * {@code MethodRef.Param.Typed} whose source is {@code ParamSource.Context} keys by
         ...
         * {@code ContextArgumentClassifier} when the per-name fold finds
        ...
        /**
         * Two or more {@code OutputField} producers reach the same SDL return type with
         * disagreeing {@code DomainReturnType} arms. Each producer's emitted resolver puts a
         ...
         * {@code GraphitronSchemaBuilder}.
         *
         * <p>The same rejection is attached to every demoted producer in the group, so each
         * surfaces independently in the validator's per-{@code GraphitronField.UnclassifiedField}
        ...
        record MultiProducerDomainTypeDisagreement(
             ...
             * {@code DomainReturnType} arm, on the rule a rejection follows: a rejection is a fact
         ...
         * skipped by {@code OrderByResolver}, producing no
         * {@code OrderBySpec.NamedOrder}; a request selecting only unannotated values then generates
    ...
    sealed interface InvalidSchema extends Rejection permits InvalidSchema.DirectiveConflict, InvalidSchema.CaseFoldCollision, InvalidSchema.Structural {
         ...
         * to {@code GraphitronType.UnclassifiedType} carrying its own {@code CaseFoldCollision}
         * with this same {@code group} and the demoted member's {@code origin}, so the validator
         * surfaces one {@link no.sikt.graphitron.model.diagnostics.ValidationError} per member from either
         * entry point.
         *
         ...
         *
         * <p>Producer: {@code GraphitronSchemaBuilder} case-fold
         * uniqueness pass.
         */
    ...
    /**
     * Recognised but not yet generator-supported. {@code summary} is the whole
     * human-readable message: a deferred rejection states the fact that the
     * feature is not yet generated and stands alone, so {@link #message()} is a
     * pure function of {@code summary} and composes no further prose.
     * {@code stubKey} is the live variant-class anchor the generator's
     * {@code TypeFetcherGenerator.STUBBED_VARIANTS} stubbing keys on.
     */
    record Deferred(
        String summary,
        StubKey stubKey
        ...
        @Override public Rejection prefixedWith(String prefix) {
            return new Deferred(prefix + summary, stubKey);
        }
    }
     ...
     * {@link no.sikt.graphitron.model.derive.AuthoredClaimRejectionRows}) reach one rule and a
    ...
    /**
     * {@link Deferred} factory for sites with no specific variant-class anchor. Equivalent to
     * {@link #deferred(String, Class)} with {@code fieldClass} = {@code null}.
     */
    static Rejection deferred(String summary) {
        return new Deferred(summary, new StubKey.VariantClass(null));
    }
