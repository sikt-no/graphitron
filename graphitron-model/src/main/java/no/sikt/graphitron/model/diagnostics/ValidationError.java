...
/**
 * A schema validation error produced by {@code GraphitronSchemaValidator}.
 *
 * <p>{@code location.getSourceName()} carries the source file path, populated automatically
 * when the schema is parsed via {@code SchemaLoader} (which uses
...
public record ValidationError(String coordinate, Rejection rejection, SourceLocation location) {
     ...
     * convention, shared by {@code GraphitronSchemaValidator}'s {@code validateUnclassifiedType}
