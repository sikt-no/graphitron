...
public class JooqCatalog {
     ...
     * diagnostic.
     */
    public static void verifyTablesClassPresent(Schema schema, ClassLoader codegenLoader) {
        verifyTablesClassPresent(schema.getName(), schema.getClass().getPackageName(), codegenLoader);
    }

    public static void verifyTablesClassPresent(String schemaName, String packageName, ClassLoader codegenLoader) {
        try {
     ...
     * {@code GraphitronType.UnclassifiedType}.
     */
     ...
     * so a stray-dot value degrades to the ordinary {@link RoutineResolution.NotInCatalog} rejection.
     *
     ...
     * or {@link RoutineResolution.NotInCatalog}), identity cannot fire, so the predicate falls back to a bare
     ...
     * <em>orientation</em> predicate {@code BuildContext#synthesizeFkJoin} and
     * {@code BuildContext#resolveFkSlots} share to decide join/population direction.
     *
     ...
     * {@code BuildContext#synthesizeFkJoin} by class identity instead of round-tripping through a
     ...
     * the table SQL name; the classifier boundary surfaces it as an {@code GraphitronType.UnclassifiedType} via
     ...
     *
     * <p>Classifier usage: call this first; if present, fail the type as {@code GraphitronType.UnclassifiedType}
     ...
     */
    public record StatedConstant(boolean declared, Object value) {
        public static StatedConstant absent() {
            return new StatedConstant(false, null);
        }

        public static StatedConstant of(Object value) {
     ...
     */
    public static NodeMetadataFacts reduceNodeMetadata(StatedConstant typeId, StatedConstant keyColumns) {
     ...
     */
    public static Optional<NodeIdMetadata> validateNodeIdMetadata(
            String tableSqlName,
            Object typeIdRaw,
     ...
     */
    public static String generateRoleName(List<String> sourceColumns, List<String> targetColumns) {
    ...
    public record TableEntry(String javaFieldName, Table<?> table, JooqCatalog catalog) {
         ...
         * {@link no.sikt.graphitron.model.jooq.TableRef#allColumns()}: the interface base/detail
        ...
        /**
         * Materialises a {@link no.sikt.graphitron.model.jooq.TableRef} from this entry. The
         ...
         */
        public no.sikt.graphitron.model.jooq.TableRef toTableRef(String sqlName) {
            ...
            return new no.sikt.graphitron.model.jooq.TableRef(
                sqlName,
                javaFieldName,
                tableClassName(),
                recordClassName(),
     ...
     * {@code BuildContext#synthesizeFkJoin} distinguish "endpoint
