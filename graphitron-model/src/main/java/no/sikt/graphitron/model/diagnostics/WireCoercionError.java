 ...
 * {@code CallSiteExtraction.Direct}; a mismatch surfaces here instead.
 *
    ...
    WireCoercionError.NodeIdDecodedType
{
     ...
     * type at the call site, so the generated {@code (declaredType) wireValue} cast would throw
     * at runtime. Class names are fully qualified; {@code site} names where the leaf lives (for
     * the message and LSP related-information).
     */
     ...
     * {@code IllegalArgumentException} at runtime for those values. Populated from the
     * {@code EnumMappingResolver} parity result so the value-name comparison is single-homed
     * with the column/arg enum path.
     */
