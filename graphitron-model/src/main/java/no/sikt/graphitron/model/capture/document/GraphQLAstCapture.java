 ...
 *
 * <p>The anchors are one statement each, an insert over a select over the entry rows, so the
 * derivation is the statement rather than a walk that reads rows into Java and writes them back.
 * Nothing is read into memory there and nothing is decided there.
 *
 * <p>They do not wait for the corpus to build. They come from the entries, so a corpus graphql-java
 * refused still has its anchors, which is the ordinary state of a schema while somebody is editing
 * it. Whether it built is {@code graphql_schema_problem}'s to say.
 *
 * <p>The anchors' own sweep is per graph, not per file, which is what makes it an anchor's sweep: a
 * coordinate survives while any document still declares it. It takes the graphitron rows hanging
 * off a coordinate it drops, those references cascading, which is what lets this gatherer run ahead
 * of the decode rather than behind an emptying pass.
 *
...
public final class GraphQLAstCapture {
     ...
     */
    public static void anchor(DSLContext dsl, String graph, LocalDateTime readAt) {
        elements(dsl, graph, readAt);
        typeElements(dsl, graph, readAt);
        ...
        enumValues(dsl, graph, readAt);
        fieldArguments(dsl, graph, readAt);
        unionMembers(dsl, graph, readAt);
        implementsInterfaces(dsl, graph, readAt);
        directives(dsl, graph, readAt);
