 ...
 * </ul>
 *
 * <p>Both arms flow into {@code ValidationReport} exactly as warnings did before sealing, so the LSP
 * replay and the MCP projection need no new transport. {@code location.getSourceName()} carries the
 * source file path when the schema was parsed via {@code SchemaLoader}.
 */
