...
COMMENT ON COLUMN intent_class_assignable.supertype_name IS 'the fully-qualified binary name of a type the subtype can stand in for, a nested one spelled with the $ the JVM uses. Frequently not a census row at all, which is the ordinary case at the end of a chain and the reason the relation this closes over carries no foreign key on it either';
