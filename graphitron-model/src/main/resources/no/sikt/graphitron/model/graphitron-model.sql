...
INSERT INTO meta_materialize VALUES
  ('intent_node_id_instruction_live', 'intent_node_id_instruction',
   'Every relation resolving what a @nodeId instruction means is a multiple of this one, and the deepest of them reached 2528 instantiations in a single read against the tree''s previous worst of 765, because H2 inlines a view wherever it is named and the chain names this one three times over. The rule is correct as a view and only ruinous to re-evaluate, which is the registration case; the impossibility case would have to argue no view could state it, and one does.'),
