...
INSERT INTO meta_materialize VALUES
  ...
  ('intent_node_id_instruction_live', 'intent_node_id_instruction',
   'Three view bodies name this relation and one of them, the decode slot, names it twice through a local alias that is itself named twice, so a single read of the slot relation expands to several whole evaluations of this rule. Its own cost is internal rather than inherited: measured against a consumer schema with 95 classpath sources and a 388-row catalog census, one evaluation is 26 s while every relation it reads answers in under a second, the whole of it in one union arm that joins a local alias to a second local alias derived from the first. H2 inlines a non-recursive WITH exactly like a view and eliminates no common subexpression, so that inner alias is recomputed once per driving row, and no rewrite restores the sharing: widening the inner alias to carry the outer one''s columns measured 94 s, driving the arm from the inner alias measured 66 s, and spelling the join''s null-safe comparisons as IS NOT DISTINCT FROM changed nothing. Snapshotting the inner alias into a table put the arm at 0.7 s, which is the shape of the fix and the reason this is a registration rather than a rewrite. What it buys downstream is the whole of the reason: the decode slot relation went from not answering in 400 s to 278 ms with these rows stored, and the decode defect view a build reads sits directly on it, so before this registration a consumer with a schema this size had a build that did not finish. Refresh is one evaluation of the rule per capture and it is the most expensive refresh in this registry; the narrower registration that would cut it is the inner alias, which is a local alias rather than a named relation today and wants promoting to one before it can be registered.');
