...
-- mostly cannot: the bulk of the grains include a column that is nullable and meaningfully so,
-- and H2 refuses a primary key over one. So the index is an index, and where it exists it is declared
...
COMMENT ON COLUMN intent_argument_scope_table.basis IS 'which rung answered, carried unchanged from intent_field_scope_table, whose comment says what the two values mean. Every argument of one field carries the same value, the rung being the field''s and not the argument''s; a reader wanting one rung filters on it and owns having chosen';
...
CREATE VIEW intent_errors_field_live
...
COMMENT ON VIEW intent_errors_field_live IS 'This states the rule and is evaluated on demand. The canonical name intent_errors_field beside it is the table this view is materialized into on the capture cadence, which is what every reader spells and what the registration in meta_materialize records; a reader naming this relation instead is asking for on-demand evaluation and will get it. The rule itself, and what each column means, is documented on intent_errors_field.';
COMMENT ON COLUMN intent_errors_field_live.graph_name IS 'the graph_name of a row of this rule, materialized into intent_errors_field.graph_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_errors_field_live.type_name IS 'the type_name of a row of this rule, materialized into intent_errors_field.type_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_errors_field_live.field_name IS 'the field_name of a row of this rule, materialized into intent_errors_field.field_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_errors_field_live.ordinal IS 'the ordinal of a row of this rule, materialized into intent_errors_field.ordinal, whose comment carries what the value means';
COMMENT ON COLUMN intent_errors_field_live.container_name IS 'the container_name of a row of this rule, materialized into intent_errors_field.container_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_errors_field_live.container_kind IS 'the container_kind of a row of this rule, materialized into intent_errors_field.container_kind, whose comment carries what the value means';

CREATE TABLE intent_errors_field (
  graph_name     VARCHAR NOT NULL,
  type_name      VARCHAR,
  field_name     VARCHAR,
  ordinal        INTEGER,
...
COMMENT ON INDEX ix_argmapping_pair_use_site IS 'Serves the use-site coordinate every reader of a pair''s resolution departs from: intent_argmapping_projection_defect joins this relation on these four columns in six of its arms, intent_node_id_decode_slot in one, and both reach it through readers of their own. The smallest claim of the four coordinate indexes here, and stated as what this index alone is worth rather than as what its readers gained, because those two figures are far apart and the larger one is not this index''s doing. Measured on the read-cost gate''s twelve-unit fixture with statistics current, declaring this index on top of the rest: intent_argmapping_projection_defect goes from 22014 scans to 21394, intent_argmapping_key_column_candidate from 5328 to 5248, intent_resolved_node_key_projection from 8274 to 8194, intent_argmapping_binding_leaf from 178 to 138. Those readers do fall from 33794, 8273 and 8274 across the five indexes together, and almost all of that is the spelling index rather than this one. Three per cent is a thin claim and it is made on the shape as much as the figure: four readers, no reader dearer, and the coordinate is the one this relation''s own comment says every reader of a pair''s resolution departs from. The other coordinate a reader spells here, the routine parameter name intent_argmapping_bound_parameter_type''s NOT EXISTS matches on, was measured as a second index and bought ten scans on a sixty-nine scan relation, which is the instrument''s per-naming floor rather than a cost, so it was not declared. Not UNIQUE: the site literal and the use-site key are a discriminated coordinate rather than this relation''s grain, whose tail is the pair''s own position and parameter.';
...
SELECT f.graph_name, 'INPUT_FIELD', f.type_name, f.field_name, NULL, p.path, p.path,
 ...
 WHERE f.field_name = 'id' AND f.named_type = 'ID' AND tn.candidates = 1
   AND NOT EXISTS (SELECT 1 FROM graphitron_field_node_id n
                    WHERE n.graph_name = f.graph_name AND n.type_name = f.type_name
                      AND n.field_name = f.field_name);
COMMENT ON VIEW intent_node_id_instruction_live IS 'This states the rule and is evaluated on demand. The canonical name intent_node_id_instruction beside it is the table this view is materialized into on the capture cadence, which is what every reader spells and what the registration in meta_materialize records; a reader naming this relation instead is asking for on-demand evaluation and will get it. The rule itself, and what each column means, is documented on intent_node_id_instruction.';
COMMENT ON COLUMN intent_node_id_instruction_live.graph_name IS 'the graph_name of a row of this rule, materialized into intent_node_id_instruction.graph_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_instruction_live.site IS 'the site of a row of this rule, materialized into intent_node_id_instruction.site, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_instruction_live.type_name IS 'the type_name of a row of this rule, materialized into intent_node_id_instruction.type_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_instruction_live.field_name IS 'the field_name of a row of this rule, materialized into intent_node_id_instruction.field_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_instruction_live.argument_name IS 'the argument_name of a row of this rule, materialized into intent_node_id_instruction.argument_name, whose comment carries what the value means';
...
COMMENT ON COLUMN intent_node_id_instruction_live.source_column IS 'the source_column of a row of this rule, materialized into intent_node_id_instruction.source_column, whose comment carries what the value means';

CREATE TABLE intent_node_id_instruction (
  graph_name             VARCHAR NOT NULL,
  site                   VARCHAR,
  type_name              VARCHAR,
  field_name             VARCHAR,
