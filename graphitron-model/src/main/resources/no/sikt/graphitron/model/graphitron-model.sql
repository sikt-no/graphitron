...
CREATE VIEW intent_node_id_decode_endpoint_live
...
COMMENT ON VIEW intent_node_id_decode_endpoint_live IS 'This states the rule and is evaluated on demand. The canonical name intent_node_id_decode_endpoint beside it is the table this view is materialized into on the capture cadence, which is what every reader spells and what the registration in meta_materialize records; a reader naming this relation instead is asking for on-demand evaluation and will get it. The rule itself, and what each column means, is documented on intent_node_id_decode_endpoint.';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.graph_name IS 'the graph_name of a row of this rule, materialized into intent_node_id_decode_endpoint.graph_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.site IS 'the site of a row of this rule, materialized into intent_node_id_decode_endpoint.site, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.type_name IS 'the type_name of a row of this rule, materialized into intent_node_id_decode_endpoint.type_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.field_name IS 'the field_name of a row of this rule, materialized into intent_node_id_decode_endpoint.field_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.argument_name IS 'the argument_name of a row of this rule, materialized into intent_node_id_decode_endpoint.argument_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.path IS 'the path of a row of this rule, materialized into intent_node_id_decode_endpoint.path, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.use_site IS 'the use_site of a row of this rule, materialized into intent_node_id_decode_endpoint.use_site, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.node_type_name IS 'the node_type_name of a row of this rule, materialized into intent_node_id_decode_endpoint.node_type_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.navigation IS 'the navigation of a row of this rule, materialized into intent_node_id_decode_endpoint.navigation, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.from_source_name IS 'the from_source_name of a row of this rule, materialized into intent_node_id_decode_endpoint.from_source_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.from_schema IS 'the from_schema of a row of this rule, materialized into intent_node_id_decode_endpoint.from_schema, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.from_table IS 'the from_table of a row of this rule, materialized into intent_node_id_decode_endpoint.from_table, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.to_source_name IS 'the to_source_name of a row of this rule, materialized into intent_node_id_decode_endpoint.to_source_name, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.to_schema IS 'the to_schema of a row of this rule, materialized into intent_node_id_decode_endpoint.to_schema, whose comment carries what the value means';
COMMENT ON COLUMN intent_node_id_decode_endpoint_live.to_table IS 'the to_table of a row of this rule, materialized into intent_node_id_decode_endpoint.to_table, whose comment carries what the value means';

CREATE TABLE intent_node_id_decode_endpoint (
  graph_name       VARCHAR,
  site             VARCHAR,
  type_name        VARCHAR,
  field_name       VARCHAR,
...
COMMENT ON COLUMN intent_node_id_decode_hop_column.path IS 'the occurrence path on INPUT_FIELD; NULL on ARGUMENT, as on the hop';
COMMENT ON COLUMN intent_node_id_decode_hop_column.position IS 'the hop''s own 0-based position along the path, carried from the hop; with the pair position beside it and the coordinate above, this is the key';
...
WITH RECURSIVE lifted (graph_name, site, type_name, field_name, argument_name, path,
  ...
  SELECT l.graph_name, l.site, l.type_name, l.field_name, l.argument_name, l.path,
    ...
    JOIN intent_node_id_decode_hop_column hc
     ...
     AND hc.field_name = l.field_name
     AND (hc.argument_name = l.argument_name
          OR (hc.argument_name IS NULL AND l.argument_name IS NULL))
     AND (hc.path = l.path OR (hc.path IS NULL AND l.path IS NULL))
     AND hc.position = l.position + 1
     AND UPPER(hc.from_column_name) = UPPER(l.arrived_column_name)
)
SELECT e.graph_name, e.site, e.type_name, e.field_name, e.argument_name, e.path,
  ...
  LEFT JOIN lifted l
   ...
   AND l.graph_name = e.graph_name AND l.site = e.site AND l.type_name = e.type_name
   AND l.field_name = e.field_name
...
COMMENT ON COLUMN intent_node_id_decode_column.path IS 'the occurrence path on INPUT_FIELD; NULL on ARGUMENT, as on the endpoint relation';
...
INSERT INTO meta_materialize VALUES
  ...
  ('intent_node_id_decode_endpoint_live', 'intent_node_id_decode_endpoint',
   'Three relations read this one, each being a different reduction over the same two endpoints, and as a view each of them paid for its whole subtree rather than for the forty rows. Measured against a real schema, one evaluation is five seconds and the two relations directly above it were seven and a half each; over the materialized table they are two and a half. The shared departure this relation was factored out to state once was being computed once per reader, which is the registration case exactly.'),
  ('intent_node_id_decode_hop_column_live', 'intent_node_id_decode_hop_column',
   'A recursive walk lifts a decode''s columns along this relation, and a recursive step joins its input once per row the walk has accumulated, so a view here is evaluated as many times as the walk has rows. Measured against a real schema, that recursion took a hundred and forty-six seconds to produce twenty rows; over the materialized table it is instant. Nothing about how the step names the relation changes this, both a serialized-key equality and the original null-safe match being instant against a table and neither helping against the view, so this is the registration case with no rewrite standing behind it.');
