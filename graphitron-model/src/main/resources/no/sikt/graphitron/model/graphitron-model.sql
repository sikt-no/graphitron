...
SELECT l.graph_name, l.site, l.use_site, l.type_name, l.field_name, l.position, ap.param_name,
  ...
  JOIN graphitron_argument_path_segment sg
   ...
   AND sg.field_name = l.field_name AND sg.argument_path = l.argument_path
   AND sg.position = l.segment_position + 1
  LEFT JOIN graphql_argument la
    ON l.bound_kind = 'ARGUMENT' AND la.graph_name = l.graph_name
   AND la.type_name = l.bound_type_name AND la.field_name = l.bound_field_name
   AND la.argument_name = l.bound_argument_name
  LEFT JOIN graphql_field lf
    ON l.bound_kind = 'INPUT_FIELD' AND lf.graph_name = l.graph_name
   AND lf.type_name = l.bound_type_name AND lf.field_name = l.bound_field_name
 WHERE NOT l.node_id_declared AND l.trailing_segments = 1
   AND COALESCE(la.named_type, lf.named_type) = 'ID';
COMMENT ON VIEW intent_argmapping_projection_defect IS 'What is wrong with an argMapping binding that opens, or should have opened, a @nodeId: one row per defective pair, in a closed verdict vocabulary of four, over the binding leaf and the resolved key columns alone. The rejections that close the silent hole this family had, where a path bound a node id and the base64 wire id went to the database verbatim with nothing in the build saying a word. Every arm is a positive statement about a captured population rather than a negative space maintained by hand: the leaf relation says what each path bound and whether that thing declares a decode, and three of these arms are the ways a declared decode fails to become a projection while the fourth is a projection asked for where no decode was declared at all. Which arm fires among the first three is decided by trailing_segments and nothing else, and node_id_declared separates them from the fourth, so the arms are disjoint by construction and no precedence rule is needed. Zero trailing segments means the author did not ask for a projection at all, and BARE_NODE_ID is that fact whether or not the directive names a type; the remedy differs in a second clause the consumer adds from node_type_ref, not in a second verdict, because the defect is one. Exactly one trailing segment means the author did ask, and then the resolution either succeeds (a row of intent_resolved_node_key_projection and no row here) or names what stopped it: MISSING_TYPE_NAME where the directive carries no typeName: and there is no containing table at this position to infer one from, UNKNOWN_KEY_COLUMN where the trailing segment matches no resolved key column of the named type. Two or more trailing segments is deliberately not an arm: the walk rejects walking through a scalar leaf and keeps rejecting it after the grammar admits one trailing segment, so an arm here would double-report a rejection the error stream already carries. UNDECLARED_NODE_ID is the fourth arm and is drawn by exactly that boundary rather than despite it. The grammar widening admits one trailing segment after an ID and asks nothing about the directive, because the head of a path is reached through a slot map carrying types and not directives; so an ID with no @nodeId on it is admitted by the walk, resolves no projection, and would reach an emitter as an uninterpreted segment read straight off the wire map. That is this family''s own failure mode arriving through the widening, and this arm is the only thing standing where the walk''s rejection used to. Its named_type = ''ID'' predicate is the disjointness rule and not a convenience: on any other leaf type the walk still rejects the trailing segment itself, so widening this arm past ID would double-report, and the arm covers precisely the shapes the walk stopped judging. The leaf''s type is read off the SDL corpus by the leaf''s own kind, graphql_argument on an ARGUMENT leaf and graphql_field on an INPUT_FIELD one, which is the same fork bound_kind already governs elsewhere in this family. Two further shapes stay out. A path that bound nothing has no leaf row and is ArgBindingMap.of''s rejection, a head naming no slot in scope and a path-step @condition resolving against an empty slot map both. And a projection that resolves at a site whose emitter is not wired yet is a deferral rather than an author defect, which is why it is not a verdict here: whether an emitter exists is a fact about the generator''s own code and not about the schema, so its arm lives with the consumer that knows the wired set (no.sikt.graphitron.rewrite.derive.ArgmappingProjectionDefects) rather than being asserted by a view that cannot see it. Every arm is use-keyed rather than definition-keyed, which is the point of resolving at the pair''s grain: one input type can be consumed by a routine call with no containing table and by a table-bound mutation where inference works, so an author told to add typeName: is being asked to satisfy a use-site constraint and the message has to name the use site that is asking. Locations are the owning directive application''s, carried from intent_argmapping_pair, so a message points at the argMapping the author wrote rather than at the input type''s declaration. There is no message column: the closed vocabulary plus the witness columns are the fact base, and the prose belongs with the consumer that composes it, which is also where the wording converges with the two hand-written sites stating this same condition elsewhere. Nor is there a rendered candidate list, though a message about a named type wants one: a consumer joining intent_resolved_node_key_column on the graph and node_type_ref gets the columns as rows in key order, and a render here would have to be split apart to be used, which is the one thing no reader of this schema does.';
...
COMMENT ON COLUMN intent_argmapping_projection_defect.verdict IS 'which defect, in a closed vocabulary of four: BARE_NODE_ID where a declared decode names no key column to project out of it, MISSING_TYPE_NAME where a projection is asked for against a @nodeId carrying no typeName:, UNKNOWN_KEY_COLUMN where the trailing segment names no resolved key column of the named type, UNDECLARED_NODE_ID where a key column is named after an ID that declares no @nodeId at all. Disjoint by node_id_declared between the fourth and the rest, and by trailing_segments among the first three, so this column is a discriminator a consumer switches on and never a precedence to re-test';
COMMENT ON COLUMN intent_argmapping_projection_defect.bound_kind IS 'ARGUMENT where the defective leaf is a field argument, INPUT_FIELD where it is an input field; carried from the leaf, and the fork twice over: which @nodeId relation the directive sits on where there is one, and which SDL relation the leaf''s own declared type is read from on the UNDECLARED_NODE_ID arm, where there is not';
